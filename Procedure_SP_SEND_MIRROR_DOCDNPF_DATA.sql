CREATE PROCEDURE SP_SEND_MIRROR_DOCDNPF_DATA
(	
	 @CREQXNID VARCHAR(50)
...
*/
BEGIN
	DECLARE @DTSQL NVARCHAR(MAX),@NSPID INT,@CMEMOID VARCHAR(50),@CSTEP VARCHAR(5)
...
LBLTABLEINFO:
	...
	SELECT DISTINCT 'DOCDNPF_DEBITNOTE_PROFORMA_MST_UPLOAD' AS TARGET_TABLENAME,A.* ,@CMEMOID AS XN_ID FROM DebitNote_Proforma_MST(NOLOCK) A 
    WHERE A.Memo_Id=@CMEMOID  
	
	SET @CSTEP=230
	---- SEND THE DNPF MEMO DETAIL TABLE
	 SELECT DISTINCT 'DOCDNPF_DEBITNOTE_PROFORMA_DET_UPLOAD' AS TARGET_TABLENAME,A.* FROM DebitNote_Proforma_DET (NOLOCK) A WHERE A.Memo_Id=@CMEMOID  
   
	
	SET @CSTEP=240
	---- SEND THE DEBITNOTE MEMO SKU TABLE
	 SELECT DISTINCT 'DOCDNPF_SKU_UPLOAD' AS TARGET_TABLENAME,A.*,@CMEMOID AS DNPF_MEMO_ID FROM SKU A  (NOLOCK)
	...
	SELECT DISTINCT 'DOCDNPF_ARTICLE_UPLOAD' AS TARGET_TABLENAME,A.*,@CMEMOID AS DNPF_MEMO_ID FROM ARTICLE A  (NOLOCK)
	JOIN #TMPARTICLE B ON A.ARTICLE_CODE =B.ARTICLE_CODE 
	

	SELECT DISTINCT 'DOCDNPF_UOM_UPLOAD' AS TARGET_TABLENAME,UOM.*,@CMEMOID AS DOCDNPF_MEMO_ID FROM #TMPARTICLE A
	...
	SELECT DISTINCT 'DOCDNPF_SECTIOND_UPLOAD' AS TARGET_TABLENAME,SECTIOND.*,@CMEMOID AS DOCDNPF_MEMO_ID FROM #TMPARTICLE A
	JOIN ARTICLE B (NOLOCK) ON A.article_code =B.ARTICLE_CODE 
	...
	SELECT DISTINCT 'DOCDNPF_SECTIONM_UPLOAD' AS TARGET_TABLENAME,SECTIONM.*,@CMEMOID AS DOCDNPF_MEMO_ID FROM #TMPARTICLE A
	JOIN ARTICLE B (NOLOCK) ON A.article_code =B.ARTICLE_CODE 
	JOIN SECTIOND  (NOLOCK) ON SECTIOND.sub_section_code =B.sub_section_code 
    ...
    SELECT DISTINCT 'DOCDNPF_PARA1_UPLOAD' AS TARGET_TABLENAME,A.*,@CMEMOID AS DOCDNPF_MEMO_ID FROM PARA1 A
	JOIN #TMPPARA B (NOLOCK) ON A.PARA1_CODE  =B.PARA1_CODE 

	SET @CSTEP=380
	---- SEND THE PARA2 RELATED TO GIVEN DNPF MEMO

	SELECT DISTINCT 'DOCDNPF_PARA2_UPLOAD' AS TARGET_TABLENAME,A.*,@CMEMOID AS DOCDNPF_MEMO_ID FROM PARA2 A
	JOIN #TMPPARA B (NOLOCK) ON A.PARA2_CODE  =B.PARA2_CODE 

	SET @CSTEP=390
	---- SEND THE PARA3 RELATED TO GIVEN DNPF MEMO
	SELECT DISTINCT 'DOCDNPF_PARA3_UPLOAD' AS TARGET_TABLENAME,A.*,@CMEMOID AS DOCDNPF_MEMO_ID FROM PARA3 A
	JOIN #TMPPARA B (NOLOCK) ON A.PARA3_CODE  =B.PARA3_CODE 

	SET @CSTEP=400
	---- SEND THE PARA4 RELATED TO GIVEN DNPF MEMO
	SELECT DISTINCT 'DOCDNPF_PARA4_UPLOAD' AS TARGET_TABLENAME,A.*,@CMEMOID AS DOCDNPF_MEMO_ID FROM PARA4 A
	JOIN #TMPPARA B (NOLOCK) ON A.PARA4_CODE  =B.PARA4_CODE 

	SET @CSTEP=410
	---- SEND THE PARA5 RELATED TO GIVEN DNPF MEMO
	SELECT DISTINCT 'DOCDNPF_PARA5_UPLOAD' AS TARGET_TABLENAME,A.*,@CMEMOID AS DOCDNPF_MEMO_ID FROM PARA5 A
	JOIN #TMPPARA B (NOLOCK) ON A.PARA5_CODE  =B.PARA5_CODE 

	SET @CSTEP=420
	---- SEND THE PARA6 RELATED TO GIVEN DNPF MEMO
	SELECT DISTINCT 'DOCDNPF_PARA6_UPLOAD' AS TARGET_TABLENAME,A.*,@CMEMOID AS DOCDNPF_MEMO_ID FROM PARA6 A
	...
	SELECT  DISTINCT 'DOCDNPF_ARTICLE_FIX_ATTR_UPLOAD' AS TARGET_TABLENAME,@CMEMOID AS DOCDNPF_MEMO_ID,A.* FROM ARTICLE_FIX_ATTR A (NOLOCK)  
	 JOIN ARTICLE B (NOLOCK) ON A.ARTICLE_CODE=B.ARTICLE_CODE  
	...
	SET @CSTEP=440
	 ...
	 BEGIN  
		 ...
		 SET @CCMD=N'
		SELECT  DISTINCT ''DOCDNPF_ATTR'+RTRIM(LTRIM(RTRIM(LTRIM(STR(@BLOOP)))))+'_MST_UPLOAD'' AS TARGET_TABLENAME,'''+@CMEMOID+''' AS DOCDNPFF_MEMO_ID,ATTR'+RTRIM(LTRIM(STR(@BLOOP)))+'_MST.*   
