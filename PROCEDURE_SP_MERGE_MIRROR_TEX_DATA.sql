CREATE PROCEDURE SP_MERGE_MIRROR_TEX_DATA
(
	@CMEMOID VARCHAR(50)
   ,@CLOCID VARCHAR(3)
...
BEGIN TRANSACTION 
	...
	SELECT @CTEMPMASTERTABLE=@CSOURCEDB+'TMP_TEX_TILL_LIFTS_'+LTRIM(RTRIM(@CTABLE_SUFFIX))

LBLMERGE:
    
    SET @CSTEP=250
	SET @CTABLENAME='TILL_EXPENSE_MST'
	SET @CTMP_TABLENAME='TMP_TEX_'+@CTABLENAME+'_'+LTRIM(RTRIM(@CTABLE_SUFFIX))
	...
	SET @CSTEP=320						  
	SET @CTABLESSTR='TILL_EXPENSE_MST,TILL_EXPENSE_DET'
...
END TRY
BEGIN CATCH
	SET @CERRMSG='P:SP_MERGE_MIRROR_TEX_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
END CATCH
EXIT_PROC:
