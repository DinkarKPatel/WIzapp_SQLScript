create PROCEDURE SP3S_IMPORT_SLS_DATA_UPLOAD_RND_CN
(    
 @iDateMode		INT,
 @CLOCID VARCHAR(10),          
 @NMODE INT=0      ,    
...
select * from pmt01106 where quantity_in_stock=0
INSERT SLS_IMPORT_DATA	( address0, address1, address2, ADDRESS9, area, BIN_ID, cancelled, cgst_amount, city, CMD_DISCOUNT_AMOUNT, CMD_DISCOUNT_PERCENTAGE, 
CMM_DISCOUNT_AMOUNT, CMM_OTHER_CHARGES, CUS_GST_NO, cus_gst_state, CUSTOMER_FNAME, CUSTOMER_LNAME, customer_name, dept_id, EMAIL, EMP_NAME, errormsg, 
gst_percentage, hsn_code, igst_amount, memo_dt, memo_no, MRP, net, net_amount, processed, product_code, quantity, REMARKS, row_id, sgst_amount, SP_ID, state, 
STORE_CODE, USER_CUSTOMER_CODE, xn_value_without_gst )  
SELECT 	''  address0, ''address1, ''address2, ''ADDRESS9, ''area, '000' BIN_ID, 0 cancelled, 0 cgst_amount, '' city, 0 CMD_DISCOUNT_AMOUNT, 0 CMD_DISCOUNT_PERCENTAGE, 
0 CMM_DISCOUNT_AMOUNT, 0 CMM_OTHER_CHARGES, '' CUS_GST_NO, '' cus_gst_state, ''CUSTOMER_FNAME, ''CUSTOMER_LNAME, ''customer_name, 'HO' dept_id,'' EMAIL, '' EMP_NAME, '' errormsg, 
0 gst_percentage, '' hsn_code, 0 igst_amount, GETDATE() memo_dt, 'LATER_RAMAN' memo_no, 0 MRP,-580 net,-580 net_amount,0 processed, 'JM4774223'product_code,-1 quantity, 'REPAIR AND DEFECTIVE' REMARKS,
NEWID() row_id, 0 sgst_amount, 'ROHIT' SP_ID, '' state,'' STORE_CODE, '9871055311' USER_CUSTOMER_CODE, 0 xn_value_without_gst 
UNION ALL
SELECT 	''  address0, ''address1, ''address2, ''ADDRESS9, ''area, '000' BIN_ID, 0 cancelled, 0 cgst_amount, '' city, 0 CMD_DISCOUNT_AMOUNT, 0 CMD_DISCOUNT_PERCENTAGE, 
0 CMM_DISCOUNT_AMOUNT, 0 CMM_OTHER_CHARGES, '' CUS_GST_NO, '' cus_gst_state, ''CUSTOMER_FNAME, ''CUSTOMER_LNAME, ''customer_name, 'HO' dept_id,'' EMAIL, '' EMP_NAME, '' errormsg, 
0 gst_percentage, '' hsn_code, 0 igst_amount, GETDATE() memo_dt, 'LATER_RAMAN' memo_no, 0 MRP,-680 net,-680 net_amount,0 processed, 'JM4774224'product_code,-1 quantity, 'REPAIR AND DEFECTIVE' REMARKS,
NEWID() row_id, 0 sgst_amount, 'ROHIT' SP_ID, '' state,'' STORE_CODE, '9871055311' USER_CUSTOMER_CODE, 0 xn_value_without_gst 

UPDATE SLS_IMPORT_DATA SET memo_dt='02-08-2022',DEPT_ID='JM' WHERE SP_ID='ROHIT'

SELECT * FROM SLS_IMPORT_DATA WHERE SP_ID='ROHIT'

DECLARE @iDateMode		INT=0,
 @CLOCID VARCHAR(10) ='HO',          
 @NMODE INT=0      ,    
 @CSPID VARCHAR(50)='ROHIT' ,
 @CUSER_CODE CHAR(7)= '0000000',
 @memo_no	VARCHAR(50)='LATER_ROHIT',
 @memo_dt	VARCHAR(50)='2022-08-31',
 @CBINID	VARCHAR(10)='000'
 */
BEGIN          
...
FROM #SLSIMPORT_CUSTDYM WHERE ISNULL(USER_CUSTOMER_CODE,'')<>''    
 ...
 END    
  ...
  --UPDATE SLS_IMPORT_DATA SET MEMO_DT=CONVERT(VARCHAR(20),GETDATE(),111) WHERE ISNULL(MEMO_DT,'')=''         
	...
	,ISNULL(A.gst_percentage,0) sisloc_gst_percentage,A.xn_value_without_gst AS sisloc_taxable_value,ISNULL(A.cgst_amount,0)+ISNULL(A.sgst_amount,0)  AS sisloc_lgst_amount,ISNULL(A.igst_amount,0) AS sisloc_igst_amount
	 ,CAST (0 AS NUMERIC(14,2)) AS xn_value_without_gst
    FROM SLS_IMPORT_DATA A  (NOLOCK)         
	...
	*/
	--if @cRETAIN_EXCEL_NRV_SISLOC_SALEIMP='1'
	SET @CCMD=N'SELECT 0 AS SR_NO,MEMO_NO AS BILL_NO,MEMO_DT AS CM_DT,DEPT_ID,A.PRODUCT_CODE,QUANTITY,A.NET AS AMOUNT,          
    CAST(ISNULL(CANCELLED,0) AS BIT) AS CANCELLED,'''' AS ERROR_MESSAGE,ABS(ISNULL(NULL,(CASE WHEN QUANTITY<0 THEN -1 * ABS(ISNULL(A.CMD_DISCOUNT_AMOUNT,0)) ELSE ABS(ISNULL(A.CMD_DISCOUNT_AMOUNT,0)) END)+A.NET)/QUANTITY) AS MRP,
	...
	ELSE A.NET-(ISNULL(A.cgst_amount,0)+ISNULL(A.sgst_amount,0)+ISNULL(A.igst_amount,0)) END xn_value_without_gst
    FROM SLS_IMPORT_DATA A  (NOLOCK)         
    JOIN SKU B (NOLOCK) ON A.PRODUCT_CODE=B.PRODUCT_CODE 
	...
	sisloc_eoss_discount_percentage,sisloc_eoss_discount_amount,sisloc_mrp ,sisloc_gst_percentage,sisloc_taxable_value,sisloc_lgst_amount,sisloc_igst_amount,xn_value_without_gst)            
    EXEC SP_EXECUTESQL @CCMD          
 ...
 -- SELECT * FROM #SLSIMPORT           
  ...
  ,A.REMARKS,A.NET,A.NET_AMOUNT    ,ISNULL(GST.gst_state_code,'') party_state_code
 ...
 UPDATE A SET PARTY_STATE_CODE =CASE WHEN ISNULL(B.GST_STATE_CODE   ,'') ='' THEN '00' ELSE B.GST_STATE_CODE END       
  ...
  ,A.DISCOUNT_AMT    
  ,A.HSN_CODE   ,A.gst_percentage ,A.igst_amount ,A.cgst_amount,A.sgst_amount     
  ,A.REMARKS,A.NET,A.NET_AMOUNT,ISNULL(GST.gst_state_code,'') AS  party_state_code
  ,A.sis_net,A.sisloc_eoss_discount_percentage,A.sisloc_eoss_discount_amount,A.sisloc_mrp
  ,A.sisloc_gst_percentage,A.sisloc_taxable_value,A.sisloc_lgst_amount,A.sisloc_igst_amount,A.xn_value_without_gst
  INTO #NEWBILLS          
  FROM #SLSIMPORT A          
...
-- SET @CMEMONOVAL=@CLOCID+@CLOCID+'-'+@CBILL_NO        
  ...
  INSERT SLS_CMM01106_UPLOAD (SP_ID, REMARKS, SENT_FOR_RECON, PARTY_TYPE, AC_CODE, MANUAL_DISCOUNT, MANUAL_ROUNDOFF          
   ...
   , SENT_FOR_GR,xn_item_type,oh_tax_method ,party_state_code,DELIVERY_MODE,location_Code  )            
  SELECT DISTINCT @CSPID,'IMPORTED SALE.' +ISNULL(REMARKS,'') AS REMARKS,0 AS SENT_FOR_RECON,1 AS PARTY_TYPE,'0000000000' AS AC_CODE          
    ...
    ,DEPT_ID+'-'+BILL_NO AS REF_NO,0 AS SENT_FOR_GR  , 1 as xn_item_type         ,1 as oh_tax_method,party_state_code,0 AS DELIVERY_MODE,
    @CDEPT_ID as Location_code
  FROM #EXISTINGBILLS          
...
--SELECT * FROM CMM01106(NOLOCK) WHERE CM_ID IN (SELECT CM_ID FROM #NEWBILLS)           
 ...
 PRINT 'PROCESSING STEP #'+@CSTEP+'#'+CONVERT(VARCHAR,GETDATE(),113)          
   ...
   ,1 AS CURRENCY_CONVERSION_RATE,'REPAIR AND DEFECTIVE' AS REMARKS,'' AS GV_SRNO,'' AS GV_SCRATCH_NO           
   FROM #EXISTINGBILLS A          
   JOIN PAYMODE_MST B ON B.paymode_code='0000004'          
   WHERE CASH<>0          
...
BEGIN CATCH         
  ...
  SET @CERRMSG='SP3S_SYNCH_MBO_SLS_DATA_UPLOAD_NEW  : AT STEP - '+@CSTEP+', MESSAGE - '+ERROR_MESSAGE()    
  PRINT       @CERRMSG    
  GOTO END_PROC           
END CATCH          
...
END_PROC:          
  ...
  if OBJECT_ID('tempdb..#SP3S_IMPORT_SLS_DATA_UPLOAD_NEW','U') IS NOT NULL
  DROP TABLE #SP3S_IMPORT_SLS_DATA_UPLOAD_NEW
