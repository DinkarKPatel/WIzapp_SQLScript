CREATE PROCEDURE SP3S_GET_VCHTYPE_FKEYDATA
...
BEGIN
	BEGIN TRY
		...
		SET @CFILTERCONDITION= 'B.VOUCHER_CODE IN (SELECT VOUCHER_CODE FROM '+@CTEMPTABLE+' )'
		
		SET @NSTEP=20
		
		EXEC UPDATEMASTERXN_MIRROR
		@CSOURCEDB='',
		@CSOURCETABLE='VCHTYPE',
		@CDESTDB=@CDESTDB,
		@CDESTTABLE='VCHTYPE',
		@CKEYFIELD1='VOUCHER_CODE',
		@LINSERTONLY=0,
	...
	BEGIN CATCH
		SELECT @CERRORMSG='PROCEDURE SP3S_GET_VCHTYPE_FKEYDATA : '+ISNULL(ERROR_PROCEDURE(),'NULL P')+'STEP :'+STR(ISNULL(@NSTEP,0))+ ' LINE NO. :'+
...
END
----'END OF PROCEDURE SP3S_GET_VCHTYPE_FKEYDATA'
