CREATE PROCEDURE SP_SALEAPPROVAL        
(        
  @IMODE INT ,        
  @CWHERE VARCHAR(4000)='',        
  @CFINYEAR VARCHAR(5)='',        
...
BEGIN         
 ...
 ELSE IF @IMODE=2 ------GET APM01106 MASTER BY MEMOID-------        
 GOTO LBL2        
         
 ELSE IF @IMODE=3 ------GET APD01106 DETAIL BY MEMOID-------        
 GOTO LBL3        
...
LBL2:        
 ...
 CAST(0 AS NUMERIC(5,0)) AS SP_ID      
 FROM APM01106 A  
...
LBL3:        
    ...
    UOM.UOM_TYPE, D.FIX_MRP,D.PRODUCT_NAME, UOM.UOM_CODE , UOM.UOM_NAME       
    ...
    B.PRODUCT_CODE AS ORG_PRODUCT_CODE, 
    CAST(CASE WHEN CHARINDEX('@',B.PRODUCT_CODE)=0 THEN '' ELSE 
    (SUBSTRING(B.PRODUCT_CODE,CHARINDEX('@',B.PRODUCT_CODE)+1,15)) END AS VARCHAR(100))  AS BATCH_LOT_NO,
     D.BATCH_NO,D.EXPIRY_DT  ,CAST(0 AS NUMERIC(5,0)) AS SP_ID  ,E.stock_na    
    FROM APM01106 A         
    ...
    LEFT OUTER JOIN      
    ( 
		SELECT A.APD_ROW_ID,SUM(A.QUANTITY) AS RET_QTY FROM APPROVAL_RETURN_DET A    
		JOIN APPROVAL_RETURN_MST C ON C.MEMO_ID=A.MEMO_ID    
		WHERE C.CANCELLED=0    
...
LBL4:      
   EXECUTE SP_NAVIGATE 'APM01106',@NNAVMODE,@CWHERE,@CFINYEAR,'MEMO_NO','MEMO_DT','MEMO_ID',@CWHERECLAUSE  
 GOTO LAST        
...
LBL5:        
 ...
 JOIN SKU D ON B.PRODUCT_CODE = D.PRODUCT_CODE        
 LEFT OUTER JOIN         
 (SELECT CMD_ROW_ID,SUM(A.QUANTITY) AS RET_QTY FROM CMA01106 A         
...
LBL7:        
 ...
 SELECT TOP 1 A.ROW_ID FROM APD01106 A        
    LEFT OUTER JOIN         
...
LBL9:      
 ...
 SELECT B.MEMO_ID,B.MEMO_NO,B.MEMO_DT,B.FIN_YEAR  FROM APD01106 A         
  JOIN APM01106 B ON B.MEMO_ID=A.MEMO_ID      
  LEFT OUTER JOIN       
  (SELECT CMD_ROW_ID,SUM(A.QUANTITY) AS RETURN_QUANTITY      
   FROM CMA01106 A JOIN APD01106 B ON A.CMD_ROW_ID=B.ROW_ID      
   JOIN APM01106 C ON C.MEMO_ID=B.MEMO_ID       
   ...
   AND B.CANCELLED=0      
 UNION       
 SELECT MEMO_ID,MEMO_NO,MEMO_DT,FIN_YEAR  FROM APM01106 WHERE FIN_YEAR=@CFINYEAR      
...
LBL10:        
 ...
 E.DT_CREATED AS ART_DT_CREATED,E.ARTICLE_NO,             
    ...
    FROM APM01106 A         
    JOIN APD01106 B ON A.MEMO_ID = B.MEMO_ID         
    JOIN CMA01106 CMA ON B.ROW_ID = CMA.CMD_ROW_ID         
    JOIN SKU D ON B.PRODUCT_CODE = D.PRODUCT_CODE        
    LEFT JOIN ARTICLE E ON D.ARTICLE_CODE = E.ARTICLE_CODE             
    LEFT JOIN PARA1 F ON D.PARA1_CODE = F.PARA1_CODE             
    LEFT JOIN PARA2 G ON D.PARA2_CODE = G.PARA2_CODE             
...
LBLRETURN:   
 ...
 SELECT APD_ROW_ID FROM APPROVAL_RETURN_DET A
