create VIEW VW_PO_QBF AS          
  ...
  ,CMP.COUNTRY AS COMPANY_COUNTRY,CMP.MOBILE AS COMPANY_MOBILE,CMP.CONTACT_NAME AS COMPANY_CONTACT_NAME,CMP.TDS_AC_NO AS COMPANY_TDS_AC_NO,CMP.COMPANY_NAME,CMP.ALIAS AS COMPANY_ALIAS  
  ,CMP.ADDRESS1 AS COMPANY_ADDRESS1,CMP.ADDRESS2 AS COMPANY_ADDRESS2,CMP.PHONES_FAX AS COMPANY_PHONES_FAX,CMP.CST_NO AS COMPANY_CST_NO,CMP.CST_DT AS COMPANY_CST_DT  
  ...
  ,CMP.WEB_ADDRESS AS COMPANY_WEB_ADDRESS,CMP.GRP_CODE AS COMPANY_GRP_CODE,CMP.ADDRESS9 AS COMPANY_ADDRESS9,CA.AREA_NAME AS COMPANY_AREA,CMP.CITY AS COMPANY_CITY,CS.[STATE] AS COMPANY_STATE  
  ,CRG.REGION_NAME AS COMPANY_REGION,  
  LCT.DEPT_ID AS LOCATION_DEPT_ID,LCT.DEPT_NAME AS LOCATION_DEPT_NAME,  LCT.DEPT_NAME AS MST_DEPT_NAME,
  A.po_for_dept_id ,'' as MST_po_for_dept_name,
  LA.PINCODE as LOCATION_PINCODE ,LCT.LOC_GST_NO as LOCATION_GST_NO,  
  LCT.MAJOR_DEPT_ID AS LOCATION_MAJOR_DEPT_ID,LCT.AREA_CODE AS LOCATION_AREA_CODE  
  ,LCT.PRIMARY_EMAIL AS LOCATION_PRIMARY_EMAIL,LCT.PRIMARY_EMAIL_SMTP AS LOCATION_PRIMARY_EMAIL_SMTP,LCT.PRIMARY_EMAIL_PWD AS LOCATION_PRIMARY_EMAIL_PWD,LCT.PRIMARY_EMAIL_SSL AS LOCATION_PRIMARY_EMAIL_SSL  
  ...
  ,LCT.DATE_OF_OPENING AS LOCATION_DATE_OF_OPENING,LCT.REPORT_BLOCKED AS LOCATION_REPORT_BLOCKED,LCT.INACTIVE AS LOCATION_INACTIVE,LCT.LOC_TYPE AS LOCATION_LOC_TYPE,LCT.ACCOUNTS_POSTING_DEPT_ID   
  AS LOCATION_ACCOUNTS_POSTING_DEPT_ID,LCT.CONTROL_AC_CODE AS LOCATION_CONTROL_AC_CODE,LCT.UPD_PURINFO AS LOCATION_UPD_PURINFO,LCT.SSPL_REG_KEY AS LOCATION_SSPL_REG_KEY,LCT.WIZCOM_ENABLED AS LOCATION_WIZCOM_ENABLED  
  ...
  D.XN_VALUE_WITH_GST,A.DELIVERY_DT,LCT.LOC_GST_DT AS LOCATION_GST_DT
  ...
  ,A.XN_ITEM_TYPE
  ,XN_ITEM_TYPE_DESC=CASE A.XN_ITEM_TYPE WHEN 1 THEN 'INVENTORY' WHEN 3 THEN 'ASSETS' WHEN 2 THEN 'CONSUMABLES' ELSE 'SERVICES' END,
  ApprovedLevelNo,d.tolerance_percentage,D.Gst_Cess_Percentage ,D.Gst_Cess_Amount ,a.packing_Dt,
  c.username as APPROVED_USER, cast('' as varchar(100)) as AREA_UOM_NAME,
   cast(0 as Numeric(10,2)) as area_length,cast(0 as Numeric(10,2)) as area_width,cast(0 as Numeric(10,2)) as area_sqare,
   cast(0 as Numeric(10,2)) as area_rate_pp,cast(0 as Numeric(10,2)) as RATE_AREA_SQUARE,
   cast('' as varbinary(max)) as PROD_IMAGE,shlm.AC_NAME OEM_ac_name,
   a.shipping_address,a.shipping_address2,a.shipping_address3,
   a.shipping_area_code,a.shipping_area_name,
   a.shipping_pin,a.shipping_city_name,a.shipping_state_name
   ,A.location_Code
