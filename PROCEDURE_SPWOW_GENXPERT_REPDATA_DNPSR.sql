CREATE PROCEDURE SPWOW_GENXPERT_REPDATA_DNPSR
...
BEGIN
	...
	JOIN rmm01106 C (NOLOCK) ON dnps_mst.prt_rm_id=c.rm_id
	join SKU_NAMES (NOLOCK) ON sku_names.product_code=dnps_det.PRODUCT_CODE
	JOIN location SourceLocation (NOLOCK) ON SourceLocation.dept_id=LEFT(dnps_mst.PS_ID,2) 
	JOIN bin SourceBin on SourceBin.bin_id=dnps_det.bin_id
	JOIN LOC_NAMES (NOLOCK) on LOC_NAMES.dept_id=LEFT(dnps_mst.PS_ID,2)

	    Left Outer join BARCODEWISE_EOSS_SCHEMES_INFO EOSSSCH (NOLOCK) ON EOSSSCH.location_id=LEFT(dnps_mst.PS_ID,2)
