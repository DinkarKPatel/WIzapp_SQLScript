CREATE PROCEDURE SPWOW_GENXPERT_REPDATA_IRR_CIP
...
BEGIN
	...
	join SKU_NAMES (NOLOCK) ON sku_names.product_code=ird01106.PRODUCT_CODE
   ...
   LEFT JOIN lm01106 supplier_lm01106 on supplier_lm01106.ac_code=SKU_NAMES.ac_code
	...
	LEFT JOIN XN_ITEM_TYPE_DESC_mst (NOLOCK) on XN_ITEM_TYPE_DESC_mst.XN_ITEM_TYPE=1
  	...
  	WHERE (irm01106.irm_memo_dt BETWEEN [DFROMDT] AND [DTODT]) AND ISNULL(new_product_code,'''')<>''''  AND [WHERE]       
	group by [GROUPBY]'
