...
,SKU.PRODUCT_NAME 
,SKU.PURCHASE_PRICE,
LCT.DEPT_NAME AS MST_DEPT_NAME    ,A.location_Code
FROM CNPS_MST A (NOLOCK)           
JOIN LMV01106 B (NOLOCK)  ON A.AC_CODE = B.AC_CODE          
JOIN CNPS_DET C (NOLOCK) ON A.PS_ID = C.PS_ID          
...
LEFT OUTER JOIN AREA LA(NOLOCK) ON LCT.AREA_CODE=LA.AREA_CODE  
LEFT OUTER JOIN CITY LC(NOLOCK) ON LA.CITY_CODE=LC.CITY_CODE  
LEFT OUTER JOIN STATE LS(NOLOCK) ON LC.STATE_CODE=LS.STATE_CODE  
LEFT OUTER JOIN REGIONM LRG(NOLOCK) ON LS.REGION_CODE=LRG.REGION_CODE  
