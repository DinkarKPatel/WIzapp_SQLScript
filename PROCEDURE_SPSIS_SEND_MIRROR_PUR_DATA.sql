create  PROCEDURE SPSIS_SEND_MIRROR_PUR_DATA
(
...
LBLTABLEINFO:
	...
	SET @CSTEP=60
	SELECT @cCols=COALESCE(@cCols+',','')+colname 	FROM ebo_xnssending_cols WHERE tablename='PIM01106' 

	SET @cCmdCols=N'
	   SELECT ''PUR_PIM01106_upload'' AS TARGET_TABLENAME,'+@cCols+' FROM PIM01106  (NOLOCK)   WHERE MRR_ID='''+@CMEMOID+''''
	PRINT @cCmdCols
	...
	SET @CSTEP=65	
	SET @cCols=NULL
	SELECT @cCols=COALESCE(@cCols+',','')+'PID01106.'+colname FROM ebo_xnssending_cols	WHERE tablename='PID01106' 

	SET @cCOls=REPLACE(@cCOls,'PID01106.PRODUCT_CODE','CASE WHEN ISNULL(C.VENDOR_EAN_NO,'''')<>'''' THEN C.VENDOR_EAN_NO ELSE PID01106.PRODUCT_CODE END AS PRODUCT_CODE')


	PRINT @cCOls
	SET @cCmdCols=N'
				SELECT ''PUR_PID01106_upload'' AS TARGET_TABLENAME,'+@cCols+' 
				FROM PID01106  (NOLOCK)   
				JOIN SKU C (NOLOCK) ON C.PRODUCT_CODE=PID01106.PRODUCT_CODE
				JOIN LMP01106 LMP (NOLOCK) ON LMP.AC_CODE =C.AC_CODE
				WHERE SUBSTRING(LMP.AC_GST_NO,3,10)='''+@CPANNO+'''
				AND PID01106.MRR_ID='''+@CMEMOID+''''
	
	PRINT @cCmdCols

	EXEC SP_EXECUTESQL @cCmdCols

	EXEC SPSIS_GET_CURSOR_SKUNAMES @cXNTYPE='PUR',@cTABLENAME='PID01106',@cKEYNAME='Mrr_id',@cKEYVALUE=@CMEMOID,@CERRMSG=@CERRMSG OUTPUT 
	IF ISNULL(@CERRMSG,'')<>'' 
		GOTO END_PROC
...
BEGIN CATCH
	SET @CERRMSG='P: SPSIS_SEND_MIRROR_PUR_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
