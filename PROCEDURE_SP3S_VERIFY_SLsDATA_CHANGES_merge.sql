CREATE PROCEDURE SP3S_VERIFY_SLsDATA_CHANGES_merge
@cMemoId VARCHAR(40),
@nSpId VARCHAR(50)=''
AS
BEGIN
	...
	PRINT 'gen tempdata updatestr for cmm01106'
	...
	PRINT 'gen tempdata updatestr for cmd01106'
	EXEC SP3S_GENFILTERED_UPDATESTR
	@cSpId=@cSpId ,
	@cInsSpId=@cInsSpId,
	@cTableName='CMd01106',
	...
	PRINT 'gen tempdata updatestr for paymode_xn_det'
	EXEC SP3S_GENFILTERED_UPDATESTR
	@cSpId=@cSpId ,
	@cInsSpId=@cInsSpId,
	...
	SELECT @CFILTERCONDITION=' b.cm_id='''+@cMemoId+''' ',
		   @cUploadTableName='sls_coupon_redemption_info_'+@cSuffix,
		   @CINSSPIDCol=''
	...
	EXEC UPDATEMASTERXN_MIRROR @CSOURCEDB='',@CSOURCETABLE='coupon_redemption_info',@CDESTDB=''
								...
								,@LINSERTONLY=1,@CFILTERCONDITION=@CFILTERCONDITION,@LUPDATEONLY=0
								,@BALWAYSUPDATE=0,@BUPDATEXNS=1,@CINSSPID=@CINSSPID,@CINSSPIDCol=@CINSSPIDCol
								,@CSEARCHTABLE='coupon_redemption_info',@cXnType='SLS'	

	SET @cSpIdCol='sp_id'

	PRINT 'gen tempdata updatestr for coupon_redemption_info'
