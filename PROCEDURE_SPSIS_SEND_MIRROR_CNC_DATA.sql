create PROCEDURE SPSIS_SEND_MIRROR_CNC_DATA
(
...
LBLTABLEINFO:
	...
	SET @CSTEP=60
	SELECT @cCols=COALESCE(@cCols+',','')+colname 	FROM ebo_xnssending_cols WHERE tablename='ICM01106' 

	SET @cCmdCols=N'
	   SELECT ''CNC_ICM01106_UPLOAD'' AS TARGET_TABLENAME,'+@cCols+' FROM ICM01106  (NOLOCK)   WHERE cnc_memo_ID='''+@CMEMOID+''''
	PRINT @cCmdCols
	...
	SET @CSTEP=65	
	SET @cCols=NULL
	SELECT @cCols=COALESCE(@cCols+',','')+'ICD01106.'+colname FROM ebo_xnssending_cols	WHERE tablename='ICD01106' 

	SET @cCOls=REPLACE(@cCOls,'ICD01106.PRODUCT_CODE','CASE WHEN ISNULL(C.VENDOR_EAN_NO,'''')<>'''' THEN C.VENDOR_EAN_NO ELSE ICD01106.PRODUCT_CODE END AS PRODUCT_CODE')

	PRINT @cCOls
	SET @cCmdCols=N'
		SELECT ''CNC_ICD01106_UPLOAD'' AS TARGET_TABLENAME,'+@cCols+' 
		FROM ICD01106  (NOLOCK)   
		JOIN SKU C (NOLOCK) ON C.PRODUCT_CODE=ICD01106.PRODUCT_CODE
		JOIN LMP01106 LMP (NOLOCK) ON LMP.AC_CODE =C.AC_CODE
		WHERE SUBSTRING(LMP.AC_GST_NO,3,10)='''+@CPANNO+'''
		AND ICD01106.cnc_memo_ID='''+@CMEMOID+''''
	
	PRINT @cCmdCols

	EXEC SP_EXECUTESQL @cCmdCols

	EXEC SPSIS_GET_CURSOR_SKUNAMES @cXNTYPE='CNC',@cTABLENAME='ICD01106',@cKEYNAME='cnc_memo_ID',@cKEYVALUE=@CMEMOID,@CERRMSG=@CERRMSG OUTPUT 
	IF ISNULL(@CERRMSG,'')<>'' 
		GOTO END_PROC
...
BEGIN CATCH
	SET @CERRMSG='P: SPSIS_SEND_MIRROR_CNC_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
