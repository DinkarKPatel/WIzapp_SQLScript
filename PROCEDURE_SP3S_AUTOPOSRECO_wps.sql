CREATE PROCEDURE SP3S_AUTOPOSRECO_wps
@nMode NUMERIC(1,0),
@cLocIdPara CHAR(2)='',
...
BEGIN TRY
	...
	BEGIN
		...
		SELECT TOP 5000 cancelled,ps_dt memo_dt,ps_id  memo_id,a.subtotal as amount,
		ISNULL(total_quantity,0) as quantity,'WPS' as XN_TYPE,a.LAST_UPDATE
		FROM wps_mst A WITH (NOLOCK)
	...
	BEGIN
		...
		FROM wps_mst B (NOLOCK) JOIN #auto_posreco_data_upload a (NOLOCK) ON a.memo_id=b.ps_id 
		WHERE a.xn_type='WPS'

		SET @cStep='70'
		UPDATE B SET HO_SYNCH_LAST_UPDATE=''
		FROM wps_mst B (NOLOCK) JOIN #auto_posreco_data_mismatch a (NOLOCK) ON a.memo_id=b.ps_id 
	END
...
BEGIN CATCH
	SET @cErrormsg='Error in Procedure SP3S_GETPOSRECO_WPS_DATA at Step#'+@cStep+' '+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
