...
CREATE PROCEDURE SAVETRAN_PRD_APP
(
...
BEGIN
	...
	SET @CMASTERTABLENAME	= 'PRD_APM01106'
	SET @CDETAILTABLENAME	= 'PRD_APD01106'

	SET @CTEMPMASTERTABLENAME	= 'TEMP_PRD_APM01106_'+LTRIM(RTRIM(STR(@NSPID)))
	SET @CTEMPDETAILTABLENAME	= 'TEMP_PRD_APD01106_'+LTRIM(RTRIM(STR(@NSPID)))
	...
	BEGIN TRY
		...
		IF @NUPDATEMODE<>3
		BEGIN
			EXEC SP_VALIDATEXN_BEFORESAVE 'PRD_APP',@NSPID,'0000000',@NUPDATEMODE,@CCMDOUTPUT OUTPUT,@BNEGSTOCKFOUND OUTPUT
		...
		ELSE
		BEGIN
			EXEC VALIDATEXN_BEFORE_EDIT 'PRD_APP',@CMEMOID,1,@CWIZAPPUSERCODE,@CERRORMSG OUTPUT,@CEXPRERRORMSG OUTPUT
		...
		BEGIN	
			...
			SET @CCMD = 'UPDATE ' + @CTEMPMASTERTABLE + ' SET ' + @CMEMONO+'=''' + @CMEMONOVAL+''',' + 
						@CKEYFIELD1+' = '''+@CKEYFIELDVAL1+''', DEPT_ID='''+@CLOCATIONID+''''
			PRINT @CCMD
			EXEC SP_EXECUTESQL @CCMD
		
			SET @CCMD = 'UPDATE ' + @CTEMPDETAILTABLE + ' SET '+@CKEYFIELD1+' = '''+@CKEYFIELDVAL1+''',BIN_ID=''000'''
		...
		BEGIN
			...
			-- UPDATING TRANSACTION TABLE (APD01106) FROM TEMP TABLE
			SET @NSTEP = 110		-- UPDATING TRANSACTION TABLE
				...
				-- DELETING EXISTING ENTRIES FROM PRD_APD01106 TABLE WHERE ROW_ID NOT FOUND IN TEMPTABLE
		...
		EXEC UPDATERFNET 'APP_PRD',@CKEYFIELDVAL1
...
END_PROC:
	...
	BEGIN
		...
		BEGIN
			UPDATE PRD_APM01106 SET LAST_UPDATE=GETDATE() WHERE MEMO_ID=@CKEYFIELDVAL1
			COMMIT TRANSACTION
		END	
		ELSE
			ROLLBACK
