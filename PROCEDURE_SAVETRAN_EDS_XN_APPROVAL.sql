create PROCEDURE SAVETRAN_EDS_XN_APPROVAL
(
...
BEGIN
	...
	SET @CMASTERTABLENAME	= 'EDS_XN_APPROVAL'

	SET @CTEMPMASTERTABLENAME	= 'TEMP_EDS_XN_APPROVAL_'+LTRIM(RTRIM(STR(@NSPID)))
	...
	BEGIN TRY
		...
		WHERE XN_TYPE='EDS' AND INACTIVE=0
		...
		SET @CCMD=N'UPDATE A 
					SET APPROVEDLEVELNO=(CASE WHEN ISNULL(C.APPROVAL_STATUS,0)=1 AND 
  						 ...
  						 FROM EOSSDNM A
  						 JOIN '+@CTEMPMASTERTABLE+' C ON A.MEMO_ID=C.MEMO_ID '
  		PRINT @CCMD  						   
		...
		BEGIN	
			...
			SET @NSTEP = 113	
			SET @CCMD = 'UPDATE A SET A.LEVEL_NO=(CASE WHEN ISNULL(A.LEVEL_NO,0)=0 
			              THEN B.LEVEL_NO ELSE A.LEVEL_NO END),
						 ROW_ID=NEWID() FROM ' +
						 @CTEMPMASTERTABLE + ' A 
						 JOIN XN_APPROVAL_CHECKLIST_LEVEL_USERS B ON A.USER_CODE=B.USER_CODE
						 JOIN XN_APPROVAL_CHECKLIST_LEVELS C ON C.LEVEL_NO=B.LEVEL_NO AND C.XN_TYPE=''EDS'''
...
END
------------------------------------------------------ END OF PROCEDURE SAVETRAN_EDS_XN_APPROVAL
