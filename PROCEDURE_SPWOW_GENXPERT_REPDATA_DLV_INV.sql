CREATE PROCEDURE SPWOW_GENXPERT_REPDATA_DLV_INV  --(lOC 3 DIGIT CHANGE BY sANJAY :22-11-2024 (LEFT CHANGES OF LEFT(inv_ID,2) BY OTHER DEVELOPER)
...
BEGIN
    ...
    Left Outer join BARCODEWISE_EOSS_SCHEMES_INFO EOSSSCH (NOLOCK) ON EOSSSCH.location_id=inm01106.location_code
	...
	LEFT JOIN sku_xfp (NOLOCK) ON sku_xfp.dept_id=INM01106.party_dept_id AND sku_xfp.product_code=ind01106.product_code
	...
	JOIN lm01106 party_lm01106 on party_lm01106.ac_code=TargetLocation.control_ac_code

  	WHERE INM01106.inv_dt BETWEEN [DFROMDT] AND [DTODT]  AND INM01106.inv_mode=2 AND INM01106.cancelled=0
	AND SourceLocation.loc_gst_NO=TargetLocation.loc_gst_NO	AND [WHERE]  group by [GROUPBY]'
	...
	WHERE b.xn_type='WSLOH' AND a.xn_type='DLV_INV'

	
	SELECT TOP 1 @cXntypeSearch=xn_type FROM wow_xpert_xntypes_alias WHERE xn_type_alias='DLV_INV'
	...
	BEGIN
		...
		SET @cBaseExprOutput=REPLACE(@cBaseExprOutput,'sum(ind01106.xn_value_with_gst)','sum(inm01106.'+@cOhCaption+')')
	...
	update #wow_xpert_rep_det SET xn_type='DLV_INV' WHERE xn_type='WSLOH'
END