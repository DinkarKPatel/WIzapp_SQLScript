CREATE PROCEDURE SPWOW_GENXPERT_REPDATA_SNC_PFI
...
BEGIN
	...
	SELECT @cBaseExpr='[LAYOUT_COLS]   from [DATABASE].dbo.snc_barcode_det snc_barcode_det (NOLOCK)       
	JOIN [DATABASE].dbo.snc_det snc_det (NOLOCK) ON snc_det.row_id=snc_barcode_det.REFROW_ID  
	JOIN  [DATABASE].dbo.snc_mst snc_mst (NOLOCK) ON snc_det.memo_ID = snc_mst.memo_ID         
	join SKU_NAMES (NOLOCK) ON sku_names.product_code=snc_barcode_det.PRODUCT_CODE
	JOIN location SourceLocation (NOLOCK) ON SourceLocation.dept_id= snc_mst.location_code/*LEFT(snc_mst.memo_ID,2)*//*Rohit 05-11-2024*/
	JOIN bin SourceBin on SourceBin.bin_id=snc_det.bin_id
	JOIN LOC_NAMES (NOLOCK) on LOC_NAMES.dept_id=snc_mst.location_code/*LEFT(snc_mst.memo_ID,2)*//*Rohit 05-11-2024*/

	    Left Outer join BARCODEWISE_EOSS_SCHEMES_INFO EOSSSCH (NOLOCK) ON EOSSSCH.location_id=snc_mst.location_code/*LEFT(snc_mst.memo_ID,2)*//*Rohit 05-11-2024*/
	and EOSSSCH.PRoduct_code= SKU_NAMES.Product_Code

	JOIN lm01106 party_lm01106 on party_lm01106.ac_code=snc_mst.ac_code
	Left Outer  JOIN lmp01106 party_lmp01106 on party_lmp01106.ac_code=snc_mst.ac_code
	...
	LEFT JOIN XN_ITEM_TYPE_DESC_mst (NOLOCK) on XN_ITEM_TYPE_DESC_mst.XN_ITEM_TYPE=1
  	...
  	WHERE (snc_mst.receipt_dt BETWEEN [DFROMDT] AND [DTODT]) AND snc_mst.cancelled=0 
	 AND [WHERE]       
	group by [GROUPBY]'
