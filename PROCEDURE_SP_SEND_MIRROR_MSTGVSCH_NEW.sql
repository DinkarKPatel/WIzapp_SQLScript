CREATE PROCEDURE DBO.SP_SEND_MIRROR_MSTGVSCH_NEW
(
...
AS
 BEGIN
   DECLARE @DTSQL NVARCHAR(MAX),@CSTEP VARCHAR(10),@CFILTERCONDITION NVARCHAR(MAX)
	...
	SELECT DISTINCT  'MSTGVSCH_GV_SCHEME_MST_MIRROR' AS TARGET_TABLENAME,GV_SCHEME_MST.*,1 AS PICK_FROM_CURRENT_DB FROM GV_SCHEME_MST (NOLOCK)
    ...
    SELECT DISTINCT  'MSTGVSCH_GV_SCHEME_1_MIRROR' AS TARGET_TABLENAME,GV_SCHEME_1.*,1 AS PICK_FROM_CURRENT_DB FROM GV_SCHEME_1 (NOLOCK)
	...
	SELECT DISTINCT  'MSTGVSCH_GV_SCHEME_LOCS_MIRROR' AS TARGET_TABLENAME,GV_SCHEME_LOCS.*,1 AS PICK_FROM_CURRENT_DB FROM GV_SCHEME_LOCS (NOLOCK)
	...
	BEGIN CATCH
	    SET @CERRORMSG='P: SP_SEND_MIRROR_MSTGVSCH_NEW, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()+ 'ERROR QUERY || '+@DTSQL
	    GOTO END_PROC
    END CATCH 
