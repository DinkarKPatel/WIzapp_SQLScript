CREATE PROCEDURE SP3S_Reporting_expressions_New
AS
BEGIN
	...
	DELETE FROM xtreme_reports_exp_new 
	
	

	INSERT xtreme_reports_exp_new (master_col,loc_join_col,bin_join_col,XN_NO_COL,XN_DT_COL,base_expr)
	...
	SELECT 'pending_JOBWORK_TRADING_qty_opt' as master_col,'a.dept_id' as loc_join_col,'a.bin_id' as bin_join_col,'' as xn_no_col,'' as xn_dt_col,'[LAYOUT_COL],
	 ...
	 SELECT 'ops_qty_pmt' as master_col,'a.dept_id' as loc_join_col,'a.bin_id' as bin_join_col,'' as xn_no_col,'' as xn_dt_col,'[LAYOUT_COL],    
	  ...
	  SUM(a.quantity_in_stock*sku_names.pp)as OBP1,	 
	...
	INSERT xtreme_reports_exp_new (master_col,loc_join_col,bin_join_col,XN_NO_COL,XN_DT_COL,base_expr)
	 ...
	 INSERT xtreme_reports_exp_new (master_col,loc_join_col,bin_join_col,XN_NO_COL,XN_DT_COL,base_expr)    --(-1) avilable inside CHANGES MADE BY CHANDAN ON 24-06-2019
	...
	group by [GROUPBY]' AS base_expr    --- 3ecs
	 ...
	 INSERT xtreme_reports_exp_new (master_col,loc_join_col,bin_join_col,xnparty_join_col,XN_NO_COL,XN_DT_COL,base_expr)    
	...
	'''LM''+B.AC_CODE' as xnparty_join_col,'b.mrr_no' as xn_no_col,'b.receipt_dt' as xn_dt_col,'[LAYOUT_COL],[CALCULATIVE_COL]    
	 ...
	 IF @bHoloc=0
		...
		'''LM''+B.AC_CODE' as xnparty_join_col,'b.mrr_no' as xn_no_col,'b.receipt_dt' as xn_dt_col,'[LAYOUT_COL],[CALCULATIVE_COL]    
		 from [DATABASE].dbo.PID01106 A WITH(NOLOCK)    
		 JOIN  [DATABASE].dbo.PIM01106 B WITH(NOLOCK) ON A.MRR_ID = B.MRR_ID
		 LEFT OUTER JOIN [DATABASE].dbo.PARCEL_DET PD  WITH(NOLOCK) ON B.INV_ID = PD.REF_MEMO_ID 
         LEFT OUTER JOIN [DATABASE].dbo.parcel_mst  WITH(NOLOCK) on PD.parcel_memo_id = parcel_mst.parcel_memo_id 
		  AND parcel_mst.CANCELLED=0
         LEFT OUTER JOIN [DATABASE].dbo.ANGM  WITH(NOLOCK)  ON parcel_mst.angadia_code = ANGM.Angadia_code 
		 
		 [JOIN]
		 WHERE b.RECEIPT_DT BETWEEN [DFROMDT] AND [DTODT]  AND  B.CANCELLED = 0
		 and ISNULL(parcel_mst.xn_type,''WSL'')= ''WSL'' 
		  AND b.inv_mode=2 AND [WHERE] 
		 group by [GROUPBY]' AS base_expr
	ELSE
		 INSERT xtreme_reports_exp_new (master_col,loc_join_col,bin_join_col,xnparty_join_col,XN_NO_COL,XN_DT_COL,base_expr)    
		...
		from [DATABASE].dbo.InD01106 A WITH(NOLOCK)    
		 JOIN  [DATABASE].dbo.PIM01106 B WITH(NOLOCK) ON A.INV_ID = B.INV_ID
		 JOIN  [DATABASE].dbo.INM01106 c WITH(NOLOCK) ON c.inv_ID = a.inv_ID	 		 
		LEFT OUTER JOIN [DATABASE].dbo.PARCEL_DET PD  WITH(NOLOCK) ON A.INV_ID = PD.REF_MEMO_ID 
         LEFT OUTER JOIN [DATABASE].dbo.parcel_mst  WITH(NOLOCK) on PD.parcel_memo_id = parcel_mst.parcel_memo_id 
		 AND parcel_mst.CANCELLED=0
         LEFT OUTER JOIN [DATABASE].dbo.ANGM  WITH(NOLOCK)  ON parcel_mst.angadia_code = ANGM.Angadia_code 
	...
	[JOIN]
	 ...
	 INSERT xtreme_reports_exp_new (master_col,loc_join_col,loc_join_col_2,bin_join_col,bin_join_col_2,
	...
	[JOIN_2]
	 ...
	 from [DATABASE].dbo.RMD01106 A WITH(NOLOCK)
	 JOIN  [DATABASE].dbo.RMM01106 B WITH(NOLOCK) ON A.RM_ID = B.RM_ID   
	 LEFT OUTER JOIN [DATABASE].dbo.PARCEL_DET PD  WITH(NOLOCK) ON B.RM_ID = PD.REF_MEMO_ID 
         LEFT OUTER JOIN [DATABASE].dbo.parcel_mst  WITH(NOLOCK) on PD.parcel_memo_id = parcel_mst.parcel_memo_id 
		   AND parcel_mst.CANCELLED=0
         LEFT OUTER JOIN [DATABASE].dbo.ANGM  WITH(NOLOCK)  ON parcel_mst.angadia_code = ANGM.Angadia_code 
	...
	[JOIN]
	 ...
	 AND ISNULL(PARCEL_MST.XN_TYPE,''PRT'')= ''PRT''
	 AND [WHERE]  
	 group by [GROUPBY]' AS base_expr
	...
	[JOIN]
	 ...
	 -- APPROVAL RETURN    
	INSERT xtreme_reports_exp_new (master_col,loc_join_col,bin_join_col,XN_NO_COL,XN_DT_COL,base_expr,product_code_col)    
	 ...
	 [JOIN]
	 WHERE c.memo_dt BETWEEN [DFROMDT] AND [DTODT]  AND c.cancelled=0   
	 AND  [WHERE]    
	...
	group by [GROUPBY]' AS base_expr,'APD_product_code' as product_code_col
	 ...
	 INSERT xtreme_reports_exp_new (master_col,loc_join_col,bin_join_col,xnparty_join_col,XN_NO_COL,XN_DT_COL,base_expr,product_code_col)    
	...
	'b.memo_no' as xn_no_col_2,'b.memo_dt' as xn_dt_col_2,'[LAYOUT_COL],[CALCULATIVE_COL_2]    
	 ...
	 INSERT xtreme_reports_exp_new (master_col,loc_join_col,bin_join_col,XN_NO_COL,XN_DT_COL,
	...
	group by [GROUPBY]' AS base_expr
	 ...
	 INSERT xtreme_reports_exp_new (master_col,loc_join_col,loc_join_col_2,bin_join_col,bin_join_col_2,  --(-1) avilable inside CHANGES MADE BY CHANDAN ON 24-06-2019
	...
	group by [GROUPBY_2]' AS base_expr
	 ...
	 INSERT xtreme_reports_exp_new (master_col,loc_join_col,bin_join_col,XN_NO_COL,XN_DT_COL,base_expr) --(-1) avilable inside 
	...
	INSERT xtreme_reports_exp_new (master_col,loc_join_col,loc_join_col_2,loc_join_col_3,loc_join_col_4, --(-1) avilable inside 
	...
	INSERT xtreme_reports_exp_new (master_col,loc_join_col,loc_join_col_2,loc_join_col_3,loc_join_col_4,loc_join_col_5,--(-1) avilable inside 
	...
	INSERT xtreme_reports_exp_new (master_col,loc_join_col,bin_join_col,
	...
	INSERT xtreme_reports_exp_new (master_col,loc_join_col,loc_join_col_2,bin_join_col,bin_join_col_2,xnparty_join_col,xnparty_join_col_2,
	...
	---- Net Debit note Pack Slip Qty  --(-1) avilable inside CHANGES MADE BY CHANDAN ON 24-06-2019
	  INSERT xtreme_reports_exp_new (master_col,loc_join_col,loc_join_col_2,bin_join_col,bin_join_col_2,xnparty_join_col,
	  xnparty_join_col_2,
	  XN_NO_COL,XN_DT_COL,XN_NO_COL_2,XN_DT_COL_2,base_expr)    
	  --CREATE NONCLUSTERED INDEX IX_PSID_RMD_INCL ON [dbo].[rmd01106] ([PS_ID]) INCLUDE ([product_code],[quantity],[rm_id],[RFNET],[item_tax_amount],[BIN_ID])
 ...
 -- CONSUMPTION OF BARCODES IN SPLIT/COMBINE     --(-1) avilable inside CHANGES MADE BY CHANDAN ON 24-06-2019
	...
	group by [GROUPBY]' AS base_expr
	 ...
	 -- OPS JOB WORK ISSUE     --(-1) avilable inside CHANGES MADE BY CHANDAN ON 24-06-2019
	  INSERT xtreme_reports_exp_new (master_col,loc_join_col,bin_join_col,XN_NO_COL,XN_DT_COL,xnparty_join_col,base_expr)     
	...
	group by [GROUPBY]' AS base_expr
	 ...
	 -- CBS JOB WORK ISSUE    --(-1) avilable inside CHANGES MADE BY CHANDAN ON 24-06-2019 
	  INSERT xtreme_reports_exp_new (master_col,loc_join_col,bin_join_col,XN_NO_COL,XN_DT_COL,xnparty_join_col,base_expr)     --(-1) avilable inside 
	...
	group by [GROUPBY]' AS base_expr
	  ...
	  INSERT xtreme_reports_exp_new (master_col,loc_join_col,bin_join_col,xnparty_join_col,XN_NO_COL,XN_DT_COL,base_expr)     --(-1) avilable inside 
 ...
 --(-1) avilable inside CHANGES MADE BY CHANDAN ON 24-06-2019
	INSERT xtreme_reports_exp_new (master_col,loc_join_col,loc_join_col_2,bin_join_col,bin_join_col_2,xnparty_join_col,xnparty_join_col_2,XN_NO_COL,XN_DT_COL,XN_NO_COL_2,XN_DT_COL_2,base_expr)     --(-1) avilable inside 
	...
	--WSL xtreme_reports_exp_new ADDED BY CHANDAN ON 26-06-2019
	...
	--WSR xtreme_reports_exp_new ADDED BY CHANDAN ON 26-06-2019
