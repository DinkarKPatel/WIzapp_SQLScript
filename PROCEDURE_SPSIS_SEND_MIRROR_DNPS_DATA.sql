create PROCEDURE SPSIS_SEND_MIRROR_DNPS_DATA
(
...
LBLTABLEINFO:
	...
	SET @CSTEP=60
	SELECT @cCols=COALESCE(@cCols+',','')+colname 	FROM ebo_xnssending_cols WHERE tablename='DNPS_MST' 

	SET @cCmdCols=N'
	   SELECT ''DNPS_DNPS_MST_upload'' AS TARGET_TABLENAME,'+@cCols+' FROM DNPS_MST(NOLOCK)   WHERE PS_ID='''+@CMEMOID+''''
	PRINT @cCmdCols
	...
	SET @CSTEP=65	
	SET @cCols=NULL
	SELECT @cCols=COALESCE(@cCols+',','')+'DNPS_DET.'+colname FROM ebo_xnssending_cols	WHERE tablename='DNPS_DET' 

	SET @cCOls=REPLACE(@cCOls,'DNPS_DET.PRODUCT_CODE','CASE WHEN ISNULL(C.VENDOR_EAN_NO,'''')<>'''' THEN C.VENDOR_EAN_NO ELSE DNPS_DET.PRODUCT_CODE END AS PRODUCT_CODE')

	PRINT @cCOls
	SET @cCmdCols=N'
		SELECT ''DNPS_DNPS_DET_UPLOAD'' AS TARGET_TABLENAME,'+@cCols+' 
		FROM DNPS_DET  (NOLOCK)   
		JOIN SKU C (NOLOCK) ON C.PRODUCT_CODE=DNPS_DET.PRODUCT_CODE
		JOIN LMP01106 LMP (NOLOCK) ON LMP.AC_CODE =C.AC_CODE
		WHERE SUBSTRING(LMP.AC_GST_NO,3,10)='''+@CPANNO+'''
		AND DNPS_DET.PS_ID='''+@CMEMOID+''''
	...
	EXEC SPSIS_GET_CURSOR_SKUNAMES @cXNTYPE='DNPS',@cTABLENAME='DNPS_DET',@cKEYNAME='PS_ID',@cKEYVALUE=@CMEMOID,@CERRMSG=@CERRMSG OUTPUT 
	IF ISNULL(@CERRMSG,'')<>'' 
		GOTO END_PROC
...
BEGIN CATCH
	SET @CERRMSG='P: SPSIS_SEND_MIRROR_DNPS_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
