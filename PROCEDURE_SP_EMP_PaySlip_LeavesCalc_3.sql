CREATE PROCEDURE SP_EMP_PAYSLIP_LEAVESCALC_3                      
(                      
 @CEMPID VARCHAR(20),                       
 @NYEAR INT,                       
 @NMONTH INT,                      
...
BEGIN                      
 ...
 BEGIN TRY                      
	 ...
	 WHERE A.TIME_IN > B.SHIFT_TIME_IN AND A.TIME_IN <= B.LATE_CUTOFF AND FL_COUNT=0                      
	                      
	 UPDATE A SET SL_COUNT=SL_COUNT+1 FROM @TATTENDANCE A JOIN EMP_SHIFTS B ON A.SHIFT_ID=B.SHIFT_ID                      
	 WHERE A.TIME_OUT < B.SHIFT_TIME_OUT AND A.TIME_OUT >= B.EARLY_CUTOFF AND FL_COUNT=0                      
...
--	 DEALLOCATE LEAVESCUR    
	 ...
	 SELECT @NCURDRFL4=COUNT(*) FROM   EMP_ATTENDANCE WHERE EMP_ID=@CEMPID
		AND ATTENDANCE_DT BETWEEN @DFROMDT AND @DTODT AND DATENAME(W,ATTENDANCE_DT)=@CWEEKLYOFF1
...
END               
--*************************************** END OF PROCEDURE SP_EMP_PAYSLIP_LEAVESCALC_3
