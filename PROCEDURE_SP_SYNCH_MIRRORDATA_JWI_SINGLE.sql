CREATE PROCEDURE SP_SYNCH_MIRRORDATA_JWI_SINGLE
(
    @CLOCID VARCHAR(3)
...
BEGIN TRY
	...
	SELECT @CTEMPMASTERTABLE=@CSOURCEDB+'JWI_JOBWORK_ISSUE_MST_MIRROR',
		   @CTEMPDETAILTABLE=@CSOURCEDB+'JWI_JOBWORK_ISSUE_DET_MIRROR',
		   @CTEMPDETAILTABLE1=@CSOURCEDB+'JWI_PMT01106_MIRROR'
...
LBLSTART:
    ...
    FROM JWI_JOBWORK_ISSUE_MST_MIRROR B (NOLOCK)
    LEFT OUTER JOIN MIRROR_SYNCH_LOG C (NOLOCK) ON C.MEMO_ID=B.ISSUE_ID AND C.XN_TYPE='JWI'
	...
	BEGIN
		...
		SELECT @CFINYEAR=FIN_YEAR FROM JWI_JOBWORK_ISSUE_MST_MIRROR (NOLOCK) WHERE ISSUE_ID=@CMEMOID
...
LBLMERGE:
	...
	SET @CTMP_TABLENAME='JWI_JOBWORK_ISSUE_MST_MIRROR'
	...
	SET @CTMP_TABLENAME='JWI_JOBWORK_ISSUE_DET_MIRROR'
	...
	@cTempTable='JWI_PMT01106_MIRROR',  
	@cMemoIdCol='JWI_MEMO_ID',  
	@cMemoId =@CMEMOID

    
    SET @CSTEP=330
    SELECT TOP 1 @CERRMSG=ERRMSG FROM MIRROR_SYNCH_LOG (NOLOCK) WHERE XN_TYPE='JWI' AND MEMO_ID=@CMEMOID
    
    IF ISNULL(@CERRMSG,'')=''
    BEGIN 
		SET @CSTEP=340
	    		
		DELETE A FROM JWI_JOBWORK_ISSUE_MST_MIRROR A WHERE A.ISSUE_ID=@CMEMOID AND A.VERSION_NO<=@NVERSIONNO

		DELETE A FROM JWI_JOBWORK_ISSUE_DET_MIRROR A WHERE A.ISSUE_ID=@CMEMOID AND A.VERSION_NO<=@NVERSIONNO
		DELETE A FROM JWI_PMT01106_MIRROR A WHERE A.JWI_MEMO_ID=@CMEMOID
...
BEGIN CATCH
	SET @CERRMSG='P:SP_SYNCH_MIRRORDATA_JWI_SINGLE, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO EXIT_PROC
END CATCH
