CREATE PROCEDURE SAVETRAN_MIS_PRD
(
...
BEGIN
	...
	BEGIN TRY
		...
		SET @CTEMPDETAILTABLENAME	= 'TEMP_PRD_ISSUE_MATERIAL_DET_'+LTRIM(RTRIM(STR(@NSPID)))					
		SET @CTEMPDETAILTABLENAME1	= 'TEMP_PRD_ISSUE_ROW_MATERIAL_DET_'+LTRIM(RTRIM(STR(@NSPID)))			
		...
		BEGIN
			...
			SET @CCMD = N'UPDATE PRD_ISSUE_MATERIAL_MST SET CANCELLED = 1,LAST_UPDATE=GETDATE() ' + 
		...
		SET @CMASTERTABLENAME	= 'PRD_ISSUE_MATERIAL_MST'
		SET @CDETAILTABLENAME	= 'PRD_ISSUE_MATERIAL_DET'
        SET @CDETAILTABLENAME1	= 'PRD_ISSUE_ROW_MATERIAL_DET'
        
		SET @CTEMPMASTERTABLENAME	= 'TEMP_PRD_ISSUE_MATERIAL_MST_'+LTRIM(RTRIM(STR(@NSPID)))
...
PRINT 'GENERATING NEW KEY... START'			
		...
		---------------
		  ...
		  SET @CCMD = N'UPDATE B SET REF_MATERIAL_ROW_ID=A.ROW_ID FROM ' + @CTEMPDETAILTABLE1 + ' A
		  JOIN ' + @CTEMPDETAILTABLE + ' B ON A.OLD_ROW_ID=B.REF_MATERIAL_ROW_ID' 
		  PRINT @CCMD 
          EXEC SP_EXECUTESQL @CCMD  
              
              
          SET @CCMD = N'DELETE  A  FROM ' + @CDETAILTABLENAME1 + ' A
       ...
       ----------------
		...
		EXEC SAVETRAN_UPDSKU_PRD 'MIS',@CKEYFIELDVAL1,@CERRORMSG OUTPUT 
		IF ISNULL(@CERRORMSG,'')<>''
				GOTO END_PROC
