CREATE  VIEW VW_AGENCY_ISSUE_MATERIAL_LIST_PENDING      
AS       
SELECT        
...
(      
  ...
  FROM PRD_AGENCY_ISSUE_MATERIAL_DET_PENDING A (NOLOCK)       
  JOIN PRD_AGENCY_ISSUE_MATERIAL_MST_PENDING B (NOLOCK) ON A.MEMO_ID=B.MEMO_ID     
  ...
  (     
   ...
   JOIN PRD_AGENCY_ISSUE_MATERIAL_DET_PENDING  C (NOLOCK) ON C.ROW_ID=A.REF_ROW_ID      
   WHERE B.CANCELLED=0     
