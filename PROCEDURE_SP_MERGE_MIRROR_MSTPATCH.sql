CREATE PROCEDURE DBO.SP_MERGE_MIRROR_MSTPATCH
  (@NMODE INT
  ,@XN_TYPE VARCHAR(50) ='MSTPATCH'
  )
AS
 BEGIN
...
BEGIN TRY
	  ...
	  BEGIN
			SET @CERRORMSG='P:SP_MERGE_MIRROR_MSTPATCH, CANNOT CALL MERGING PROC AT HEAD OFFICE'
			GOTO END_PROC
  	  END					
	 ...
	 --INSERT DATA INTO LOCATION TABLE
	   ...
	   SET @CSOURCETABLE ='MSTPATCH_PATCH_MRP_SETUP_MIRROR'
	   SET @CKEYFIELD1 ='OLD_MRP'
	   EXEC UPDATEMASTERXN
...
BEGIN CATCH
	SET @CERRORMSG='P:SP_MERGE_MIRROR_MSTPATCH, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
...
END_PROC:
	...
	IF ISNULL(@CERRORMSG,'')='' 
	BEGIN   
	   --DROP TEMP TABLE RELATED TO DTM
	 DELETE FROM MSTPATCH_PATCH_MRP_SETUP_MIRROR
...
--END PROCEDURE SP_MERGE_MIRROR_MSTPATCH
