CREATE PROCEDURE SP_PRD_SHORT_RAW_MATERIAL
(
...
IF OBJECT_ID('TEMPDB..#TMPORDER_RM_REQ','U') IS NOT NULL
   DROP TABLE #TMPORDER_RM_REQ
	...
	SELECT   MST.MEMO_ID AS ORDER_ID
		   ...
		   ,ISNULL(CONVERT(NUMERIC(12,2),(AVG_QTY+ISNULL(A.ADD_AVG_QTY,0)) * ISNULL(CT.ISSUE_QTY,0)),0) AS REQUIRED_QTY 
	...
	JOIN
	(
	 SELECT A.MEMO_ID, A.ARTICLE_CODE ,A.PARA1_CODE,A.PARA2_CODE,B.REF_WO_ID,
	  SUM(A.ISSUE_QTY) AS ISSUE_QTY
	 FROM PRD_STK_TRANSFER_MATERIAL_DET A
	 JOIN
	 (
	   SELECT A.REF_WO_ID ,A.MEMO_ID
	   FROM PRD_STK_TRANSFER_DET A
	   JOIN PRD_STK_TRANSFER_MST B ON A.MEMO_ID=B.MEMO_ID
	   WHERE B.CANCELLED =0
	   GROUP BY A.REF_WO_ID ,A.MEMO_ID
	 ) B ON A.MEMO_ID=B.MEMO_ID
	 WHERE (@CORDER_ID='' OR  B.REF_WO_ID=@CORDER_ID)
	 GROUP BY A.MEMO_ID,A.ARTICLE_CODE ,A.PARA1_CODE,A.PARA2_CODE,B.REF_WO_ID
...
IF OBJECT_ID('TEMPDB..#TMPORDER_RM_REQ_CUTTING','U') IS NOT NULL
   DROP TABLE #TMPORDER_RM_REQ_CUTTING
   
	SELECT 'PENDING FOR CUTTING' AS TRAN_TYPE,  A.*,
	       ISNULL(T.QUANTITY,0) AS ISSUE_QTY,
	       A.REQUIRED_QTY-ISNULL(T.QUANTITY,0) AS PENDING_QTY
	INTO #TMPORDER_RM_REQ_CUTTING
	...
	(
	 ...
	 FROM
	 (
	 SELECT A.REF_WO_ID,A.MEMO_ID, A.COMPONENT_CODE,C.ARTICLE_CODE,A.COM_PARA1_CODE,A.COM_PARA2_CODE,AR.UOM_CODE,
	 SUM(QUANTITY) AS  QUANTITY
	 FROM PRD_STK_TRANSFER_DET A
	 JOIN PRD_STK_TRANSFER_MST B ON A.MEMO_ID=B.MEMO_ID
	 JOIN PRD_SKU C ON A.NEW_PRODUCT_UID=C.PRODUCT_UID
	 JOIN ARTICLE AR ON AR.ARTICLE_CODE=C.ARTICLE_CODE
	 WHERE B.CANCELLED=0
	 GROUP BY A.REF_WO_ID,A.MEMO_ID,A.COMPONENT_CODE,C.ARTICLE_CODE,A.COM_PARA1_CODE,A.COM_PARA2_CODE,AR.UOM_CODE
	 UNION ALL
	 SELECT A.REF_WO_ID,A.REF_CUTTING_ID AS MEMO_ID,A.COMPONENT_CODE,C.ARTICLE_CODE,A.COM_PARA1_CODE,A.COM_PARA2_CODE,AR.UOM_CODE,
	 SUM(QUANTITY) AS  QUANTITY
	 ...
	 JOIN PRD_SKU C ON A.NEW_PRODUCT_UID=C.PRODUCT_UID
	  JOIN ARTICLE AR ON AR.ARTICLE_CODE=C.ARTICLE_CODE
	 WHERE B.CANCELLED=0
	 GROUP BY A.REF_WO_ID,A.REF_CUTTING_ID,A.COMPONENT_CODE,C.ARTICLE_CODE,A.COM_PARA1_CODE,A.COM_PARA2_CODE,AR.UOM_CODE
	 ) A
...
IF OBJECT_ID('TEMPDB..#TMPISSUE_RM_REQ_DETAILS','U') IS NOT NULL
   DROP TABLE #TMPISSUE_RM_REQ_DETAILS
	...
	SELECT *FROM #TMPORDER_RM_REQ_CUTTING 
	
	--SELECT * FROM #TMPORDER_RM_REQ A
