CREATE PROCEDURE SPWOW_GENXPERT_REPDATA_WPSR
...
BEGIN
	...
	JOIN [DATABASE].dbo.INM01106  (nolock) on INM01106.INV_ID=wps_mst.wsl_inv_id  
	...
	LEFT OUTER JOIN location TargetLocation on TargetLocation.dept_id=wps_mst.party_dept_id
	...
	JOIN lm01106 party_lm01106 on party_lm01106.ac_code=wps_mst.ac_code
   ...
   LEFT JOIN lm01106 supplier_lm01106 on supplier_lm01106.ac_code=SKU_NAMES.ac_code
	...
	WHERE inm01106.inv_dt BETWEEN [DFROMDT] AND [DTODT]  AND wps_mst.cancelled=0 
	AND INM01106.cancelled=0   AND ISNULL(INM01106.PENDING_GIT,0)=0
	AND [WHERE]       
	group by [GROUPBY]'
