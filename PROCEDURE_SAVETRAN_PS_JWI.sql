...
create PROCEDURE SAVETRAN_PS_JWI--(LocId 3 digit change by Sanjay:06-11-2024)
(
...
BEGIN
	...
	SET @CDETAILTABLENAME	= 'POST_SALES_JOBWORK_ISSUE_DET'
	...
	BEGIN TRY
		...
		BEGIN
			...
			SELECT TOP 1 @cLocationId=location_code FROM POST_SALES_JOBWORK_ISSUE_MST (NOLOCK) WHERE issue_id=@CMEMOID
			

			 	
			EXEC VALIDATEXN_BEFORE_EDIT 'JWI',@CMEMOID,0,@CWIZAPPUSERCODE,@CERRORMSG OUTPUT,@CEXPRERRORMSG OUTPUT,@CLOCATIONID
		...
		BEGIN	
		   ...
		   IF ISNULL(@CLOCATIONID,'')=''
			 BEGIN
				SET @CERRORMSG = 'STEP- ' + LTRIM(STR(@NSTEP)) + ' 1.LOCATION ID CAN NOT BE BLANK  '  
				GOTO END_PROC    
			 END
		...
		BEGIN				-- START OF EDITMODE
			...
			BEGIN
				...
				-- UPDATING SENTTOHO FLAG
				SET @CCMD = N'UPDATE ' + @CTEMPMASTERTABLE + ' SET SENT_TO_HO = 0,LAST_UPDATE=GETDATE(),bin_id=''999'''
				EXEC SP_EXECUTESQL @CCMD
...
END_PROC:
	...
	UPDATE post_sales_jobwork_issue_mst WITH (ROWLOCk) SET last_update=getdate() WHERE issue_id=@CKEYFIELDVAL1

	IF @@TRANCOUNT>0
	BEGIN
		IF ISNULL(@CERRORMSG,'')='' AND ISNULL(@CCMDOUTPUT,'')='' AND ISNULL(@BNEGSTOCKFOUND,0)=0
		BEGIN
		    UPDATE post_sales_jobwork_issue_mst WITH (ROWLOCk) SET HO_SYNCH_LAST_UPDATE='' WHERE issue_id=@CKEYFIELDVAL1 
			commit TRANSACTION

			exec SP3S_INSUPDATE_ITEM_STATUS 'PSJWI',@CKEYFIELDVAL1,@NUPDATEMODE
		END
		ELSE
			ROLLBACK
	END
	...
	EXEC SP_DROPTEMPTABLES_XNS 'XNSPSJWI',@NSPID	
END		
------------------------------------------------------ END OF PROCEDURE SAVETRAN_PS_JWI
