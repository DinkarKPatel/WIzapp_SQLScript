create PROCEDURE SPSIS_SEND_MIRROR_APR_DATA
(
...
LBLTABLEINFO:
	...
	SET @CSTEP=60
	SELECT @cCols=COALESCE(@cCols+',','')+colname 	FROM ebo_xnssending_cols WHERE tablename='approval_return_mst' 

	SET @cCmdCols=N'
	   SELECT ''APR_APPROVAL_RETURN_MST_UPLOAD'' AS TARGET_TABLENAME,'+@cCols+' FROM approval_return_mst  (NOLOCK)   WHERE MEMO_ID='''+@CMEMOID+''''
	PRINT @cCmdCols
	...
	SET @CSTEP=65	
	SET @cCols=NULL
	SELECT @cCols=COALESCE(@cCols+',','')+'approval_return_det.'+colname FROM ebo_xnssending_cols	WHERE tablename='approval_return_det' 

	SET @cCOls=REPLACE(@cCOls,'APPROVAL_RETURN_DET.APD_PRODUCT_CODE','CASE WHEN ISNULL(C.VENDOR_EAN_NO,'''')<>'''' THEN C.VENDOR_EAN_NO ELSE APPROVAL_RETURN_DET.APD_PRODUCT_CODE END AS APD_PRODUCT_CODE')
	PRINT @cCOls
	SET @cCmdCols=N'
		SELECT ''APR_APPROVAL_RETURN_DET_UPLOAD'' AS TARGET_TABLENAME,'+@cCols+' 
		FROM approval_return_det  (NOLOCK)   
		JOIN SKU C (NOLOCK) ON C.PRODUCT_CODE=approval_return_det.APD_PRODUCT_CODE
		JOIN LMP01106 LMP (NOLOCK) ON LMP.AC_CODE =C.AC_CODE
		WHERE SUBSTRING(LMP.AC_GST_NO,3,10)='''+@CPANNO+'''
		AND approval_return_det.MEMO_ID='''+@CMEMOID+''''
	
	PRINT @cCmdCols

	EXEC SP_EXECUTESQL @cCmdCols

	EXEC SPSIS_GET_CURSOR_SKUNAMES @cXNTYPE='APR',@cTABLENAME='APPROVAL_RETURN_DET',@cKEYNAME='MEMO_ID',@cKEYVALUE=@CMEMOID,@CERRMSG=@CERRMSG OUTPUT 
	IF ISNULL(@CERRMSG,'')<>'' 
		GOTO END_PROC
...
BEGIN CATCH
	SET @CERRMSG='P: SPSIS_SEND_MIRROR_APR_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
