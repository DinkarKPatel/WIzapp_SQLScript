CREATE PROC SP_INSERT_MIRRORLOG
...
BEGIN TRY
	DECLARE @XN_TYPE VARCHAR(50),@T_NAME VARCHAR(50),@P_COLUMN VARCHAR(50),@D_COLUMN VARCHAR(50),
			@ORDERED INT, @QRY NVARCHAR(MAX) ,@CCURLOCID VARCHAR(10),@CHOLOCID VARCHAR(10),@CCUTOFFDATE VARCHAR(20)
	...
	SET @CCUTOFFDATE=(CASE WHEN @DCUTOFFDATE='' THEN CONVERT(VARCHAR,DATEADD(DAY,-15,GETDATE()),110)
					       ELSE CONVERT(VARCHAR,@DCUTOFFDATE,110) END)	
	...
	SELECT 'DOCPO','POM01106','PO_ID','PO_DT',20 UNION
	...
	SELECT 'CUS','CUSTDYM','CUSTOMER_CODE','LAST_UPDATE',260  UNION
	SELECT 'STREC','STK_RECON_HIST_MST','RECON_ID','LAST_UPDATE',260  UNION  
	SELECT 'PSHBD','HOLD_BACK_DELIVER_MST','MEMO_ID','LAST_UPDATE',270  UNION  
	SELECT 'PSJWI','POST_SALES_JOBWORK_ISSUE_MST','ISSUE_ID','LAST_UPDATE',270  UNION  
	SELECT 'PSJWR','POST_SALES_JOBWORK_RECEIPT_MST','RECEIPT_ID','LAST_UPDATE',280  UNION  
	SELECT 'PSDLV','SLS_DELIVERY_MST','MEMO_ID','LAST_UPDATE',290 UNION 
	SELECT 'DSM','DSM01106','DS_ID','LAST_UPDATE',300 

	
	DELETE FROM MIRROR_XNS_PENDING
	IF CURSOR_STATUS('GLOBAL','CURR_MIRROR') IN (0,1)  
	BEGIN  
	  CLOSE CURR_MIRROR  
	  DEALLOCATE CURR_MIRROR  
	END
    EXEC SP_EXECUTESQL SP_INSERT_MIRROR_RECON_LOG  
	BEGIN TRAN
	...
	BEGIN
		SET @QRY = N'UPDATE B  SET LAST_UPDATE=A.LAST_UPDATE FROM MIRRORLOG B
					JOIN  '+@T_NAME+' A ON A.'+@P_COLUMN+'=B.XN_ID AND B.XN_TYPE='''+@XN_TYPE+'''
					WHERE A.'+@D_COLUMN+' < '''+@CCUTOFFDATE+''' AND 
		...
		SET @QRY = N'INSERT INTO MIRRORLOG  (XN_TYPE,XN_ID,LAST_UPDATE)
					SELECT '''+@XN_TYPE+''' AS XN_TYPE,A.'+@P_COLUMN+' AS XN_ID ,A.LAST_UPDATE FROM 
					'+@T_NAME+' A LEFT OUTER JOIN MIRRORLOG B ON A.'+@P_COLUMN+'=B.XN_ID AND B.XN_TYPE='''+@XN_TYPE+'''
					WHERE A.'+@D_COLUMN+' < '''+@CCUTOFFDATE+''' AND B.XN_ID IS NULL'+
					(CASE WHEN @XN_TYPE='PUR' THEN ' AND (A.INV_MODE<>2 OR A.RECEIPT_DT<>'''')' ELSE '' END)+ 
					(CASE WHEN @XN_TYPE='WSR' THEN ' AND (A.MODE<>2 OR A.RECEIPT_DT<>'''')' ELSE '' END)
