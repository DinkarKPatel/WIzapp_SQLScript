CREATE PROCEDURE SPWOW_GENXPERT_REPDATA_UNC
...
BEGIN
	...
	JOIN location SourceLocation (NOLOCK) ON SourceLocation.dept_id= icm01106.location_code/*LEFT(icm01106.cnc_memo_ID,2)*//*Rohit 05-11-2024*/
	JOIN bin SourceBin on SourceBin.bin_id=icd01106.bin_id
	JOIN LOC_NAMES (NOLOCK) on LOC_NAMES.dept_id=icm01106.location_code/*LEFT(icm01106.cnc_memo_ID,2)*//*Rohit 05-11-2024*/

	    Left Outer join BARCODEWISE_EOSS_SCHEMES_INFO EOSSSCH (NOLOCK) ON EOSSSCH.location_id=icm01106.location_code/*LEFT(icm01106.cnc_memo_ID,2)*//*Rohit 05-11-2024*/
	...
	LEFT JOIN sku_current_xfp (NOLOCK) ON sku_current_xfp.dept_id=icm01106.location_code/*LEFT(icm01106.cnc_memo_ID,2)*//*Rohit 05-11-2024*/ AND sku_current_xfp.product_code=icd01106.PRODUCT_CODE
    ...
    left outer join loc_names groupSuppler on groupSuppler.dept_id= sku_xfp.challan_source_location_code
	 ...
	 AND icm01106.cnc_type=2   AND ISNULL(icm01106.stock_adj_note,0)=0 AND icm01106.cancelled=0 
	 AND [WHERE]       
	group by [GROUPBY]'
