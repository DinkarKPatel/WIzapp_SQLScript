...
,T1.FIN_YEAR ,COM.COMPANY_CODE ,T1.MEMO_TYPE,L.DEPT_ID  AS LOC_ID,L.DEPT_NAME AS LOC_NAME ,
 F.FORM_NAME AS TAX_TYPE,T2.TAX_PERCENTAGE ,T2.TAX_AMOUNT,ISNULL(T10.NO_OF_BOXES,0) AS NO_OF_BOXES    
FROM INM01106 T1
...
(
 ...
 FROM IND01106 GROUP BY INV_ID,ITEM_FORM_ID,ITEM_TAX_PERCENTAGE
 ) T2 ON T2.INV_ID=T1.INV_ID 
 
LEFT OUTER JOIN   
(  
  SELECT  COUNT(DISTINCT BOX_NO) AS NO_OF_BOXES,INV_ID FROM IND01106 GROUP BY INV_ID
