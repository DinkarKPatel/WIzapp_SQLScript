CREATE PROCEDURE SP3S_AUTOPOSRECO_APP
@nMode NUMERIC(1,0),
@cLocIdPara CHAR(2)='',
...
BEGIN TRY
	...
	BEGIN
		...
		ISNULL(total_quantity,0) as quantity,'APP' as XN_TYPE,a.LAST_UPDATE
	...
	BEGIN
		...
		LEFT OUTER JOIN apm01106 b (NOLOCK)	ON a.memo_id=b.memo_ID
	...
	BEGIN
		...
		FROM apm01106 B (NOLOCK) JOIN #auto_posreco_data_upload a (NOLOCK) ON a.memo_id=b.memo_ID 
		WHERE a.xn_type='APP'

		SET @cStep='70'
		UPDATE B SET HO_SYNCH_LAST_UPDATE=''
		FROM apm01106 B (NOLOCK) JOIN #auto_posreco_data_mismatch a (NOLOCK) ON a.memo_id=b.memo_ID 
	END
...
BEGIN CATCH
	SET @cErrormsg='Error in Procedure SP3S_GETPOSRECO_APP_DATA at Step#'+@cStep+' '+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
