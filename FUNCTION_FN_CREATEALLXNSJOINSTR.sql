CREATE  FUNCTION FN_CREATEALLXNSJOINSTR
(
	@CXNTYPE VARCHAR(10),
...
BEGIN
	...
	IF ISNULL(@CNEXTPARENTTABLENAME,'')<>ISNULL(@CPARENTTABLENAME,'') AND @CNEXTPARENTTABLENAME<>''
	    SELECT @CJOINSTR = DBO.FN_CREATEALLXNSJOINSTR(@CXNTYPE, @CPARENTTABLENAME,@CNEXTPARENTTABLENAME,
						   @CNEXTPARENTPARANAME,@CJOINSTR,@CWHERECLAUSE,@CMEMOID)		
	ELSE
	BEGIN
		SELECT @CPARENTKEYFIELD=KEYFIELD FROM XNSINFO WHERE TABLENAME=@CPARENTTABLENAME
