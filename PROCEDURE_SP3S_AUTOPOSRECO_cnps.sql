CREATE PROCEDURE SP3S_AUTOPOSRECO_cnps
@nMode NUMERIC(1,0),
@cLocIdPara CHAR(2)='',
...
BEGIN TRY
	...
	BEGIN
		...
		ISNULL(total_quantity,0) as quantity,'cnps' as XN_TYPE,a.LAST_UPDATE
		FROM cnps_mst A WITH (NOLOCK)
	...
	BEGIN
		...
		LEFT OUTER JOIN cnps_mst b (NOLOCK)	ON a.memo_id=b.ps_id
	...
	BEGIN
		...
		FROM cnps_mst B (NOLOCK) JOIN #auto_posreco_data_upload a (NOLOCK) ON a.memo_id=b.ps_id 
		WHERE a.xn_type='cnps'

		SET @cStep='70'
		UPDATE B SET HO_SYNCH_LAST_UPDATE=''
		FROM cnps_mst B (NOLOCK) JOIN #auto_posreco_data_mismatch a (NOLOCK) ON a.memo_id=b.ps_id 
	END
...
BEGIN CATCH
	SET @cErrormsg='Error in Procedure SP3S_GETPOSRECO_cnps_DATA at Step#'+@cStep+' '+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
