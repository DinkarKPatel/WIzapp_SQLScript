create PROCEDURE SPSIS_SEND_MIRROR_JWR_DATA
(
...
LBLTABLEINFO:
	...
	SET @CSTEP=60
	SELECT @cCols=COALESCE(@cCols+',','')+colname 	FROM ebo_xnssending_cols WHERE tablename='JOBWORK_RECEIPT_MST' 

	SET @cCmdCols=N'
	   SELECT ''JWR_JOBWORK_RECEIPT_MST_UPLOAD'' AS TARGET_TABLENAME,'+@cCols+' FROM JOBWORK_RECEIPT_MST  (NOLOCK)   WHERE RECEIPT_ID='''+@CMEMOID+''''
	PRINT @cCmdCols
	...
	SELECT @cCols=COALESCE(@cCols+',','')+'JOBWORK_RECEIPT_DET.'+colname FROM ebo_xnssending_cols	WHERE tablename='JOBWORK_RECEIPT_DET' 
	
	SET @cCOls=REPLACE(@cCOls,'JOBWORK_RECEIPT_DET.PRODUCT_CODE','CASE WHEN ISNULL(C.VENDOR_EAN_NO,'''')<>'''' THEN C.VENDOR_EAN_NO ELSE JOBWORK_RECEIPT_DET.PRODUCT_CODE END AS PRODUCT_CODE')

	PRINT @cCOls
	SET @cCmdCols=N'
		SELECT ''JWR_JOBWORK_RECEIPT_DET_UPLOAD'' AS TARGET_TABLENAME,'+@cCols+' 
		FROM JOBWORK_RECEIPT_DET  (NOLOCK)   
		JOIN SKU C (NOLOCK) ON C.PRODUCT_CODE=JOBWORK_RECEIPT_DET.PRODUCT_CODE
		JOIN LMP01106 LMP (NOLOCK) ON LMP.AC_CODE =C.AC_CODE
		WHERE SUBSTRING(LMP.AC_GST_NO,3,10)='''+@CPANNO+'''
		AND JOBWORK_RECEIPT_DET.RECEIPT_ID='''+@CMEMOID+''''
	...
	EXEC SPSIS_GET_CURSOR_SKUNAMES @cXNTYPE='JWR',@cTABLENAME='JOBWORK_RECEIPT_DET',@cKEYNAME='RECEIPT_ID',@cKEYVALUE=@CMEMOID,@CERRMSG=@CERRMSG OUTPUT 
	IF ISNULL(@CERRMSG,'')<>'' 
		GOTO END_PROC
...
BEGIN CATCH
	SET @CERRMSG='P: SPSIS_SEND_MIRROR_JWR_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
