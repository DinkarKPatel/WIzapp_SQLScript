...
AS    
 SELECT  A.DEPT_ID,    
   ...
   convert(numeric(11,3),A.QUANTITY_OB) AS XN_QTY,    -- Did this conversion so that column value of other xn types should be retained upto 4 decimal places
 ...
 SELECT  B.DEPT_ID,    
   ...
   convert(numeric(12,3),A.QUANTITY) AS XN_QTY,    
   A.MRP AS XN_NET,    
   CONVERT(NUMERIC(10,2),0) AS XN_DA,CONVERT(NUMERIC(10,2),0) AS TAX_AMOUNT    
   ,'000'  AS [BIN_ID],
   cast('' as varchar(100)) AS BATCHLOTNO ,
 ...
 SELECT C.MAJOR_DEPT_ID AS DEPT_ID,      
   ...
   convert(numeric(12,3),ABS(A.QUANTITY)) AS XN_QTY,      
 ...
 SELECT b.location_Code AS DEPT_ID,    
   (CASE WHEN b.MODE=2 THEN 'CHI'   
    ELSE   
    (CASE WHEN (B.BIN_TRANSFER=1 OR B.MODE=3) THEN 'API' ELSE 'WSR' END)  
    END) AS XN_TYPE,    
   (CASE WHEN b.MODE=2 THEN B.RECEIPT_DT ELSE CN_DT END) AS XN_DT,    
   B.CN_NO AS XN_NO,    
 ...
 LEFT JOIN RMM01106 C (NOLOCK) ON c.rm_id=b.rm_id AND c.cancelled=0
  JOIN (SELECT TOP 1 VALUE FROM CONFIG WHERE CONFIG_OPTION='LOCATION_ID') LOC ON 1=1
 JOIN (SELECT TOP 1 VALUE FROM CONFIG WHERE CONFIG_OPTION='HO_LOCATION_ID') HO ON 1=1 
 WHERE B.CANCELLED = 0 AND B.CN_TYPE<>2 AND (b.MODE<>2 OR (B.RECEIPT_DT<>'' AND loc.value<>ho.value)) 
 ...
 cast('' as varchar(50)) AS Pick_list_id
 FROM RMD01106 A WITH(NOLOCK)    
 JOIN CNM01106 B WITH(NOLOCK) ON A.RM_ID = B.RM_ID
 JOIN rmm01106 d (nolock) on d.rm_id=a.rm_id
 WHERE B.CANCELLED = 0 AND b.mode=2 AND d.cancelled=0
 AND B.RECEIPT_DT<>'' 
 ...
 UNION ALL  
  ...
  SELECT  B.location_code/*LEFT(A.MEMO_ID,2)*//*Rohit 05-11-2024*/ AS [DEPT_ID],   
   ...
   A.PRODUCT_CODE AS PRODUCT_CODE,    
   '' AS XN_PARTY_CODE,     
   convert(numeric(12,3),ABS(A.QTY)) AS XN_QTY,    
   CONVERT(NUMERIC(10,2),0) AS XN_NET,    
   CONVERT(NUMERIC(10,2),0) AS XN_DA,    
   CONVERT(NUMERIC(10,2),0) AS TAX_AMOUNT    
   ,'000'  AS [BIN_ID],
 ...
 UNION ALL
  SELECT B.location_code/*LEFT(B.ISSUE_ID,2)*//*Rohit 05-11-2024*/ AS DEPT_ID
	    ...
	    ,A.PRODUCT_CODE
	   ,'LM'+D.AC_CODE AS XN_PARTY_CODE
	   ,convert(numeric(12,3),ABS(A.STOCK_QTY)) AS XN_QTY
	   ,CONVERT(NUMERIC(10,2),0) AS XN_NET
	   ,CONVERT(NUMERIC(10,2),0) AS XN_DA  
	   ,CONVERT(NUMERIC(10,2),0) AS TAX_AMOUNT
	   ,A.BIN_ID  AS [BIN_ID]
 ...
 UNION ALL  
  ...
  'SLS' AS XN_TYPE,       
   ...
   'CUS'+b.CUSTOMER_CODE AS XN_PARTY_CODE,       
    convert(numeric(12,3),sum(A.GRQuantity -A.RecoQty))  AS XN_QTY,      
    0 AS XN_NET,      
    0 AS XN_DA,  
    0  as TAX_AMOUNT   
...
--2
  ...
  SELECT b.DEPT_ID  AS DEPT_ID
	    ...
	    ,b.PRODUCT_CODE
	   ,'' AS XN_PARTY_CODE
	   ,convert(numeric(12,3),b.quantity_in_stock) AS XN_QTY
	   ,CONVERT(NUMERIC(10,2),0) AS XN_NET
	   ,CONVERT(NUMERIC(10,2),0) AS XN_DA  
	   ,CONVERT(NUMERIC(10,2),0) AS TAX_AMOUNT
	   ,b.BIN_ID  AS [BIN_ID]
