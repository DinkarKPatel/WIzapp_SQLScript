create  PROCEDURE SPSIS_SEND_MIRROR_PRT_DATA
(
...
LBLTABLEINFO:
	...
	SET @CSTEP=60
	SELECT @cCols=COALESCE(@cCols+',','')+colname 	FROM ebo_xnssending_cols WHERE tablename='RMM01106' 

	SET @cCmdCols=N'
	   SELECT ''PRT_RMM01106_upload'' AS TARGET_TABLENAME,'+@cCols+' FROM RMM01106  (NOLOCK)   WHERE RM_ID='''+@CMEMOID+''''
	PRINT @cCmdCols
	...
	SET @CSTEP=65	
	SET @cCols=NULL
	SELECT @cCols=COALESCE(@cCols+',','')+'RMD01106.'+colname FROM ebo_xnssending_cols	WHERE tablename='RMD01106' 

	SET @cCOls=REPLACE(@cCOls,'RMD01106.PRODUCT_CODE','CASE WHEN ISNULL(C.VENDOR_EAN_NO,'''')<>'''' THEN C.VENDOR_EAN_NO ELSE RMD01106.PRODUCT_CODE END AS PRODUCT_CODE')

	PRINT @cCOls
	SET @cCmdCols=N'
				SELECT ''PRT_RMD01106_upload'' AS TARGET_TABLENAME,'+@cCols+' 
				FROM RMD01106  (NOLOCK)   
				JOIN SKU C (NOLOCK) ON C.PRODUCT_CODE=RMD01106.PRODUCT_CODE
				JOIN LMP01106 LMP (NOLOCK) ON LMP.AC_CODE =C.AC_CODE
				WHERE SUBSTRING(LMP.AC_GST_NO,3,10)='''+@CPANNO+'''
				AND RMD01106.RM_ID='''+@CMEMOID+''''
	...
	EXEC SPSIS_GET_CURSOR_SKUNAMES @cXNTYPE='PRT',@cTABLENAME='RMD01106',@cKEYNAME='RM_ID',@cKEYVALUE=@CMEMOID,@CERRMSG=@CERRMSG OUTPUT 
	IF ISNULL(@CERRMSG,'')<>'' 
		GOTO END_PROC
...
BEGIN CATCH
	SET @CERRMSG='P: SPSIS_SEND_MIRROR_PRT_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
