CREATE PROCEDURE SP_MERGE_MIRROR_PSDLV_DATA
(
	@CMEMOID VARCHAR(50)
   ,@CLOCID VARCHAR(3)
...
BEGIN
     ...
     DECLARE @TXNSSENDINFO TABLE (ORG_TABLENAME VARCHAR(50),TMP_TABLENAME VARCHAR(50),XN_ID VARCHAR(40))  
      ...
      BEGIN TRY
		...
		SET @CTABLESUFFIX=@CTABLE_SUFFIX
			...
			SET @CTMP_TABLENAME='TMP_PSDLV_'+@CTABLENAME+'_'+LTRIM(RTRIM(@CTABLE_SUFFIX))
			...
			SET @CCMD = N'DELETE A FROM SLS_DELIVERY_DET A LEFT OUTER JOIN 
	...
	END TRY
	BEGIN CATCH
		SET @CERRMSG='P:SP_MERGE_MIRROR_PSDLV_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	END CATCH
	EXIT_PROC:
	IF ISNULL(@CERRMSG,'')='' AND @@TRANCOUNT>0
	BEGIN
		COMMIT
	      SET @CTABLESSTR='SLS_DELIVERY_MST,SLS_DELIVERY_DET,XNSTABLELIST'
