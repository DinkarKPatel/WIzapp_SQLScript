...
AS    
 ...
 ((A.QUANTITY<0 AND (ISNULL(C.SLR_RECON_REQD,0)<>1 OR B.CM_DT<ISNULL(C.SLR_RECON_CUTOFF_DATE,'')))  
  OR A.QUANTITY>0)  
 ...
 (CASE WHEN B.CNC_TYPE=1 THEN 'CNC' ELSE 'UNC' END) 	   	
  END) AS XN_TYPE,
   ...
   CONVERT(VARCHAR(40),'') AS XN_PARTY_CODE,     
   A.QUANTITY AS XN_QTY,    
   (CASE WHEN STOCK_NA=1 THEN A.RATE ELSE 0 END) AS XN_NET,    
   CONVERT(NUMERIC(10,2),0) AS XN_DA,CONVERT(NUMERIC(10,2),0) AS TAX_AMOUNT    
   ,A.BIN_ID  AS [BIN_ID],
   cast('' as varchar(100)) AS BATCHLOTNO   ,
   cast(''  as varchar(50)) AS Order_id,
 ...
 WHERE B.CANCELLED = 0 AND c.cancelled=0   AND ISNULL(B.PENDING_GIT,0)=0
 ...
 UNION ALL  
  ...
  SELECT C.location_code/*LEFT(c.cm_id,2)*//*Rohit 05-11-2024*/ AS DEPT_ID,      
   'SLR' AS XN_TYPE,       
   b.memo_DT AS XN_DT,      
   b.memo_NO AS XN_NO,      
   'CMM'+c.cm_ID AS XN_ID,    
   e.PRODUCT_CODE AS PRODUCT_CODE,      
   'CUS'+c.CUSTOMER_CODE AS XN_PARTY_CODE,       
   cast(sum(abs(A.QUANTITY)) as numeric(10,3)) AS XN_QTY,      
   0 AS XN_NET,      
   0 AS XN_DA,  
   0  as TAX_AMOUNT   
   ,'000'  AS [BIN_ID],  
 ...
 JOIN CMD01106 e (NOLOCK) ON A.CMD_ROW_ID=e.ROW_ID  
 JOIN CMM01106 C (NOLOCK) ON C.CM_ID=e.CM_ID  
 JOIN LOCATION D (NOLOCK) ON D.DEPT_ID=  C.location_code/*LEFT(C.CM_NO,2)*//*Rohit 05-11-2024*/
 ...
 AND b.cancelled=0  
 GROUP BY C.location_code/*LEFT(c.CM_ID,2)*//*Rohit 05-11-2024*/, e.PRODUCT_CODE,e.BIN_ID,b.memo_dt,b.memo_no,c.cm_id,c.CUSTOMER_CODE  
