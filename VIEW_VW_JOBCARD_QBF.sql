...
,C.VALUE_ADDITION  
,A.ORD_PLAN_MODE AS MST_JOBCARD_MST_MODE  
--,A.WIP AS MST_WIP   
,D_CONSUME.* 
FROM ORD_PLAN_MST A (NOLOCK)                  
JOIN LMV01106     B (NOLOCK) ON A.AC_CODE = B.AC_CODE              
JOIN ORD_PLAN_DET C (NOLOCK) ON A.MEMO_ID = C.MEMO_ID        
...
JOIN UOM U (NOLOCK) ON D.UOM_CODE = U.UOM_CODE      
JOIN ORD_PLAN_BARCODE_DET BC (NOLOCK) ON BC.REFROW_ID=C.ROW_ID  
...
,BC.PRODUCT_CODE AS COM_PRODUCT_CODE
,CD.QUANTITY AS COM_QUANTITY
,CD.AVG_QUANTITY AS COM_AVG_QUANTITY
,CD.PURCHASE_PRICE AS COM_PURCHASE_PRICE
,CD.BIN_ID AS COM_BIN_ID
,CD.SRNO AS COM_SRNO
,CD.AUTO_SRNO AS COM_AUTO_SRNO
,CD.LAST_UPDATE AS COM_LAST_UPDATE
,CD.ROW_ID AS COM_ROW_ID
,CD.REF_ROW_ID AS COM_REF_ROW_ID
--,CD.WIP AS COM_WIP
--,CD.WIP_UID AS COM_WIP_UID
--,CD.WASTAGE_QUANTITY AS COM_WASTAGE_QUANTITY
,CD.MANUAL_QTY AS COM_MANUAL_QTY
FROM ORD_PLAN_MST A (NOLOCK)
JOIN LMV01106 B (NOLOCK)ON A.AC_CODE = B.AC_CODE              
JOIN ORD_PLAN_DET C (NOLOCK) ON A.MEMO_ID = C.MEMO_ID  
JOIN ORD_PLAN_BOM_DET CD(NOLOCK) ON A.MEMO_ID = CD.MEMO_ID              
JOIN ARTICLE D (NOLOCK)  ON C.ARTICLE_CODE = D.ARTICLE_CODE              
JOIN SECTIOND E (NOLOCK) ON D.SUB_SECTION_CODE = E.SUB_SECTION_CODE               
JOIN SECTIONM F (NOLOCK) ON E.SECTION_CODE = F.SECTION_CODE               
JOIN PARA1 P1 (NOLOCK) ON C.PARA1_CODE = P1.PARA1_CODE              
...
JOIN BIN      (NOLOCK) ON BIN.BIN_ID = A.BIN_ID      
JOIN UOM U    (NOLOCK) ON D.UOM_CODE = U.UOM_CODE      
JOIN ORD_PLAN_BARCODE_DET BC 
			  (NOLOCK) ON BC.REFROW_ID=C.ROW_ID  
JOIN SKU      (NOLOCK) ON SKU.PRODUCT_CODE = BC.PRODUCT_CODE      
)D_CONSUME 
ON D_CONSUME.CMEMO_ID=A.MEMO_ID
AND SKU.PRODUCT_CODE=D_CONSUME.COM_PRODUCT_CODE
