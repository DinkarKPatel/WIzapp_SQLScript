CREATE PROCEDURE SAVETRAN_DOC_PRD
(
...
BEGIN
	...
	BEGIN TRY
		...
		SET @CTEMPDETAILTABLENAME	= 'TEMP_PRD_DEPARTMENT_OUTPUT_DET_'+LTRIM(RTRIM(STR(@NSPID)))					
				
		SET @CTEMPDETAILTABLENAME1	= 'TEMP_PRD_DEPARTMENT_RMCONSUMPTION_DET_'+LTRIM(RTRIM(STR(@NSPID)))					
		...
		SELECT PRODUCT_UID,QUANTITY_IN_STOCK,WIP AS XN_STOCK INTO #TMPXNSTK FROM PRD_PMT WHERE 1=2
		...
		BEGIN
			...
			SET @CCMD = N'UPDATE PRD_DEPARTMENT_OUTPUT_MST SET CANCELLED = 1,LAST_UPDATE=GETDATE() ' + 
		...
		SET @CDETAILTABLENAME	= 'PRD_DEPARTMENT_OUTPUT_DET'
		SET @CDETAILTABLENAME1	= 'PRD_DEPARTMENT_RMCONSUMPTION_DET'
		--SET @CDETAILTABLENAME2	= 'INSPECTION_XN_DET'
				

		SET @CTEMPMASTERTABLENAME	= 'TEMP_PRD_DEPARTMENT_OUTPUT_MST_'+LTRIM(RTRIM(STR(@NSPID)))
...
PRINT 'GENERATING NEW KEY... START'			
		...
		EXEC SAVETRAN_UPDSKU_PRD 'DOC',@CKEYFIELDVAL1,@CERRORMSG OUTPUT 
		IF ISNULL(@CERRORMSG,'')<>''
				GOTO END_PROC
