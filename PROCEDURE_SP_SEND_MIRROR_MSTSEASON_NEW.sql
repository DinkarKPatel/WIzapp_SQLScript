CREATE PROCEDURE DBO.SP_SEND_MIRROR_MSTSEASON_NEW
(
...
AS
 BEGIN
   DECLARE @DTSQL NVARCHAR(MAX),@CSTEP VARCHAR(10),@CFILTERCONDITION NVARCHAR(MAX)
	...
	SELECT DISTINCT  'MSTSEASON_SEASON_MST_MIRROR' AS TARGET_TABLENAME,SEASON_MST.*,1 AS PICK_FROM_CURRENT_DB FROM SEASON_MST (NOLOCK)
	...
	BEGIN CATCH
	    SET @CERRORMSG='P: SP_SEND_MIRROR_MSTSEASON, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()+ 'ERROR QUERY || '+@DTSQL
	    GOTO END_PROC
    END CATCH 
