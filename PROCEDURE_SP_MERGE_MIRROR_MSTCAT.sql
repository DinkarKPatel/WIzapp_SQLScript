CREATE PROCEDURE DBO.SP_MERGE_MIRROR_MSTCAT
  (@NMODE INT
  ,@XN_TYPE VARCHAR(50) ='MSTCAT'
  )
AS
 BEGIN
...
BEGIN TRY
	  ...
	  BEGIN
			SET @CERRORMSG='P:SP_MERGE_MIRROR_MSTCAT, CANNOT CALL MERGING PROC AT HEAD OFFICE'
			GOTO END_PROC
  	  END					
	  ...
	  --DELETE ALREADY EXISTING RECORDS FOR [MASTER TABLE
	   	 DELETE FROM CATGRPDET
	     DELETE FROM CATGRPMST
	 ...
	 --INSERT DATA INTO LOCATION TABLE
	   ...
	   SET @CSOURCETABLE ='MSTCAT_CATGRPMST_MIRROR'
	   SET @CKEYFIELD1 ='GROUP_CODE'
	   EXEC UPDATEMASTERXN
	   ...
	   SET @CSOURCETABLE ='MSTCAT_CATGRPDET_MIRROR'
	   SET @CKEYFIELD1 ='GROUP_CODE'
	   EXEC UPDATEMASTERXN
	   @CSOURCEDB=@CSOURCEDB, @CSOURCETABLE=@CSOURCETABLE,
	   @CDESTDB='',@CDESTTABLE=@CTABLENAME,@CKEYFIELD1=@CKEYFIELD1,
...
BEGIN CATCH
	SET @CERRORMSG='P:SP_MERGE_MIRROR_MSTCAT, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
...
END_PROC:
	...
	BEGIN   
	   ...
	   DELETE FROM MSTCAT_CATGRPDET_MIRROR
	   DELETE FROM MSTCAT_CATGRPMST_MIRROR
	END
		
	SELECT @XN_TYPE AS MEMO_ID,@CERRORMSG AS ERRMSG   	  
END

--END PROCEDURE SP_MERGE_MIRROR_MSTCAT
