CREATE PROCEDURE SPCALCULATE_ADI
(
	 @DFROMDT DATETIME=''
	,@DTODT DATETIME
...
BEGIN
SET NOCOUNT ON
PRINT 'SP ''SPCALCULATE_ADI'' CALLED'
	DECLARE @DLOOPDT DATETIME,@CPARA_NAME VARCHAR(500),@NINQTY NUMERIC(10,2),@NOUTQTY NUMERIC(10,2),
			...
			,@CCMD NVARCHAR(MAX),@CWHERECLAUSE VARCHAR(MAX),@CERRMSG VARCHAR(500),@CSTEP VARCHAR(5),@CDFROMDT VARCHAR(20),@CDTODT VARCHAR(20);	
	BEGIN TRY
	SET @CSTEP=10
	IF ISNULL(@DTODT,'')=''
	BEGIN
		SET @CERRMSG='P:SPCALCULATE_ADI,'+CHAR(13)+'STEP:'+@CSTEP+','+CHAR(13)+'MESSAGE:NO PROCESS DATE DEFINED FOR PROCESSING.....'
		GOTO EXIT_PROC		
	END	
	SET @CSTEP=20
	IF ISNULL(@DFROMDT,'')=''
	...
	SET @CSTEP=80
	CREATE TABLE #TMPADIOB(PARA_NAME VARCHAR(500),CBS_VAL NUMERIC(18,2))
	
	SET @CSTEP=90
	SET @CWHERECLAUSE=' WHERE XN_DT <'''+@CDFROMDT
				   +''' AND ARTICLE.STOCK_NA=0 
					    AND LOCATION.INACTIVE=0 '+(CASE WHEN ISNULL(@CFILTERCOLS,'')='' THEN '' ELSE ' AND ' END)
					    +@CFILTERCOLS
    PRINT '90 '+ @CWHERECLAUSE					    
	SET @CSTEP=100
	SET @CCMD=N'SELECT '+(CASE WHEN @CMODE=0 THEN 'LOCATION.MAJOR_DEPT_ID AS DEPT_ID' ELSE @CLAYOUTCOLS END)+'
				,SUM(CASE WHEN XN_TYPE IN (''OPS'', ''PUR'',''CHI'',''SLR'',''UNC'',''APR'', ''WSR'', ''PFI'', ''BCG'',''MRP'',''DCI'',''PSB'',''JWR'') THEN XN_QTY*(SKU.PURCHASE_PRICE-ISNULL(SKU_OH.DISCOUNT_AMOUNT,0)+(CASE WHEN POST_TAX_SEPARATELY=1 THEN 0 ELSE ISNULL(SKU_OH.TAX_AMOUNT,0) END)) 
						  WHEN XN_TYPE IN (''PRT'',''CHO'',''SLS'',''CNC'',''APP'',''WSL'',''CIP'',''DCO'',''MIP'',''CSB'',''JWI'',''DLM'')			    THEN -XN_QTY*(SKU.PURCHASE_PRICE-ISNULL(SKU_OH.DISCOUNT_AMOUNT,0)+(CASE WHEN POST_TAX_SEPARATELY=1 THEN 0 ELSE  ISNULL(SKU_OH.TAX_AMOUNT,0) END)) 
						  ELSE 0 
					 END
					 ) AS CBS_VAL 
				FROM '+@CRFDB+' A (NOLOCK)
				...
				'+@CWHERECLAUSE+'
				GROUP BY '+(CASE WHEN @CMODE=0 THEN 'LOCATION.MAJOR_DEPT_ID' 
							ELSE @CLAYOUTCOLS END)
	PRINT '100 '+ISNULL(@CCMD,'NULL CCMD')
	INSERT #TMPADIOB(PARA_NAME,CBS_VAL)
	...
	IF OBJECT_ID('TEMPDB..#TMPADI1','U') IS NOT NULL
		DROP TABLE #TMPADI1
	SET @CSTEP=120
	CREATE TABLE #TMPADI1(PARA_NAME VARCHAR(500),XN_DT DATETIME,IN_VAL NUMERIC(18,2),OUT_VAL NUMERIC(18,2)
	...
	SET @CWHERECLAUSE=' WHERE XN_DT BETWEEN '''+@CDFROMDT+''' AND '''+@CDTODT+''' AND ARTICLE.STOCK_NA=0 AND LOCATION.INACTIVE=0 '
					  +(CASE WHEN ISNULL(@CFILTERCOLS,'')='' THEN '' ELSE ' AND ' END)
						+@CFILTERCOLS	
	SET @CSTEP=140
	SET @CCMD=N'SELECT '+(CASE WHEN @CMODE=0 THEN 'LOCATION.MAJOR_DEPT_ID AS DEPT_ID' 
				ELSE @CLAYOUTCOLS END)+'
	,A.XN_DT
	,SUM((CASE WHEN A.XN_TYPE IN (''OPS'',''PUR'',''CHI'',''SLR'',''UNC'',''APR'',''WSR'',''PFI'',''BCG'',''MRP'',''DCI'',''PSB'',''JWR'') THEN (A.XN_QTY*(SKU.PURCHASE_PRICE-ISNULL(SKU_OH.DISCOUNT_AMOUNT,0)+(CASE WHEN FORM.POST_TAX_SEPARATELY=1 THEN 0 ELSE  ISNULL(SKU_OH.TAX_AMOUNT,0) END))) ELSE 0 END )) AS IN_VAL
	,SUM((CASE WHEN A.XN_TYPE IN (''PRT'',''CHO'',''SLS'',''CNC'',''APP'',''WSL'',''CIP'',''DCO'',''MIP'',''CSB'',''JWI'',''DLM'') 		   THEN (A.XN_QTY*(SKU.PURCHASE_PRICE-ISNULL(SKU_OH.DISCOUNT_AMOUNT,0)+(CASE WHEN FORM.POST_TAX_SEPARATELY=1 THEN 0 ELSE  ISNULL(SKU_OH.TAX_AMOUNT,0) END))) ELSE 0 END)) AS OUT_VAL 
	...
	'+@CWHERECLAUSE+'
	GROUP BY '+(CASE WHEN @CMODE=0 THEN 'LOCATION.MAJOR_DEPT_ID' 
	ELSE @CLAYOUTCOLS END)+',XN_DT'	

	PRINT ISNULL(@CCMD,'NULL COMMAND')
	SET @CSTEP=150
	INSERT #TMPADI1(PARA_NAME,XN_DT,IN_VAL,OUT_VAL,CBS_VAL)
	EXEC SP_EXECUTESQL @CCMD
	...
	SET @CSTEP=170
	INSERT #TMPADI1 (PARA_NAME,XN_DT,IN_VAL,OUT_VAL,CBS_VAL)
	SELECT PARA_NAME,DT, 0 AS IN_VAL, 0 AS OUT_VAL,0 AS CBS_VAL
	FROM TIME_LINE,( SELECT DISTINCT PARA_NAME FROM #TMPADI1 ) B
	WHERE TIME_LINE.DT  BETWEEN @DFROMDT AND @DTODT
	AND CONVERT(VARCHAR,DT,110)+B.PARA_NAME
	NOT IN (SELECT CONVERT(VARCHAR,XN_DT,110)+PARA_NAME FROM #TMPADI1)
	
	SET @CSTEP=180
	PRINT 'ADI-5'+CONVERT(VARCHAR,GETDATE(),113)
	SELECT PARA_NAME,XN_DT,IN_VAL,OUT_VAL,CBS_VAL
	INTO #TMPADI2 FROM #TMPADI1
	ORDER BY PARA_NAME,XN_DT
	...
	IF @CMODE='0'
		INSERT #TMPADIOB 
		SELECT DISTINCT A.DEPT_ID,0 AS CBS_VAL FROM LOCATION A
		LEFT OUTER JOIN #TMPADIOB B ON A.DEPT_ID=B.PARA_NAME
		WHERE A.MAJOR_DEPT_ID=A.DEPT_ID AND B.PARA_NAME IS NULL
	ELSE
		INSERT #TMPADIOB 
		SELECT DISTINCT A.PARA_NAME,0 AS CBS_VAL FROM #TMPADI1 A
		LEFT OUTER JOIN #TMPADIOB B ON A.PARA_NAME=B.PARA_NAME
		WHERE B.PARA_NAME IS NULL
	
	SET @CSTEP=210
	DECLARE ADICBSCUR CURSOR FOR SELECT PARA_NAME,CBS_VAL FROM #TMPADIOB
	SET @CSTEP=220
	PRINT 'ADI-6'+CONVERT(VARCHAR,GETDATE(),113)
	OPEN ADICBSCUR
	FETCH NEXT FROM ADICBSCUR INTO @CPARA_NAME,@NCBSVAL
	SET @CSTEP=230
	...
	BEGIN	
		INSERT #TMPCBSPP
		SELECT PARA_NAME,CBS_VAL FROM #TMPADI2 WHERE XN_DT=@DTODT
	END
	
	SET @CSTEP=270
	IF OBJECT_ID('TEMPDB..#TMPADI3','U') IS NOT NULL
	BEGIN	
		INSERT #TMPADI3(PARA_NAME,ADI)
		SELECT PARA_NAME,ROUND(AVG(CBS_VAL),2) AS ADI
		FROM #TMPADI2 GROUP BY PARA_NAME
		ORDER BY PARA_NAME
	END
END TRY
BEGIN CATCH
	SET @CERRMSG='P:SPCALCULATE_ADI,'+CHAR(13)+'STEP:'+@CSTEP+','+CHAR(13)+'MESSAGE:'+ERROR_MESSAGE()
	GOTO EXIT_PROC
END CATCH
EXIT_PROC:
	IF ISNULL(@CERRMSG,'')<>''
...
END
--END OF PROCEDURE - SPCALCULATE_ADI
