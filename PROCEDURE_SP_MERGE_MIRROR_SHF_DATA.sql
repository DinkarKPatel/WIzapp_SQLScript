CREATE PROCEDURE SP_MERGE_MIRROR_SHF_DATA
(
	@CMEMOID VARCHAR(50)
   ,@CLOCID VARCHAR(3)
...
BEGIN TRANSACTION 
	...
	SELECT @CTEMPMASTERTABLE=@CSOURCEDB+'TMP_TILL_SHIFT_MST_'+LTRIM(RTRIM(@CTABLE_SUFFIX))

LBLMERGE:
    
    SET @CSTEP=250
	SET @CTABLENAME='TILL_SHIFT_MST'
	SET @CTMP_TABLENAME='TMP_SHF_'+@CTABLENAME+'_'+LTRIM(RTRIM(@CTABLE_SUFFIX))
	...
	SET @CSTEP=320						  
	SET @CTABLESSTR='TILL_SHIFT_MST,TILL_SHIFT_DET'
...
END TRY
BEGIN CATCH
	SET @CERRMSG='P:SP_MERGE_MIRROR_SHF_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
END CATCH
EXIT_PROC:
	...
	ELSE IF ISNULL(@CERRMSG,'')<>'' AND @@TRANCOUNT>0
		ROLLBACK
END	
---END OF PROCEDURE - SP_MERGE_MIRROR_SHF_DATA

/*
exec sp_merge_mirrordata '12','shf','12-20191112-003',0

select * from mirrorxnsinfo
*/