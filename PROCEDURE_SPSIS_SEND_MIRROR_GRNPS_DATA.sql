create PROCEDURE SPSIS_SEND_MIRROR_GRNPS_DATA
(
...
LBLTABLEINFO:
	...
	SET @CSTEP=60
	SELECT @cCols=COALESCE(@cCols+',','')+colname 	FROM ebo_xnssending_cols WHERE tablename='GRN_PS_MST' 

	SET @cCmdCols=N'
	   SELECT ''GRNPS_GRN_PS_MST_UPLOAD'' AS TARGET_TABLENAME,'+@cCols+' FROM GRN_PS_MST  (NOLOCK)   WHERE MEMO_ID='''+@CMEMOID+''''
	PRINT @cCmdCols
	...
	SET @CSTEP=65	
	SET @cCols=NULL
	SELECT @cCols=COALESCE(@cCols+',','')+'GRN_PS_DET.'+colname FROM ebo_xnssending_cols	WHERE tablename='GRN_PS_DET' 

	SET @cCOls=REPLACE(@cCOls,'GRN_PS_DET.PRODUCT_CODE','CASE WHEN ISNULL(C.VENDOR_EAN_NO,'''')<>'''' THEN C.VENDOR_EAN_NO ELSE GRN_PS_DET.PRODUCT_CODE END AS PRODUCT_CODE')

	PRINT @cCOls
	SET @cCmdCols=N'
		SELECT ''GRNPS_GRN_PS_DET_UPLOAD'' AS TARGET_TABLENAME,'+@cCols+' 
		FROM GRN_PS_DET  (NOLOCK)   
		JOIN SKU C (NOLOCK) ON C.PRODUCT_CODE=GRN_PS_DET.PRODUCT_CODE
		JOIN LMP01106 LMP (NOLOCK) ON LMP.AC_CODE =C.AC_CODE
		WHERE SUBSTRING(LMP.AC_GST_NO,3,10)='''+@CPANNO+'''
		AND GRN_PS_DET.MEMO_ID='''+@CMEMOID+''''
	
	PRINT @cCmdCols

	EXEC SP_EXECUTESQL @cCmdCols

	EXEC SPSIS_GET_CURSOR_SKUNAMES @cXNTYPE='GRNPS',@cTABLENAME='GRN_PS_DET',@cKEYNAME='MEMO_ID',@cKEYVALUE=@CMEMOID,@CERRMSG=@CERRMSG OUTPUT 
	IF ISNULL(@CERRMSG,'')<>'' 
		GOTO END_PROC
...
BEGIN CATCH
	SET @CERRMSG='P: SPSIS_SEND_MIRROR_GRNPS_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
