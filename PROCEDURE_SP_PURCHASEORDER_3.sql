CREATE PROCEDURE SP_PURCHASEORDER_3    
@CWHERE   NVARCHAR(MAX)='',    
@CFINYEAR  VARCHAR(5)='',    
@CDEPTID  VARCHAR(4)='',    
...
begin  
   ...
   SELECT ROW_ID =CAST('' AS VARCHAR(100)),  
    ...
    BEGIN  
         ...
         EXEC SP3S_CALCULATE_FCNET_PO @CMEMOID=@CWHERE,@BCALLEDFORLIST=1  
  
    END  
  
  
  IF EXISTS (SELECT TOP 1 product_code FROM pod01106 (NOLOCK) WHERE po_id=@CWHERE AND product_code<>'')  
  BEGIN  
   SELECT  A.*,  CAST(0 AS NUMERIC(14,0)) AS SERIAL_NO,'' as DT_CREATED,0 as GEN_EAN_CODES,      
    sku.ARTICLE_NO,sku.ARTICLE_NAME, art.coding_scheme, CAST(0 AS BIT) as PARA1_SET,CAST(0 AS BIT) as PARA2_SET,'0000000' as UOM_CODE,   
   i.uom_name as UOM_NAME, 1 as UOM_TYPE,    
    P1.PARA1_NAME,P2.PARA2_NAME,p3.PARA3_NAME,p4.PARA4_NAME, p5.PARA5_NAME, p6.PARA6_NAME, sku.STOCK_NA,0 as GEN_EAN_CODES,      
   ...
   '' as FORM_NAME,sku.ARTICLE_NAME AS PRODUCT_NAME ,'' AS PI_PO_ROW_ID ,  
   0 as FIX_PRICE_ARTICLE,ARTICLE_ALIAS ,'' as ONLINE_PRODUCT_CODE,SUB_SECTION_ALIAS,  
    SECTION_ALIAS,'' as BUYER_ORDER_REMARKS,'' as P1_SET,'' AS P2_SET  
   ...
   attr25_key_name,art.PERISHABLE ,'' as BATCH_NO, cast('' as Datetime) as Expiry_Dt,art.article_desc,  
   ...
   LEFT OUTER JOIN SKU_names  sku   (NOLOCK) ON SKU.PRODUCT_CODE=A.PRODUCT_CODE   
  ...
  BEGIN  
   SELECT  A.*,  CAST(0 AS NUMERIC(14,0)) AS SERIAL_NO,'' as DT_CREATED,0 as GEN_EAN_CODES,      
    ARTICLE_NO, ARTICLE_NAME, art.coding_scheme as coding_scheme, CAST(0 AS BIT) as PARA1_SET, CAST(0 AS BIT) as PARA2_SET,'0000000' as UOM_CODE,   
   i.uom_name as UOM_NAME, 1 as UOM_TYPE,    
   ...
   0 as FIX_PRICE_ARTICLE,art.alias ARTICLE_ALIAS ,'' as ONLINE_PRODUCT_CODE,sd.alias SUB_SECTION_ALIAS,  
    sm.alias SECTION_ALIAS,'' as BUYER_ORDER_REMARKS,'' as P1_SET,'' AS P2_SET,  
    ...
    isnull(a25.attr25_key_name,'') as attr25_key_name  
     ...
     cast('' as Datetime) as Expiry_Dt,article_desc,  
