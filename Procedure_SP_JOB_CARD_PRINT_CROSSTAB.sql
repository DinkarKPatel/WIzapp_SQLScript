create  PROCEDURE SP_JOB_CARD_PRINT_CROSSTAB  
	  ...
	  if object_id ('tempdb..#tmpjobcard','u') is not null
		...
		(
		   SELECT BIN.BIN_NAME,OPD.WOD_ROW_ID, opd.MEMO_ID , 
				...
				sku_names .para4_name ,sku_names .para5_name ,sku_names .para6_name ,opd.PARA2_CODE 
		...
		SELECT  OPM.MEMO_ID,OPM.MEMO_NO,OPM.MEMO_DT,LMV01106.AC_NAME,LOC.DEPT_NAME,OPD.BIN_NAME,OPM.ORD_PLAN_MODE,  
		...
		opd .para4_name ,opd .para5_name ,opd .para6_name 
		into #tmpjobcard
		...
		GROUP BY OPM.MEMO_ID,OPM.MEMO_NO,OPM.MEMO_DT,LMV01106.AC_NAME,LOC.DEPT_NAME,OPD.BIN_NAME,OPM.ORD_PLAN_MODE,  
		...
		Declare @DTSQL nvarchar(max),@COL varchar(1000)

		SELECT @COL=COALESCE(@COL,'')+'['+SIZE+'],' FROM (SELECT  SIZE   FROM #tmpjobcard group by SIZE)A 
		join para2 (nolock) on para2.para2_name  =a.SIZE 
		order by para2.PRINTCOLUMNNO
		
		if right(@COL,1)=','
		set @COL=substring (@COL,1,len(@COL)-1)

		SET @DTSQL=N' SELECT MEMO_ID,MEMO_NO,MEMO_DT,AC_NAME,DEPT_NAME,BIN_NAME,ORD_PLAN_MODE,  
		       SECTION_NAME,SUB_SECTION_NAME, COLOR,STYLE,  
			   ARTICLE_NAME,ARTICLE_NO,UOM_NAME,CANCELLED,HSN_CODE,  
			   MRP,WHOLESALE_PRICE, LEDGER_ADD ,PARA4_NAME ,PARA5_NAME ,PARA6_NAME  ,'+@COL+'
		FROM #TMPJOBCARD 
		pivot (sum(QUANTITY) for size in('+@COL+')) as P1
		'
    print @DTSQL
