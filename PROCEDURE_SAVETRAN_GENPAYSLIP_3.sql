create PROCEDURE SAVETRAN_GENPAYSLIP_3
(                      
	 @NUPDATEMODE INT=1,
	 @CREFID VARCHAR(20),                       
	 @NYEAR INT,                       
...
--	 
	 ...
	 BEGIN
		 ...
		 WHERE A.EMP_ID=@CEMPID AND A.PAY_ID NOT IN ('PAY0002','PAY0001','PAY0004')
		 
		 
		 ---GETTING ALL DEDUCTIONS AGAINST LOAN TAKEN BY AN EMPLOYEE
IF OBJECT_ID('TEMPDB..#ALLLOANS','U') IS NOT NULL
	DROP TABLE #ALLLOANS

IF OBJECT_ID('TEMPDB..#LOAN_PAID','U') IS NOT NULL
	DROP TABLE #LOAN_PAID	

IF OBJECT_ID('TEMPDB..#BAL_LOAN','U') IS NOT NULL
	DROP TABLE #BAL_LOAN

SELECT LOAN_ID,APPROVED_AMOUNT AS LOAN_AMOUNT,EMI_AMOUNT
INTO #ALLLOANS
FROM EMP_LOAN_MST 
WHERE EMP_ID=@CEMPID AND LOAN_DATE NOT BETWEEN @DFROMDT AND @DTODT	AND LOAN_STATUS=1 
	  AND ISNULL(APPROVED_AMOUNT,0)>0

		IF EXISTS(SELECT TOP 1 'U' FROM #ALLLOANS)
		BEGIN
			SELECT D.LOAN_ID,SUM(ISNULL(D.AMOUNT,0)) AS PAID_AMOUNT
			INTO #LOAN_PAID
			FROM EMP_PAYSLIP_DET D
			JOIN EMP_PAYSLIP_MST M ON M.PAYSLIP_ID=D.PAYSLIP_ID
			WHERE M.CANCELLED=0 AND M.EMP_ID=@CEMPID AND D.PAY_ID='PAY0004' 
			GROUP BY D.LOAN_ID 

			SELECT AL.LOAN_ID,(AL.LOAN_AMOUNT-ISNULL(LP.PAID_AMOUNT,0)) AS BAL_AMOUNT,AL.EMI_AMOUNT
			INTO #BAL_LOAN
			FROM #ALLLOANS AL
			LEFT JOIN #LOAN_PAID LP ON AL.LOAN_ID=LP.LOAN_ID
			WHERE AL.LOAN_AMOUNT>ISNULL(LP.PAID_AMOUNT,0)

			INSERT EMP_PAYSLIP_DET	( ROW_ID, PAYSLIP_ID, PAY_ID, LOAN_ID, PAY_TYPE, AMOUNT, ORG_AMOUNT )
			SELECT  @CLOCATIONID+CONVERT(VARCHAR(40),NEWID()) AS ROW_ID,@CPAYSLIPID AS PAYSLIP_ID,
				   'PAY0004' AS PAY_ID, LOAN_ID,1 AS PAY_TYPE
				  ,(CASE WHEN BAL_AMOUNT<EMI_AMOUNT THEN BAL_AMOUNT ELSE EMI_AMOUNT END) AS AMOUNT
				  ,(CASE WHEN BAL_AMOUNT<EMI_AMOUNT THEN BAL_AMOUNT ELSE EMI_AMOUNT END) AS ORG_AMOUNT 
			 FROM #BAL_LOAN 
			/*
			INSERT EMP_PAYSLIP_DET	( ROW_ID, PAYSLIP_ID, PAY_ID, LOAN_ID, PAY_TYPE, AMOUNT, ORG_AMOUNT )
			 SELECT  LEFT(@CLOCATIONID,2)+CONVERT(VARCHAR(40),NEWID()) AS ROW_ID,@CPAYSLIPID AS PAYSLIP_ID,
			 ...
			 AND LOAN_DATE NOT BETWEEN @DFROMDT AND @DTODT	
			*/ 
		END
		 SET @NSTEP = 98
	 ...
	 BEGIN	 
	   ...
	   , @BALWAYSUPDATE = 1  
		...
		--SET @CCMD=N'UPDATE '+@CTEMPDETAILTABLENAME+' SET AMOUNT=ORG_AMOUNT WHERE PAY_ID=''PAY0004'''
	 ...
	 FROM EMP_PAYSLIP_MST A JOIN 
	 (SELECT A.PAYSLIP_ID,SUM(CASE WHEN PAY_TYPE=2 THEN AMOUNT ELSE 0 END) AS EARNINGS,
...
END_PROC:
    ...
    SELECT * FROM @TRETMSG    
END               
--*************************************** END OF PROCEDURE SAVETRAN_GENPAYSLIP_3
