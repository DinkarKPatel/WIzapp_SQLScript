create PROCEDURE SAVETRAN_GENPAYSLIP
(                      
 @NUPDATEMODE INT=1,
 @CREFID VARCHAR(20),                       
 @NYEAR INT,                       
...
BEGIN TRY                      
	 ...
	 @CPAYSLIPVERSION VARCHAR(2),@NNEXTYEAR INT
	 ...
	 SELECT TOP 1 @CPAYSLIPVERSION=VALUE FROM CONFIG (NOLOCK) WHERE CONFIG_OPTION='PAYSLIP_VERSION'
	 
	 IF ISNULL(@CPAYSLIPVERSION,'')<>''
	 BEGIN
		IF @CPAYSLIPVERSION='1'
			EXEC  SAVETRAN_GENPAYSLIP_1 
			...
			@NSPID=@NSPID
		ELSE
		IF @CPAYSLIPVERSION='2'
			EXEC  SAVETRAN_GENPAYSLIP_2 
			@NUPDATEMODE=@NUPDATEMODE,
			@CREFID=@CREFID,                       
			@NYEAR=@NYEAR,                       
			@NMONTH=@NMONTH,
			...
			@NSPID=@NSPID	
		ELSE
		IF @CPAYSLIPVERSION='3'
			EXEC  SAVETRAN_GENPAYSLIP_3 
			@NUPDATEMODE=@NUPDATEMODE,
			@CREFID=@CREFID,                       
			@NYEAR=@NYEAR,                       
			@NMONTH=@NMONTH,
	 ...
	 BEGIN
		 ...
		 SELECT TOP 1 @CPFPCT=VALUE FROM CONFIG WHERE CONFIG_OPTION='PF'
		 ...
		 WHERE A.EMP_ID=@CEMPID AND A.PAY_ID NOT IN ('PAY0002','PAY0001')  
		 
		 SET @NSTEP = 98
		 SELECT @NTOTALEARNINGS=@NGROSSSALARY+SUM(AMOUNT) FROM EMP_PAYSLIP_DET
		 WHERE PAYSLIP_ID=@CPAYSLIPID AND PAY_TYPE=2
		 
		 SET @NTOTALEARNINGS=ISNULL(@NTOTALEARNINGS,0)
		 
		 SET @NSTEP = 99
		 INSERT EMP_PAYSLIP_DET	( ROW_ID, PAYSLIP_ID, PAY_ID, LOAN_ID, PAY_TYPE, AMOUNT, ORG_AMOUNT )
		 SELECT  @CLOCATIONID+CONVERT(VARCHAR(40),NEWID()) AS ROW_ID,@CPAYSLIPID AS PAYSLIP_ID,
		 'PAY0002' AS PAY_ID,'' AS LOAN_ID,1 AS PAY_TYPE,@NTOTALEARNINGS*@NESIPCT/100 AS AMOUNT,@NGROSSSALARY*@NESIPCT/100 
		 AS ORG_AMOUNT 
		 FROM EMP_MST A WHERE A.EMP_ID=@CEMPID AND ESI_ENABLED=1
		 UNION
		 SELECT  @CLOCATIONID+CONVERT(VARCHAR(40),NEWID()) AS ROW_ID,@CPAYSLIPID AS PAYSLIP_ID,
		 'PAY0001' AS PAY_ID,'' AS LOAN_ID,1 AS PAY_TYPE,@NTOTALEARNINGS*@NPFPCT/100 AS AMOUNT,@NGROSSSALARY*@NESIPCT/100 
		 AS ORG_AMOUNT 
	 ...
	 BASIC_SALARY=(CASE WHEN @NUPDATEMODE=1 THEN (BASIC_SALARY/31)*@NSALARYDAYS ELSE BASIC_SALARY END)
	 FROM EMP_PAYSLIP_MST A JOIN (SELECT A.PAYSLIP_ID,SUM(CASE WHEN PAY_TYPE=2 THEN AMOUNT ELSE 0 END) AS EARNINGS,
		   SUM(CASE WHEN PAY_TYPE=1 THEN AMOUNT ELSE 0 END) AS DEDUCTIONS FROM  	
...
BEGIN CATCH
	...
	SELECT @CERRORMSG='PROCEDURE SAVETRAN_GENPAYSLIP STEP :'+STR(ISNULL(@NSTEP,0))+ ' LINE NO. :'+
...
END               
--*************************************** END OF PROCEDURE SAVETRAN_GENPAYSLIP
