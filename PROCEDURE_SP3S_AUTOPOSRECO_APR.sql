CREATE PROCEDURE SP3S_AUTOPOSRECO_APR
@nMode NUMERIC(1,0),
@cLocIdPara CHAR(2)='',
...
BEGIN TRY
	...
	BEGIN
		...
		SELECT TOP 5000 cancelled,memo_dt memo_dt,memo_ID  memo_id,
		ISNULL(total_quantity,0) as quantity,'APR' as XN_TYPE,a.LAST_UPDATE
		FROM approval_return_mst A WITH (NOLOCK)
	...
	BEGIN
		...
		LEFT OUTER JOIN approval_return_mst b (NOLOCK)	ON a.memo_id=b.memo_ID
	...
	BEGIN
		...
		FROM approval_return_mst B (NOLOCK) JOIN #auto_posreco_data_upload a (NOLOCK) ON a.memo_id=b.memo_ID 
		WHERE a.xn_type='APR'

		SET @cStep='70'
		UPDATE B SET HO_SYNCH_LAST_UPDATE=''
		FROM approval_return_mst B (NOLOCK) JOIN #auto_posreco_data_mismatch a (NOLOCK) ON a.memo_id=b.memo_ID 
	END
...
BEGIN CATCH
	SET @cErrormsg='Error in Procedure SP3S_GETPOSRECO_APR_DATA at Step#'+@cStep+' '+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
