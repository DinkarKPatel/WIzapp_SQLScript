CREATE PROCEDURE SP3S_IMPORT_SLS_DATA    
(    
 @CLOCID VARCHAR(10),          
 @NMODE INT=0      ,    
 @CSPID VARCHAR(50)    
...
BEGIN TRANSACTION          
 ...
 ELSE    
 BEGIN    
 EXEC GETNEXTKEY_OPT 'STATE', 'STATE_CODE', 7, @CLOCID, 1,'',0, '',@cstate_code OUTPUT         
 ...
 EXEC GETNEXTKEY'CITY', 'CITY_CODE', 7, 'JM', 1,'',0 ,@cCity_Code OUTPUT         
 ...
 ELSE    
 BEGIN    
 EXEC GETNEXTKEY_OPT 'AREA', 'AREA_CODE', 7, @CLOCID, 1,'',0, '',@cArea_Code OUTPUT         
...
FROM #SLSIMPORT_CUSTDYM WHERE ISNULL(USER_CUSTOMER_CODE,'')<>''    
 ...
 END    
  ...
  --UPDATE SLS_IMPORT_DATA SET MEMO_DT=CONVERT(VARCHAR(20),GETDATE(),111) WHERE ISNULL(MEMO_DT,'')=''         
    ...
    CAST(ISNULL(CANCELLED,0) AS BIT) AS CANCELLED,'''' AS ERROR_MESSAGE,A.MRP,ISNULL(A.CMD_DISCOUNT_AMOUNT,0) AS DISCOUNT_AMT,A.NET_AMOUNT CASH,0 CC_AMOUNT,'''' CC_NAME          
 ,A.CMM_DISCOUNT_AMOUNT,A.CMM_OTHER_CHARGES,ISNULL(A.HSN_CODE  ,B.HSN_CODE) AS HSN_CODE ,A.gst_percentage ,A.igst_amount ,A.cgst_amount,A.sgst_amount     
 ,A.NET,ISNULL(A.NET_AMOUNT,A.NET) AS NET_AMOUNT
    FROM SLS_IMPORT_DATA A  (NOLOCK)         
    LEFT JOIN SKU B (NOLOCK) ON A.PRODUCT_CODE=B.PRODUCT_CODE WHERE A.SP_ID='''+@CSPID+''' AND DEPT_ID='''+@CLOCID+''''          
    PRINT @CCMD          
 ...
 HSN_CODE   ,gst_percentage ,igst_amount ,cgst_amount,sgst_amount,NET,NET_AMOUNT )            
...
--return     
 ...
 -- SELECT * FROM #SLSIMPORT           
  ...
  INTO #NEWBILLS          
  FROM #SLSIMPORT A          
  LEFT JOIN CMM01106 B(NOLOCK) ON A.DEPT_ID+'-'+ISNULL(A.BILL_NO,'')=B.REF_NO AND A.FIN_YEAR=B.FIN_YEAR          
...
-- SET @CMEMONOVAL=@CLOCID+@CLOCID+'-'+@CBILL_NO        
  ...
  SELECT DISTINCT 'IMPORTED SALE.' +ISNULL(REMARKS,'') AS REMARKS,0 AS SENT_FOR_RECON,1 AS PARTY_TYPE,'0000000000' AS AC_CODE          
    ...
    ,@cCUSTOMER_CODE AS CUSTOMER_CODE, CANCELLED,'0000000' AS USER_CODE,GETDATE() AS LAST_UPDATE          
  ...
  FROM #EXISTINGBILLS          
  GROUP BY DEPT_ID,CM_NO,CM_DT,CANCELLED,CM_ID,FIN_YEAR,BILL_NO ,REMARKS     
  UNION           
  SELECT DISTINCT 'IMPORTED SALE.'+ ISNULL(REMARKS,'') AS REMARKS,0 AS SENT_FOR_RECON,1 AS PARTY_TYPE,'0000000000' AS AC_CODE          
    ,0 AS MANUAL_DISCOUNT,0 AS MANUAL_ROUNDOFF,'000' AS BIN_ID,0 AS PATCHUP_RUN          
...
--SELECT * FROM CMM01106(NOLOCK) WHERE CM_ID IN (SELECT CM_ID FROM #NEWBILLS)           
  ...
  */          
  INSERT CMD01106 ( PRODUCT_CODE, QUANTITY, MRP, NET, DISCOUNT_PERCENTAGE, DISCOUNT_AMOUNT, ROW_ID, LAST_UPDATE          
   ...
   , NRM_ID ,HSN_CODE   ,gst_percentage ,igst_amount ,cgst_amount,sgst_amount )            
  SELECT    A.PRODUCT_CODE, A.QUANTITY, A.MRP,A.NET AS NET,    
  ...
  --ELSE 0 END)          
  --  AS DISCOUNT_AMOUNT          
  0 AS DISCOUNT_PERCENTAGE,ISNULL(A.DISCOUNT_AMT,0) AS DISCOUNT_AMOUNT    
  ...
  --ELSE 0 END)          
  --  AS DISCOUNT_AMOUNT          
  ,0 AS DISCOUNT_PERCENTAGE,ISNULL(DISCOUNT_AMT,0) AS DISCOUNT_AMOUNT    
    ,A.DEPT_ID+CONVERT(VARCHAR(40),NEWID()) AS ROW_ID,GETDATE() AS LAST_UPDATE          
    ,0 AS TAX_PERCENTAGE,0 AS TAX_AMOUNT,'0000000' AS EMP_CODE,'' AS SLSDET_ROW_ID          
    ,'000' AS BIN_ID,0 AS OLD_MRP,'' AS REF_SLS_MEMO_ID,0 AS REALIZE_SALE          
    ,A.CM_ID AS CM_ID,0 AS RFNET,1 AS TAX_TYPE,1 AS TAX_METHOD,'' AS EAN,'0000000' AS EMP_CODE1          
...
END_PROC:          
  ...
  --DELETE FROM SLS_IMPORT_DATA WHERE DEPT_ID=@CLOCID           
  ...
  --    INSERT SLS_MBODATA_ERROR_DETAILS ( PRODUCT_CODE, REF_NO, DEPT_ID, ERROR_MSG )           
  --    SELECT PRODUCT_CODE, MEMO_NO,@CLOCID AS  DEPT_ID, ISNULL(@CERRMSG,'') FROM SLS_IMPORT_DATA WHERE DEPT_ID=@CLOCID          
  --  ELSE          
  --    INSERT SLS_MBODATA_ERROR_DETAILS ( PRODUCT_CODE, REF_NO, DEPT_ID, ERROR_MSG )           
  --    SELECT PRODUCT_CODE, MEMO_NO,@CLOCID AS  DEPT_ID, ISNULL(@CERRMSG,'') FROM SLS_IMPORT_DATA     
  -- WHERE DEPT_ID=@CLOCID    AND ISNULL(ERRORMSG,'')<>''          
  --END          
  IF EXISTS (SELECT PRODUCT_CODE FROM @TERRORDETAILS )    
 BEGIN    
 ...
 END    
  ...
  ELSE          
    SELECT PRODUCT_CODE, MEMO_NO AS REF_NO,@CLOCID AS  DEPT_ID, ISNULL(@CERRMSG,'')     
 ERRMSG FROM SLS_IMPORT_DATA           
    WHERE SP_ID=@CSPID        
 ...
 IF OBJECT_ID('TEMPDB..#SLSIMPORT_CUSTDYM','U') IS NOT NULL      
  ...
  (      
	...
	UPDATE A SET A.errormsg=ISNULL(A.errormsg,'')+CHAR(13)+(CASE WHEN B.HSN_CODE IS NULL  THEN 'HSN Code not found.' ELSE '' END)
	  ...
	  WHERE A.SP_ID=@CSPID AND B.HSN_CODE IS NULL
...
FROM #SLSIMPORT_CUSTDYM WHERE ISNULL(USER_CUSTOMER_CODE,'')<>''
 ...
 END
  ...
  --SELECT '1'   
    SET @CCMD=N'SELECT 0 AS SR_NO,MEMO_NO AS BILL_NO,MEMO_DT AS CM_DT,DEPT_ID,A.PRODUCT_CODE,QUANTITY,A.NET AS AMOUNT,      
    CAST(ISNULL(CANCELLED,0) AS BIT) AS CANCELLED,'''' AS ERROR_MESSAGE,A.MRP,A.CMD_DISCOUNT_AMOUNT AS DISCOUNT_AMT,A.NET_AMOUNT CASH,0 CC_AMOUNT,'''' CC_NAME      
	,A.CMM_DISCOUNT_AMOUNT,A.CMM_OTHER_CHARGES,A.HSN_CODE   ,A.gst_percentage	,A.igst_amount ,A.cgst_amount,A.sgst_amount 
	,A.NET,A.NET_AMOUNT
    FROM SLS_IMPORT_DATA A  (NOLOCK)     
    LEFT JOIN SKU B (NOLOCK) ON A.PRODUCT_CODE=B.PRODUCT_CODE WHERE A.SP_ID='''+@CSPID+''' AND DEPT_ID='''+@CLOCID+''''      
    PRINT @CCMD      
...
--	SET @CMEMONOVAL=@CLOCID+@CLOCID+'-'+@CBILL_NO    
  ...
  SELECT DISTINCT 'IMPORTED SALE.' +ISNULL(REMARKS,'') AS REMARKS,0 AS SENT_FOR_RECON,1 AS PARTY_TYPE,'0000000000' AS AC_CODE      
    ...
    ,CM_DT,1 AS CM_MODE      
    ,SUM(CASE WHEN NET>0 THEN NET  ELSE 0 END) AS SUBTOTAL      
    ,'0000000' AS DT_CODE,0 AS DISCOUNT_PERCENTAGE,SUM(CMM_DISCOUNT_AMOUNT) AS DISCOUNT_AMOUNT      
    ,ISNULL(NET_AMOUNT      ,0)
    ,@cCUSTOMER_CODE AS CUSTOMER_CODE, CANCELLED,'0000000' AS USER_CODE,GETDATE() AS LAST_UPDATE      
    ,0 AS EXEMPTED,0 AS SENT_TO_HO,GETDATE() AS CM_TIME, CM_ID,'' AS REF_CM_ID      
    , FIN_YEAR,SUM(CMM_OTHER_CHARGES) AS ATD_CHARGES,0 AS COPIES_PTD,0 AS ROUND_OFF,1 AS MEMO_TYPE,1 AS PAY_MODE,0 AS SMS_SENT      
    ,1 AS AUTOENTRY,0 AS CASH_TENDERED      
    ,0 AS PAYBACK,'' AS ECOUPON_ID,'' AS CAMPAIGN_GC_OTP      
    ,0 AS SALESSETUPINEFFECT,@CUSER_CODE AS EDT_USER_CODE,0 AS GV_AMOUNT      
    ,DEPT_ID+'-'+BILL_NO AS REF_NO,0 AS SENT_FOR_GR  , 1 as xn_item_type     
  FROM #EXISTINGBILLS      
  GROUP BY DEPT_ID,CM_NO,CM_DT,CANCELLED,CM_ID,FIN_YEAR,BILL_NO ,REMARKS  ,ISNULL(NET_AMOUNT   ,0)
  UNION       
  SELECT DISTINCT 'IMPORTED SALE.'+ ISNULL(REMARKS,'') AS REMARKS,0 AS SENT_FOR_RECON,1 AS PARTY_TYPE,'0000000000' AS AC_CODE      
    ,0 AS MANUAL_DISCOUNT,0 AS MANUAL_ROUNDOFF,'000' AS BIN_ID,0 AS PATCHUP_RUN      
    ,SUM(CASE WHEN NET<0 THEN NET ELSE 0 END) AS SUBTOTAL_R,'' AS PASSPORT_NO,'' AS TICKET_NO,'' AS FLIGHT_NO,0 AS MRP_WSP,0 AS MANUAL_BILL,0 AS FC_RATE      
...
--SELECT * FROM CMM01106(NOLOCK) WHERE CM_ID IN (SELECT CM_ID FROM #NEWBILLS)       
  ...
  */      
  ...
  --ELSE 0 END)      
  --  AS DISCOUNT_AMOUNT      
  0 AS DISCOUNT_PERCENTAGE,A.DISCOUNT_AMT AS DISCOUNT_AMOUNT
    ,A.DEPT_ID+CONVERT(VARCHAR(40),NEWID()) AS ROW_ID,GETDATE() AS LAST_UPDATE      
    ,0 AS TAX_PERCENTAGE,0 AS TAX_AMOUNT,'0000000' AS EMP_CODE,'' AS SLSDET_ROW_ID,      
    '000' AS BIN_ID,0 AS OLD_MRP,'' AS REF_SLS_MEMO_ID,0 AS REALIZE_SALE      
    ,A.CM_ID AS CM_ID,0 AS RFNET,1 AS TAX_TYPE,1 AS TAX_METHOD,'' AS EAN,'0000000' AS EMP_CODE1      
  ...
  --ELSE 0 END)      
  --  AS DISCOUNT_AMOUNT      
  ,0 AS DISCOUNT_PERCENTAGE,DISCOUNT_AMT AS DISCOUNT_AMOUNT
    ,A.DEPT_ID+CONVERT(VARCHAR(40),NEWID()) AS ROW_ID,GETDATE() AS LAST_UPDATE      
    ,0 AS TAX_PERCENTAGE,0 AS TAX_AMOUNT,'0000000' AS EMP_CODE,'' AS SLSDET_ROW_ID      
    ,'000' AS BIN_ID,0 AS OLD_MRP,'' AS REF_SLS_MEMO_ID,0 AS REALIZE_SALE      
    ,A.CM_ID AS CM_ID,0 AS RFNET,1 AS TAX_TYPE,1 AS TAX_METHOD,'' AS EAN,'0000000' AS EMP_CODE1      
