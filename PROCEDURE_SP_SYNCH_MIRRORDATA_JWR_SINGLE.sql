create PROCEDURE SP_SYNCH_MIRRORDATA_JWR_SINGLE
(
    @CLOCID VARCHAR(3)
...
BEGIN TRY
	...
	SELECT @CTEMPMASTERTABLE=@CSOURCEDB+'JWR_JOBWORK_RECEIPT_MST_MIRROR',
		   @CTEMPDETAILTABLE=@CSOURCEDB+'JWR_JOBWORK_RECEIPT_DET_MIRROR',
		   @CTEMPDETAILTABLE1=@CSOURCEDB+'JWR_PMT01106_MIRROR'
...
LBLSTART:
    ...
    FROM JWR_JOBWORK_RECEIPT_MST_MIRROR B (NOLOCK)
    LEFT OUTER JOIN MIRROR_SYNCH_LOG C (NOLOCK) ON C.MEMO_ID=B.RECEIPT_ID AND C.XN_TYPE='JWR'
	...
	BEGIN
		...
		SELECT @CFINYEAR=FIN_YEAR FROM JWR_JOBWORK_RECEIPT_MST_MIRROR (NOLOCK) WHERE RECEIPT_ID=@CMEMOID
		...
		SELECT TOP 1 @CPREVMEMONOSEARCH=RECEIPT_NO FROM JOBWORK_RECEIPT_MST (NOLOCK) WHERE RECEIPT_NO=@CPREVMEMONO AND FIN_YEAR=@CFINYEAR
...
LBLMERGE:
	...
	SET @CTMP_TABLENAME='JWR_JOBWORK_RECEIPT_MST_MIRROR'
	...
	SET @CTMP_TABLENAME='JWR_JOBWORK_RECEIPT_DET_MIRROR'
	...
	@cTempTable='JWR_PMT01106_MIRROR',  
	@cMemoIdCol='JWR_MEMO_ID',  
	@cMemoId =@CMEMOID
    
    SET @CSTEP=330
    SELECT TOP 1 @CERRMSG=ERRMSG FROM MIRROR_SYNCH_LOG (NOLOCK) WHERE XN_TYPE='JWR' AND MEMO_ID=@CMEMOID
    
    IF ISNULL(@CERRMSG,'')=''
    BEGIN 
		SET @CSTEP=340
	    		
		DELETE A FROM JWR_JOBWORK_RECEIPT_MST_MIRROR A WHERE A.RECEIPT_ID=@CMEMOID AND A.VERSION_NO<=@NVERSIONNO

		DELETE A FROM JWR_JOBWORK_RECEIPT_DET_MIRROR A WHERE A.RECEIPT_ID=@CMEMOID AND A.VERSION_NO<=@NVERSIONNO
		DELETE A FROM JWR_PMT01106_MIRROR A WHERE A.JWR_memo_id=@CMEMOID 
...
BEGIN CATCH
	SET @CERRMSG='P:SP_SYNCH_MIRRORDATA_JWR_SINGLE, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO EXIT_PROC
END CATCH
