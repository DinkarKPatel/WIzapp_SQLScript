...
BEGIN
	...
	SELECT 'ops_qty_opt' as master_col,'a.dept_id' as loc_join_col,'a.bin_id' as bin_join_col,'' as xn_no_col,'' as xn_dt_col,'[LAYOUT_COL],    
	 ...
	 0 as OBXPC,0 as OBSP,0 as OBP1,
	 0 as OBW,SUM(CASE WHEN cnc_type=1 and stock_adj_type =2  THEN -rate 
	 WHEN cnc_type=2 THEN rate ELSE 0 END) as OBM,
	 0 as OBPWD,0 AS OBXPWG ,0 as OBXFDEP	
	...
	SELECT 'git_qty_opt' as master_col,'a.dept_id' as loc_join_col,'a.bin_id' as bin_join_col,'XN_NO' as xn_no_col,'XN_DT' as xn_dt_col,'[LAYOUT_COL],
	 ...
	 JOIN location c (NOLOCK) ON a.dept_id=c.dept_id [JOIN]
	 WHERE [WHERE]  
	 group by [GROUPBY]'
	...
	'''LM''+B.AC_CODE' as xnparty_join_col,'b.mrr_no' as xn_no_col,'b.receipt_dt' as xn_dt_col,'[LAYOUT_COL],[CALCULATIVE_COL]    
	 ...
	 IF @bHoloc=0
		...
		'''LM''+B.AC_CODE' as xnparty_join_col,'b.mrr_no' as xn_no_col,'b.receipt_dt' as xn_dt_col,'[LAYOUT_COL],[CALCULATIVE_COL]    
         ...
         LEFT OUTER JOIN [DATABASE].dbo.parcel_mst  WITH(NOLOCK) on PD.parcel_memo_id = parcel_mst.parcel_memo_id 
		 and parcel_mst.xn_type= ''WSL''  AND parcel_mst.CANCELLED=0
         LEFT OUTER JOIN [DATABASE].dbo.ANGM  WITH(NOLOCK)  ON parcel_mst.angadia_code = ANGM.Angadia_code 
	...
	[JOIN_2]
	 ...
	 LEFT OUTER JOIN [DATABASE].dbo.PARCEL_DET PD  WITH(NOLOCK) ON B.RM_ID = PD.REF_MEMO_ID 
         LEFT OUTER JOIN [DATABASE].dbo.parcel_mst  WITH(NOLOCK) on PD.parcel_memo_id = parcel_mst.parcel_memo_id 
		 and parcel_mst.xn_type= ''PRT''  AND parcel_mst.CANCELLED=0
         LEFT OUTER JOIN [DATABASE].dbo.ANGM  WITH(NOLOCK)  ON parcel_mst.angadia_code = ANGM.Angadia_code 
	...
	UNION ALL
	 ...
	 FROM year_wise_cbsstk_depcn_det A WITH (NOLOCK) 
	 JOIN location LOc (NOLOCK) ON loc.dept_id=a.dept_id [JOIN_2]
