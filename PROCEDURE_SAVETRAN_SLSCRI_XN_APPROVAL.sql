...
create PROCEDURE SAVETRAN_SLSCRI_XN_APPROVAL
(
...
BEGIN
	...
	SET @CMASTERTABLENAME	= 'SLSCRI_XN_APPROVAL'

	SET @CTEMPMASTERTABLENAME	= 'TEMP_SLSCRI_XN_APPROVAL_'+LTRIM(RTRIM(STR(@NSPID)))
	...
	BEGIN TRY
		...
		WHERE XN_TYPE='SLSCRI' AND INACTIVE=0
		
		
		SET @CCMD=N'DELETE FROM A FROM CREDITBILLS A
		...
		BEGIN	
			...
			SET @CCMD = 'UPDATE A SET A.LEVEL_NO=(CASE WHEN ISNULL(A.LEVEL_NO,0)=0 THEN B.LEVEL_NO ELSE A.LEVEL_NO END),
						 ...
						 JOIN XN_APPROVAL_CHECKLIST_LEVELS C ON C.LEVEL_NO=B.LEVEL_NO AND C.XN_TYPE=''SLSCRI'''
...
END
------------------------------------------------------ END OF PROCEDURE SAVETRAN_SLSCRI_XN_APPROVAL
