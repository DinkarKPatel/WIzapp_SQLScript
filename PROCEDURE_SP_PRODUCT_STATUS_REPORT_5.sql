CREATE PROCEDURE SP_PRODUCT_STATUS_REPORT_5
(
 @nQueryID	INT,  
 @DFM_DT DATETIME='',  
 @DTO_DT DATETIME='',  
...
BEGIN
	   ...
	   JOIN
	   (
	     SELECT A.PRODUCT_CODE ,1 AS INV_QTY 
	   ...
	   begin
		 ...
		 SET @DTSQL=N'SELECT A.DEPT_ID as [Location Name], A.BUYER_NAME as [Party] ,A.ORDER_NO as [Order No] ,  
				  ...
				  (A.ORDER_QTY - isnull(a.ShortCloseQty,0))-(isnull(('+@CWIP_QTY+'),0)+ISNULL(TTM.TTM_QTY,0)) AS [WIP Qty]  ,
		   ...
		   (  
			...
			SELECT MEMO_ID,ARTICLE_CODE,PARA1_CODE,PARA2_CODE,'+@CCOLUMN_NAME+'  
			FROM  
			(  
			 ...
			 GROUP BY A.MEMO_ID,A.ARTICLE_CODE,A.PARA1_CODE,A.PARA2_CODE,JOBWORK_NAME,JOBWORK_NAME+''1''  
			 ) A  
			  PIVOT  
				 (  
			 ...
			 GROUP BY MEMO_ID,ARTICLE_CODE,PARA1_CODE,PARA2_CODE 
		   ...
		   (  
			...
			GROUP BY A.MEMO_ID,A.ARTICLE_CODE,A.PARA1_CODE,A.PARA2_CODE 
		   ) TTM ON A.ARTICLE_CODE=TTM.ARTICLE_CODE AND A.PARA1_CODE=TTM.PARA1_CODE AND A.PARA2_CODE=TTM.PARA2_CODE  
	  ...
	  [Article],[Color] ,[Size] 
	   ...
	   sum([ShortCloseQty]) AS [ShortCloseQty]
	  FROM ##TMPDETAILS'
