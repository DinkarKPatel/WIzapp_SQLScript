create PROCEDURE DBO.SP_MERGE_MIRROR_MSTSTKLVL
  ( @NMODE INT
    ,@XN_TYPE VARCHAR(50) ='MSTSTKLVL'
   )
AS
 BEGIN
...
BEGIN TRY
	  ...
	  BEGIN
			SET @CERRORMSG='P:SP_MERGE_MIRROR_MSTSTKLVL, CANNOT CALL MERGING PROC AT HEAD OFFICE'
			GOTO END_PROC
  	  END					
	  ...
	  DELETE A  FROM LOC_STOCK_LEVEL_DET A (nolock) 
	  join LOC_STOCK_LEVEL_MST b on a.MEMO_ID =b.MEMO_DT 
	  
	  WHERE b.location_code=@CHODEPTID
	  DELETE  FROM LOC_STOCK_LEVEL_MST WHERE location_code =@CHODEPTID
	 ...
	 --INSERT DATA INTO LOCATION TABLE
	   ...
	   SET @CSTEP = 40
	   SET @CTABLENAME = 'LOC_STOCK_LEVEL_MST'
	   SET @CSOURCETABLE ='MSTSTKLVL_LOC_STOCK_LEVEL_MST_MIRROR'
	   ...
	   SET @CSTEP = 50
	   SET @CTABLENAME = 'LOC_STOCK_LEVEL_DET'
	   SET @CSOURCETABLE ='MSTSTKLVL_LOC_STOCK_LEVEL_DET_MIRROR'
...
BEGIN CATCH
	SET @CERRORMSG='P:SP_MERGE_MIRROR_MSTSTKLVL, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
...
END_PROC:
	...
	BEGIN   
	   ...
	   DELETE FROM MSTSTKLVL_LOC_STOCK_LEVEL_DET_MIRROR
	   DELETE FROM MSTSTKLVL_LOC_STOCK_LEVEL_MST_MIRROR  
	   
	END
		
	SELECT @XN_TYPE AS MEMO_ID,@CERRORMSG AS ERRMSG   	  
END
--END PROCEDURE SP_MERGE_MIRROR_MSTSTKLVL
