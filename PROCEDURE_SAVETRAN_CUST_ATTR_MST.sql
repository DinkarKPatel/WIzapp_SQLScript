create PROCEDURE SAVETRAN_CUST_ATTR_MST  
(  
 @SOURCE  VARCHAR(100),  
 @SOURCE1 VARCHAR(100),  
 @DEST  VARCHAR(100),  
...
UPDATE CONFIG_CUST_ATTR SET TABLE_CAPTION=@CONFIG,OPEN_KEY=@OPENKEY WHERE TABLE_NAME=@DEST  
...
SET @STEP=3    
SET @SNO=SUBSTRING(@DEST,10,CHARINDEX('_',@DEST)-5)  
SET @SNO=SUBSTRING(REPLACE(@DEST,'CUST_ATTR',''),1,CHARINDEX('_',REPLACE(@DEST,'CUST_ATTR',''))-1)  
SET @SNO=REPLACE(REPLACE(@DEST,'CUST_ATTR',''),'_MST','')  
--SELECT @SNO
...
DECLARE @CNEWKEYVAL VARCHAR(100),@LOC VARCHAR(10),@TAB VARCHAR(100),@NAME VARCHAR(100),@CODE VARCHAR(100),@ID VARCHAR(100)  
...
SET @CCMD+='USING (SELECT KEY_CODE=CASE WHEN KEY_CODE LIKE ''LATER%'' OR KEY_CODE LIKE ''%-%'' THEN GEN_KEY_CODE ELSE KEY_CODE END,KEY_NAME,GEN_KEY_CODE,KEY_ALIAS,ISNULL(INACTIVE,0)INACTIVE FROM ['+@SOURCE+']) S ON D.CUST_ATTR'+@SNO+'_KEY_CODE=S.KEY_CODE '+CHAR(13)    
SET @CCMD+='WHEN MATCHED THEN UPDATE SET D.CUST_ATTR'+@SNO+'_KEY_NAME=S.KEY_NAME,D.CUST_ATTR'+@SNO+'_ALIAS=S.KEY_ALIAS,D.CUST_ATTR'+@SNO+'_INACTIVE=S.INACTIVE '+CHAR(13)    
SET @CCMD+='WHEN NOT MATCHED THEN INSERT (CUST_ATTR'+@SNO+'_KEY_CODE,CUST_ATTR'+@SNO+'_KEY_NAME,CUST_ATTR'+@SNO+'_ALIAS,CUST_ATTR'+@SNO+'_INACTIVE) VALUES(S.KEY_CODE,S.KEY_NAME,S.KEY_ALIAS,S.INACTIVE);'    
...
BEGIN CATCH    
  SET @ERROR='PROCEDURE SAVETRAN_CUST_ATTR_MST: STEP- ' + LTRIM(CAST(@STEP AS VARCHAR)) + ' SQL ERROR: #' + LTRIM(STR(ERROR_NUMBER())) + ' ' + ERROR_MESSAGE()    
END CATCH    
