CREATE PROCEDURE SP3s_XPERTREPORTING
(
@iQueryId Int,
...
LBL1:
	...
	SELECT 'X' AS XN_TYPE,a.rep_code,a.rep_id,a.rep_name,
	...
	Else  'Stock Analysis' End  as rep_type,isnull(a.inActive,0)as inActive, 
	--b.rep_type,
	isnull(a.user_rep_type,'ALL')as user_rep_type,b.rep_category ,a.remarks,c.username ,a.Last_update,a.XPERT_REP_CODE
	FROM rep_mst a (NOLOCK) 
	JOIN reportType b (NOLOCK) ON a.rep_code=b.rep_code 
	JOIN USERS C on a.user_code = c.user_code 
	WHERE  isnull(a.report_item_type,1) = 1 and a.rep_code= 'X001'  AND isnull(a.xn_history,0)<> 1
	AND ISNULL(a.XPERT_REP_CODE ,'') NOT IN ('', 'R6','XNHISTORY')
    And b.rep_category Not in ('MSC','SPR','XNO') and b.rep_code <> 'C001'  and a.user_code = @cUserCode

	UNION 
	SELECT 'Y' AS XN_TYPE,a.rep_code,a.rep_id,a.rep_name,	
	case when a.XPERT_REP_CODE='R2' then 'Transaction Analysis'  
	when  a.XPERT_REP_CODE='R3' then 'Transaction Summary'
	when  a.XPERT_REP_CODE='R4' then 'Pendency Analysis'	
	when  a.XPERT_REP_CODE='R5' then 'Customer Analysis'	
	...
	WHERE   a.rep_code not in ('Z001','C001') and r.user_code = @cUserCode
	And isnull(a.XPERT_REP_CODE,'') <> ''  and a.rep_code= 'X001' AND isnull(a.xn_history,0)<> 1
	UNION 
	SELECT 'Z' AS XN_TYPE,a.rep_code,a.rep_id,a.rep_name,'Custom Report',isnull(a.inActive,0)as inActive, 
	isnull(b.rep_type,'ALL')as user_rep_type,b.rep_category ,a.remarks,c.username,a.Last_update,a.XPERT_REP_CODE
	FROM rep_mst a (NOLOCK) 
	Left outer JOIN reportType b (NOLOCK) ON a.rep_code=b.rep_code 
	JOIN USERS C (nolock) on a.user_code = c.user_code 
	WHERE  isnull(a.XPERT_REP_CODE,'') <> '' and b.rep_category  in ('SPR')  and a.user_code = @cUserCode and a.rep_code= 'X001'
