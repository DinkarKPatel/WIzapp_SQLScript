create PROCEDURE SP3S_NORMALIZE_BARCODE
(
   @CDEPT_ID VARCHAR(5)='',
   @cspid varchar(50),
   @cUserCode char(10)='',
...
BEGIN
	  ...
	  SELECT PRODUCT_CODE, isnull(mrp,0) as mrp,bin_id 
		 INTO #tmpitemUnique
	  FROM XN_BARCODENORMALIZED_UPLOAD (nolock)
	  WHERE SP_ID =@CSPID 
	  GROUP BY PRODUCT_CODE, isnull(mrp,0),bin_id 
	...
	FROM XN_BARCODENORMALIZED_UPLOAD A (NOLOCK)
	JOIN #TMPITEM B ON A.PRODUCT_CODE =B.BARCODE_WOBATCH 
	AND A.SRNO =B.SRNO and isnull(a.mrp,0)=b.mrp  and a.bin_id=b.BIN_ID
	WHERE A.SP_ID=@CSPID

	 if @bAllowNegative=1 and exists(select top 1 'u' from XN_BARCODENORMALIZED_UPLOAD a (nolock) WHERE A.SP_ID=@CSPID  AND ISNULL(a.ORG_PRODUCT_CODE,'')='')
	 begin
	    
		UPDATE B SET ORG_PRODUCT_CODE= A.PRODUCT_CODE 
		FROM  sku A (nolock)
		JOIN XN_BARCODENORMALIZED_UPLOAD B  (NOLOCK) ON LEFT(A.PRODUCT_CODE, ISNULL(NULLIF(CHARINDEX ('@',A.PRODUCT_CODE)-1,-1),LEN(A.PRODUCT_CODE )))=B.PRODUCT_CODE 
	...
	begin
		...
		SELECT Product_code, Org_product_code,count(*) as Quantity ,
			 case when isnull(Org_product_code,'')='' then @CERRMSG else ''  end  AS ERRMSG,
			 isnull(a.mrp ,0) as mrp,a.bin_id 
		FROM XN_BARCODENORMALIZED_UPLOAD A (NOLOCK)
		WHERE A.SP_ID=@CSPID
		group by Product_code,Org_product_code,isnull(a.mrp ,0),a.bin_id 
