...
CREATE Procedure SP3S_CALCULATE_FCNET_PURQT
(
 @CMEMOID VARCHAR(50),
...
BEGIN
		...
		BEGIN
		 ...
		 END
          ...
          INSERT INTO #TMPFCRATE(ROW_ID,FC_NET,FC_GST_PERCENTAGE,FC_TAX_METHOD,IS_IGST)
			SELECT A.ROW_ID ,
			       FC_NET = ((A.PURCHASE_PRICE*A.QUANTITY)-(A.PURCHASE_PRICE*A.QUANTITY*ISNULL(B.DISCOUNT_PERCENTAGE,0)/100))/B.fc_rate,
			...
			JOIN purchase_quotation_master B (NOLOCK) ON A.PQ_ID=B.PQ_ID 
		...
		BEGIN
		 ...
		 DECLARE @NFCOTH NUMERIC(10,2),@NFCFREIGHT NUMERIC(10,2)
		   ...
		   FROM purchase_quotation_master WITH (NOLOCK) WHERE PQ_ID= @CMEMOID
