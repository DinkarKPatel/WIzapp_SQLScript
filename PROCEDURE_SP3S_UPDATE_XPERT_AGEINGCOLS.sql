CREATE PROCEDURE SP3S_UPDATE_XPERT_AGEINGCOLS
@cRepTableName varchar(100),
@cTempDb varchar(100),
...
BEGIN TRY
	...
	BEGIN
		...
		BEGIN
			...
			@nPrevAgeDays+1  as ageing_days,'ageing_'+(CASE WHEN @CPrevAgeingCol='purchase ageing' THEN '1' 
			WHEN @CPrevAgeingCol='Sale ageing' THEN '2' ELSE '3' END) rep_ageing_col
		...
		(CASE WHEN ageing_col='purchase ageing' THEN '1' WHEN ageing_col='Sale ageing' THEN '2' ELSE '3' END)
	...
	BEGIN
		...
		SET @cStep='76'
		SET @cCmd=N'UPDATE '+@cTempDb+'['+@cRepTableName+'] SET '+@cRepAgeingColUpd+'='''+@cRepColUpdExpr+
					''' WHERE ['+@cAgeingCol+']'+(CASE WHEN CHARINDEX('-',@cRepColUpdExpr)=0 THEN @cRepColUpdExpr
	...
	--IF EXISTS (SELECT TOP 1 col_expr FROM #rep_det WHERE col_expr='ageing_1')
	--BEGIN
	--	SET @cCmd=N'UPDATE a SET ageing_1=(CASE WHEN '
	--END
...
BEGIN CATCH
	SET @cErrormsg='Error in Procedure SP3S_UPDATE_XPERT_AGEINGCOLS at Step:'+@cStep+' '+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
