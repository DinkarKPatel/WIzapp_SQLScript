CREATE PROCEDURE SP3S_REPROCESS_WSR_BILL_DISCOUNT
(
 @NSPID varchar(40)=''
...
BEGIN
 BEGIN TRY
	  ...
	  set @cStep=10
		...
		SELECT   @NTOTAL_DISCOUNT_AMOUNT= ISNULL(DISCOUNT_AMOUNT,0)  FROM #tWsrMstTable
	       
		LBLCALCULATE_PRT: 
	    
		set @cStep=20
	    SELECT  @NITEM_DISCOUNT_AMOUNT= SUM(ISNULL(cnmdiscOUNTAMOUNT,0))  FROM #tWsrDetTable
		...
		(SELECT A.*,SR=ROW_NUMBER() OVER (ORDER BY ROW_ID)
			...
			UPDATE CTE SET cnmdiscOUNTAMOUNT=cnmdiscOUNTAMOUNT+'+LTRIM(RTRIM(str(@CIN_VALUE,10,2)))+' WHERE SR<='''+STR(@NTOTALDIFF)+''''
