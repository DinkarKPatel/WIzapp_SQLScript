...
CREATE PROCEDURE SAVETRAN_PRT_XN_APPROVAL
(
...
BEGIN
	...
	SET @CMASTERTABLENAME	= 'PRT_XN_APPROVAL'

	SET @CTEMPMASTERTABLENAME	= 'TEMP_PRT_XN_APPROVAL_'+LTRIM(RTRIM(STR(@NSPID)))
	...
	BEGIN TRY
		...
		SET @CCMD=N'UPDATE A 
					SET APPROVEDLEVELNO=(CASE WHEN ISNULL(C.APPROVAL_STATUS,0)=1 AND 
  						 ...
  						 FROM RMM01106 A
  						 JOIN '+@CTEMPMASTERTABLE+' C ON A.RM_ID=C.RM_ID '
  		PRINT @CCMD  						   
		...
		BEGIN	
			...
			SET @CCMD = 'UPDATE A SET A.LEVEL_NO=(CASE WHEN ISNULL(A.LEVEL_NO,0)=0 THEN B.LEVEL_NO ELSE A.LEVEL_NO END),
						 ...
						 JOIN XN_APPROVAL_CHECKLIST_LEVELS C ON C.LEVEL_NO=B.LEVEL_NO AND C.XN_TYPE=''PRT'''
...
END
------------------------------------------------------ END OF PROCEDURE SAVETRAN_PRT_XN_APPROVAL
