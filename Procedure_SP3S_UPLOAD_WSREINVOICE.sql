...
CREATE PROCEDURE SP3S_UPLOAD_WSREINVOICE    
(        
@CXNID VARCHAR(50)=''        
)        
AS        
BEGIN        
   ...
   set @CEINVOICE_PREFIX=ISNULL(@CEINVOICE_PREFIX,'')
    ...
    WHERE A.XN_TYPE ='wsr' AND B.REF_MEMO_ID=@CXNID          
    AND A.CANCELLED =0        
...
set @NPICKINGSHIPPINGADDRESS=0
 ...
 IF OBJECT_ID('TEMPDB..#TMPINVOICE','U') IS NOT NULL  
   ...
   'CRN' AS TYP,  
   NO=CASE WHEN (@CEINVOICE_PREFIX<>'' AND LEFT(RTRIM(LTRIM(A.cn_no)),1)='0') THEN @CEINVOICE_PREFIX ELSE '' END +RTRIM(LTRIM(A.cn_no)),    --RTRIM(LTRIM(A.cn_no)),
   -- a.INV_NO  AS NO,---- EVERY  NO CAHNE NOT MATCH INV_NO BOTH ARE DIFFERENT      
   convert(varchar,A.cn_dt,103) AS INV_DT,  -- CONVERT INVDT NOT TIME      
   ...
   lm.ac_gst_state_code  AS POS,      
   LM.ADDRESS1 AS ADDR1 ,-- LM.ADDRESS1 SHOULD NOT BE BLANK      
   case when isnull(LM.ADDRESS2,'')='' then '...' else LM.ADDRESS2 end  AS ADDR2,  --LM.ADDRESS2 SHOULD NOT BE BLANK      
   LM.AREA_NAME +' '+lm.CITY  B_LOC ,lm.PINCODE AS B_PIN,-- LM.PINCODE PIN CODE MATCH ADDRESS        
   lm.ac_gst_state_code AS B_STCD--LM.AC_GST_STATE_CODE  STCD IS NOT SAME STATECODE FOR b_STCd      
   ,case when isnull( LM.MOBILE,'')='' then lm.PHONES_O else lm.mobile end AS B_PH,LM.E_MAIL  AS B_EM ,  
  
 --******DispDtls******   
   L.DEPT_NAME AS DIS_NM,
   L.ADDRESS1 AS DIS_ADDR1,  
   L.ADDRESS2 AS DIS_ADDR2,        
   ...
   l.gst_state_code AS DIS_STCD,  --mANADATORY STCD SENDER GSTNO     
    ...
    --******ShipDtls******   
    UPPER(LM.Ac_gst_no) SHIP_GSTIN,
	LM.AC_NAME  SHIP_LGLNM,
    LM.AC_NAME SHIP_TRDNM,
    LM.ADDRESS0 SHIP_ADDR1,
	LM.ADDRESS1 SHIP_ADDR2,
	lm.AREA_NAME +' '+ lm.city SHIP_LOC,
    lm.PINCODE  SHIP_PIN,
	lm.ac_gst_state_code   SHIP_STCD,

    ROW_NUMBER () OVER (ORDER BY cnd.HSN_CODE )  AS SLNO,
   cnd.HSN_CODE AS PRDDESC,        
   CASE WHEN XN_ITEM_TYPE =4 THEN 'Y' ELSE  'N' END AS ISSERVC,
   cnd.HSN_CODE AS HSNCD,
   cnd.HSN_CODE AS BARCDE,        
   CAST(SUM(cnd.INVOICE_QUANTITY) AS NUMERIC(10,3)) AS QTY,
   CAST(SUM(cnd.SCHEME_QUANTITY) AS NUMERIC(10,3)) AS FREEQTY,
    CASE WHEN ISNULL(UOM.GST_UOM_NAME,'')<>'' THEN LEFT(UOM.GST_UOM_NAME,3) ELSE UOM.UOM_NAME END AS UNIT,     
   
   
   CAST((XN_VALUE_WITHOUT_GST+(CND.DISCOUNT_AMOUNT +CND.CNMDISCOUNTAMOUNT)) /INVOICE_QUANTITY  AS NUMERIC(14,2)) AS UNITPRICE,
   CAST((SUM(XN_VALUE_WITHOUT_GST+(CND.DISCOUNT_AMOUNT +CND.CNMDISCOUNTAMOUNT)))   AS NUMERIC(14,2)) AS TOTAMT, --SUBT
   CAST(sum(isnull(cnd.DISCOUNT_AMOUNT,0) +isnull(cnd.CNMDISCOUNTAMOUNT,0))  AS NUMERIC(10,2)) AS DISCOUNT,

   
   --CAST(XN_VALUE_WITHOUT_GST /INVOICE_QUANTITY  AS NUMERIC(14,2)) AS UNITPRICE,
   --CAST(SUM(XN_VALUE_WITHOUT_GST) AS NUMERIC(10,2)) AS TOTAMT, --SUBT
   --cast(0  as numeric(10,2)) AS DISCOUNT,
   ...
   cnd.GST_PERCENTAGE AS GSTRT,
   CAST(SUM(cnd.IGST_AMOUNT) AS NUMERIC(14,2)) AS IGSTAMT,
   CAST(SUM(cnd.CGST_AMOUNT) AS NUMERIC(14,2)) AS CGSTAMT,        
   CAST(SUM(cnd.CGST_AMOUNT) AS NUMERIC(14,2)) AS SGSTAMT,
   isnull(cnd.GST_CESS_PERCENTAGE,0) AS CESRT,
   SUM(isnull(cnd.gst_CESS_AMOUNT,0)) AS  CESAMT,        
  ...
  --*******ValDtls***********  
   ...
   ASSVAL= cast(@NTABLABLEVALUE+isnull(a.other_charges_taxable_value,0)+isnull(a.freight_taxable_value,0)+isnull(a.insurance_taxable_value,0)  as numeric(18,2)) ,--+ISNULL(@NDISCOUNT_AMOUNT,0)+A.DISCOUNT_AMOUNT ,--net_rate,---ASSVAL WITHOUT TAX VALE  --SUBT
   CGSTVAL=cast(@NCGSTVAL+isnull(a.other_charges_cgst_amount ,0)+isnull(a.freight_cgst_amount ,0)+isnull(a.insurance_cgst_amount ,0) as numeric(18,2)) ,-- cgst_amount, ----@NCGSTVAL,    
   SGSTVAL=cast(@NSGSTVAL+isnull(a.other_charges_sgst_amount ,0)+isnull(a.freight_sgst_amount ,0)+isnull(a.insurance_sgst_amount ,0) as numeric(18,2)),--sgst_amount,-- , 
   IGSTVAL=cast(@NIGSTVAL+isnull(a.other_charges_Igst_amount ,0)+isnull(a.freight_igst_amount ,0)+isnull(a.insurance_igst_amount ,0) as numeric(18,2)),--IND.IGST_AMOUNT , --- ,   
   ...
   TOTINVVALFC=CAST((@NTABLABLEVALUE+@NCGSTVAL+@NSGSTVAL+@NIGSTVAL+@NCESSaMOUNT
    +a.FREIGHT +a.OTHER_CHARGES+isnull(a.insurance,0)+
   case when a.OH_TAX_METHOD <>2 then 
  ...
  --*******PayDtls***********  
   ...
   INVNO=CASE WHEN (@CEINVOICE_PREFIX<>'' AND  LEFT(RTRIM(LTRIM(A.cn_no)),1)='0') THEN @CEINVOICE_PREFIX ELSE '' END +RTRIM(LTRIM(A.cn_no)),
   INVDT= convert(varchar(10),A.cn_dt,103),  --A.INV_DT ,        
   OTHREFNO='OTH-'+A.cn_no ,--MANADTORY    
   ...
   DISTANCE=CAST(isnull(a.ewaydistance,0) AS NUMERIC(10,0)),-- NOT BE 0  
   ...
   JOIN SKU_NAMES SN (NOLOCK) ON SN.PRODUCT_CODE =cnd.PRODUCT_CODE         
   JOIN LOC_VIEW L (NOLOCK) ON L.DEPT_ID =LEFT(A.cn_id,2)        
   LEFT JOIN GST_STATE_DET GD (NOLOCK) ON GD.GST_STATE_CODE =L.GST_STATE_CODE AND A.receipt_dt BETWEEN GD.FM_DT AND GD.TO_DT         
  ...
  WHERE A.cn_id=@CXNID    and ISNULL(a.IRN_QR_CODE,'')='' and a.receipt_dt <>''
   group by UPPER(L.LOC_GST_NO) ,  SUPPLY_TYPE_CODE, A.party_state_code,  
   --******SellerDtls******  
   RTRIM(LTRIM(A.cn_no)),  
   CASE WHEN XN_ITEM_TYPE =4 THEN 'Y' ELSE  'N' END,
   convert(varchar,A.cn_dt,103),A.FIN_YEAR,  -- CONVERT INVDT NOT TIME      
   ...
   lm.ac_gst_state_code,     
   LM.ADDRESS1  ,-- LM.ADDRESS1 SHOULD NOT BE BLANK      
   case when isnull(LM.ADDRESS2,'')='' then '...' else LM.ADDRESS2 end ,  --LM.ADDRESS2 SHOULD NOT BE BLANK      
 ...
 --******DispDtls******   
   UPPER(LM.Ac_gst_no ),LM.Ac_NAME  ,LM.ADDRESS0 ,LM.ADDRESS1 ,lm.AREA_NAME +' '+ lm.city ,lm.PINCODE    ,
   cnd.HSN_CODE,
    CASE WHEN ISNULL(UOM.GST_UOM_NAME,'')<>'' THEN LEFT(UOM.GST_UOM_NAME,3) ELSE UOM.UOM_NAME END,       
   cnd.GST_PERCENTAGE ,isnull(cnd.GST_CESS_PERCENTAGE,0) ,      
   CAST((XN_VALUE_WITHOUT_GST+(CND.DISCOUNT_AMOUNT +CND.CNMDISCOUNTAMOUNT)) /INVOICE_QUANTITY  AS NUMERIC(14,2)) ,
   A.ROUND_OFF ,  --A.ROUND_OFF,        
   A.total_amount,   --A.NET_AMOUNT,    ---TOTINVVALUE  ASSVALUE+TAX VALE    
   CAST((a.FREIGHT +a.OTHER_CHARGES+isnull(a.insurance,0)+
   case when a.OH_TAX_METHOD <>2 then 
   ISNULL(A.FREIGHT_IGST_AMOUNT,0)+ISNULL(A.FREIGHT_SGST_AMOUNT,0)+ISNULL(A.FREIGHT_CGST_AMOUNT,0) 
   +ISNULL(A.OTHER_CHARGES_IGST_AMOUNT,0)+ISNULL(A.OTHER_CHARGES_SGST_AMOUNT,0)+ISNULL(A.OTHER_CHARGES_CGST_AMOUNT,0)
   +ISNULL(A.insurance_IGST_AMOUNT,0)+ISNULL(A.insurance_SGST_AMOUNT,0)+ISNULL(A.insurance_CGST_AMOUNT,0) 
  ...
  A.CREDIT_DAYS ,A.total_amount,
   ...
   A.OTHER_CHARGES_TAXABLE_VALUE,A.FREIGHT_TAXABLE_VALUE,A.INSURANCE_TAXABLE_VALUE,
   A.other_charges_cgst_amount ,A.other_charges_Sgst_amount,A.other_charges_Igst_amount,
   A.FREIGHT_cgst_amount ,A.INSURANCE_cgst_amount,
   A.FREIGHT_Sgst_amount ,A.INSURANCE_Sgst_amount,
   A.FREIGHT_Igst_amount ,A.INSURANCE_Igst_amount,
   A.FREIGHT,A.OTHER_CHARGES ,A.OH_TAX_METHOD ,A.INSURANCE ,A.cn_dt ,A.cn_no,
   case when ISNULL(TR.TRANSPORTER_GST_NO,'')='' then '' else TR.TRANSPORTER_GST_NO end,lm.ac_gst_state_code
  ...
  IF OBJECT_ID ('TEMPDB..#TMPOVERHEAD','U') IS NOT NULL
	 ...
	 SELECT TOP 1 * INTO #TMPOVERHEAD FROM #TMPINVOICE
		 ...
		 SELECT 	TOP 1  TAXSCH, SUPTYP, REGREV, ECMGSTIN, IGSTONINTRA, TYP, NO, A.INV_DT, GSTIN, LGLNM, TRDNM, ADDRESS1, ADDRESS2, LOC, PIN, STCD, PH, EM, B_GSTIN, B_LGLNM, B_TRDNM, 
		 POS, ADDR1, ADDR2, B_LOC, B_PIN, B_STCD, B_PH, B_EM, DIS_NM, DIS_ADDR1, DIS_ADDR2, DIS_LOC, DIS_PIN, DIS_STCD, SHIP_GSTIN, SHIP_LGLNM, SHIP_TRDNM, SHIP_ADDR1, SHIP_ADDR2, SHIP_LOC, 
		 SHIP_PIN, SHIP_STCD, SLNO,'Insurance' PRDDESC, ISSERVC, insurance_hsn_code  HSNCD,'Insurance' BARCDE,1 QTY,0 FREEQTY, UNIT, 
		 ...
		 ;with CTE as 
		 (
			select NEWSRNO =ROW_NUMBER () OVER (ORDER BY CASE WHEN BARCDE IN('OTHER CHARGES','FREIGHT','INSURANCE') THEN 1 ELSE 0 END ,SLNO ),* from  #TMPINVOICE a
		 )
  ...
  ORDER BY SLNO  
   ...
   DECLARE @CERRMSG VARCHAR(1000)
	...
	IF EXISTS (SELECT TOP 1 'U'  FROM cnm01106  A (NOLOCK) WHERE cn_id =@CXNID AND isnull(EINV_IRN_NO,'')<>'' )
	BEGIN
   ...
   IF ISNULL(@CERRMSG,'')<>''
	...
	SELECT @CERRMSG AS ERRMSG WHERE 1=2
