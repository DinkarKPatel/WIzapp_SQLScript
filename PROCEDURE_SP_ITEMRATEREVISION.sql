CREATE PROCEDURE SP_ITEMRATEREVISION    
...
LBLNAVIGATE:  -- CREATING LOOK UP TABLE 
	 ...
	 EXEC SP_NAVIGATE 'IRM01106',@NNAVMODE,@CMEMO,@CFINYEAR,'IRM_MEMO_NO','IRM_MEMO_DT','IRM_MEMO_ID',@CWHERE        
...
LBLDETAIL:    
	...
	SKU.PURCHASE_PRICE,SKU.FIX_MRP,SKU.PRODUCT_NAME ,
	...
	,SKU.ONLINE_PRODUCT_CODE AS ONLINE_BAR_CODE,'' AS ONLINE_PRODUCT_CODE,
	CAST((CASE WHEN SKU.PURCHASE_PRICE>0 THEN (SKU.MRP - SKU.PURCHASE_PRICE) * 100 / SKU.PURCHASE_PRICE ELSE 0 END  ) AS NUMERIC(14,2))AS SKU_MP ,  
	CAST((CASE WHEN SKU.MRP>0 THEN (SKU.MRP - SKU.PURCHASE_PRICE) * 100 / SKU.MRP ELSE 0 END  ) AS NUMERIC(14,2))AS SKU_MD 	,
	PARA1.para1_set ,para2.para2_set ,SKU.AC_CODE 	
 ...
 LBLPARA:
  ...
  LBLSCAN:
         ...
         EXEC SP_CHECKSTOCK @CMEMO,@NNAVMODE,@CWHERE,0,0,'0000000'
   ...
   LBLPREFIX:
    ...
    LBLMASTERCOL:
		...
		UNION ALL
		SELECT 'NEW ARTICLE NO' AS MASTER_COL,'NEW_ARTICLE_NO' AS MASTER_COL_EXPR,'' AS MAPPED_COL	
		UNION ALL
		...
		UNION ALL		
		SELECT value AS MASTER_COL,replace(CONFIG_OPTION,'CAPTION','NAME') AS MASTER_COL_EXPR,'' AS MAPPED_COL 
        FROM  CONFIG  WHERE CONFIG_OPTION like 'PARA%_CAPTION' and value <> ''
		UNION ALL
		...
		UNION ALL				
		SELECT 'NEW FIX MRP' AS MASTER_COL,'NEW_FIX_MRP' AS MASTER_COL_EXPR,'' AS MAPPED_COL
		UNION ALL
		SELECT 'NEW MRP' AS MASTER_COL,'NEW_MRP' AS MASTER_COL_EXPR,'' AS MAPPED_COL		
		UNION ALL
		SELECT 'WS PRICE' AS MASTER_COL,'WS_PRICE' AS MASTER_COL_EXPR,'' AS MAPPED_COL
		UNION ALL
		SELECT 'NEW WS PRICE' AS MASTER_COL,'NEW_WSP' AS MASTER_COL_EXPR,'' AS MAPPED_COL
		UNION ALL			
		...
		UNION ALL
		SELECT 'HSN' AS MASTER_COL,'OLD_HSN_CODE' AS MASTER_COL_EXPR,'' AS MAPPED_COL
		UNION ALL
		SELECT 'NEW HSN' AS MASTER_COL,'NEW_HSN_CODE' AS MASTER_COL_EXPR,'' AS MAPPED_COL
		UNION ALL
	 ...
	 LBLTMASTER:
	  ...
	  IF OBJECT_ID('_TMPMASTERS','U') IS NOT NULL 
		...
		B.CODING_SCHEME,A.PRODUCT_CODE,UOM_NAME,AC_NAME,A.ONLINE_PRODUCT_CODE  INTO _TMPMASTERS FROM SKU A 
	...
	LBLTATTR:	
			...
			INTO _TMPARTATTR  FROM ART_ATTR A JOIN ATTR_KEY B ON 
	...
	LBLMEMOATTR:
		...
		FROM SKU A1 (NOLOCK)
		JOIN IRD01106 B1 (NOLOCK) ON A1.PRODUCT_CODE = B1.PRODUCT_CODE
...
LBLTFC:	
        ...
        IF OBJECT_ID('_TMPFCRATE','U') IS NOT NULL 
			DROP TABLE _TMPFCRATE	
			
		SELECT CAST('' AS VARCHAR(100)) AS product_code,CAST('' AS VARCHAR(100)) AS fc_code,CAST(0 AS NUMERIC(14,2)) AS fc_rate 
		INTO _TMPFCRATE  
		FROM sku_fc_prices A WHERE 1=2
