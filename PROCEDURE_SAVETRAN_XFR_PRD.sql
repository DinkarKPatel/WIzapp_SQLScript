CREATE PROCEDURE SAVETRAN_XFR_PRD
(
...
BEGIN
	...
	DECLARE @CTEMPDBNAME			VARCHAR(100),
			...
			@CDETAILTABLENAME_QTY   VARCHAR(100),
			@CTEMPMASTERTABLENAME	VARCHAR(100),
			@CTEMPDETAILTABLENAME	VARCHAR(100),
			@CTEMPDETAILTABLENAME_QTY	VARCHAR(100),
			@CTEMPMASTERTABLE		VARCHAR(100),
			@CTEMPDETAILTABLE		VARCHAR(100),
			@CTEMPDETAILTABLE_QTY   VARCHAR(100),
	...
	BEGIN TRY
		...
		SET @CTEMPDETAILTABLENAME	= 'TEMP_PRD_STK_TRANSFER_DET_'+LTRIM(RTRIM(STR(@NSPID)))					
		SET @CTEMPDETAILTABLENAME_QTY 	= 'TEMP_PRD_STK_TRANSFER_MATERIAL_DET_'+LTRIM(RTRIM(STR(@NSPID)))		
		...
		BEGIN
		   ...
		   --
			...
			DECLARE @CPRODUCT_CODE VARCHAR(100)

			SET @CPRODUCT_CODE=''
			SELECT TOP 1 @CPRODUCT_CODE=A.PRODUCT_CODE 
			...
			WHERE B.CANCELLED =0
			) C ON A.NEW_PRODUCT_UID  =C.PRODUCT_UID 
			WHERE B.MEMO_ID =@CKEYFIELDVAL1
		    
		    IF ISNULL(@CPRODUCT_CODE,'')<>''
			BEGIN
                SET @CERRORMSG = 'STEP- ' + LTRIM(STR(@NSTEP)) + ' TRANSACTION FOND  IN AGENCY ISSUE PLEASE CHECK -' +ISNULL(@CPRODUCT_CODE,'')
				GOTO END_PROC 
			...
			JOIN
			(
			SELECT A.REF_CUTTING_ID   
			FROM PRD_STK_TRANSFER_DET_PENDING  A
			JOIN PRD_STK_TRANSFER_MST_PENDING B ON A.MEMO_ID =B.MEMO_ID 
			WHERE B.CANCELLED =0
			AND A.REF_CUTTING_ID =@CKEYFIELDVAL1
			GROUP BY A.REF_CUTTING_ID
			) C ON A.MEMO_ID   =C.REF_CUTTING_ID  
			WHERE B.MEMO_ID =@CKEYFIELDVAL1
		    
		    IF ISNULL(@CPRODUCT_CODE,'')<>''
			BEGIN
                SET @CERRORMSG = 'STEP- ' + LTRIM(STR(@NSTEP)) + ' TRANACTION FOUND IN PENDING CUTTING PLEASE CHECK -' +ISNULL(@CPRODUCT_CODE,'')
				GOTO END_PROC 
			...
			SET @CCMD = N'UPDATE PRD_STK_TRANSFER_MST SET CANCELLED = 1,LAST_UPDATE=GETDATE() ' + 
		...
		--SET @CCMD=N'DELETE FROM '+@CTEMPDETAILTABLENAME+' WHERE ISNULL(PRODUCT_CODE,'''')='''''
		--EXEC SP_EXECUTESQL @CCMD
		...
		SET @CMASTERTABLENAME	= 'PRD_STK_TRANSFER_MST'
		SET @CDETAILTABLENAME	= 'PRD_STK_TRANSFER_DET'
		SET @CDETAILTABLENAME_QTY 	= 'PRD_STK_TRANSFER_MATERIAL_DET'

		SET @CTEMPMASTERTABLENAME	= 'TEMP_PRD_STK_TRANSFER_MST_'+LTRIM(RTRIM(STR(@NSPID)))
		SET @CTEMPMASTERTABLE	= @CTEMPDBNAME + @CTEMPMASTERTABLENAME
		SET @CTEMPDETAILTABLE	= @CTEMPDBNAME + @CTEMPDETAILTABLENAME
		SET @CTEMPDETAILTABLE_QTY 	= @CTEMPDBNAME + @CTEMPDETAILTABLENAME_QTY 
		...
		BEGIN	
			...
			BEGIN
				...
				SET @CCMD = 'UPDATE ' + @CTEMPDETAILTABLE_QTY + ' SET '+@CKEYFIELD1+' = '''+@CKEYFIELDVAL1+''''
				PRINT @CCMD
				EXEC SP_EXECUTESQL @CCMD
				
				---- MAKE AN ENTRY INTO PURCHASE IMPORT LOG TABLE TO MARK THE PURCHAE IMPORT FROM WSL INVOICE
				IF ISNULL('','')<>''
				BEGIN
					SET @NSTEP = 140
					--SET @CCMD=N'INSERT PUR_IMPORT_LOG (INV_ID,MRR_ID,RECEIPT_DT)
					--			SELECT '''+@CWSLINVID+''','''+@CKEYFIELDVAL1+''',RECEIPT_DT FROM '+@CTEMPMASTERTABLE
					--EXEC SP_EXECUTESQL @CCMD			 	
				END
			END
		...
		-------------CHANGING
		 SET @CCMD=N'ALTER TABLE '+@CTEMPDETAILTABLE_QTY +' ADD OLD_ROW_ID VARCHAR(40)'  
         EXEC SP_EXECUTESQL @CCMD  
     
         SET @NSTEP = 225  
     
   -- UPDATING ROW_ID IN TEMP TABLES  
        SET @CCMD = N'UPDATE ' + @CTEMPDETAILTABLE_QTY + ' SET OLD_ROW_ID=ROW_ID,ROW_ID = ''' + @CLOCATIONID + ''' + CONVERT(VARCHAR(38), NEWID())  
        WHERE LEFT(ROW_ID,5) = ''LATER'''  
        PRINT @CCMD
        EXEC SP_EXECUTESQL @CCMD  
        
      --  SELECT * FROM TEMP_PRD_STK_TRANSFER_MATERIAL_DET_69
		
		SET @CCMD = N'UPDATE B SET REF_ROW_ID=A.ROW_ID FROM ' + @CTEMPDETAILTABLE_QTY + ' A
		JOIN ' + @CTEMPDETAILTABLE + ' B ON A.OLD_ROW_ID=B.REF_ROW_ID'  
		PRINT @CCMD
        EXEC SP_EXECUTESQL @CCMD  
        
       -- SELECT * FROM TEMP_PRD_STK_TRANSFER_DET_69
        
        
        SET @CCMD = N'DELETE A FROM ' + @CDETAILTABLENAME_QTY + ' A 
                    JOIN ' + @CDETAILTABLENAME + ' B ON A.ROW_ID=B.REF_ROW_ID
					WHERE B.' + @CKEYFIELD1 + ' = ''' + @CKEYFIELDVAL1 + '''
					AND B.ROW_ID IN 
					(
						SELECT A.ROW_ID 
		...
		SET @NSTEP = 240		-- UPDATING TRANSACTION TABLE - INSERTING NEW ENTRIES
	     ...
	     EXEC UPDATEMASTERXN 
			  @CSOURCEDB	= @CTEMPDBNAME
			, @CSOURCETABLE = @CTEMPDETAILTABLENAME_QTY 
			, @CDESTDB		= ''
			, @CDESTTABLE	= @CDETAILTABLENAME_QTY 
			, @CKEYFIELD1	= 'ROW_ID'
			, @BALWAYSUPDATE = 1
			--, @LUPDATEXNS	= 1	      
		...
		EXEC SAVETRAN_UPDSKU_PRD 'XFR',@CKEYFIELDVAL1,@CERRORMSG OUTPUT 
		IF ISNULL(@CERRORMSG,'')<>''
				GOTO END_PROC
...
END_PROC:
   ...
   --SELECT * FROM  PRD_STK_TRANSFER_DET WHERE MEMO_ID ='0101117000000100000004'
   --ORDER BY NEW_PRODUCT_UID 
   
   -- SELECT * FROM PRD_SKU WHERE PRODUCT_UID  IN
   --(
   --SELECT NEW_PRODUCT_UID FROM  PRD_STK_TRANSFER_DET WHERE MEMO_ID ='0101117000000100000004')
   --ORDER BY PRODUCT_UID 
   
   --SELECT * FROM PRD_PMT  WHERE PRODUCT_UID  IN
   --(
   --SELECT NEW_PRODUCT_UID FROM  PRD_STK_TRANSFER_DET WHERE MEMO_ID ='0101117000000100000004')
   --ORDER BY PRODUCT_UID 
