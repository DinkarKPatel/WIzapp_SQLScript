...
create PROCEDURE SAVETRAN_ARC_XN_APPROVAL
(
...
BEGIN
	...
	DECLARE @CTEMPDBNAME			VARCHAR(100),
			...
			,@ICURRENTLEVEL INT
			,@IMAXLEVEL INT,@CLOCID	VARCHAR(4)
	...
	SET @CMASTERTABLENAME	= 'ARC_XN_APPROVAL'

	SET @CTEMPMASTERTABLENAME	= 'TEMP_ARC_XN_APPROVAL_'+LTRIM(RTRIM(STR(@NSPID)))

	SET @CTEMPMASTERTABLE	= @CTEMPDBNAME + @CTEMPMASTERTABLENAME
	
	SET @CERRORMSG			= ''
	
	SET @CCMD = 'SELECT @CLOCID= location_code  FROM '+  @CTEMPMASTERTABLE +' where sp_id ='''+@NSPID +''' ' 
	...
	BEGIN TRY
		...
		WHERE XN_TYPE='CRDISS' AND INACTIVE=0
		

		IF ISNULL(@CLOCATIONID,'')=''
		BEGIN
		    SET @CERRORMSG = 'STEP- ' + LTRIM(STR(@NSTEP)) + ' location id not fuld please check : ' 
			GOTO END_PROC
		END

		
		SET @CCMD=N'DELETE FROM A FROM CARDISSUE A
		JOIN '+@CTEMPMASTERTABLE+' B ON A.ADV_REC_ID=B.ADV_REC_ID' 
		PRINT @CCMD
		EXEC SP_EXECUTESQL @CCMD
		
		SET @CCMD=N'SELECT ADV_REC_ID,APPROVEDLEVELNO=(CASE WHEN ISNULL(C.APPROVAL_STATUS,0)=1 AND 
		...
		BEGIN	
			...
			SET @CCMD = 'UPDATE A SET A.LEVEL_NO=(CASE WHEN ISNULL(A.LEVEL_NO,0)=0 THEN B.LEVEL_NO ELSE A.LEVEL_NO END),
						 ...
						 JOIN XN_APPROVAL_CHECKLIST_LEVELS C ON C.LEVEL_NO=B.LEVEL_NO AND C.XN_TYPE=''CRDISS'''
...
END
------------------------------------------------------ END OF PROCEDURE SAVETRAN_ARC_XN_APPROVAL
