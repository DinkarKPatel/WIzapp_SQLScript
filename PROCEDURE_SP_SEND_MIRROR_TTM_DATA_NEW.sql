CREATE PROCEDURE SP_SEND_MIRROR_TTM_DATA_NEW
(
...
LBLTABLEINFO:
	...
	SET @CSTEP=65
	SELECT DISTINCT 'TTM_TRANSFER_TO_TRADING_BOM_MIRROR' AS TARGET_TABLENAME,A.*  FROM TRANSFER_TO_TRADING_BOM A (NOLOCK)  WHERE A.MEMO_ID=@CMEMOID
	
	SET @CSTEP=66
	SELECT DISTINCT 'TTM_TRANSFER_TO_TRADING_JOBS_MIRROR' AS TARGET_TABLENAME,A.* FROM TRANSFER_TO_TRADING_JOBS A (NOLOCK)  WHERE A.MEMO_ID=@CMEMOID
	
	SET @CSTEP=67
	SELECT DISTINCT 'TTM_TRANSFER_TO_TRADING_SUB_DET_MIRROR' AS TARGET_TABLENAME,A.*  FROM TRANSFER_TO_TRADING_SUB_DET A (NOLOCK)  WHERE A.MEMO_ID=@CMEMOID
	
	SET @CSTEP=68
	SELECT DISTINCT 'TTM_PMT01106_MIRROR' AS TARGET_TABLENAME,A.DEPT_ID,A.product_code,A.BIN_ID,A.quantity_in_stock,
...
END TRY
BEGIN CATCH
	SET @CERRMSG='P: SP_SEND_MIRROR_TTM_DATA_NEW, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH 
