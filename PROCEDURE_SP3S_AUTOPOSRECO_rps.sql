CREATE PROCEDURE SP3S_AUTOPOSRECO_rps
@nMode NUMERIC(1,0),
@cLocIdPara CHAR(2)='',
...
BEGIN TRY
	...
	BEGIN
		...
		SELECT TOP 5000 cancelled,cm_dt memo_dt,cm_id  memo_id,
		ISNULL(total_quantity,0) as quantity,'rps' as XN_TYPE,a.LAST_UPDATE
		FROM rps_mst A WITH (NOLOCK)
	...
	BEGIN
		...
		LEFT OUTER JOIN rps_mst b (NOLOCK)	ON a.memo_id=b.cm_id
	...
	BEGIN
		...
		FROM rps_mst B (NOLOCK) JOIN #auto_posreco_data_upload a (NOLOCK) ON a.memo_id=b.cm_id 
		WHERE a.xn_type='rps'

		SET @cStep='70'
		UPDATE B SET HO_SYNCH_LAST_UPDATE=''
		FROM rps_mst B (NOLOCK) JOIN #auto_posreco_data_mismatch a (NOLOCK) ON a.memo_id=b.cm_id 
	END
...
BEGIN CATCH
	SET @cErrormsg='Error in Procedure SP3S_GETPOSRECO_RPS_DATA at Step#'+@cStep+' '+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
