CREATE PROCEDURE SP_PRD_PRODUCTION_STATUS_REPORT_WO_UPC    
(    
 @DASON AS DATETIME='2018-10-20',    
...
DECLARE @DTSQL NVARCHAR(MAX),@CART_STATUS VARCHAR(MAX),@AAC_STATUS VARCHAR(MAX),    
        @CMERCHANTSTATUS VARCHAR(MAX), @CENABLED_UPC   VARCHAR(10)
    
 
 SELECT @CENABLED_UPC=VALUE  FROM CONFIG WHERE CONFIG_OPTION='ENABLED_UPC' AND VALUE=1
 
 --IF ISNULL(@CENABLED_UPC,'')<>'1'
 --BEGIN
    
 --   EXEC SP3S_PRODUCTION_STATUS_REPORT
 --   @DASON,
	--@CARTICLE_CODE,
	--@CAC_CODE,
	--@MERCHANT_CODE,
...
SET @CARTICLE_NO_CONTAINING=' AND '+ @CARTICLE_NO_CONTAINING  
 ...
 SET @DTSQL=N' SELECT DISTINCT M.AC_CODE,ISNULL(PMT1.ORDER_ID ,B.ORDER_ID) AS ORDER_ID,
    ISNULL(PMT1.WO_ID,ISNULL(A.MEMO_ID,'''')) AS MEMO_ID ,B.ARTICLE_CODE,B.PARA1_CODE,B.PARA2_CODE,         
     SUM(ISNULL(PRD_QTY,A.QUANTITY)) AS QTY, SUM(ISNULL(PRD_QTY,A.QUANTITY)) AS PRD_QUANTITY,  
    ALLOCATE_QTY=CAST(0 AS NUMERIC(10,0)),  
    WO_QTY=CAST(0 AS NUMERIC(10,0)),  
    WIP_STOCK=CAST(0 AS NUMERIC(10,0)),  
    M.ORDER_NO,M.ORDER_DT,M.DELIVERY_DT,  
    DATEDIFF(DAY,M.DELIVERY_DT,GETDATE()) AS DTD,  
    SR=ROW_NUMBER() OVER (PARTITION BY  B.ARTICLE_CODE,ISNULL(PMT1.ORDER_ID ,B.ORDER_ID),M.AC_CODE ORDER BY B.ARTICLE_CODE,ISNULL(PMT1.ORDER_ID ,B.ORDER_ID),M.AC_CODE )  
  ...
  WHERE C.CANCELLED=0     
  AND ISNULL(C.MARK_AS_COMPLETED,0)<>2
  ...
  LEFT JOIN  
  (  
   SELECT A.ORDER_ID,B.ARTICLE_SET_CODE, A.WO_ID,A.PARA1_CODE,A.PARA2_CODE,COUNT(*) AS PRD_QTY   
   FROM PRD_UPCPMT A  
   JOIN PRD_WO_MST B ON A.WO_ID=B.MEMO_ID
   WHERE ISNULL(ORDER_ID,'''')<>'''' 
   AND PRODUCT_CODE<>''''  AND B.CANCELLED=0
   AND ISNULL(B.MARK_AS_COMPLETED,0)<>2
   GROUP BY A.ORDER_ID,B.ARTICLE_SET_CODE, A.WO_ID,A.PARA1_CODE,A.PARA2_CODE  
  ) PMT1 ON PMT1.ORDER_ID=M.ORDER_ID  
  AND B.ARTICLE_CODE=PMT1.ARTICLE_SET_CODE   
  AND B.PARA1_CODE=PMT1.PARA1_CODE  
  AND B.PARA2_CODE=PMT1.PARA2_CODE   
  JOIN BUYER_ORDER_MST MST ON MST.ORDER_ID=ISNULL(PMT1.ORDER_ID ,B.ORDER_ID)
  JOIN USERS U ON U.USER_CODE=MST.USER_CODE 
  JOIN ARTICLE ART ON ART.ARTICLE_CODE=B.ARTICLE_CODE
  WHERE  M.CANCELLED=0   
  AND MST.CANCELLED=0 
  AND M.ORDER_DT<='''+CONVERT(VARCHAR(11),@DASON,120)+'''   
  AND M.ORDER_DT>=''2017-01-01''     
  AND ISNULL(PMT1.WO_ID,ISNULL(A.MEMO_ID,''''))<>''''
  AND ('''+RTRIM(LTRIM(@CART_STATUS))+'''='''' OR B.ARTICLE_CODE '+@CARTICLE_CODE+')      
  AND ('''+RTRIM(LTRIM(@AAC_STATUS))+'''='''' OR MST.AC_CODE '+@CAC_CODE+')       
  AND ('''+RTRIM(LTRIM(@CAC_CODE_NOTIN))+'''='''' OR MST.AC_CODE<>'''+@CAC_CODE_NOTIN+''')
  '+@CARTICLE_NO_CONTAINING+   
 ' GROUP BY M.AC_CODE,ISNULL(PMT1.ORDER_ID ,B.ORDER_ID),ISNULL(PMT1.WO_ID,ISNULL(A.MEMO_ID,'''')),B.ARTICLE_CODE,B.PARA1_CODE,B.PARA2_CODE ,  
  M.ORDER_NO,M.ORDER_DT,M.DELIVERY_DT,  
  DATEDIFF(DAY,M.DELIVERY_DT,GETDATE()) '    
  PRINT @DTSQL    
  INSERT INTO #TMPWO    
  ...
  SET @DTSQL=N' SELECT DISTINCT  ISNULL(PMT1.ORDER_ID ,B.ORDER_ID) AS ORDER_ID,ISNULL(PMT1.WO_ID,ISNULL(A.MEMO_ID,'''')) AS MEMO_ID ,  
  ...
  ISNULL(ISNULL(PMT1.JOB_CODE,ISNULL(A.JOB_CODE,'''')),'''') AS JOB_CODE        
  FROM BUYER_ORDER_DET B     
  JOIN BUYER_ORDER_MST M ON M.ORDER_ID=B.ORDER_ID     
  LEFT JOIN    
  (    
  ...
  LEFT JOIN  
  (  
   SELECT A.ORDER_ID, A.WO_ID,B.ARTICLE_SET_CODE, A.PARA1_CODE,A.PARA2_CODE,A.PRODUCT_CODE,
   A.QUANTITY_IN_STOCK,A.JOB_CODE  
   FROM PRD_UPCPMT A  
   JOIN PRD_WO_MST B ON A.WO_ID=B.MEMO_ID
   WHERE ISNULL(A.ORDER_ID,'''')<>''''  AND B.CANCELLED=0
   AND PRODUCT_CODE<>'''' 
   AND ISNULL(B.MARK_AS_COMPLETED,0)<>2
  ) PMT1 ON PMT1.ORDER_ID=M.ORDER_ID  
  AND B.ARTICLE_CODE=PMT1.ARTICLE_SET_CODE   
 ...
 JOIN  
 (  
  SELECT WIP_PRODUCT_CODE  AS PRODUCT_CODE FROM PRD_TRANSFER_MAIN_DET  A  
  JOIN PRD_TRANSFER_MAIN_MST B ON A.MEMO_ID =B.MEMO_ID   
  WHERE B.CANCELLED =0  
 ) B ON A.PRODUCT_CODE=B.PRODUCT_CODE   
 
 --DELETE FROM #TMPALLOCATION WHERE ISNULL(PRODUCT_CODE,'') =''
 --DELETE FROM #TMPWO WHERE ISNULL(PRODUCT_CODE,'') =''
  ...
  FROM #TMPALLOCATION WHERE MEMO_ID<>'' AND ISNULL(JOB_CODE,'')=''  
  ...
  JOIN  
  (  
  SELECT ORDER_ID,MEMO_ID,PARA1_CODE,PARA2_CODE,  
  ...
  IF OBJECT_ID('TEMPDB..#TMPWSP','U') IS NOT NULL  
    ...
    SELECT A.ORDER_ID AS ORDER_ID,
            A.PACKSLIP_ARTICLE_CODE AS PACKSLIP_ARTICLE_CODE , 
            A.PARA1_CODE AS PARA1_CODE ,A.PARA2_CODE AS PARA2_CODE ,
            A.WO_ID AS WO_ID,
   ...
   FROM
    (
     ...
     WHERE ISNULL(A.PS_ID,'') ='' AND 
     TM.CANCELLED =0 AND B.CANCELLED =0
 ...
 SELECT  A.ARTICLE_CODE,A.PARA1_CODE,A.PARA2_CODE  
     ,A.AC_CODE,A.ORDER_ID,A.ORDER_NO,  
     A.MEMO_ID,  
     A.WO_QTY,A.WIP_STOCK  
 ...
 LEFT JOIN #TMPWSP WPS ON A.ORDER_ID=WPS.ORDER_ID  
   ...
   --AND A.AC_CODE=WPS.AC_CODE 
   AND A.PARA1_CODE=WPS.PARA1_CODE AND A.PARA2_CODE=WPS.PARA2_CODE   
   AND A.MEMO_ID =WPS.WO_ID 
 GROUP BY A.ARTICLE_CODE,A.PARA1_CODE,A.PARA2_CODE  
     ,A.AC_CODE,A.ORDER_ID,A.ORDER_NO, A.MEMO_ID,A.WO_QTY,A.WIP_STOCK  
...
(
 ...
 IF OBJECT_ID('TEMPDB..#TMPORDER_WO','U') IS NOT NULL    
   ...
   JOIN
   (
    SELECT DISTINCT  ORDER_ID,ARTICLE_CODE,A.PARA1_CODE ,A.PARA2_CODE ,ITEM_EMP_CODE,ITEM_MERCHANT_CODE
    FROM 
    BUYER_ORDER_DET A
   ) DET ON DET.ORDER_ID =A.ORDER_ID AND A.ARTICLE_CODE =DET.ARTICLE_CODE   
 ...
 --DELETE FROM #TMPORDER_WO WHERE QTY =ISNULL(DLVRD,0) AND ISNULL(WIP_STOCK,0)=0
 ...
 --) B ON A.PRODUCT_CODE=B.PRODUCT_CODE   
  ...
  JOIN   
 (    
  SELECT  ISNULL(UPC.ORDER_ID,ORD.ORDER_ID ) AS ORDER_ID,
          A.ARTICLE_CODE ,A.REF_PRD_WORKORDER_MEMOID ,
          A.PARA1_CODE ,A.PARA2_CODE ,    
  SUM(PMT.QUANTITY_IN_STOCK) AS QUANTITY_IN_STOCK     
  FROM PRD_TRANSFER_MAIN_DET A
  JOIN PRD_TRANSFER_MAIN_MST  B ON A.MEMO_ID =B.MEMO_ID 
  JOIN PMT01106 PMT ON PMT.PRODUCT_CODE =A.PRODUCT_CODE 
  LEFT JOIN PRD_UPCPMT UPC ON UPC.PRODUCT_CODE =A.WIP_PRODUCT_CODE 
  LEFT OUTER JOIN PRD_WO_ORDERS ORD ON ORD.MEMO_ID =A.REF_PRD_WORKORDER_MEMOID   
  WHERE B.CANCELLED =0
  GROUP BY ISNULL(UPC.ORDER_ID,ORD.ORDER_ID ),
          A.ARTICLE_CODE ,A.REF_PRD_WORKORDER_MEMOID ,
          A.PARA1_CODE ,A.PARA2_CODE 
 ) STK ON  A.ORDER_ID =STK.ORDER_ID AND A.ARTICLE_CODE =STK.ARTICLE_CODE   
   AND A.PARA1_CODE=STK.PARA1_CODE AND A.PARA2_CODE =STK.PARA2_CODE  
   AND A.MEMO_ID =STK.REF_PRD_WORKORDER_MEMOID
   --   DROP TABLE TMPORDER_WO
   --SELECT * INTO TMPORDER_WO FROM #TMPORDER_WO
   
   
    DELETE FROM #TMPORDER_WO WHERE QTY =ISNULL(DLVRD,0) --AND ISNULL(WIP_STOCK,0)=0
    
    DELETE A  FROM #TMPORDER_WO A
    JOIN
    (
    
    SELECT REF_PRD_WORKORDER_MEMOID AS WO_ID,PARA1_CODE,PARA2_CODE ,A.ARTICLE_CODE,
           SUM(A.QTY) AS QTY
    FROM PRD_TRANSFER_MAIN_DET A
    JOIN PRD_TRANSFER_MAIN_MST B ON A.MEMO_ID =B.MEMO_ID 
    WHERE B.CANCELLED =0
    AND ISNULL(B.MODE,0)=1
    GROUP BY REF_PRD_WORKORDER_MEMOID,PARA1_CODE,PARA2_CODE ,A.ARTICLE_CODE
    ) B ON A.MEMO_ID =B.WO_ID 
    AND A.ARTICLE_CODE =B.ARTICLE_CODE 
    AND A.PARA1_CODE =B.PARA1_CODE 
    AND A.PARA2_CODE =B.PARA2_CODE 
    WHERE A.PENDING_WSL =B.QTY 
 ...
 --   DROP TABLE TMPT
 --   SELECT A.*,  
 --MST1.MEMO_NO AS WO_NO,  
 --MST1.MEMO_DT  AS WO_DT,  
 ...
 -- ,ISNULL([DYING],0) AS [DYING],ISNULL([HAND_EMB],0) AS [HAND_EMB],ISNULL([STICHING_FINISHING],0) AS [STICHING_FINISHING]    
 --,(SELECT TOP 1 JOB_CODE FROM JOBS WHERE JOB_NAME='DYING') AS [DYING_1],(SELECT TOP 1 JOB_CODE FROM JOBS WHERE JOB_NAME='HAND_EMB') AS [HAND_EMB_1],(SELECT TOP 1 JOB_CODE FROM JOBS WHERE JOB_NAME='STICHING_FINISHING') AS [STICHING_FINISHING_1]  
 --INTO TMPT
 --FROM #TMPORDER_WO A    
 ...
 --  ,SUM(CASE WHEN JOB_NAME= 'DYING' THEN ISSUE_QTY ELSE 0 END) AS [DYING],SUM(CASE WHEN JOB_NAME= 'HAND_EMB' THEN ISSUE_QTY ELSE 0 END) AS [HAND_EMB],SUM(CASE WHEN JOB_NAME= 'STICHING_FINISHING' THEN ISSUE_QTY ELSE 0 END) AS [STICHING_FINISHING]    
 -- FROM #TMPJOBS A    
 ...
 --AND A.PARA1_CODE=B.PARA1_CODE    
 --AND A.PARA2_CODE=B.PARA2_CODE    
 --LEFT JOIN PRD_WO_MST MST1 ON MST1.MEMO_ID=A.MEMO_ID  
 --ORDER BY A.ARTICLE_NO, ORDER_DT,ORDER_NO,MST1.MEMO_DT,MST1.MEMO_NO
 
 --DROP TABLE TMPJ
 --   SELECT * INTO TMPJ FROM #TMPJOBS
 --  EXEC SP_PRD_PRODUCTION_STATUS_REPORT_WO_UPC '2017-01-14','IN (''010001063'')','','','',0   
 ...
 SET @DTSQL=N' SELECT A.ARTICLE_NO,A.PARA1_NAME,A.PARA2_NAME,  
   ...
   AND A.PARA1_CODE=B.PARA1_CODE    
   AND A.PARA2_CODE=B.PARA2_CODE
   AND A.MEMO_ID=A.WORK_ORDER_ID  
   LEFT JOIN PRD_WO_MST MST1 ON MST1.MEMO_ID=A.MEMO_ID  
   WHERE  ('''+RTRIM(LTRIM(STR(@NPENDING_WO)))+'''=''0'' OR A.WO >0)   
   GROUP BY A.MEMO_ID,MST1.MEMO_NO,MST1.MEMO_DT, A.ARTICLE_NO,A.PARA1_NAME,A.PARA2_NAME  ,QUANTITY_IN_STOCK'  
 PRINT @DTSQL      
 EXEC SP_EXECUTESQL @DTSQL  
...
--EXEC SP_PRD_PRODUCTION_STATUS_REPORT_UPC '2017-01-03'  
 ...
 LEFT OUTER JOIN     
 (    
  SELECT A.ORDER_ID,A.WORK_ORDER_ID ,A.ARTICLE_CODE,A.PARA1_CODE,A.PARA2_CODE     
  '+@CCOLNAME+'    
  FROM #TMPJOBS A    
  GROUP BY A.ORDER_ID,A.WORK_ORDER_ID ,A.ARTICLE_CODE,A.PARA1_CODE,A.PARA2_CODE     
     
 ) B ON A.ORDER_ID=B.ORDER_ID    
 AND A.ARTICLE_CODE=B.ARTICLE_CODE    
 AND A.PARA1_CODE=B.PARA1_CODE    
 AND A.PARA2_CODE=B.PARA2_CODE 
  AND A.MEMO_ID=B.WORK_ORDER_ID    
 LEFT JOIN PRD_WO_MST MST1 ON MST1.MEMO_ID=A.MEMO_ID  
 WHERE ('''+RTRIM(LTRIM(@CMERCHANTSTATUS))+'''='''' OR A.MERCHANT_CODE '+@MERCHANT_CODE+')  
