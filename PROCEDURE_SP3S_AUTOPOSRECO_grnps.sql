CREATE PROCEDURE SP3S_AUTOPOSRECO_grnps
@nMode NUMERIC(1,0),
@cLocIdPara CHAR(2)='',
...
BEGIN TRY
	...
	BEGIN
		...
		ISNULL(total_quantity,0) as quantity,'grnps' as XN_TYPE,a.LAST_UPDATE
		FROM grnmemo_mst A WITH (NOLOCK)
	...
	BEGIN
		...
		LEFT OUTER JOIN grnmemo_mst b (NOLOCK)	ON a.memo_id=b.memo_id
	...
	BEGIN
		...
		FROM grnmemo_mst B (NOLOCK) JOIN #auto_posreco_data_upload a (NOLOCK) ON a.memo_id=b.memo_id 
		WHERE a.xn_type='grnps'

		SET @cStep='70'
		UPDATE B SET HO_SYNCH_LAST_UPDATE=''
		FROM grnmemo_mst B (NOLOCK) JOIN #auto_posreco_data_mismatch a (NOLOCK) ON a.memo_id=b.memo_id 
	END
...
BEGIN CATCH
	SET @cErrormsg='Error in Procedure SP3S_GETPOSRECO_grnmemo_DATA at Step#'+@cStep+' '+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
