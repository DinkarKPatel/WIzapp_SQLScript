create PROCEDURE SP3S_SYNCH_UPLOADDATA_ATD_OPT  
(  
    @CSPID VARCHAR(50)  
...
DECLARE @CSTEP VARCHAR(100),@DTSQL NVARCHAR(MAX),@NMERGE_ORDER NUMERIC(5)  
    ...
    ,@CERRMSGOUT VARCHAR(1000),@CTABLESSTR VARCHAR(MAX),@CCMD NVARCHAR(MAX),@CMEMOID VARCHAR(50),@CLOCID VARCHAR(3),@CSOURCEDB VARCHAR(200)  
...
BEGIN TRANSACTION   
 ...
 select @CMEMOID=row_id from ATD_EMP_WPAYATT_UPLOAD  with (nolock) where sp_id =LTRIM(RTRIM((@CSPID)))  
   
  
  
  SET @CFILTERCONDITION = 'B.ROW_ID='''+@CMEMOID+''' AND B.SP_ID='''+LTRIM(RTRIM((@CSPID)))+''''  
   
  
 SET @CTABLENAME='EMP_WPAYATT'  
 SET @CTMP_TABLENAME='ATD_'+@CTABLENAME+'_'+LTRIM(RTRIM(@CTABLE_SUFFIX))  
 ...
 EXEC UPDATEMASTERXN_MIRROR @CSOURCEDB=@CSOURCEDB,@CSOURCETABLE=@CTMP_TABLENAME,@CDESTDB=@CMERGEDB  
  ...
  SET @CFILTERCONDITION = 'B.ATD_MEMO_ID='''+@CMEMOID+''' AND B.SP_ID='''+LTRIM(RTRIM((@CSPID)))+''''  
...
IF (SELECT COUNT(*)  FROM EMP_ATTENDANCE A WITH (NOLOCK) 
JOIN ATD_EMP_WPAYATT_UPLOAD B WITH (NOLOCK) ON  A.EMP_ID = B.EMP_ID AND CONVERT(VARCHAR,A.ATTENDANCE_DT,110)=CONVERT(VARCHAR,B.IST_TIME,110)  
WHERE A.TIME_IN = '' AND A.TIME_OUT=''  
AND B.ROW_ID=@CMEMOID  AND B.SP_ID=LTRIM(RTRIM((@CSPID))))>0
begin

	 SET @CCMD = N'DELETE A FROM EMP_ATTENDANCE A WITH (Nolock) JOIN '+@CSOURCEDB+'['+@CTMP_TABLENAME+'] B with (Nolock) ON  
		 A.EMP_ID = B.EMP_ID AND CONVERT(VARCHAR,A.ATTENDANCE_DT,110)=CONVERT(VARCHAR,B.IST_TIME,110)  
		 WHERE A.TIME_IN = '''' AND A.TIME_OUT=''''  
		 AND B.ROW_ID='''+@CMEMOID+''' AND B.SP_ID='''+LTRIM(RTRIM((@CSPID)))+''' '  
 ...
 SET @CSTEP=300  
 SET @CCMD = N'UPDATE A   SET A.TIME_OUT = CONVERT(DATETIME,CONVERT(VARCHAR,B.IST_TIME,108)),  A.EMPIMAGE_OUT=B.EMPIMAGE  
    FROM EMP_ATTENDANCE A  WITH (Nolock) JOIN '+@CSOURCEDB+'['+@CTMP_TABLENAME+'] B with (NOLOCK)  
    ON A.EMP_ID = B.EMP_ID AND CONVERT(VARCHAR,A.ATTENDANCE_DT,110)=CONVERT(VARCHAR,B.IST_TIME,110)  
    WHERE (A.TIME_OUT='''' or A.TIME_OUT=A.time_in)  
    and B.ROW_ID='''+@CMEMOID+''' AND B.SP_ID='''+LTRIM(RTRIM((@CSPID)))+''''  
 PRINT @CCMD  
 EXEC SP_EXECUTESQL @CCMD  
 ...
 SET @CCMD = 'SELECT   
    ...
    FROM '+@CSOURCEDB+'['+@CTMP_TABLENAME+'] A with (nolock)
	LEFT OUTER JOIN  EMP_ATTENDANCE B with (nolock) ON A.EMP_ID = B.EMP_ID AND  
    CONVERT(VARCHAR,B.ATTENDANCE_DT,110)=CONVERT(VARCHAR,A.IST_TIME,110)  
    JOIN EMP_MST C with (nolock) ON A.EMP_ID = C.EMP_ID  
    JOIN EMP_SHIFTS D with (nolock) ON D.SHIFT_ID = A.SHIFT_ID   
    WHERE B.EMP_ID IS NULL  
    AND A.ROW_ID='''+@CMEMOID+''' AND a.SP_ID='''+LTRIM(RTRIM((@CSPID)))+''''      
...
END TRY  
BEGIN CATCH  
 SET @CERRMSG='P:SP3S_SYNCH_UPLOADDATA_ATD_OPT, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()  
END CATCH  
EXIT_PROC:  
 ...
 ELSE IF ISNULL(@CERRMSG,'')<>'' AND @@TRANCOUNT>0  
  ...
  DELETE A FROM ATD_EMP_WPAYATT_UPLOAD A with (rowlock) WHERE SP_ID=@CSPID   
  DELETE A FROM ATD_EMP_MST_UPLOAD A with (rowlock) WHERE SP_ID=@CSPID   
