CREATE PROCEDURE SP3S_PPC_GET_USER_ROLE      
(      
 @NMODE INT,      
 @NSPID INT=0,      
 @NVIEWSTATUS INT=1,      
...
BEGIN TRY      
 ...
 SET @NSTEP = 20         
    SELECT FORM_NAME,FORM_OPTION,DISPLAY_FORM_NAME,DISPLAY_NAME,GROUP_NAME INTO #TMPUSER_ROLL_DET      
    FROM PPC_MODULES WHERE 1=2      
     
 IF @GROUP='' AND @MODULES=''--DEFAULT      
    INSERT INTO #TMPUSER_ROLL_DET      
    SELECT DISTINCT FORM_NAME,FORM_OPTION,DISPLAY_FORM_NAME,DISPLAY_NAME,GROUP_NAME      
    FROM PPC_MODULES WHERE USER_CODE='0000000' AND DISPLAY_FORM_NAME<>''      
        
 IF @GROUP='' AND @MODULES!=''--MODULES BUT NOT GROUP      
    INSERT INTO #TMPUSER_ROLL_DET      
    SELECT DISTINCT FORM_NAME,FORM_OPTION,DISPLAY_FORM_NAME,DISPLAY_NAME,GROUP_NAME      
    FROM PPC_MODULES WHERE USER_CODE='0000000' AND DISPLAY_FORM_NAME<>''      
        
    IF @GROUP!='' AND @MODULES=''--GROUP BUT NOT MODULE      
    INSERT INTO #TMPUSER_ROLL_DET      
    SELECT DISTINCT FORM_NAME,FORM_OPTION,DISPLAY_FORM_NAME,DISPLAY_NAME,GROUP_NAME      
    FROM PPC_MODULES WHERE USER_CODE='0000000'      
    AND GROUP_NAME=@GROUP      
     
 IF @GROUP!='' AND @MODULES!=''--GROUP AND MODULE      
    INSERT INTO #TMPUSER_ROLL_DET      
    SELECT DISTINCT FORM_NAME,FORM_OPTION,DISPLAY_FORM_NAME,DISPLAY_NAME,GROUP_NAME      
    FROM PPC_MODULES WHERE USER_CODE='0000000' AND DISPLAY_FORM_NAME=@MODULES      
    AND GROUP_NAME=@GROUP      
              
    SELECT ROLE_ID, ROLE_NAME INTO #TOTALUSER FROM USER_ROLE_MST WHERE ISNULL(ROLE_NAME,'')<>'' 
    --SUPER ROLE SKIPPED:VISHAL_30 MAR 2018
    AND ROLE_ID<>'0000000'
	--SUPER ROLE SKIPPED:VISHAL_30 MAR 2018
    ...
    SELECT DISTINCT FORM_NAME,FORM_OPTION,DISPLAY_FORM_NAME,DISPLAY_NAME,NEWID(),DISPLAY_FORM_NAME,GROUP_NAME 
    FROM #TMPUSER_ROLL_DET      
 ...
 SET @NCNT=1      
    ...
    SET @CCMD=N'ALTER TABLE #USERPER ADD ['+@CCOLUMN_NAME+'] BIT'      
...
SELECT *,SR=ROW_NUMBER () OVER (PARTITION BY DISPLAY_FORM_NAME ORDER BY DISPLAY_FORM_NAME)      
 ...
 BEGIN      
  ...
  DECLARE @CLOCID CHAR(2)      
  SELECT TOP 1 @CLOCID=VALUE FROM CONFIG WHERE  CONFIG_OPTION='LOCATION_ID'       
        
  SET @NSTEP = 100      
  SET @CTEMPTABLE='TEMP_USER_ROLE_DET_'+LTRIM(RTRIM(STR(@NSPID)))      
  ...
  SET @CCMD = N'INSERT INTO USER_ROLE_DET(ROLE_ID,FORM_NAME,FORM_OPTION,VALUE,ROW_ID,LAST_UPDATE,      
     GROUP_NAME,S_NO,DISPLAY_FORM_NAME,DISPLAY_NAME,DEPT_ID)      
     SELECT A.ROLE_ID,A.FORM_NAME,A.FORM_OPTION,A.VALUE,NEWID() AS ROW_ID,GETDATE(),      
     B.GROUP_NAME,B.S_NO,B.DISPLAY_FORM_NAME,B.DISPLAY_NAME,'''+@CLOCID+''' FROM       
     (      
       SELECT A.ROLE_ID,A.FORM_NAME,A.FORM_OPTION,A.VALUE,A.GROUP_NAME      
       FROM '+@CTEMPTABLE+'      
       A LEFT OUTER JOIN USER_ROLE_DET B       
       ON A.ROLE_ID=B.ROLE_ID AND A.FORM_NAME=B.FORM_NAME AND A.FORM_OPTION=B.FORM_OPTION            
       WHERE B.ROLE_ID IS NULL AND B.FORM_NAME IS NULL AND B.FORM_OPTION IS NULL      
      ) A      
      JOIN PPC_MODULES B ON A.FORM_NAME = B.FORM_NAME AND A.FORM_OPTION=B.FORM_OPTION AND A.GROUP_NAME=B.GROUP_NAME       
...
BEGIN CATCH      
 SET @CERRMSG = 'PROCEDURE SP3S_PPC_GET_USER_ROLE: STEP- ' + LTRIM(STR(@NSTEP)) + ' SQL ERROR: #' + LTRIM(STR(ERROR_NUMBER())) + ' ' + ERROR_MESSAGE()        
 GOTO END_PROC      
END CATCH         
