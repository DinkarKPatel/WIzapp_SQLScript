CREATE  VIEW VW_PRD_SPLIT_N_COMBINE_QBF 
...
,MU.USERNAME AS MAJOR_USERNAME,'' AS PRODUCT_NAME    
...
,A.WIP AS MST_WIP   
FROM PRD_SNC_MST (NOLOCK) A                
JOIN LMV01106 (NOLOCK) B ON A.AC_CODE = B.AC_CODE            
JOIN PRD_SNC_DET (NOLOCK) C ON A.MEMO_ID = C.MEMO_ID      
...
JOIN PRD_SNC_BARCODE_DET BC (NOLOCK) ON BC.REFROW_ID=C.ROW_ID
JOIN PRD_SKU SKU (NOLOCK)  ON SKU.PRODUCT_UID = BC.PRODUCT_UID    
