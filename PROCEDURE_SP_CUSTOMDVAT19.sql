CREATE PROCEDURE SP_CUSTOMDVAT19
(
	@DFROMDT DATETIME,
	@DTODT DATETIME,
...
BEGIN
/*
	THIS PROCEDURE RETURNS DETAIL OF INTERSTATE PURCHASES AND DEBIT NOTES.
*/
	...
	ELSE
		...
		SELECT	
				...
				, B.CST_NO  AS TIN
				, SUM( PID.RFNET-PID.TAX_AMOUNT ) AS IST_PUR_AMT
				,(PID.TAX_PERCENTAGE) AS IST_TAX_PERCENTAGE
				,SUM(PID.TAX_AMOUNT) AS IST_TAX_AMOUNT
				,SUM(PID.RFNET) AS IST_TOTAL_AMOUNT
				,F.FORM_NAME		
		...
		AND A.BILL_NO <> ''  
		AND A.CANCELLED=0  
		AND  ROUND(A.SUBTOTAL,0) <>0  AND F.POST_TAX_SEPARATELY=0
		GROUP  BY	  A.INV_DT 
					, A.BILL_NO
					, B.AC_NAME
		...
		JOIN PRD_PID01106 PID (NOLOCK) ON A.MRR_ID=PID.MRR_ID
		JOIN FORM F (NOLOCK) ON PID.FORM_ID=F.FORM_ID
		JOIN LMV01106 B (NOLOCK) ON A.AC_CODE = B.AC_CODE  
		JOIN @LOCLIST LOC  ON LEFT(A.MRR_ID,2) = LOC.DEPT_ID
		WHERE A.INV_DT BETWEEN @DFROMDT AND @DTODT 
	...
	ELSE
		...
		SELECT	'PURCHASE RETURN' AS XN_TYPE
				...
				,COALESCE(B.ADDRESS0,B.ADDRESS1,B.ADDRESS2,B.AREA_NAME,B.CITY,B.STATE,B.REGION_NAME) AS SELLER_ADDRESS
				, ( B.CST_NO ) AS TIN
				,SUM(C.RFNET-C.ITEM_TAX_AMOUNT) AS IST_PUR_AMT
				, C.ITEM_TAX_PERCENTAGE AS IST_TAX_PERCENTAGE		
				,SUM(C.ITEM_TAX_AMOUNT ) AS IST_TAX_AMOUNT
				,SUM(C.RFNET) AS IST_TOTAL_AMOUNT
				,F.FORM_NAME	
				,SUM(C.QUANTITY) AS QUANTITY
				,A.FREIGHT
		FROM RMM01106 A  (NOLOCK)
		JOIN LMV01106 B (NOLOCK) ON A.AC_CODE = B.AC_CODE  
		JOIN RMD01106 C (NOLOCK) ON A.RM_ID = C.RM_ID  
		JOIN FORM F (NOLOCK) ON C.ITEM_FORM_ID=F.FORM_ID
		JOIN @LOCLIST LOC  ON LEFT(A.RM_ID,2) = LOC.DEPT_ID
		WHERE A.RM_DT BETWEEN @DFROMDT AND @DTODT AND
		ROUND(A.SUBTOTAL,2) <>0 
		AND A.CANCELLED=0 AND A.MEMO_TYPE=1 AND F.POST_TAX_SEPARATELY=0
 		GROUP BY  A.RM_DT 
				, A.RM_NO 
				, B.AC_NAME
				,COALESCE(B.ADDRESS0,B.ADDRESS1,B.ADDRESS2,B.AREA_NAME,B.CITY,B.STATE,B.REGION_NAME)
		...
		JOIN FORM F (NOLOCK) ON C.ITEM_FORM_ID=F.FORM_ID
		JOIN @LOCLIST LOC  ON LEFT(A.RM_ID,2) = LOC.DEPT_ID
		WHERE A.RM_DT BETWEEN @DFROMDT AND @DTODT AND  ROUND(A.SUBTOTAL,2) <>0 AND A.CANCELLED=0 AND F.POST_TAX_SEPARATELY=0
 		GROUP BY  A.RM_DT 
				, A.RM_NO 
				, B.AC_NAME
				,COALESCE(B.ADDRESS0,B.ADDRESS1,B.ADDRESS2,B.AREA_NAME,B.CITY,B.STATE,B.REGION_NAME)
				...
				,A.FREIGHT
END
---END OF PROCEDURE - SP_CUSTOMDVAT19
