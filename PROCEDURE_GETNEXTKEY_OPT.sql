create PROCEDURE GETNEXTKEY_OPT  
...
begin try
	...
	BEGIN
		PRINT 'CREATE NEW KEYS TABLE FOR CMM'
		SET @CCMD=N'SELECT * INTO ['+@CKEYSTABLE+'] FROM KEYS_CMM WHERE 1=2'
		EXEC SP_EXECUTESQL @CCMD	

		print 'create trigger for new keys table:'+@CKEYSTABLE
		EXEC SP3S_CREATE_TRG_KEYSCMM
		@cKeysTable=@CKEYSTABLE
	END
	
	declare @bCalledFromSls bit
	
	Set @cStep='20'
	set @bCalledFromSls=0
	If Left(@CKEYSTABLE,8)='keys_cmm'
		set @bCalledFromSls=1
...
Set @cStep='30'
	...
	--select @clastkeyval,@CTABLENAME,@CCOLNAME,
	--@NWIDTH	,  
	--@CPREFIX,   
	--@NLZEROS,  
	--@CFINYEAR,
	...
	IF @NLZEROS <> 0  and @bCalledFromSls=0     -- ZERO SUFFING IS NOT ENABLED  
		SET @NTEMPWIDTH = @NWIDTH
	
Set @cStep='40'	
	If @bCalledFromSls=1 
	begin
		IF ISNULL(@CLASTKEYVAL,'')=''
			SELECT @NTEMPWIDTH = @NWIDTH, @CLASTKEYVAL=@CPREFIX+replicate('0',@NTEMPWIDTH-len(ltrim(rtrim(@cPrefix))))

		IF len(@cLastkeyval)=10
			select @NTEMPWIDTH=12,@clastkeyval=REPLACE(@cLastkeyval,'-','-00')
...
begin catch
	set @cErrormsg='Érror in Procedure Getnextkey_opt at step#'+@cStep+''+ERROR_MESSAGE()
	goto end_proc
end catch	

end_proc:
	print 'error in getnextkey_opt :'+ @cErrormsg
END