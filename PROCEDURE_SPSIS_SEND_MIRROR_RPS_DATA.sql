create PROCEDURE SPSIS_SEND_MIRROR_RPS_DATA
(
...
LBLTABLEINFO:
	...
	SET @CSTEP=60
	SELECT @cCols=COALESCE(@cCols+',','')+colname 	FROM ebo_xnssending_cols WHERE tablename='RPS_MST' 

	SET @cCmdCols=N'
	   SELECT ''RPS_RPS_MST_upload'' AS TARGET_TABLENAME,'+@cCols+' FROM RPS_MST(NOLOCK)   WHERE CM_ID='''+@CMEMOID+''''
	PRINT @cCmdCols
	...
	SET @CSTEP=65	
	SET @cCols=NULL
	SELECT @cCols=COALESCE(@cCols+',','')+'RPS_DET.'+colname FROM ebo_xnssending_cols	WHERE tablename='RPS_DET' 

	SET @cCOls=REPLACE(@cCOls,'RPS_DET.PRODUCT_CODE','CASE WHEN ISNULL(C.VENDOR_EAN_NO,'''')<>'''' THEN C.VENDOR_EAN_NO ELSE RPS_DET.PRODUCT_CODE END AS PRODUCT_CODE')


	PRINT @cCOls
	SET @cCmdCols=N'
		SELECT ''RPS_RPS_DET_upload'' AS TARGET_TABLENAME,'+@cCols+' 
		FROM RPS_DET  (NOLOCK)   
		JOIN SKU C (NOLOCK) ON C.PRODUCT_CODE=RPS_DET.PRODUCT_CODE
		JOIN LMP01106 LMP (NOLOCK) ON LMP.AC_CODE =C.AC_CODE
		WHERE SUBSTRING(LMP.AC_GST_NO,3,10)='''+@CPANNO+'''
		AND RPS_DET.CM_ID='''+@CMEMOID+''''
	...
	EXEC SPSIS_GET_CURSOR_SKUNAMES @cXNTYPE='RPS',@cTABLENAME='RPS_DET',@cKEYNAME='CM_ID',@cKEYVALUE=@CMEMOID,@CERRMSG=@CERRMSG OUTPUT 
	IF ISNULL(@CERRMSG,'')<>'' 
		GOTO END_PROC
...
BEGIN CATCH
	SET @CERRMSG='P: SPSIS_SEND_MIRROR_RPS_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
