CREATE PROCEDURE PPC_SAVETRAN_AGENCY_ISSUE_FIRST
(
...
BEGIN
	...
	SET @CMASTERTABLENAME	= 'PPC_AGENCY_ISSUE_FG_FIRST_MST'
	SET @CDETAILTABLENAME1	= 'PPC_AGENCY_ISSUE_FG_FIRST_DET'
    SET @CPOTERMSMASTER='PPC_ISSUE_FI_TERMS'
	
	
	SET @CTEMPMASTERTABLENAME	= 'PPC_PPC_AGENCY_ISSUE_FG_FIRST_MST_UPLOAD'
	SET @CTEMPDETAILTABLENAME1	= 'PPC_PPC_AGENCY_ISSUE_FG_FIRST_DET_UPLOAD'
	SET @CPOTERMSTEMP='PPC_PPC_ISSUE_FI_TERMS_UPLOAD'
	...
	BEGIN TRY
     ...
     IF @NUPDATEMODE = 3
		BEGIN
			...
			--IF BARCODE IN SECOND ISSUE 
			SET @NSTEP = 30		
					
			IF EXISTS(SELECT TOP 1'U' FROM PPC_AGENCY_ISSUE_FG_FIRST_MST A
			JOIN PPC_AGENCY_ISSUE_FG_FIRST_DET B ON A.MEMO_ID =B.MEMO_ID
			JOIN 
			(
			 SELECT PRODUCT_CODE FROM PPC_AGENCY_ISSUE_FG_DET A
			 JOIN PPC_AGENCY_ISSUE_FG_MST B ON A.MEMO_ID=B.MEMO_ID
			 WHERE B.CANCELLED=0
			) C ON B.PRODUCT_CODE =C.PRODUCT_CODE 
			WHERE A.MEMO_ID =@CXNMEMOID)
			BEGIN
			   
				SET @CERRORMSG = 'STEP- ' + LTRIM(STR(@NSTEP)) + ' BARCODE HAS ISSUE FOR ANOTHER JOB PLEASE CHECK...'
				GOTO END_PROC  
			...
			JOIN 
			(
			 SELECT PRODUCT_CODE FROM PPC_AGENCY_REC_FG_DET A
			...
			BEGIN
				...
				SET @CERRORMSG = 'STEP- ' + LTRIM(STR(@NSTEP)) + ' BARCODE HAS RECEIVE FOR ANOTHER JOB PLEASE CHECK...'
				GOTO END_PROC  
			...
			JOIN 
			(
			 SELECT PRODUCT_CODE FROM PPC_FG_BARCODE_PRINT_DET A
			...
			BEGIN
				...
				SET @CERRORMSG = 'STEP- ' + LTRIM(STR(@NSTEP)) + ' BARCODE HAS PRINT CAN NOT CANCELLED PLEASE CHECK...'
				GOTO END_PROC  
       ...
       IF NOT EXISTS(SELECT TOP 1 'U' FROM   PPC_PPC_AGENCY_ISSUE_FG_FIRST_DET_UPLOAD WHERE SP_ID=LTRIM(RTRIM(STR(@NSPID))))
       BEGIN
       ...
       END
		...
		BEGIN				-- START OF EDITMODE
			...
			DELETE FROM PPC_AGENCY_ISSUE_FG_FIRST_DET WHERE MEMO_ID = @CKEYFIELDVAL1
			
	    END					-- END OF EDITMODE
        
        
    
        SET @CCMD = N'UPDATE ' + @CTEMPDETAILTABLE1 + ' SET ROW_ID =  CONVERT(VARCHAR(40), NEWID()),MEMO_ID='''+@CKEYFIELDVAL1+'''
		...
		EXEC UPDATEMASTERXN 
	     ...
	     ---BARCODE INSERTION--
		  ...
		  SELECT ROW_ID,ARTICLE_CODE,BILL_NO,AC_CODE,PARA1_CODE,SIZEGROUP_CODE,PARA2_CODE,SELECT_QTY,BO_DET_ROW_ID 
		  INTO #TMPBARCODE  
		  FROM PPC_PPC_AGENCY_ISSUE_FG_FIRST_DET_UPLOAD WHERE SP_ID=LTRIM(RTRIM(STR(@NSPID))) 
		  ...
		  @CROW_ID VARCHAR(100),@BO_DET_ROW_ID VARCHAR(100)
		  ...
		  IF OBJECT_ID ('TEMPDB..#TMPBARCODEDET','U') IS NOT NULL
		   ...
		   FROM #TMPBARCODE   
				...
				JOIN
				(
				 SELECT A.REF_ROW_ID, A.JOB_CODE FROM PPC_BO_ART_JOBS A
				 WHERE JOB_ORDER=1
				 GROUP BY A.REF_ROW_ID, A.JOB_CODE
				...
				WHERE DET.ARTICLE_CODE=@CARTICLE_CODE
				...
				ORDER BY SKU.PRODUCT_CODE
				 ...
				 INSERT PPC_AGENCY_ISSUE_FG_FIRST_DET	( MEMO_ID,PRODUCT_CODE,ROW_ID,QUANTITY ) 
				 SELECT @CKEYFIELDVAL1 AS MEMO_ID,PRODUCT_CODE,ROW_ID =CAST(NEWID() AS VARCHAR(100)) ,  QUANTITY=1
				 FROM #TMPBARCODEDET

				DELETE FROM #TMPBARCODE WHERE ROW_ID =@CROW_ID 
			END
			
			
		SET @FILTER=@FILTER+ ' AND B.PRODUCT_CODE<>'''''
	    SET @NSTEP = 180
	 ...
	 UPDATE PPC_AGENCY_ISSUE_FG_FIRST_DET SET QUANTITY=1 WHERE MEMO_ID = @CKEYFIELDVAL1  AND QUANTITY=0
...
--EXEC PPC_SAVETRAN_AGENCY_ISSUE_FIRST 1,'122','01117','','01'	
		...
		END	  
		 ...
		 IF NOT EXISTS (SELECT TOP 1 '' FROM PPC_AGENCY_ISSUE_FG_FIRST_DET WHERE MEMO_ID=@CKEYFIELDVAL1)
		 BEGIN
		    SET @CERRORMSG = 'STEP- ' + LTRIM(STR(@NSTEP)) + ' ERROR FOR GENERATE BARCODE ....'
			GOTO END_PROC
		 END
...
END_PROC:
	...
	IF ISNULL(@CERRORMSG,'') = ''
	BEGIN
	   DELETE FROM PPC_PPC_AGENCY_ISSUE_FG_FIRST_MST_UPLOAD WHERE SP_ID=LTRIM(RTRIM(STR(@NSPID)))
	   DELETE FROM PPC_PPC_AGENCY_ISSUE_FG_FIRST_DET_UPLOAD WHERE SP_ID=LTRIM(RTRIM(STR(@NSPID)))	
	   DELETE FROM PPC_PPC_ISSUE_FI_TERMS_UPLOAD WHERE SP_ID=LTRIM(RTRIM(STR(@NSPID)))	
