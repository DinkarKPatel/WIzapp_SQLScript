CREATE PROCEDURE SP_SYNCH_MIRRORDATA_TLF_SINGLE
(
    @CLOCID VARCHAR(3)
...
BEGIN TRY
	...
	SELECT @CTEMPMASTERTABLE=@CSOURCEDB+'TLF_TILL_LIFTS_MIRROR'
...
LBLSTART:
    ...
    FROM TLF_TILL_LIFTS_MIRROR B (NOLOCK)
    LEFT OUTER JOIN MIRROR_SYNCH_LOG C (NOLOCK) ON C.MEMO_ID=B.MEMO_ID AND C.XN_TYPE='TLF'
    WHERE LEFT(B.MEMO_ID,2)=@CLOCID AND C.MEMO_ID IS NULL
    ORDER BY B.MEMO_ID DESC
	...
	SET @BADDMODE=0
	IF NOT EXISTS (SELECT TOP 1 'U' FROM TILL_LIFTS WHERE memo_id =@CMEMOID)
	   SET @BADDMODE=1
...
LBLMERGE:
	...
	SET @CSTEP=50
	SET @CTABLENAME='TILL_LIFTS'
	SET @CTMP_TABLENAME='TLF_TILL_LIFTS_MIRROR'
   ...
   DELETE A FROM TLF_TILL_LIFTS_MIRROR A WITH (ROWLOCK)  WHERE A.MEMO_ID=@CMEMOID 
...
END TRY
BEGIN CATCH
	SET @CERRMSG='P:SP_SYNCH_MIRRORDATA_TLF_SINGLE,MEMO ID :'+@CMEMOID+' STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO EXIT_PROC
END CATCH
...
END	
---END OF PROCEDURE - SP_SYNCH_MIRRORDATA_TLF_SINGLE
