create PROCEDURE SPSIS_SEND_MIRROR_BCO_DATA
(
...
LBLTABLEINFO:
	...
	SELECT @cCols=COALESCE(@cCols+',','')+colname 	FROM ebo_xnssending_cols WHERE tablename='FLOOR_ST_MST' 

	SET @cCmdCols=N'
	   SELECT ''BCO_FLOOR_ST_MST_upload'' AS TARGET_TABLENAME,'+@cCols+' FROM FLOOR_ST_MST  (NOLOCK)   WHERE MEMO_ID='''+@CMEMOID+''''
	PRINT @cCmdCols
	...
	SET @CSTEP=65	
	SET @cCols=NULL
	SELECT @cCols=COALESCE(@cCols+',','')+'FLOOR_ST_DET.'+colname FROM ebo_xnssending_cols	WHERE tablename='FLOOR_ST_DET' 

	SET @cCOls=REPLACE(@cCOls,'FLOOR_ST_DET.PRODUCT_CODE','CASE WHEN ISNULL(C.VENDOR_EAN_NO,'''')<>'''' THEN C.VENDOR_EAN_NO ELSE FLOOR_ST_DET.PRODUCT_CODE END AS PRODUCT_CODE')

	PRINT @cCOls
	SET @cCmdCols=N'
		SELECT ''BCO_FLOOR_ST_DET_upload'' AS TARGET_TABLENAME,'+@cCols+' 
		FROM FLOOR_ST_DET  (NOLOCK)   
		...
		AND FLOOR_ST_DET.MEMO_ID='''+@CMEMOID+''''
	...
	EXEC SPSIS_GET_CURSOR_SKUNAMES @cXNTYPE='BCO',@cTABLENAME='FLOOR_ST_DET',@cKEYNAME='MEMO_ID',@cKEYVALUE=@CMEMOID,@CERRMSG=@CERRMSG OUTPUT 
	IF ISNULL(@CERRMSG,'')<>'' 
		GOTO END_PROC
...
BEGIN CATCH
	SET @CERRMSG='P: SPSIS_SEND_MIRROR_BCO_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
