CREATE PROCEDURE PPC_SAVETRAN_BARCODE_PRINT
(
...
BEGIN
	...
	SET @CMASTERTABLENAME	= 'PPC_FG_BARCODE_PRINT_MST'
	SET @CDETAILTABLENAME1	= 'PPC_FG_BARCODE_PRINT_DET'

	
	SET @CTEMPMASTERTABLENAME	= 'PPC_PPC_FG_BARCODE_PRINT_MST_UPLOAD'
	SET @CTEMPDETAILTABLENAME1	= 'PPC_PPC_FG_BARCODE_PRINT_DET_UPLOAD'
	...
	BEGIN TRY
       ...
       IF NOT EXISTS(SELECT TOP 1 'U' FROM   PPC_PPC_FG_BARCODE_PRINT_DET_UPLOAD WHERE SP_ID=LTRIM(RTRIM(STR(@NSPID))))
       BEGIN
       ...
       END
		...
		BEGIN				-- START OF EDITMODE
			...
			DELETE FROM PPC_FG_BARCODE_PRINT_DET WHERE MEMO_ID = @CKEYFIELDVAL1
		...
		EXEC UPDATEMASTERXN 
	     ...
	     ---BARCODE INSERTION--
		  ...
		  INTO #TMPBARCODE  
		  FROM PPC_PPC_FG_BARCODE_PRINT_DET_UPLOAD WHERE SP_ID=LTRIM(RTRIM(STR(@NSPID)))  
		  ...
		  IF OBJECT_ID ('TEMPDB..#TMPBARCODEDET','U') IS NOT NULL
		   ...
		   FROM #TMPBARCODE   
				...
				SKU.ARTICLE_CODE,
				SKU.PARA3_CODE,
				SKU.PARA2_CODE,P2.PARA2_NAME ,PMT.PRODUCT_CODE ,
				SR=ROW_NUMBER() OVER (ORDER BY B.BO_DET_ROW_ID, SKU.PARA2_CODE,PMT.PRODUCT_CODE)
				INTO #TMPBARCODEDET
				...
				JOIN
			   (
				SELECT  AC_CODE, PRODUCT_CODE,CAST(1 AS INT) AS ISSUE_QTY
				...
				LEFT OUTER JOIN
				(
					SELECT B.AC_CODE, PRODUCT_CODE,CAST(1 AS INT) AS ISSUE_QTY
				...
				WHERE SKU.ARTICLE_CODE=@CARTICLE_CODE
				AND DET.BILL_NO=@CBILL_NO
				AND DET.ROW_ID=@CROW_ID
				AND TPR.AC_CODE=@CAC_CODE
				...
				AND PR.PRODUCT_CODE IS NULL
				 ...
				 INSERT PPC_FG_BARCODE_PRINT_DET	( MEMO_ID,PRODUCT_CODE,ROW_ID,QUANTITY ) 
			...
			IF NOT EXISTS(SELECT TOP 1 'U' FROM PPC_FG_BARCODE_PRINT_DET WHERE MEMO_ID=@CKEYFIELDVAL1)     
			BEGIN
			   SET @CERRORMSG = 'STEP- ' + LTRIM(STR(@NSTEP)) + ' BARCODE DOES NOT EXISTS...'
				 GOTO END_PROC 
...
END_PROC:
	...
	IF ISNULL(@CERRORMSG,'') = ''
	BEGIN
	   DELETE FROM PPC_PPC_FG_BARCODE_PRINT_MST_UPLOAD WHERE SP_ID=LTRIM(RTRIM(STR(@NSPID)))
	   DELETE FROM PPC_PPC_FG_BARCODE_PRINT_DET_UPLOAD WHERE SP_ID=LTRIM(RTRIM(STR(@NSPID)))	
