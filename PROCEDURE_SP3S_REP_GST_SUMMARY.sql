CREATE PROCEDURE SP3S_REP_GST_SUMMARY
(
  @DFMDATE DATETIME,
  @DTODATE DATETIME,
  @CDEPT_ID VARCHAR(5)='',
  @CGSTN_NO VARCHAR(100)='',
  @NTYPE INT=1--1 FOR SUMMARY,2 FOR MEMO WISE 3 FOR HSN CODE WISE
)
AS
BEGIN
   IF @NTYPE=3
      ...
      SELECT 1 AS SR ,0 AS GST_TYPE,'SLS' AS XN_TYPE, 'RETAIL SALE' AS DETAILS, 
           ...
           SUM(ISNULL(B.TAX_AMOUNT,0)) AS VAT_AMOUNT 
      FROM CMM01106 A (NOLOCK) 
      JOIN #TMCMID TMP ON A.CM_ID =TMP.CM_ID 
  ...
  --***WHOSALE***
      ...
      INSERT INTO #TMPSUMMARY(SR,GST_TYPE,XN_TYPE,DETAILS,MEMO_ID,MEMO_NO ,MEMO_DT ,ROUND_OFF   ,GSTN_ID,DEPT_ID,DEPT_NAME,XN_VALUE_WITH_GST,
      ...
      SELECT 2 AS SR ,0 AS GST_TYPE,'WSL' AS XN_TYPE, 'WHOLESALE' AS DETAILS, 
           ...
           a.location_Code AS DEPT_ID,
           B.DEPT_NAME ,
           ISNULL(A.OTHER_CHARGES,0)+ ISNULL(A.OTHER_CHARGES_IGST_AMOUNT,0)+ISNULL(A.OTHER_CHARGES_CGST_AMOUNT,0)+ISNULL(A.OTHER_CHARGES_SGST_AMOUNT,0) 
           +ISNULL(A.PACKING,0)+ISNULL(A.PACKING_IGST_AMOUNT,0)+ISNULL(A.PACKING_CGST_AMOUNT,0)+ISNULL(A.PACKING_SGST_AMOUNT,0) 
           +ISNULL(A.INSURANCE ,0)+ISNULL(A.INSURANCE_IGST_AMOUNT,0)+ISNULL(A.INSURANCE_CGST_AMOUNT,0)+ISNULL(A.INSURANCE_SGST_AMOUNT,0) 
           ...
           CASE WHEN ISNULL(OTHER_CHARGES_IGST_AMOUNT,0) >0 THEN A.OTHER_CHARGES ELSE 0 END +
           CASE WHEN ISNULL(PACKING_IGST_AMOUNT,0) >0 THEN A.PACKING ELSE 0 END+
           CASE WHEN ISNULL(INSURANCE_IGST_AMOUNT,0) >0 THEN A.INSURANCE ELSE 0 END+
           CASE WHEN ISNULL(FREIGHT_IGST_AMOUNT,0) >0 THEN A.FREIGHT ELSE 0 END AS IGST_TAXABLE_VALUE,
           
           CASE WHEN ISNULL(OTHER_CHARGES_IGST_AMOUNT,0) >0 THEN A.OTHER_CHARGES_GST_PERCENTAGE  
                WHEN ISNULL(PACKING_IGST_AMOUNT,0) >0 THEN A.PACKING_GST_PERCENTAGE 
                WHEN ISNULL(INSURANCE_IGST_AMOUNT,0) >0 THEN A.INSURANCE_GST_PERCENTAGE 
                WHEN ISNULL(FREIGHT_IGST_AMOUNT,0) >0 THEN A.FREIGHT_GST_PERCENTAGE ELSE 0 END AS IGST_RATE,
          
           CASE WHEN ISNULL(OTHER_CHARGES_IGST_AMOUNT,0) >0 THEN ISNULL(A.OTHER_CHARGES_IGST_AMOUNT,0) ELSE 0 END +
           CASE WHEN ISNULL(PACKING_IGST_AMOUNT,0) >0 THEN  ISNULL(A.PACKING_IGST_AMOUNT,0) ELSE 0 END+
           CASE WHEN ISNULL(INSURANCE_IGST_AMOUNT,0) >0 THEN ISNULL(A.INSURANCE_IGST_AMOUNT,0) ELSE 0 END+
           CASE WHEN ISNULL(FREIGHT_IGST_AMOUNT,0) >0 THEN ISNULL(FREIGHT_IGST_AMOUNT,0) ELSE 0 END AS IGST_AMOUNT,
          
           
           
           CASE WHEN ISNULL(OTHER_CHARGES_CGST_AMOUNT,0) >0 THEN A.OTHER_CHARGES ELSE 0 END +
           CASE WHEN ISNULL(PACKING_CGST_AMOUNT,0) >0 THEN A.PACKING ELSE 0 END+
           CASE WHEN ISNULL(INSURANCE_CGST_AMOUNT,0) >0 THEN A.INSURANCE ELSE 0 END+
           CASE WHEN ISNULL(FREIGHT_CGST_AMOUNT,0) >0 THEN A.FREIGHT ELSE 0 END AS CGST_TAXABLE_VALUE,
           
           CASE WHEN ISNULL(OTHER_CHARGES_CGST_AMOUNT,0) >0 THEN A.OTHER_CHARGES_GST_PERCENTAGE  
                WHEN ISNULL(PACKING_CGST_AMOUNT,0) >0 THEN A.PACKING_GST_PERCENTAGE 
                WHEN ISNULL(INSURANCE_CGST_AMOUNT,0) >0 THEN A.INSURANCE_GST_PERCENTAGE 
                WHEN ISNULL(FREIGHT_CGST_AMOUNT,0) >0 THEN A.FREIGHT_GST_PERCENTAGE ELSE 0 END AS CGST_RATE,
           
           CASE WHEN ISNULL(OTHER_CHARGES_CGST_AMOUNT,0) >0 THEN ISNULL(A.OTHER_CHARGES_CGST_AMOUNT,0) ELSE 0 END +
           CASE WHEN ISNULL(PACKING_CGST_AMOUNT,0) >0 THEN  ISNULL(A.PACKING_CGST_AMOUNT,0) ELSE 0 END+
           CASE WHEN ISNULL(INSURANCE_CGST_AMOUNT,0) >0 THEN ISNULL(A.INSURANCE_CGST_AMOUNT,0) ELSE 0 END+
           CASE WHEN ISNULL(FREIGHT_CGST_AMOUNT,0) >0 THEN ISNULL(FREIGHT_CGST_AMOUNT,0) ELSE 0 END AS CGST_AMOUNT,
           
           
           CASE WHEN ISNULL(OTHER_CHARGES_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=0 THEN A.OTHER_CHARGES ELSE 0 END +
           CASE WHEN ISNULL(PACKING_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=0 THEN A.PACKING ELSE 0 END+
           CASE WHEN ISNULL(INSURANCE_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=0 THEN A.INSURANCE ELSE 0 END+
           CASE WHEN ISNULL(FREIGHT_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=0 THEN A.FREIGHT ELSE 0 END AS SGST_TAXABLE_VALUE,
           
           CASE WHEN ISNULL(OTHER_CHARGES_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=0 THEN A.OTHER_CHARGES_GST_PERCENTAGE  
                WHEN ISNULL(PACKING_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=0 THEN A.PACKING_GST_PERCENTAGE 
                WHEN ISNULL(INSURANCE_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=0 THEN A.INSURANCE_GST_PERCENTAGE 
                WHEN ISNULL(FREIGHT_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=0 THEN A.FREIGHT_GST_PERCENTAGE ELSE 0 END AS SGST_RATE,
           
           CASE WHEN ISNULL(OTHER_CHARGES_SGST_AMOUNT,0) >0  AND ISNULL(C.UT,0)=0 THEN ISNULL(A.OTHER_CHARGES_SGST_AMOUNT,0) ELSE 0 END +
           CASE WHEN ISNULL(PACKING_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=0 THEN  ISNULL(A.PACKING_SGST_AMOUNT,0) ELSE 0 END+
           CASE WHEN ISNULL(INSURANCE_SGST_AMOUNT,0) >0  AND ISNULL(C.UT,0)=0 THEN ISNULL(A.INSURANCE_SGST_AMOUNT,0) ELSE 0 END+
           CASE WHEN ISNULL(FREIGHT_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=0 THEN ISNULL(FREIGHT_SGST_AMOUNT,0) ELSE 0 END AS SGST_AMOUNT,
           
          -- CASE WHEN ISNULL(C.UT,0)=0 THEN ISNULL(OTHER_CHARGES_SGST_AMOUNT,0)+ISNULL(PACKING_SGST_AMOUNT,0)+ISNULL(INSURANCE_SGST_AMOUNT,0)+ISNULL(FREIGHT_SGST_AMOUNT,0) ELSE 0 END AS SGST_AMOUNT,
           
           CASE WHEN ISNULL(OTHER_CHARGES_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=1 THEN A.OTHER_CHARGES ELSE 0 END +
           CASE WHEN ISNULL(PACKING_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=1 THEN A.PACKING ELSE 0 END+
           CASE WHEN ISNULL(INSURANCE_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=1 THEN A.INSURANCE ELSE 0 END+
           CASE WHEN ISNULL(FREIGHT_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=0 THEN A.FREIGHT ELSE 0 END AS UTGST_TAXABLE_VALUE,
           
           CASE WHEN ISNULL(OTHER_CHARGES_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=1 THEN A.OTHER_CHARGES_GST_PERCENTAGE  
                WHEN ISNULL(PACKING_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=1 THEN A.PACKING_GST_PERCENTAGE 
                WHEN ISNULL(INSURANCE_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=1 THEN A.INSURANCE_GST_PERCENTAGE 
                WHEN ISNULL(FREIGHT_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=1 THEN A.FREIGHT_GST_PERCENTAGE ELSE 0 END AS UTGST_RATE,
                
           CASE WHEN ISNULL(OTHER_CHARGES_SGST_AMOUNT,0) >0  AND ISNULL(C.UT,0)=1 THEN ISNULL(A.OTHER_CHARGES_SGST_AMOUNT,0) ELSE 0 END +
           CASE WHEN ISNULL(PACKING_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=1 THEN  ISNULL(A.PACKING_SGST_AMOUNT,0) ELSE 0 END+
           CASE WHEN ISNULL(INSURANCE_SGST_AMOUNT,0) >0  AND ISNULL(C.UT,0)=1 THEN ISNULL(A.INSURANCE_SGST_AMOUNT,0) ELSE 0 END+
           CASE WHEN ISNULL(FREIGHT_SGST_AMOUNT,0) >0 AND ISNULL(C.UT,0)=1 THEN ISNULL(FREIGHT_SGST_AMOUNT,0) ELSE 0 END AS UTGST_AMOUNT
      ...
      AND (@CGSTN_NO='' OR B.LOC_GST_NO=@CGSTN_NO)
      AND ISNULL(A.MEMO_TYPE,0)=1
      AND (ISNULL(OTHER_CHARGES_GST_PERCENTAGE,0) >0 OR ISNULL(A.PACKING_GST_PERCENTAGE,0)>0 OR ISNULL(A.INSURANCE_GST_PERCENTAGE,0)>0 OR ISNULL(A.FREIGHT_GST_PERCENTAGE,0)>0) 
  ...
  --SELECT * FROM RMM01106		  
    ...
    INSERT INTO #TMPSUMMARY(SR,GST_TYPE,XN_TYPE,DETAILS,MEMO_ID,MEMO_NO ,MEMO_DT ,ROUND_OFF ,GSTN_ID,DEPT_ID,DEPT_NAME,XN_VALUE_WITH_GST,
      ...
      AND (@CGSTN_NO='' OR B.LOC_GST_NO=@CGSTN_NO)
      AND ISNULL(A.MEMO_TYPE,0)=1
      AND (A.OTHER_CHARGES_GST_PERCENTAGE>0 OR A.FREIGHT_GST_PERCENTAGE>0)
     UNION ALL
     SELECT 3 AS SR ,0 AS GST_TYPE,'PRT' AS XN_TYPE, 'DEBIT NOTE' AS DETAILS, 
           A.RM_ID ,
           A.RM_NO ,
  ...
  --**ADVANCE***
     ...
     CASE WHEN ARCT=2 THEN 'ADVANCE' ELSE 'OTHER CHARGES' END AS DETAILS, 
           ...
           SUM(CASE WHEN ISNULL(IGST_AMOUNT,0)>0 THEN ISNULL(B.NET_AMOUNT ,0)-(ISNULL(IGST_AMOUNT,0)+ISNULL(CGST_AMOUNT,0)+ISNULL(SGST_AMOUNT,0)) ELSE 0 END) AS IGST_TAXABLE_VALUE,
           CASE WHEN ISNULL(IGST_AMOUNT,0) >0 THEN ISNULL(B.GST_PERCENTAGE,0) ELSE 0 END AS IGST_RATE,
           SUM(CASE WHEN ISNULL(IGST_AMOUNT,0) >0 THEN ISNULL(B.IGST_AMOUNT,0) ELSE 0 END) IGST_AMOUNT,
           
           SUM(CASE WHEN ISNULL(CGST_AMOUNT,0)>0 THEN ISNULL(B.NET_AMOUNT,0)-(ISNULL(IGST_AMOUNT,0)+ISNULL(CGST_AMOUNT,0)+ISNULL(SGST_AMOUNT,0)) ELSE 0 END) AS CGST_TAXABLE_VALUE,
           CASE WHEN ISNULL(CGST_AMOUNT,0) >0 THEN ISNULL(B.GST_PERCENTAGE,0) ELSE 0 END AS CGST_RATE,
           SUM(CASE WHEN ISNULL(CGST_AMOUNT,0) >0 THEN ISNULL(B.CGST_AMOUNT,0) ELSE 0 END) CGST_AMOUNT,
           
           SUM(CASE WHEN ISNULL(SGST_AMOUNT,0)>0  AND ISNULL(D.UT,0)=0  THEN ISNULL(B.NET_AMOUNT,0)-(ISNULL(IGST_AMOUNT,0)+ISNULL(CGST_AMOUNT,0)+ISNULL(SGST_AMOUNT,0)) ELSE 0 END) AS SGST_TAXABLE_VALUE,
           CASE WHEN ISNULL(SGST_AMOUNT,0) >0 AND ISNULL(D.UT,0)=0 THEN ISNULL(B.GST_PERCENTAGE,0) ELSE 0 END AS SGST_RATE,
           SUM(CASE WHEN ISNULL(CGST_AMOUNT,0) >0  AND  ISNULL(D.UT,0)=0 THEN ISNULL(B.SGST_AMOUNT,0) ELSE 0 END) AS SGST_AMOUNT,
           SUM(CASE WHEN  ISNULL(SGST_AMOUNT,0)>0 AND ISNULL(D.UT,0)=1 THEN ISNULL(B.NET_AMOUNT,0)-(ISNULL(IGST_AMOUNT,0)+ISNULL(CGST_AMOUNT,0)+ISNULL(SGST_AMOUNT,0)) ELSE 0 END) AS UTGST_TAXABLE_VALUE,
           CASE WHEN ISNULL(SGST_AMOUNT,0) >0 AND ISNULL(D.UT,0)=1 THEN ISNULL(B.GST_PERCENTAGE,0) ELSE 0 END AS UTGST_RATE,
           SUM(CASE WHEN ISNULL(SGST_AMOUNT,0) >0 AND ISNULL(D.UT,0)=1 THEN ISNULL(B.SGST_AMOUNT,0) ELSE 0 END) AS UTGST_AMOUNT
      ...
      AND (@CGSTN_NO='' OR C.LOC_GST_NO=@CGSTN_NO)
      GROUP BY B.ADV_REC_ID , C.LOC_GST_NO,
  ...
  --END ADVANCE
    ...
    --****** 1 RETAIL SALE CREDIT NOTE**********  
     ...
     INSERT INTO #TMPSUMMARY(SR,GST_TYPE,XN_TYPE,DETAILS,MEMO_ID,MEMO_NO ,MEMO_DT ,ROUND_OFF ,GSTN_ID,DEPT_ID,DEPT_NAME,XN_VALUE_WITH_GST,
      ...
      SELECT 1 AS SR ,0 AS GST_TYPE,'SLSCR' AS XN_TYPE, 'RETAIL CREDIT NOTE' AS DETAILS, 
           ...
           LEFT (A.CM_ID,2) AS DEPT_ID,
           C.DEPT_NAME ,
           (SUM(CASE WHEN ISNULL(B.XN_VALUE_WITH_GST,0)>0 THEN ISNULL(B.XN_VALUE_WITH_GST,0) ELSE NET-CMM_DISCOUNT_AMOUNT END  )) AS XN_VALUE_WITH_GST,
      ...
      AND (@CDEPT_ID='' OR a.location_Code=@CDEPT_ID)
      AND (@CGSTN_NO='' OR C.LOC_GST_NO=@CGSTN_NO)
      AND B.QUANTITY<0 AND B.REF_SLS_MEMO_DT>'2017-06-30'
      AND ISNULL(A.MEMO_TYPE,0)=1
      AND ISNULL(B.GST_PERCENTAGE,0)>0
      GROUP BY A.CM_ID ,A.CM_NO,
    ...
    ---- INPUT OF ADVANCE ADJUSTED IN SALES
    INSERT INTO #TMPSUMMARY(SR,GST_TYPE,XN_TYPE,DETAILS,MEMO_ID,MEMO_NO ,MEMO_DT ,ROUND_OFF ,GSTN_ID,DEPT_ID,DEPT_NAME,XN_VALUE_WITH_GST,
                              IGST_TAXABLE_VALUE,IGST_RATE,IGST_AMOUNT,CGST_TAXABLE_VALUE,CGST_RATE,CGST_AMOUNT,
                              SGST_TAXABLE_VALUE,SGST_RATE,SGST_AMOUNT,UTGST_TAXABLE_VALUE,UTGST_RATE,UTGST_AMOUNT)
  ...
  INSERT INTO #TMPSUMMARY(SR,GST_TYPE,XN_TYPE,DETAILS,MEMO_ID,MEMO_NO ,MEMO_DT ,ROUND_OFF ,GSTN_ID,DEPT_ID,DEPT_NAME,XN_VALUE_WITH_GST,
      ...
      AND (@CGSTN_NO='' OR B.LOC_GST_NO=@CGSTN_NO)
      AND ISNULL(A.MEMO_TYPE,0)=1
      AND (A.FREIGHT_GST_PERCENTAGE>0 OR A.FREIGHT_GST_PERCENTAGE>0)
     UNION ALL
     SELECT 6 AS SR ,1 AS GST_TYPE,'PUR' AS XN_TYPE, 'PURCHASE' AS DETAILS, 
           A.MRR_ID ,
           A.BILL_NO ,
  ...
  --***WHOSALE****
   INSERT INTO #TMPSUMMARY(SR,GST_TYPE,XN_TYPE,DETAILS,MEMO_ID,MEMO_NO ,MEMO_DT ,ROUND_OFF ,GSTN_ID,DEPT_ID,DEPT_NAME,XN_VALUE_WITH_GST,
      ...
      AND (@CGSTN_NO='' OR B.LOC_GST_NO=@CGSTN_NO)
      AND ISNULL(A.MEMO_TYPE,0)=1
      AND ( ISNULL(A.OTHER_CHARGES_GST_PERCENTAGE,0)>0 OR ISNULL(A.INSURANCE_GST_PERCENTAGE,0)>0 OR A.FREIGHT_GST_PERCENTAGE>0)
     UNION ALL
     SELECT 7 AS SR ,1 AS GST_TYPE,'WSR' AS XN_TYPE, 'WHOLESALE CREDIT NOTE' AS DETAILS, 
           A.CN_ID ,
           A.CN_NO  ,
  ...
  BEGIN
     ...
     FROM #TMPSUMMARY A
     WHERE 
     A.XN_VALUE_WITH_GST<>0 
     GROUP BY A.SR,A.GST_TYPE ,A.XN_TYPE ,A.DETAILS ,A.GSTN_ID ,A.DEPT_NAME,
     A.IGST_RATE ,A.CGST_RATE ,A.SGST_RATE,A.UTGST_RATE
      ORDER BY DEPT_NAME,A.XN_TYPE ,SR,A.IGST_RATE , A.CGST_RATE ,A.SGST_RATE ,A.UTGST_RATE 
   END  
   ELSE IF @NTYPE=2
   BEGIN
     ...
     A.MEMO_NO,A.MEMO_DT,
     A.IGST_RATE ,A.CGST_RATE ,A.SGST_RATE,A.UTGST_RATE
     ORDER BY DEPT_NAME,A.XN_TYPE ,SR,A.MEMO_DT,A.MEMO_NO
   
   END
   ELSE IF @NTYPE=3
   BEGIN
        EXEC SP3S_REP_GST_HSN_SUMMARY @DFMDATE,@DTODATE,@CDEPT_ID,@CGSTN_NO
        RETURN
   END
   ELSE 
   BEGIN
          SELECT A.SR,A.GST_TYPE , A.XN_TYPE ,A.DETAILS ,A.GSTN_ID  ,
            CAST(SUM(A.ROUND_OFF ) AS NUMERIC(18,2)) AS  ROUND_OFF,
           -- 0 AS ROUND_OFF,
            ((CAST(SUM(A.XN_VALUE_WITH_GST ) AS NUMERIC(18,2)))) AS  XN_VALUE_WITH_GST,
            ((CAST(SUM(A.IGST_TAXABLE_VALUE) AS NUMERIC(18,2)))) AS IGST_TAXABLE_VALUE,
     ...
     WHERE 
     A.XN_VALUE_WITH_GST<>0 
     GROUP BY A.SR,A.GST_TYPE ,A.XN_TYPE ,A.DETAILS ,A.GSTN_ID ,
     A.IGST_RATE ,A.CGST_RATE ,A.SGST_RATE,A.UTGST_RATE
      ORDER BY A.XN_TYPE ,SR,A.IGST_RATE , A.CGST_RATE ,A.SGST_RATE ,A.UTGST_RATE 
   END    
