CREATE PROC SP_GENERATE_TRAN_DETAIL(@LOC VARCHAR(10),@SALE_DATE DATE,@POS VARCHAR(10)='',@APP_FOLDER VARCHAR(200))              
AS                
BEGIN  
...
WHILE @@FETCH_STATUS=0            
  BEGIN            
    ...
    DECLARE @131_ZERO INT
    SELECT 'C'TTYPE,AMOUNT,AMOUNT*XD.CURRENCY_CONVERSION_RATE AMT,PM.PAYMODE_GRP_CODE            
    INTO #C
    ...
    FROM PAYMODE_XN_DET XD(NOLOCK) JOIN PAYMODE_MST PM (NOLOCK) ON PM.PAYMODE_CODE=XD.PAYMODE_CODE            
    WHERE MEMO_ID=@CMID AND PAYMODE_NAME='CREDIT ISSUED' AND XN_TYPE='SLS' AND @131_ZERO>0           
    UNION            
    SELECT 'T',AMOUNT,AMOUNT*XD.CURRENCY_CONVERSION_RATE,PM.PAYMODE_GRP_CODE            
    FROM PAYMODE_XN_DET XD(NOLOCK) JOIN PAYMODE_MST PM (NOLOCK) ON PM.PAYMODE_CODE=XD.PAYMODE_CODE            
    WHERE MEMO_ID=@CMID AND PAYMODE_NAME!='CREDIT ISSUED' AND XN_TYPE='SLS'AND @131_ZERO>0            
    ...
    WHILE @@FETCH_STATUS=0            
      BEGIN            
		...
		+''+'^|'--REMARKS2            
		+''+'^|'--REMARKS3            
		+CASE ISNULL(@PAYMODE_NAME,'') WHEN '0000001' THEN CAST(ISNULL(@TENDER_AMOUNT_CONV,0) AS VARCHAR) WHEN '0000002' THEN CAST(ISNULL(@TENDER_AMOUNT_CONV,0) AS VARCHAR) ELSE '0.0000' 
      END            
