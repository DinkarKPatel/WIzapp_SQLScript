...
CREATE PROCEDURE DBO.SP_SEND_MIRROR_MSTLSP_NEW
(
...
AS
 BEGIN
   DECLARE @DTSQL NVARCHAR(MAX),@CSTEP VARCHAR(10),@CFILTERCONDITION NVARCHAR(MAX)
	...
	SELECT DISTINCT  'MSTLSP_LOC_SPACE_MST_MIRROR' AS TARGET_TABLENAME,LOC_SPACE_MST.*,1 AS PICK_FROM_CURRENT_DB FROM LOC_SPACE_MST (NOLOCK)
	...
	SELECT DISTINCT  'MSTLSP_LOC_SPACE_DET_MIRROR' AS TARGET_TABLENAME,LOC_SPACE_DET.*,1 AS PICK_FROM_CURRENT_DB FROM LOC_SPACE_DET (NOLOCK)
	
	END TRY
	
	BEGIN CATCH
	    SET @CERRORMSG='P: SP_SEND_MIRROR_MSTLSP_NEW, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()+ 'ERROR QUERY || '+@DTSQL
	    GOTO END_PROC
    END CATCH 
