CREATE PROCEDURE DBO.SP_SEND_MIRROR_MSTDSB_NEW
(
...
AS
 BEGIN
   DECLARE @DTSQL NVARCHAR(MAX),@CSTEP VARCHAR(10),@CFILTERCONDITION NVARCHAR(MAX)
	...
	SELECT DISTINCT  'MSTDSB_SALECONTRIBUTIONDATA_MIRROR' AS TARGET_TABLENAME,SALECONTRIBUTIONDATA.*,1 AS PICK_FROM_CURRENT_DB FROM SALECONTRIBUTIONDATA (NOLOCK)

	END TRY
	
	BEGIN CATCH
	    SET @CERRORMSG='P: SP_SEND_MIRROR_MSTDSB, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()+ 'ERROR QUERY || '+@DTSQL
	    GOTO END_PROC
    END CATCH 
