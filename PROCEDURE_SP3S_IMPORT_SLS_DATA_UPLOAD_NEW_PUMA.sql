CREATE PROCEDURE SP3S_IMPORT_SLS_DATA_UPLOAD_NEW_PUMA
(    
 @iDateMode		INT,
 @CLOCID VARCHAR(10),          
 @NMODE INT=0      ,    
...
BEGIN TRY          
  ...
  SET @CSTEP=5       
	EXEC SP_CHKXNSAVELOG 'SLS_BULK',@cSTEP,0,@cSPID,'',0
...
,cgst_amount NUMERIC(14,2)    
 ...
 ,EMAIL   VARCHAR(MAX)     
	...
	if(@iDateMode IN (1,6))
	BEGIN
		--SELECT  memo_dt=CONVERT(DATETIME,memo_dt,105) FROM SLS_IMPORT_DATA where sp_id=@CSPID AND memo_no=@memo_no AND memo_dt=@memo_dt
...
END   
	...
	UPDATE A SET A.net=(ABS(CONVERT(NUMERIC(20,2),ISNULL(A.MRP,0)))* CONVERT(NUMERIC(20,2),ISNULL(A.QUANTITY,0)))-ABS(CONVERT(NUMERIC(20,2),ISNULL(A.CMD_DISCOUNT_AMOUNT,0)))
	FROM SLS_IMPORT_DATA A    
  ...
  FROM SLS_IMPORT_DATA A  (NOLOCK)  
   WHERE A.SP_ID=@CSPID AND ISNULL(A.DEPT_ID,'')=''  
...
FROM #SLSIMPORT_CUSTDYM WHERE ISNULL(USER_CUSTOMER_CODE,'')<>''    
 ...
 --   JOIN SKU B (NOLOCK) ON A.PRODUCT_CODE=B.PRODUCT_CODE 
	...
	,CAST (0 AS NUMERIC(14,2)) AS SISLOC_EOSS_DISCOUNT_AMOUNT,ISNULL(B.MRP,0) AS SISLOC_MRP
	,ISNULL(A.gst_percentage,0) sisloc_gst_percentage,CAST (0 AS NUMERIC(14,2)) AS sisloc_taxable_value,ISNULL(A.cgst_amount,0)+ISNULL(A.sgst_amount,0)  AS sisloc_lgst_amount,ISNULL(A.igst_amount,0) AS sisloc_igst_amount
	, A.xn_value_without_gst,ISNULL(A.CMD_DISCOUNT_PERCENTAGE,0) AS CMD_DISCOUNT_PERCENTAGE
    FROM SLS_IMPORT_DATA A  (NOLOCK)         
    JOIN SKU B (NOLOCK) ON A.PRODUCT_CODE=B.PRODUCT_CODE 
	WHERE A.SP_ID='''+@CSPID+''' AND memo_no='''+@memo_no + ''' AND memo_dt='''+ @memo_dt+'''' --AND DEPT_ID='''+@CLOCID+''''  
	...
	NET,NET_AMOUNT,NET_SALE,REMARKS,party_state_code, 
	sis_net,sisloc_eoss_discount_percentage,sisloc_eoss_discount_amount,sisloc_mrp ,
	sisloc_gst_percentage,sisloc_taxable_value,sisloc_lgst_amount,sisloc_igst_amount,xn_value_without_gst,CMD_DISCOUNT_PERCENTAGE)            
    EXEC SP_EXECUTESQL @CCMD          
	...
	IF EXISTS (SELECT 'U' FROM SLS_IMPORT_DATA_MAPPING WHERE SP_ID=@CSPID AND MASTER_COL_EXPR='MRP' AND ISNULL(MAPPED_COL,'')='')
	BEGIN
		...
		ELSE IF EXISTS (SELECT 'U' FROM SLS_IMPORT_DATA_MAPPING WHERE SP_ID=@CSPID AND MASTER_COL_EXPR='CMD_DISCOUNT_AMOUNT' AND ISNULL(MAPPED_COL,'')<>'')
		BEGIN 
			UPDATE #SLSIMPORT SET MRP=(ABS(NET)+ABS(DISCOUNT_AMT))/ABS(QUANTITY) 
		...
		ELSE 
		BEGIN		
			UPDATE #SLSIMPORT SET MRP=(ABS(NET))/ABS(QUANTITY),DISCOUNT_AMT=0,CMD_DISCOUNT_PERCENTAGE=0
		END
	END
	ELSE 
	BEGIN
	...
	BEGIN
		...
		BEGIN
			UPDATE #SLSIMPORT SET XN_VALUE_WITHOUT_GST=NET-igst_amount
 ...
 UPDATE A SET PARTY_STATE_CODE =CASE WHEN ISNULL(B.GST_STATE_CODE   ,'') ='' THEN '00' ELSE B.GST_STATE_CODE END       
  ...
  INTO #NEWBILLS          
  FROM #SLSIMPORT A          
  LEFT JOIN CMM01106 B(NOLOCK) ON A.DEPT_ID+'-'+ISNULL(A.BILL_NO,'')=B.REF_NO AND A.FIN_YEAR=B.FIN_YEAR       AND CONVERT(DATETIME,A.CM_DT) =    B.CM_DT  AND B.CANCELLED=0
  LEFT OUTER JOIN gst_state_mst GST (NOLOCK) ON GST.gst_state_name=A.party_state_code
