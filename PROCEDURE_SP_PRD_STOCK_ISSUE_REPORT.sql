CREATE PROCEDURE SP_PRD_STOCK_ISSUE_REPORT
(
 @DFM_DT DATETIME,
 @DTO_DT DATETIME,
...
SELECT A.DEPARTMENT, SECTIONM.SECTION_NAME, SECTIOND.SUB_SECTION_NAME,A.PRODUCT_CODE,PARA1.PARA1_NAME,PARA3.PARA3_NAME,PARA4.PARA4_NAME 
  ,CAST(SUM( (CASE WHEN A.XN_TYPE='OPS' OR (A.XN_TYPE IN ('OPS','APR', 'PUR','PRD_SCF','PFI', 'UNC', 'WSR','JWR','AMR', 'STK-IN', 'ISSUE-MAT-IN','JWR-FIN') AND XN_DT < @DFM_DT) THEN 1 WHEN A.XN_TYPE IN ('PRT','CIP','CNC','WSL','AMI','STK-OUT','APP','CODE-CON','JWI','JWR-CON','ISSUE-MAT-OUT') AND XN_DT < @DFM_DT  THEN -1 ELSE 0 END) * (XN_QTY)) AS NUMERIC(14,2)) AS OPS_QTY
  ,CAST(SUM( (CASE WHEN A.XN_TYPE IN ('PRT','CIP','CNC','WSL','AMI','STK-OUT','APP','CODE-CON','JWI','JWR-CON','ISSUE-MAT-OUT') AND XN_DT BETWEEN @DFM_DT AND @DTO_DT  THEN -1 ELSE 0 END) * (XN_QTY)) AS NUMERIC(14,2)) AS ISSUE_QTY
  ,CAST(SUM( (CASE WHEN A.XN_TYPE='OPS' OR (A.XN_TYPE IN ('OPS','APR', 'PUR','PRD_SCF','PFI', 'UNC', 'WSR','JWR','AMR', 'STK-IN', 'ISSUE-MAT-IN','JWR-FIN') AND XN_DT <= @DTO_DT) THEN 1 WHEN A.XN_TYPE IN ('PRT','CIP','CNC','WSL','AMI','STK-OUT','APP','CODE-CON','JWI','JWR-CON','ISSUE-MAT-OUT') AND XN_DT<=@DTO_DT  THEN -1 ELSE 0 END) * (XN_QTY)) AS NUMERIC(14,2)) AS CBSPRD
 INTO #TMPISSUE
 ...
 SELECT * FROM #TMPISSUE
 WHERE ( @NISSUE_QTY  =0 OR ABS(ISSUE_QTY)>0)
 ORDER BY DEPARTMENT
