CREATE PROCEDURE SAVETRAN_RECEIVE_PRD
(
...
BEGIN
	...
	BEGIN TRY
		...
		SET @CTEMPDETAILTABLENAME	= 'TEMP_PRD_STK_RECEIVE_DET_'+LTRIM(RTRIM(STR(@NSPID)))					
		...
		BEGIN
			...
			SET @CCMD = N'UPDATE PRD_STK_RECEIVE_MST SET CANCELLED = 1,LAST_UPDATE=GETDATE() ' + 
		...
		SET @CMASTERTABLENAME	= 'PRD_STK_RECEIVE_MST'
		SET @CDETAILTABLENAME	= 'PRD_STK_RECEIVE_DET'

		SET @CTEMPMASTERTABLENAME	= 'TEMP_PRD_STK_RECEIVE_MST_'+LTRIM(RTRIM(STR(@NSPID)))
		...
		EXEC SAVETRAN_UPDSKU_PRD 'XREC',@CKEYFIELDVAL1,@CERRORMSG OUTPUT 
		IF ISNULL(@CERRORMSG,'')<>''
				GOTO END_PROC
