CREATE PROCEDURE SPWOW_MULTILEDGER 
(
	@dFromDt DATETIME='',      
	@dToDt DATETIME='',    
	@cAcCodePara CHAR(10)='',    
...
--         select @nMode
  DECLARE @cStep VARCHAR(4),@cErrormsg varchar(max),@dOpnDt DATETIME,    
   ...
   @cFinyear VARCHAR(5),@dFinyearFromDt DATETIME,@nSpId INT,@cAcName VARCHAR(300)   
  ...
  CREATE TABLE #locListC (dept_id VARCHAR(5)/*Rohit 06-11-2024*/)    
 ...
 IF @cPanno<>''
  INSERT #locListC    
  SELECT DEPT_ID from location (NOLOCK) WHERE pan_no=@cPanno
 ELSE    
  INSERT #locListC    
 ...
 -- end
  ...
  where  voucher_dt<=@dToDt AND (d.head_code is null or voucher_dt>=@dFinyearFromDt) AND
  ((VOUCHER_DT>@cViewLedgerForCancelledVouchers AND cancelled=0)    
  ...
  ''MEMO000002'') THEN ''MemoZZ1'' else ''ZZ1'' end) as vouchercode
   ...
   AND (dnhd.head_code IS NULL OR voucher_dt>='''+convert(varchar,@dFinyearFromDt,110)+''')    
...
lblDetailed:    
  ...
  SET @cStep='12'      
  PRINT 'Step#'+@cStep+':'+convert(varchar,getdate(),113)      
  create table #temp (cost_center_dept_id varchar(5)/*Rohit 06-11-2024*/,vm_id VARCHAR(50), 
  ...
  '''+convert(varchar,@dFromDt-1,110)+''' OrderDate, ''Memo'' VoucherType, 
  ...
  SET @cCmd=N'Select cost_center_dept_id,vd.vm_id, lm.'+@cLmCol+' AcCode,head_code, vm.voucher_no, vm.ref_no, vm.voucher_dt VoucherDate, 
   vm.voucher_dt OrderDate, vch.VOUCHER_TYPE_ALIAS VoucherType, vd.Narration, debit_amount, credit_amount  ,debit_amount_forex, credit_amount_forex  ,
...
--    EXEC SP_EXECUTESQL @cCmd    
  ...
  ''MEMO000002'') THEN ''MemoZZ2'' else ''ZZ2''  end) as VoucherCode
   ...
   JOIN #locListC l ON l.dept_id=vd.cost_center_dept_id    
   left outer join #tempDNObHd d on d.head_code=lm.head_code
   where  vm.voucher_dt<='''+CONVERT(VARCHAR,@dToDt,110)+''' AND 
   (d.head_code is null or voucher_dt>='''+CONVERT(VARCHAR,@dFinyearFromDt,110)+''') 
    AND ((VOUCHER_DT>'''+@cViewLedgerForCancelledVouchers+''' AND vm.cancelled=0)    
    OR ('''+@cViewLedgerForCancelledVouchers+'''<>'''' AND ISNULL(consider_for_ledger_view,0)=1     
     AND VOUCHER_DT<='''+@cViewLedgerForCancelledVouchers+'''))    
 ...
 SET @cStep='41'      
  ...
  CONVERT(VARCHAR(20) ,VoucherDate,105) AS   DISPLAY_VoucherDate,CONVERT(BIT,0) AS group_ledger,
  ...
  SET @cStep='42.8'      
  PRINT 'Step#'+@cStep+':'+convert(varchar,getdate(),113)     
	 INSERT @t(UnqId, AC_NAME,AC_NAME_forex)
...
IF @nMode>0
BEGIN
	DECLARE @nopening NUMERIC(20,2),@nclosing numeric(20,2), @nTotalDebit numeric(20,2), @nTotalCredit  numeric(20,2)

	SELECT @nopening=SUM(isnull(debitamount,0)-isnull(creditamount,0)) from #temp2 WHERE narration='opening balance'
	SELECT @nclosing=SUM(isnull(debitamount,0)-isnull(creditamount,0)) from #temp2 WHERE narration='closing balance'

	SELECT @nTotalDebit=SUM(isnull(debitamount,0)),@nTotalCredit=SUM(isnull(creditamount,0)) from #temp2 
	where voucher_no is not null

	SELECT @cAcName=ac_name FROM lm01106 (NOLOCK) WHERE ac_code=@cAcCodePara

    SELECT TOP 1 'Master' Tablename,@cAcName [Ledger], LTRIM(RTRIM(B0.ADDRESS0+' '+b0.ADDRESS1+' '+b0.ADDRESS2+' '+b1.area_name+' '+b2.city+' '+B3.state )) AS ADDRESS,
	B0.Ac_gst_no AS GST_NO,b0.CREDIT_DAYS [Cr Days],LTRIM(RTRIM(STR(ABS(@nopening))))+' '+(CASE WHEN @nopening<0 THEN 'Cr' ELSE 'Dr' END) Opening,
	@nTotalDebit DebitTotal, @nTotalCredit CreditTotal,
	LTRIM(RTRIM(STR(ABS(@nClosing))))+' '+(CASE WHEN @nopening<0 THEN 'Cr' ELSE 'Dr' END)  Closing,
	ISNULL(lm.LM_REMARKS,'') Remarks,0 td_percentage,0 cd_percentage
	from #temp2 a  
	LEFT OUTER JOIN lmp01106 b0 ON b0.ac_code=a.ac_code
  ...
  IF @nViewType=1
	  select 'Details' Tablename,replace(convert(varchar,VoucherDate,106),' ','-') AS [Date], cost_center_dept_id [Loc],Narration,VS_AC_NAME as [Vs Account],
	  voucher_no [Voucher no],ref_no [Ref No],VoucherType [Type],DebitAmount [Debit], CreditAmount [Credit],
	  runningTotal [Total]   from #temp2 a      
	  JOIN HD01106 b ON a.major_head_code=b.HEAD_CODE      
	  JOIN lm01106 c (NOLOCK) ON c.ac_code=a.ac_code
	  JOIN lm01106 d (NOLOCK) ON d.ac_code=c.major_ac_code       

	  order by VoucherCode,VoucherDate,unqid  
   ELSE
	  select 'Details' Tablename,a.ac_name [Sub Ledger],replace(convert(varchar,VoucherDate,106),' ','-') AS [Date], cost_center_dept_id [Loc],Narration,VS_AC_NAME as [Vs Account],
	  voucher_no [Voucher no],ref_no [Ref No],VoucherType [Type],DebitAmount [Debit], CreditAmount [Credit],
	  runningTotal [Total]  from #temp2 a      
	  JOIN HD01106 b ON a.major_head_code=b.HEAD_CODE      
	  JOIN lm01106 c (NOLOCK) ON c.ac_code=a.ac_code
	  JOIN lm01106 d (NOLOCK) ON d.ac_code=c.major_ac_code       
		...
		LEFT OUTER JOIN city b2 ON b2.CITY_CODE=b1.city_code
		LEFT OUTER JOIN State b3 ON b3.state_code=b2.state_code
	  order by VoucherCode,a.ac_name,VoucherDate,unqid  
END
ELSE
BEGIN
  SELECT @nMode AS A,cost_center_dept_id,a.vm_id,unqid,b.HEAD_NAME as  bs_head_name,a.head_name,a.ac_name,AcCode,
  replace(convert(varchar,getdate(),106),' ','-') AS voucherDate,VoucherType, Narration, DebitAmount, CreditAmount, runningTotal,runningTotal_forex,    
...
END 
	...
	BEGIN
	  select cost_center_dept_id,a.vm_id,unqid,b.HEAD_NAME as  bs_head_name,a.head_name,c.ac_name,AcCode,CONVERT(DATE,VoucherDate) AS VoucherDate,       
...
lblMonthly:    
 ...
 select year_no,month_no,from_dt,to_dt from dbo.FN_ACT_MONTHLIST(@dFromDt ,@dToDt)    
  ...
  --JOIN     
  --(    
  -- select 'Opening Balance' as month_name,0 as MONTH_NO,sum(credit_amount)  as CreditAmount,      
...
sum(credit_amount_forex)  as CreditAmount_forex,      
  ...
  and VOUCHER_CODE not in (select VOUCHER_CODE from vchtype where VOUCHER_TYPE like '%memo%')      
  group by lm.AC_CODE,MONTH(VOUCHER_DT),YEAR(VOUCHER_dT)    
  ) b ON a.month_no=b.month_no    
 ...
 FROM #tempDaily a    
  ...
  SET @cStep='100'    
   ...
   from #tempDailyFinal     
	...
	BEGIN
		update #tempDailyFinal SET INTEREST=CAST((CASE WHEN runningtotal<0 AND display_voucherdate LIKE '[0-9][0-9]-[0-9][0-9]-[0-9][0-9][0-9][0-9]' THEN ABS(ROUND((runningtotal * @bInterestPercent)/ 36500,2))  ELSE NULL END) AS NUMERIC(14,2))    
