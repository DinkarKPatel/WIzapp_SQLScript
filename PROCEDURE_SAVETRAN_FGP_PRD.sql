CREATE PROCEDURE SAVETRAN_FGP_PRD
(
...
BEGIN
	...
	BEGIN TRY
		...
		SET @CTEMPDETAILTABLENAME	= 'TEMP_PRD_FINISH_DET_'+LTRIM(RTRIM(STR(@NSPID)))					
				
		SET @CTEMPDETAILTABLENAME1	= 'TEMP_PRD_FINISH_CONSUMPTION_DET_'+LTRIM(RTRIM(STR(@NSPID)))					
		...
		BEGIN
			...
			SET @CCMD = N'UPDATE PRD_FINISH_MST SET CANCELLED = 1,LAST_UPDATE=GETDATE() ' + 
		...
		SET @CMASTERTABLENAME	= 'PRD_FINISH_MST'
		SET @CDETAILTABLENAME	= 'PRD_FINISH_DET'
		SET @CDETAILTABLENAME1	= 'PRD_FINISH_CONSUMPTION_DET'
		
				

		SET @CTEMPMASTERTABLENAME	= 'TEMP_PRD_FINISH_MST_'+LTRIM(RTRIM(STR(@NSPID)))
     ...
     PRINT 'GENERATING NEW KEY... START'			
	   ...
	   IF OBJECT_ID('TEMPDB..##TREFROW','U') IS NOT NULL
			DROP TABLE ##TREFROW						   
		
		SET @CCMD = N'SELECT ROW_ID,ROW_ID AS NEW_ROW_ID  INTO ##TREFROW FROM ' + @CTEMPDETAILTABLE 
		PRINT @CCMD
		EXEC SP_EXECUTESQL @CCMD
		...
		SET @CCMD = N'UPDATE  ##TREFROW SET NEW_ROW_ID = ''' + @CLOCATIONID + ''' + CONVERT(VARCHAR(40), NEWID())
		...
		SET @CCMD=N'UPDATE A SET ROW_ID=B.NEW_ROW_ID FROM '+@CTEMPDETAILTABLE+' A JOIN ##TREFROW B ON A.ROW_ID=B.ROW_ID'
		PRINT 'STEP 2.2'
		PRINT @CCMD
		EXEC SP_EXECUTESQL @CCMD	
			
        PRINT 'STEP 2.4'
		SET @CCMD=N'UPDATE A SET REF_ROW_ID = B.NEW_ROW_ID FROM '+@CTEMPDETAILTABLE1+' A JOIN ##TREFROW B ON A.REF_ROW_ID = B.ROW_ID'
		PRINT 'STEP 2.3'
		PRINT @CCMD
		...
		EXEC SAVETRAN_UPDSKU_PRD 'FGP',@CKEYFIELDVAL1,@CERRORMSG OUTPUT 
		IF ISNULL(@CERRORMSG,'')<>''
				GOTO END_PROC
...
END_PROC:
	...
	--SELECT * FROM PRD_SKU WHERE PRODUCT_UID= '61AA0E35-F396-4341-8320-27D2BDA036C8'
    --SELECT * FROM PRD_PMT WHERE PRODUCT_UID= '61AA0E35-F396-4341-8320-27D2BDA036C8'
