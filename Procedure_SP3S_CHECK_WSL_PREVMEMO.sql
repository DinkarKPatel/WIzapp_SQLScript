CREATE PROCEDURE SP3S_CHECK_WSL_PREVMEMO--(LocId 3 digit change by Sanjay:04-11-2024)
@CFINYEAR VARCHAR(5),
@CMEMONO VARCHAR(50),
...
BEGIN
	...
	BEGIN
		...
		SELECT @CFIRSTMEMONO=MIN(INV_NO) 
		...
		AND LEN(INV_NO)=LEN(@CMEMONO)
		AND LEFT(A.INV_NO,len(@cSourceLocationCode)+3)=LEFT(@CMEMONO,len(@cSourceLocationCode)+3)
...
lblChkAgain:		
		...
		SELECT TOP 1 @CPREVMEMONOSEARCH=INV_NO  
		FROM INM01106  WITH (NOLOCK) WHERE INV_NO=@CPREVMEMONO AND FIN_YEAR=@CFINYEAR
