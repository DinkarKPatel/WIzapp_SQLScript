CREATE PROCEDURE DBO.SP_SEND_MIRROR_MSTLREQ_NEW
(
...
AS
 BEGIN
   DECLARE @DTSQL NVARCHAR(MAX),@CSTEP VARCHAR(10),@CFILTERCONDITION NVARCHAR(MAX)
	...
	SELECT DISTINCT  'MSTLREQ_LOC_REQ_MIRROR' AS TARGET_TABLENAME,LOC_REQ.*,1 AS PICK_FROM_CURRENT_DB FROM LOC_REQ (NOLOCK)


	END TRY
	BEGIN CATCH
	    SET @CERRORMSG='P: SP_SEND_MIRROR_MSTLREQ_NEW, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()+ 'ERROR QUERY || '+@DTSQL
	    GOTO END_PROC
    END CATCH 
