CREATE PROCEDURE SP3S_CALC_GST_PURCHASE_QUOTATION--(LocId 3 digit change by Sanjay:06-11-2024)
@nSpId VARCHAR(40),
@CERRORMSG VARCHAR(MAX) OUTPUT,
@CLOCID varchar(4)=''
AS
BEGIN
	DECLARE @cSTEP VARCHAR(10),@CPARTYSTATECODE VARCHAR(20),@DPQDT DATETIME
	...
	SELECT row_id,@NSPID AS SP_ID,
	ROUND(((A.PURCHASE_PRICE*A.QUANTITY)-(A.PURCHASE_PRICE*A.QUANTITY*ISNULL(B.DISCOUNT_PERCENTAGE,0)/100)),2) AS NET_VALUE
	,B.BILL_LEVEL_TAX_METHOD AS TAX_METHOD,
	ROW_ID,QUANTITY,'' AS PARTY_DEPT_ID,
	CASE WHEN ISNULL(B.pq_FOR_DEPT_ID,'')<>'' THEN B.pq_FOR_DEPT_ID ELSE  B.DEPT_ID END as DEPT_ID,
	A.MRP,(CASE WHEN ISNULL(A.HSN_CODE,'') IN ('','0000000000')
	THEN C.HSN_CODE ELSE A.HSN_CODE END) AS HSN_CODE FROM PQ_PURCHASE_QUOTATION_DETAIL_UPLOAD A(NOLOCK)
	...
	WHERE A.SP_ID=@nSPId and isnull(A.QUANTITY,0)<>0
		
	SET @cSTEP = 68
	EXEC SP_CHKXNSAVELOG 'PQ',@cSTEP,0,@NSPID,'',1
		
		UPDATE A SET OH_TAX_METHOD=BILL_LEVEL_TAX_METHOD
		FROM  PQ_PURCHASE_QUOTATION_MASTER_UPLOAD A
	    WHERE SP_ID=@NSPID
	...
	SELECT TOP 1 @CPARTY_GSTN_NO=AC_GST_NO,@CPARTYSTATECODE=B.AC_GST_STATE_CODE,@DPQDT=pq_DT
	FROM PQ_PURCHASE_QUOTATION_MASTER_UPLOAD A(NOLOCK) JOIN LMP01106 B(NOLOCK) ON A.AC_CODE=B.AC_CODE WHERE SP_ID=@nSpId
				
	SELECT @BREGISTERED=ISNULL(REGISTERED_GST_DEALER,0) FROM LMP01106 A WITH (NOLOCK) 
	JOIN PQ_PURCHASE_QUOTATION_MASTER_UPLOAD B (nolock) ON A.AC_CODE=B.AC_CODE
	WHERE SP_ID=@nSPId
	...
	JOIN PQ_PURCHASE_QUOTATION_DETAIL_UPLOAD A WITH (ROWLOCK) ON B.ROW_ID=A.ROW_ID  AND a.SP_ID=b.sp_id
	WHERE A.SP_ID=@nSpId
