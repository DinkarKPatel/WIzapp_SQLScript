CREATE PROCEDURE SP_RPT_EXCISEREPORT4--(LocId 3 digit change by Sanjay:30-10-2024)
(
@FROMDT DATETIME,      
@TODT  DATETIME  
) 
...
BEGIN
   ...
   SET @CCMD=N'SELECT [SUB_SECTION_NAME],
		...
		FROM 
		(
				SELECT SECTIOND.[SUB_SECTION_NAME]
					 ,CAST(SUM( (CASE WHEN A.XN_TYPE=''OPS'' OR (A.XN_TYPE IN (''TTM'',''API'',''SCF'',''OPS'',''PRD'', ''PUR'', ''CHI'', ''SLR'',''UNC'',''APR'', ''WSR'', ''PFI'', ''PFG'', ''BCG'',''MRP'',''DCI'',''PSB'',''JWR'') AND XN_DT < '''+@FROMDATE+''' AND ARTICLE.STOCK_NA=0) THEN 1 WHEN A.XN_TYPE IN (''APO'',''SCC'',''BOC'',''PRT'',''CHO'',''SLS'',''CNC'',''APP'',''WSL'',''CIP'', ''CRM'', ''DCO'',''MIP'',''CSB'',''JWI'',''DLM'') AND XN_DT < '''+@FROMDATE+''' AND ARTICLE.STOCK_NA=0 THEN -1 ELSE 0 END) * ([XN_QTY])) AS NUMERIC(14,2)) AS OBS
					  ...
					  ,CAST(SUM( CASE WHEN A.XN_TYPE=''SLS'' AND XN_DT BETWEEN '''+@FROMDATE+''' AND '''+@TODATE+''' 
					  THEN (SKU.MRP * XN_QTY)  WHEN A.XN_TYPE=''SLR'' AND XN_DT BETWEEN '''+@FROMDATE+''' AND '''+@TODATE+''' THEN -(ABS(SKU.MRP * XN_QTY)) ELSE 0 END) AS NUMERIC(14,3)) AS NSG
