CREATE PROCEDURE PPC_SAVETRAN_PPC_AGENCY_REC_MATERIAL
(
...
BEGIN
	...
	SET @CMASTERTABLENAME	= 'PPC_AGENCY_REC_MATERIAL_MST'
	SET @CDETAILTABLENAME1	= 'PPC_AGENCY_REC_MATERIAL_DET'

	
	SET @CTEMPMASTERTABLENAME	= 'PPC_PPC_AGENCY_REC_MATERIAL_MST_UPLOAD'
	SET @CTEMPDETAILTABLENAME1	= 'PPC_PPC_AGENCY_REC_MATERIAL_DET_UPLOAD'
	...
	BEGIN TRY
		...
		BEGIN
			...
			SET @CCMD=N' UPDATE PPC_AGENCY_REC_MATERIAL_MST SET CANCELLED=1,LAST_UPDATE=GETDATE() WHERE MEMO_ID='''+@CXNMEMOID+''''
		...
		BEGIN				-- START OF EDITMODE
			...
			DELETE FROM PPC_AGENCY_REC_MATERIAL_DET WHERE MEMO_ID = @CKEYFIELDVAL1
...
END_PROC:
	...
	IF ISNULL(@CERRORMSG,'') = ''
	BEGIN
	   DELETE FROM PPC_PPC_AGENCY_REC_MATERIAL_MST_UPLOAD WHERE SP_ID=LTRIM(RTRIM(STR(@NSPID)))
	   DELETE FROM PPC_PPC_AGENCY_REC_MATERIAL_DET_UPLOAD WHERE SP_ID=LTRIM(RTRIM(STR(@NSPID)))	
