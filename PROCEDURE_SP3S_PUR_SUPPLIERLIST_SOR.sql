CREATE PROCEDURE SP3S_PUR_SUPPLIERLIST_SOR
(
@CACNAME		VARCHAR(MAX)
) 
AS
BEGIN
	...
	ISNULL(STATE.STATE,'') AS 'SUPP_ADDRESS', AC_NAME AS REPCOLNAME ,LMP.FORM_ID ,ISNULL(F.FORM_NAME,'') AS [FORM_NAME] , LMP.MP_PERCENTAGE,LMP.WP_PERCENTAGE,  
	...
	AND LM.INACTIVE = 0 AND ISNULL(LM.SOR_party,0)= 1 AND LM.AC_NAME <> '' AND (LM.AC_NAME LIKE @CACNAME  OR LM.ALIAS LIKE @CACNAME )
