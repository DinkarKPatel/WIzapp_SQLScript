CREATE PROCEDURE SAVETRAN_SLS_AFTERSAVE--(LocId 3 digit change by Sanjay:06-11-2024)
(
...
LBLSTART:	
	...
	DECLARE @CTEMPDBNAME			VARCHAR(100),
			...
			@NCALCTAXAMT NUMERIC(10,2),@BSLRRECONREQD BIT,
			@BMANUALBILL BIT,@CTEXT VARCHAR(500),@CDTCODE CHAR(7),
			@DSTARTTIME DATETIME,@BAPPRETFOUND BIT,
			@CTMPTABLEFINYEAR VARCHAR(10),@DSLRRECONCUTOFFDATE DATETIME,
	...
	IF @NUPDATEMODE NOT IN (3,5)
		SELECT @CLOCATIONID	= location_code FROM sls_cmm01106_upload (NOLOCK) WHERE sp_id=@nSpid	
	ELSE
		SELECT @CLOCATIONID	= location_code FROM CMM01106 (NOLOCK) WHERE cm_id=@cMemoId
...
BEGIN TRY	
	...
	SET @BSISLOC=0/*Rohit 13-03-2025 : Taiga - #1500 SIS LOCATION ( CASH MEMO EDIT)*/
	...
	@BSLRRECONREQD=ISNULL(SLR_RECON_REQD,0) 
	FROM LOCATION WHERE DEPT_ID=@CLOCATIONID
	...
	ELSE
	IF  @dCmDt>='2020-04-01'
		SET @NMEMONOLEN=10+len(@cLocationId) ---- (As discussed with Sir, 1st April 2020 onwards...All memos shall be generated of 12 Chars)
		...
		BEGIN	
			...
			DECLARE @nDocIndex NUMERIC(2,0)
			
			SET @nDocIndex=len(@cLocationId)+3
			...
			BEGIN
			   ...
			   SET @CCMD='IF (SUBSTRING(''' + @CMEMONOVAL+''','+str(@nDocIndex)+',1)=''N'')
			              SET @CONLINE_CN=1
			           ELSE
							   SET @CONLINE_CN=1'
				PRINT @CCMD
		...
		BEGIN				-- START OF EDITMODE
			...
			BEGIN
				...
				IF EXISTS (SELECT TOP 1 cm_id FROM rps_mst (NOLOCK) WHERE REF_CM_ID=@CKEYFIELDVAL1)
					UPDATE RPS_MST WITH (ROWLOCK) SET REF_CM_ID='',CANCELLED=1/*CANCELLED=1 Done by Rohit 25-04-2023 WizTickit : 0423-00118*/ 
				  ...
				  if @BSLRRECONREQD=1
				    DELETE FROM POSGRRecos WITH (ROWLOCK) WHERE cm_id=@CKEYFIELDVAL1
		...
		BEGIN
			...
			BEGIN
				...
				SELECT @nTotalCmdwithGst=SUM( isnull(XN_VALUE_WITHOUT_GST,0)+isnull(IGST_AMOUNT,0)+isnull(CGST_AMOUNT,0)+isnull(SGST_AMOUNT,0)+isnull(cess_amount,0)+isnull(gst_cess_amount,0)) 
    	...
    	EXEC UPDATEMASTERXN_OPT--UPDATEMASTERXN 
			...
			IF @BSLRRECONREQD=1 AND @NUPDATEMODE IN (1,2)
			BEGIN
				EXEC SP_CHKXNSAVELOG 'SLS_after',@cStep,0,@NSPID,1

				IF @NUPDATEMODE=2
				BEGIN
					DELETE a FROM POSGRRecos a WITH (ROWLOCK) LEFT JOIN SLS_cmd01106_UPLOAD b (NOLOCK) ON a.cm_id=b.cm_id AND a.CMDRowId=b.ROW_ID AND b.sp_id=@nSpId
					where a.cm_id=@CKEYFIELDVAL1 AND (b.ROW_ID IS NULL OR b.QUANTITY>0)

					UPDATE a SET GRQuantity=abs(b.QUANTITY),lastupdate=getdate()  from POSGRRecos a WITH (ROWLOCK) JOIN SLS_cmd01106_UPLOAD b (NOLOCK) ON a.cm_id=b.cm_id AND a.CMDRowId=b.ROW_ID
					WHERE b.sp_id=@nSpId AND b.QUANTITY<0
				END

				INSERT POSGRRecos	( cm_id, CMDRowId, GRQuantity, lastupdate, ProductCode, RecoQty )  
				SELECT 	a.cm_id,row_id CMDRowId,abs(A.QUANTITY) GRQuantity,getdate() lastupdate,a.PRODUCT_CODE ProductCode,0 RecoQty 
				FROM SLS_cmd01106_UPLOAD a (NOLOCK)
				LEFT JOIN POSGRRecos b (NOLOCK) ON a.ROW_ID=b.CMDRowId WHERE a.sp_id=@nSpId AND a.quantity<0 AND b.cm_id IS NULL
			END
...
--
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					EXEC SPSLS_PRINT_PRG @NLOGINSPID,@CKEYFIELDVAL1,3,@CLOCATIONID,0,@CERRORMSG OUTPUT				
				END
			END	
		END	
	...
	ELSE IF  @bValidationGvLocalFailed=0 AND (@bSisLoc=0 OR @NUPDATEMODE=3)
	BEGIN
	...
	END
	ELSE IF @bValidationGvLocalFailed=1
