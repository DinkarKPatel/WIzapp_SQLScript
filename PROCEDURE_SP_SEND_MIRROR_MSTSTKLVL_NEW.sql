CREATE PROCEDURE DBO.SP_SEND_MIRROR_MSTSTKLVL_NEW
(
...
AS
 BEGIN
   DECLARE @DTSQL NVARCHAR(MAX),@CSTEP VARCHAR(10),@CFILTERCONDITION NVARCHAR(MAX)
    ...
    SET @CSTEP=40
    ---SELECT DATA FROM LOC_STOCK_LEVEL_MST TABLE
	SET @CTABLENAME = 'LOC_STOCK_LEVEL_MST'
	SELECT DISTINCT  'MSTSTKLVL_LOC_STOCK_LEVEL_MST_MIRROR' AS TARGET_TABLENAME,LOC_STOCK_LEVEL_MST.*,1 AS PICK_FROM_CURRENT_DB FROM LOC_STOCK_LEVEL_MST (NOLOCK)
	--INSERT DATA FROM MAIN TABLE TO TEMP TABLE
    SET @CSTEP=50
    ---SELECT DATA FROM LOC_STOCK_LEVEL_DET TABLE
	SET @CTABLENAME = 'LOC_STOCK_LEVEL_DET'
	SELECT DISTINCT  'MSTSTKLVL_LOC_STOCK_LEVEL_DET_MIRROR' AS TARGET_TABLENAME,LOC_STOCK_LEVEL_DET.*,1 AS PICK_FROM_CURRENT_DB FROM LOC_STOCK_LEVEL_DET (NOLOCK)


	END TRY
	
	BEGIN CATCH
	    SET @CERRORMSG='P: SP_SEND_MIRROR_MSTSTKLVL_NEW, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()+ 'ERROR QUERY || '+@DTSQL
	    GOTO END_PROC
    END CATCH 
