create PROCEDURE SPSIS_SEND_MIRROR_WSR_DATA
(
...
LBLTABLEINFO:
	...
	SET @CSTEP=60
	SELECT @cCols=COALESCE(@cCols+',','')+colname 	FROM ebo_xnssending_cols WHERE tablename='CNM01106' 

	SET @cCmdCols=N'
	   SELECT ''WSR_CNM01106_upload'' AS TARGET_TABLENAME,'+@cCols+' FROM CNM01106  (NOLOCK)   WHERE CN_ID='''+@CMEMOID+''''
	PRINT @cCmdCols
	...
	SET @CSTEP=65	
	SET @cCols=NULL
	SELECT @cCols=COALESCE(@cCols+',','')+'CND01106.'+colname FROM ebo_xnssending_cols	WHERE tablename='CND01106' 

	SET @cCOls=REPLACE(@cCOls,'CND01106.PRODUCT_CODE','CASE WHEN ISNULL(C.VENDOR_EAN_NO,'''')<>'''' THEN C.VENDOR_EAN_NO ELSE CND01106.PRODUCT_CODE END AS PRODUCT_CODE')

	PRINT @cCOls
	SET @cCmdCols=N'
		SELECT ''WSR_CND01106_upload'' AS TARGET_TABLENAME,'+@cCols+' 
		FROM CND01106  (NOLOCK)   
		JOIN SKU C (NOLOCK) ON C.PRODUCT_CODE=CND01106.PRODUCT_CODE
		JOIN LMP01106 LMP (NOLOCK) ON LMP.AC_CODE =C.AC_CODE
		WHERE SUBSTRING(LMP.AC_GST_NO,3,10)='''+@CPANNO+'''
		AND CND01106.CN_ID='''+@CMEMOID+''''
	...
	EXEC SPSIS_GET_CURSOR_SKUNAMES @cXNTYPE='WSR',@cTABLENAME='CND01106',@cKEYNAME='CN_ID',@cKEYVALUE=@CMEMOID,@CERRMSG=@CERRMSG OUTPUT 
	IF ISNULL(@CERRMSG,'')<>'' 
		GOTO END_PROC
...
BEGIN CATCH
	SET @CERRMSG='P: SPSIS_SEND_MIRROR_WSR_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
