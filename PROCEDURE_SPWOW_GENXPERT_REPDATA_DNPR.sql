CREATE PROCEDURE SPWOW_GENXPERT_REPDATA_DNPR --(lOC 3 DIGIT CHANGE BY sANJAY :22-11-2024 (LEFT CHANGES OF LEFT(PS_ID,2) BY OTHER DEVELOPER)
...
BEGIN
	...
	JOIN [DATABASE].dbo.rmm01106  (NOLOCK) ON dnps_mst.prt_rm_id=rmm01106.rm_id   
	...
	LEFT JOIN XN_ITEM_TYPE_DESC_mst (NOLOCK) on XN_ITEM_TYPE_DESC_mst.XN_ITEM_TYPE=1
  	WHERE rmm01106.RM_dt BETWEEN [DFROMDT] AND [DTODT]  AND DNPS_MST.cancelled=0 
	 AND [WHERE]       
	group by [GROUPBY]'
