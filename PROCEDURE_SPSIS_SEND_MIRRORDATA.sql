CREATE PROCEDURE SPSIS_SEND_MIRRORDATA
...
BEGIN TRY	
	...
	BEGIN
		SET @CERRMSG='SIS XNS SENDING VIA MIRRORSERVICE CAN BE DONE FROM HEAD OFFICE....'
		GOTO EXIT_PROC
	END 
	...
	--SELECT XN_ID AS SOURCE_XN_ID,XN_TYPE AS SOURCE_XN_TYPE,XN_TYPE AS SEARCH_XN_TYPE 
	--INTO #TMPPENDINGMST
	...
	SET @CPROC_NAME='SPSIS_SEND_MIRROR_'+LTRIM(RTRIM(@CXNTYPE))+'_DATA'
	...
	BEGIN
		...
		SET @DTSQL='EXEC '+@CPROC_NAME+' '''+@CPANNO+''','''+@CREQXNID+''','''+@CCURLOCID+''','+@CACKNOWLEDGE+',@CERRMSG OUTPUT'
...
BEGIN CATCH
	SET @CERRMSG='P:SPSIS_SEND_MIRRORDATA, STEP: '+@CSTEP+',MESSAGE:'+ERROR_MESSAGE()
	
	PRINT 'CATCH IN SPSIS_SEND_MIRRORDATA'+@CERRMSG
END CATCH	 
