create PROCEDURE ACT_DAYBOOK_NEW
(	
		@NDAYBOOKMODE INT=1,
		@CDEPTID VARCHAR(4), 
		@DFROMDT DATETIME, 
...
AS
BEGIN
	DECLARE @CVMID VARCHAR(40), @CVDID VARCHAR(40), @TSTS TIMESTAMP
	...
	DECLARE @DAYBOOKC TABLE ( VOUCHER_DT DATETIME, VOUCHER_NO VARCHAR(20), VOUCHER_CODE VARCHAR(10), 
							  ...
							  DR_AC_NAME VARCHAR(100), DR_NARRATION VARCHAR(5000), DEBIT_AMOUNT NUMERIC(14,2), 
							  CASH_DEBIT_AMOUNT NUMERIC(14,2),
							  CR_VD_ID VARCHAR(40), CR_AC_NAME VARCHAR(100), CR_NARRATION VARCHAR(5000), 
	...
	BEGIN
		...
		AND A.COST_CENTER_DEPT_ID = ( CASE WHEN @CDEPTID <> '' THEN @CDEPTID ELSE A.COST_CENTER_DEPT_ID END )
	...
	BEGIN
		...
		( 
			...
			AND B.COST_CENTER_DEPT_ID = ( CASE WHEN @CDEPTID <> '' THEN @CDEPTID ELSE B.COST_CENTER_DEPT_ID END )
	...
	BEGIN
		...
		SELECT VM_ID, DR_VD_ID FROM @DAYBOOKC GROUP BY VM_ID, DR_VD_ID HAVING COUNT(*) >1 ORDER BY VM_ID, DR_VD_ID 
