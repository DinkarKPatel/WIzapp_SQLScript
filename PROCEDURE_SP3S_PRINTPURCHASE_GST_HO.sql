CREATE PROCEDURE SP3S_PRINTPURCHASE_GST_HO  
(  
 @CXN_TYPE VARCHAR(10)='',  
 @CXN_ID VARCHAR(100)=''  
)  
...
BEGIN TRY  
  ...
  DECLARE @NXN_VALUE_WITHOUT_GST NUMERIC(10,2),@NCGST_AMOUNT numeric(10,2),@NSGST_AMOUNT numeric(10,2),@NIGST_AMOUNT numeric(10,2)
    
  SELECT @PP_TOTAL='PP TOTAL: '+CAST(CAST(SUM(QUANTITY*((a.RATE)-CONVERT(NUMERIC(14,2),(A.RATE*A.DISCOUNT_PERCENTAGE/100)))) AS DECIMAL(18,2))AS VARCHAR)
  ,@MRP_TOTAL='MRP TOTAL: '+CAST(CAST(SUM(QUANTITY*MRP) AS DECIMAL(18,2))AS VARCHAR)
  ,@NXN_VALUE_WITHOUT_GST=SUM(XN_VALUE_WITHOUT_GST)
  ,@NCGST_AMOUNT=SUM(CGST_AMOUNT)
  ,@NSGST_AMOUNT=SUM(SGST_AMOUNT)
  ,@NIGST_AMOUNT=SUM(IGST_AMOUNT)
  ...
  ,CASE WHEN INV_MODE IN (1,2)
   ...
   ,ISNULL(@NCGST_AMOUNT,0)+ISNULL(@NSGST_AMOUNT,0)  +ISNULL(@NIGST_AMOUNT,0)    
    +ISNULL(PIM.OTHER_CHARGES_CGST_AMOUNT,0)+ISNULL(PIM.FREIGHT_CGST_AMOUNT,0)  
    +ISNULL(PIM.OTHER_CHARGES_SGST_AMOUNT,0)+ISNULL(PIM.FREIGHT_SGST_AMOUNT,0)  
    +ISNULL(PIM.OTHER_CHARGES_IGST_AMOUNT,0)+ISNULL(PIM.FREIGHT_IGST_AMOUNT,0)  
    AS [GSTCOLLECTION]  
   ...
   ,'APPLY CD TAX: '+CASE CAST('0' AS VARCHAR) WHEN '0' THEN 'N' ELSE 'Y' END AS [APPLY_CASH_DISC_ON_TAX]
   ...
   SELECT @AMOUNT_DIV=SUM(QUANTITY*rate)--SUM(DISCOUNT_AMOUNT)-SUM(QUANTITY*PIMDISCOUNTAMOUNT) 
   FROM ind01106 a (NOLOCK) 
   join pim01106 pim (nolock) on a.inv_id=pim.inv_id 
   WHERE MRR_ID=@CXN_ID
   ...
   SET @RATECOL='((a.RATE)-CONVERT(NUMERIC(14,2),(A.RATE*A.DISCOUNT_PERCENTAGE/100)))'   
   SET @DTSQL=N' SELECT ROW_NUMBER() OVER (ORDER BY '+ @CCOLNAME+' ) AS SR_NO, '+ @CCOLNAME+' AS PARTICULARS ,  
                 A.HSN_CODE,UOM.UOM_NAME,  
                 SUM(A.QUANTITY) AS QUANTITY,  
                 A.rate AS GROSS_PP  
                 ,'+@RATECOL+' AS RATE  
                 ,CAST(SUM(A.QUANTITY*'+@RATECOL+') AS NUMERIC(12,2)) AS AMOUNT,  
                 CAST(SUM(ISNULL(A.DISCOUNT_AMOUNT,0)+(ISNULL(A.inmDISCOUNTAMOUNT,0)*1)) AS NUMERIC(12,2))  
                 '  
                 --+CASE @RATECOL   
                 ...
                 ,A.MRP
                 ,0 as MD_PERCENTAGE,0 as MP_PERCENTAGE
                 ,CAST(SUM(A.QUANTITY*A.rate) AS NUMERIC(12,2))
       ...
       LEFT OUTER JOIN ARTICLE (NOLOCK) ON ARTICLE.ARTICLE_CODE =sku.ARTICLE_CODE      
       ...
       WHERE b.MRR_ID='''+@CXN_ID+'''  
       GROUP BY '+ @CCOLNAME+',A.HSN_CODE,UOM.UOM_NAME,A.rate,'+@RATECOL+',A.GST_PERCENTAGE ,B.TOTAL_AMOUNT ,B.ROUND_OFF,A.MRP
       ORDER BY '+ @CCOLNAME+''  
   PRINT @DTSQL  
   ...
   SELECT POD.ROW_ID AS POD_ROW_ID,  FROM_UOM_NAME,TO_UOM_NAME, 
	...
	where PID.MRR_ID=@CXN_ID AND POM.CANCELLED =0
