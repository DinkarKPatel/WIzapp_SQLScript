CREATE PROCEDURE SAVETRAN_PRD_AGENCY_RECUPC
(
...
BEGIN
	...
	SET @CDETAILTABLENAME1	= 'PRD_AGENCY_MATERIAL_RECEIPT_UPC'
	...
	SET @CTEMPDETAILTABLE3	= @CTEMPDBNAME + 'TEMP_PRD_AGENCY_ROW_MATERIAL_RECEIPT_DET'+'_'+LTRIM(RTRIM(STR(@NSPID)))
	SET @CERRORMSG			= ''
	...
	SET @NSTEP = 10		-- GETTING DEPTID INFO FROM TEMP TABLE
		...
		BEGIN
	       ...
	       IF EXISTS (SELECT TOP 1 'U' FROM PRD_UPCPMT A
		    JOIN PRD_AGENCY_MATERIAL_RECEIPT_UPC B ON A.PRODUCT_CODE=B.PRODUCT_CODE WHERE B.MEMO_ID=@CMEMO_ID AND QUANTITY_IN_STOCK <0)
		    BEGIN
