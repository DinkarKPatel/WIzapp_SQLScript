create PROCEDURE DBO.SP_SEND_MIRROR_MSTPAYMODE_NEW
(
...
AS
 BEGIN
   DECLARE @DTSQL NVARCHAR(MAX),@CSTEP VARCHAR(10),@CFILTERCONDITION NVARCHAR(MAX)
	...
	SELECT DISTINCT  'MSTPAYMODE_PAYMODE_GRP_MST_MIRROR' AS TARGET_TABLENAME,PAYMODE_GRP_MST.*,1 AS PICK_FROM_CURRENT_DB FROM PAYMODE_GRP_MST (NOLOCK)
	...
	SELECT DISTINCT  'MSTPAYMODE_PAYMODE_MST_MIRROR' AS TARGET_TABLENAME,PAYMODE_MST.*,1 AS PICK_FROM_CURRENT_DB FROM PAYMODE_MST (NOLOCK)
	...
	BEGIN CATCH
	    SET @CERRORMSG='P: SP_SEND_MIRROR_MSTPAYMODE_NEW, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()+ 'ERROR QUERY || '+@DTSQL
	    GOTO END_PROC
    END CATCH 
