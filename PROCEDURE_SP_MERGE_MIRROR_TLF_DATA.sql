CREATE PROCEDURE SP_MERGE_MIRROR_TLF_DATA
(
	@CMEMOID VARCHAR(50)
   ,@CLOCID VARCHAR(3)
...
BEGIN
	/*
		SP_MERGE_MIRROR_TLF_DATA_208_2014_02_05 : 
...
BEGIN TRANSACTION 
	...
	SELECT @CTEMPMASTERTABLE=@CSOURCEDB+'TMP_TLF_TILL_LIFTS_'+LTRIM(RTRIM(@CTABLE_SUFFIX))

LBLMERGE:
  					  
    SET @CSTEP=300
	SET @CTABLENAME='TILL_LIFTS'
	SET @CTMP_TABLENAME='TMP_TLF_'+@CTABLENAME+'_'+LTRIM(RTRIM(@CTABLE_SUFFIX))
...
END TRY
BEGIN CATCH
	SET @CERRMSG='P:SP_MERGE_MIRROR_TLF_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
END CATCH
EXIT_PROC:
	...
	ELSE IF ISNULL(@CERRMSG,'')<>'' AND @@TRANCOUNT>0
		ROLLBACK
END	
---END OF PROCEDURE - SP_MERGE_MIRROR_TLF_DATA
