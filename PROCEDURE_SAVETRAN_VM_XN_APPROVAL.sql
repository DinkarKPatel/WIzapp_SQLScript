...
create PROCEDURE SAVETRAN_VM_XN_APPROVAL
(
...
--changes by Dinkar in location id varchar(4)..
	...
	SET @CMASTERTABLENAME	= 'VM_XN_APPROVAL'

	SET @CTEMPMASTERTABLENAME	= 'TEMP_VM_XN_APPROVAL_'+LTRIM(RTRIM(STR(@NSPID)))
	...
	BEGIN TRY
		...
		SET @CCMD=N'UPDATE A 
					SET APPROVEDLEVELNO=(CASE WHEN ISNULL(C.APPROVAL_STATUS,0)=1 AND 
  						 ...
  						 FROM VM01106 A
  						 JOIN '+@CTEMPMASTERTABLE+' C ON A.VM_ID=C.VM_ID '
  		PRINT @CCMD  						   
		...
		IF ISNULL(@CLOCATIONID,'')=''
		 ...
		 END
			...
			SET @CCMD = 'UPDATE A SET A.LEVEL_NO=(CASE WHEN ISNULL(A.LEVEL_NO,0)=0 THEN B.LEVEL_NO ELSE A.LEVEL_NO END),
						 ...
						 JOIN XN_APPROVAL_CHECKLIST_LEVELS C ON C.LEVEL_NO=B.LEVEL_NO AND C.XN_TYPE=''VCH'''
