create PROCEDURE  SP3S_SYNCH_UPLOADDATA_TLF_OPT
(
    @nSpId VARCHAR(50)
...
BEGIN TRY
	...
	SELECT @CTEMPMASTERTABLE=@CSOURCEDB+'TLF_TILL_LIFTS_UPLOAD'
...
LBLSTART:
    ...
    FROM TLF_TILL_LIFTS_UPLOAD B (NOLOCK)
    WHERE sp_id=@nSpId
...
LBLMERGE:
	...
	SET @CTMP_TABLENAME='TLF_TILL_LIFTS_upload'
   ...
   DELETE A FROM TLF_TILL_LIFTS_upload A WITH (ROWLOCK)  WHERE A.MEMO_ID=@CMEMOID 
	
	

END TRY
BEGIN CATCH
	SET @CERRMSG='P:SP3S_SYNCH_UPLOADDATA_TLF_OPT,MEMO ID :'+@CMEMOID+' STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO EXIT_PROC
END CATCH
