CREATE PROC SP_APPR_RETURN_FILTER
(
...
BEGIN
	...
	JOIN
	(
	 SELECT M.MEMO_ID,SUM(QUANTITY)  AS QUANTITY ,SUM(ISNULL(QUANTITY,0)*ISNULL(APD.MRP,0)) AS NET_AMOUNT 
	 FROM APPROVAL_RETURN_DET D
	 JOIN APPROVAL_RETURN_MST M ON D.MEMO_ID =M.MEMO_ID 
	 LEFT JOIN
	 (
	 SELECT B.ROW_ID,MRP    
	 FROM APM01106 A
	...
	AND (ISNULL(@NXN_ITEM_TYPE,0)=0 OR T1.xn_item_type=@NXN_ITEM_TYPE)
	ORDER BY T1.MEMO_ID
