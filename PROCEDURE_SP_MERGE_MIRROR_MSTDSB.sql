CREATE PROCEDURE DBO.SP_MERGE_MIRROR_MSTDSB
 ( @NMODE INT
  ,@XN_TYPE VARCHAR(50) ='MSTDSB'
  )
AS
 BEGIN
...
BEGIN TRY
	  ...
	  BEGIN
			SET @CERRORMSG='P:SP_MERGE_MIRROR_MSTDSB, CANNOT CALL MERGING PROC AT HEAD OFFICE'
			GOTO END_PROC
  	  END					
	  ...
	  DELETE FROM SALECONTRIBUTIONDATA
	 ...
	 --INSERT DATA INTO LOCATION TABLE
	   ...
	   SET @CSTEP = 40
	   SET @CTABLENAME = 'SALECONTRIBUTIONDATA'
	   SET @CSOURCETABLE ='MSTDSB_SALECONTRIBUTIONDATA_MIRROR'
	   SET @CKEYFIELD1 ='REPORT_NO'
	   EXEC UPDATEMASTERXN
...
BEGIN CATCH
	SET @CERRORMSG='P:SP_MERGE_MIRROR_MSTDSB, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
...
END_PROC:
	...
	IF ISNULL(@CERRORMSG,'')='' 
	BEGIN   
	   DELETE FROM MSTDSB_SALECONTRIBUTIONDATA_MIRROR
...
--END PROCEDURE SP_MERGE_MIRROR_MSTDSB
