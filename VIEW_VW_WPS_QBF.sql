...
FORM.FORM_NAME,
BM.ORDER_NO  ,A.location_Code    
FROM WPS_MST A (NOLOCK)             
JOIN LMV01106 B (NOLOCK)  ON A.AC_CODE = B.AC_CODE            
JOIN WPS_DET C (NOLOCK) ON A.PS_ID = C.PS_ID  
LEFT OUTER JOIN BUYER_ORDER_MST BM ON BM.ORDER_ID=C.ORDER_ID          
...
LEFT OUTER JOIN EMPLOYEE T5 ON T5.EMP_CODE = C.EMP_CODE 
LEFT OUTER JOIN EMPLOYEE T9 ON T9.EMP_CODE = C.EMP_CODE1  
LEFT OUTER JOIN EMPLOYEE T10 ON T9.EMP_CODE = C.EMP_CODE2
LEFT OUTER JOIN FORM  ON FORM.FORM_ID=C.ITEM_FORM_ID 
