CREATE PROCEDURE DBO.SP_SEND_MIRROR_MSTREGINFO_NEW
(
...
AS
 BEGIN
   DECLARE @DTSQL NVARCHAR(MAX),@CSTEP VARCHAR(10),@CFILTERCONDITION NVARCHAR(MAX)
	...
	SELECT DISTINCT  'MSTREGINFO_LICENSE_INFO_HO_MIRROR' AS TARGET_TABLENAME,LICENSE_INFO_HO.*,1 AS PICK_FROM_CURRENT_DB FROM LICENSE_INFO_HO (NOLOCK)
	...
	SELECT DISTINCT  'MSTREGINFO_REG_HAND_SHAKE_MIRROR' AS TARGET_TABLENAME,REG_HAND_SHAKE.*,1 AS PICK_FROM_CURRENT_DB FROM REG_HAND_SHAKE (NOLOCK)

	
	END TRY
	
	BEGIN CATCH
	    SET @CERRORMSG='P: SP_SEND_MIRROR_MSTREGINFO, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()+ 'ERROR QUERY || '+@DTSQL
	    GOTO END_PROC
    END CATCH 
