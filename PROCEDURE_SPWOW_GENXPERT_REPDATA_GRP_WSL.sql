CREATE PROCEDURE SPWOW_GENXPERT_REPDATA_GRP_WSL
...
BEGIN
	...
	@cGrpCols VARCHAR(MAX),	@cLayoutCols VARCHAR(MAX),@cJoinStr VARCHAR(MAX),@cInsCols VARCHAR(MAX),@nLoop INT,@cXntypeSearch VARCHAR(20)
	...
	JOIN location TargetLocation on TargetLocation.dept_id=inm01106.party_dept_id
	...
	Left  outer JOIN lm01106 party_lm01106 on party_lm01106.ac_code=INM01106.ac_code
	...
	left join pim01106 cim (NOLOCK) ON cim.inv_id=inm01106.inv_id and cim.cancelled=0
	...
	JOIN bin TargetBin on TargetBin.bin_id=INM01106.target_bin_id
	LEFT JOIN sku_xfp (NOLOCK) ON sku_xfp.dept_id=inm01106.party_dept_id/*LEFT(INM01106.INV_ID,2)*//*Rohit 05-11-2024*/ AND sku_xfp.product_code=ind01106.product_code
	LEFT JOIN sku_current_xfp (NOLOCK) ON sku_current_xfp.dept_id=inm01106.party_dept_id/*LEFT(INM01106.INV_ID,2)*//*Rohit 05-11-2024*/ AND sku_current_xfp.product_code=ind01106.product_code
	...
	Left outer Join gst_state_mst Location_gst_state (nolock) on Location_gst_state.gst_state_code= SourceLocation.gst_state_code
	  Left outer Join bin MajorSourceBin on MajorSourceBin.BIN_ID=SourceBin.major_bin_id
	  	  	LEFT OUTER JOIN USERS ON USERS.USER_CODE= INM01106.USER_CODE
  	WHERE INM01106.inv_dt BETWEEN [DFROMDT] AND [DTODT]  AND INM01106.inv_mode=2 AND INM01106.cancelled=0
	AND SourceLocation.loc_gst_NO<>TargetLocation.loc_gst_NO
    ...
    Left Outer 	JOIN city  party_city on party_city.city_code=party_area.city_code
	Left Outer  JOIN state  party_state on party_state.state_code=party_city.state_code
	left join pim01106 cim (NOLOCK) ON cim.inv_id=inm01106.inv_id
	...
	WHERE b.xn_type='WSLOH' AND a.xn_type='GRP_WSL'

	
	SELECT TOP 1 @cXntypeSearch=xn_type FROM wow_xpert_xntypes_alias WHERE xn_type_alias='GRP_WSL'
	...
	update #wow_xpert_rep_det SET xn_type='GRP_WSL' WHERE xn_type='WSLOH'
END