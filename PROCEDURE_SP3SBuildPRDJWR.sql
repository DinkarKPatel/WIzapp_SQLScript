CREATE PROCEDURE SP3SBUILDPRDJWR
(
	 @CXNID			VARCHAR(50)
...
BEGIN TRY
	...
	BEGIN
		...
		SET @CCMD=N'INSERT '+@CRFDBNAME+'RF_OPT(DEPT_ID,XN_TYPE,XN_DT,XN_NO,XN_ID,PRODUCT_CODE,
				 ...
				 JOIN PRD_JOBWORK_RECEIPT_MST B (NOLOCK) ON A.RECEIPT_ID = B.ISSUE_ID  
	...
	BEGIN
		IF @nUpdateMode=4
		BEGIN
			SET @CCMD=N'DELETE A FROM '+@CRFDBNAME+'RF_OPT A
						WHERE A.XN_ID=''PRDJWR''+'''+@CXNID+''''
			PRINT @CCMD 
			EXEC SP_EXECUTESQL @CCMD			
		END
		ELSE
...
END TRY
BEGIN CATCH
	SET @CERRMSG='SP3SBUILDPRDJWR: STEP :'+@CSTEP+',ERROR :'+ERROR_MESSAGE()
END CATCH	

ENDPROC:

END
--END OF PROCEDURE - SP3SBUILDPRDJWR
