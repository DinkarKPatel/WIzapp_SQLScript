CREATE PROCEDURE SAVETRAN_XFR_PRD_PENDING
(
...
BEGIN
	...
	BEGIN TRY
		...
		SET @CTEMPDETAILTABLENAME	= 'TEMP_PRD_STK_TRANSFER_DET_PENDING_'+LTRIM(RTRIM(STR(@NSPID)))							
		...
		BEGIN
			...
			BEGIN
                SET @CERRORMSG = 'STEP- ' + LTRIM(STR(@NSTEP)) + ' MATERIAL ALREADY ISSUED FOR AGENCY -' +ISNULL(@CPRODUCT_CODE,'')
				GOTO END_PROC 
			...
			SET @CCMD = N'UPDATE PRD_STK_TRANSFER_MST_PENDING SET CANCELLED = 1,LAST_UPDATE=GETDATE() ' + 
		...
		SET @CMASTERTABLENAME	= 'PRD_STK_TRANSFER_MST_PENDING'
		SET @CDETAILTABLENAME	= 'PRD_STK_TRANSFER_DET_PENDING'


		SET @CTEMPMASTERTABLENAME	= 'TEMP_PRD_STK_TRANSFER_MST_PENDING_'+LTRIM(RTRIM(STR(@NSPID)))
		...
		PRINT 'UPDATESKU'
			...
			SET @NSTEP = 250
		EXEC SAVETRAN_UPDSKU_PRD 'XFR_PEN',@CKEYFIELDVAL1,@CERRORMSG OUTPUT 
		IF ISNULL(@CERRORMSG,'')<>''
				GOTO END_PROC
