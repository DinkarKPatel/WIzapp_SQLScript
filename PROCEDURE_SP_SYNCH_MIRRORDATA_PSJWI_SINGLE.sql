...
CREATE PROCEDURE SP_SYNCH_MIRRORDATA_PSJWI_SINGLE
(
    @CLOCID VARCHAR(3)
...
BEGIN TRY
	...
	SELECT @CTEMPMASTERTABLE=@CSOURCEDB+'PSJWI_POST_SALES_JOBWORK_ISSUE_MST_MIRROR',
		   @CTEMPDETAILTABLE=@CSOURCEDB+'PSJWI_POST_SALES_JOBWORK_ISSUE_DET_MIRROR'
...
LBLSTART:
    ...
    FROM PSJWI_POST_SALES_JOBWORK_ISSUE_MST_MIRROR B (NOLOCK)
    LEFT OUTER JOIN MIRROR_SYNCH_LOG C (NOLOCK) ON C.MEMO_ID=B.ISSUE_ID AND C.XN_TYPE='PSJWI'
    WHERE LEFT(B.ISSUE_ID,2)=@CLOCID AND ISSUE_DT>@DFREEZINGDATE 
	...
	BEGIN
		...
		SELECT @CFINYEAR=FIN_YEAR FROM PSJWI_POST_SALES_JOBWORK_ISSUE_MST_MIRROR (NOLOCK) WHERE ISSUE_ID=@CMEMOID
...
LBLMERGE:
	...
	SET @CTMP_TABLENAME='PSJWI_POST_SALES_JOBWORK_ISSUE_MST_MIRROR'
	...
	SET @CTMP_TABLENAME='PSJWI_POST_SALES_JOBWORK_ISSUE_DET_MIRROR'
    ...
    SET @CSTEP=330
    SELECT TOP 1 @CERRMSG=ERRMSG FROM MIRROR_SYNCH_LOG (NOLOCK) WHERE XN_TYPE='PSJWI' AND MEMO_ID=@CMEMOID
    
    IF ISNULL(@CERRMSG,'')=''
    BEGIN 
		SET @CSTEP=340
	    		
		DELETE A FROM PSJWI_POST_SALES_JOBWORK_ISSUE_MST_MIRROR A WHERE A.ISSUE_ID=@CMEMOID AND A.VERSION_NO<=@NVERSIONNO
		
		DELETE A FROM PSJWI_POST_SALES_JOBWORK_ISSUE_DET_MIRROR A WHERE A.ISSUE_ID=@CMEMOID AND A.VERSION_NO<=@NVERSIONNO
...
END TRY
BEGIN CATCH
	SET @CERRMSG='P:SP_SYNCH_MIRRORDATA_PSJWI_SINGLE, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO EXIT_PROC
END CATCH
