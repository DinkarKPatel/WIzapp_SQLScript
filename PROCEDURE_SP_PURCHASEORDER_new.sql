CREATE PROCEDURE SP_PURCHASEORDER_new  
(  
 @CQUERYID  NUMERIC(2),  
 @CWHERE   NVARCHAR(MAX)='',  
 @CFINYEAR  VARCHAR(5)='',  
...
AS  
BEGIN  
DECLARE @CCMD NVARCHAR(MAX),@cHEAD_CODE VARCHAR(MAX) , @CCMD2 NVARCHAR(MAX), @CCMD1 NVARCHAR(MAX)
...
LBLPOLU:  
 EXECUTE SP_NAVIGATE 'POM01106',@NNAVMODE,@CWHERE,@CFINYEAR,'PO_NO','PO_DT','PO_ID',@CWHERECLAUSE  
...
LBLMST:  
   ...
   (C.ADDRESS1 +' '+ C.ADDRESS2) AS STORE_ADDRESS ,'' AS FORM_NAME,D.ALIAS,
    ...
    XN.*  ,B.MP_PERCENTAGE,B.WP_PERCENTAGE,OEM_MST.OEM_NAME,G.DEPT_NAME AS PO_FOR_DEPT_NAME,
     ...
     ELSE 0 END AS OTHER_CHARGES_TOTAL,PT.PO_TYPE,ISNULL(TNC.TERM_NAME,'') AS TERM_NAME
	 ,SHIP.AC_NAME AS SHIPPING_FROM_AC_NAME,SHIP.ADDRESS0+' '+ SHIP.ADDRESS1 + ' ' + SHIP.ADDRESS2 + ', ' + SHIP.AREA_NAME +   
	 ' ' + SHIP.CITY + ' ' + SHIP.STATE AS 'SHIPPING_FROM_ADDRESS',SHIP.Ac_gst_no AS SHIPPING_GST_NO,
	 FC.FC_NAME AS XN_FC_NAME,ISNULL(SHIP1.AC_NAME,'') AS SHIPPING_AC_NAME,
	 CAST(0 AS BIT) AS shipping_same_as_billing_Add,ISNULL(EMP.EMP_NAME,'') AS EMP_NAME,CAST(0 AS BIT) GST_PERCENTAGE_ADDED,B.MP_CALC_BASED_ON,B.MRP_CALC_MODE
   ...
   LEFT OUTER JOIN LMV01106 SHIP (NOLOCK) ON SHIP.AC_CODE = A.SHIPPING_FROM_AC_CODE
	...
	LEFT OUTER JOIN LM01106 SHIP1 (NOLOCK) ON A.SHIPPING_AC_CODE=SHIP1.AC_CODE
	LEFT OUTER JOIN EMPLOYEE EMP (NOLOCK) ON A.PO_EMP_CODE = EMP.EMP_CODE
...
LBLDET:  
	--- THIS PART IS COMMENTED BECAZ THIS QUERY IS MAKING THE NAVIGATION PART SLOW
	--- FOR EACH & EVERY MEMO...IT WILL BE OPTIMIZED LATER ON (DATE :02-03-2015 SANJAY)
	--IF OBJECT_ID('TEMPDB..#TMPPO','U') IS NOT NULL  
	--	DROP TABLE #TMPPO  
	  
	--SELECT PO_ROW_ID AS PI_PO_ROW_ID INTO #TMPPO  FROM PID01106 A  (NOLOCK)  
	--JOIN PIM01106 B  (NOLOCK) ON A.MRR_ID = B.MRR_ID   
	--JOIN POD01106 C  (NOLOCK) ON A.PO_ROW_ID = C.ROW_ID   
	--WHERE ISNULL(PO_ROW_ID,'') <> '' AND B.CANCELLED = 0  AND  C.PO_ID = @CWHERE   
	--GROUP BY PO_ROW_ID   
	--UNION   
	--SELECT PO_ROW_ID AS PI_PO_ROW_ID FROM PO_ADJ_DET A   (NOLOCK)  
	--JOIN PO_ADJ_MST B  (NOLOCK) ON A.MEMO_ID = B.MEMO_ID   
	--JOIN POD01106 C  (NOLOCK) ON A.PO_ROW_ID = C.ROW_ID   
	--WHERE ISNULL(PO_ROW_ID,'') <> '' AND C.PO_ID = @CWHERE   
	--GROUP BY PO_ROW_ID  
...
LBL20:  
 ...
 DECLARE @cpara1Name varchar(100),@cpara2Name varchar(100),@cpara3Name varchar(100),@cpara4Name varchar(100),@cpara5Name varchar(100),@cpara6Name varchar(100),    
   ...
   SELECT @cpara6Name=CASE WHEN ISNULL(VALUE,'')='' THEN 'PARA6_NAME' ELSE QUOTENAME(VALUE) END  FROM CONFIG WHERE CONFIG_OPTION ='PARA6_CAPTION'    
	...
	declare @CATTRNAME VARCHAR(MAX)
	select  @CATTRNAME =isnull(@CATTRNAME+',','') +column_name +' AS '+QUOTENAME (case when isnull(table_caption,'')<>'' 
	        then table_caption else  column_name end)
	from config_attr 


	/*
	/*Rohit 02-01-2025 : #1081 PO SE EXCEL EXPORT HONE PAR BARCODE ME ' ARAHA H*/
	''''''''+ B.PRODUCT_CODE AS PRODUCT_CODE
	*/

 SET @CCMD= N'SELECT  distinct B.SRNO, A.PO_NO,A.PO_DT,(CASE WHEN SUBSTRING (B.PRODUCT_CODE ,1,1)=''0'' THEN CHAR(9) + B.PRODUCT_CODE ELSE B.PRODUCT_CODE END) AS PRODUCT_CODE,B.VENDOR_EAN_NO,   F.SECTION_NAME,E.SUB_SECTION_NAME,D.ARTICLE_NO,D.ARTICLE_NAME, B.HSN_CODE,   P1.PARA1_NAME AS  '+ @cpara1Name + ',   P2.PARA2_NAME AS '+ @cpara2Name + ',   P3.PARA3_NAME AS  '+ @cpara3Name + ',   PARA4_NAME AS '+ @cpara4Name + ',                  
   P5.PARA5_NAME AS '+ @cpara5Name + ',   P6.PARA6_NAME AS  '+ @cpara6Name + ',    	D.CODING_SCHEME,    B.PURCHASE_PRICE,B.MRP,B.QUANTITY,D.ALIAS AS ARTICLE_ALIAS,B.ROW_ID,
    SP2.SIZEINCHES,SP2.SIZECMS,SP2.REMARKS,    LM.AC_NAME ,lm.ALIAS as SupplierAlias,  REPLACE(ISNULL(LM.ADDRESS0,''''),'','','':'') +'' ''+REPLACE(ISNULL(LM.ADDRESS1,''''),'','','':'') +'' ''+
    REPLACE(ISNULL(LM.ADDRESS2,''''),'','','':'') AS ADDRESS,    REPLACE(LM.AREA_NAME,'','','':'') AS AREA_NAME,    REPLACE(LM.CITY,'','','':'') AS CITY,    REPLACE(LM.STATE,'','','':'') AS STATE,
    REPLACE(LM.PINCODE,'','','':'') AS PINCODE,
	--AT1.attr1_key_name,AT2.attr2_key_name,AT3.attr3_key_name,AT4.attr4_key_name,AT5.attr5_key_name,AT6.attr6_key_name,  
	--AT7.attr7_key_name,AT8.attr8_key_name,AT9.attr9_key_name,AT10.attr10_key_name,AT11.attr11_key_name,AT12.attr12_key_name,    	AT13.attr13_key_name,AT14.attr14_key_name,AT15.attr15_key_name,AT16.attr16_key_name,AT17.attr17_key_name,AT18.attr18_key_name,    
	--AT19.attr19_key_name,AT20.attr20_key_name,AT21.attr21_key_name,AT22.attr22_key_name,AT23.attr23_key_name,AT24.attr24_key_name,    
	--AT25.attr25_key_name 
	'+@CATTRNAME+'
    FROM POM01106 A(NOLOCK)  
    JOIN POD01106 B(NOLOCK) ON B.PO_ID=A.PO_ID   
    LEFT OUTER JOIN PMT01106 C(NOLOCK) ON C.PRODUCT_CODE=B.PRODUCT_CODE   AND  C.DEPT_ID=  a.location_code
	...
	SET @CCMD2 = N' LEFT OUTER JOIN article_fix_attr ATTR  (NOLOCK) ON D.article_code = ATTR.ARTICLE_CODE     
	LEFT OUTER JOIN attr1_mst at1 (NOLOCK) ON at1.attr1_key_code=ATTR.attr1_key_code    
	LEFT OUTER JOIN attr2_mst at2 (NOLOCK) ON at2.attr2_key_code=ATTR.attr2_key_code    
	LEFT OUTER JOIN attr3_mst at3 (NOLOCK) ON at3.attr3_key_code=ATTR.attr3_key_code    
	LEFT OUTER JOIN attr4_mst at4 (NOLOCK) ON at4.attr4_key_code=ATTR.attr4_key_code    
	...
	order by B.SRNO,D.Article_no,P1.para1_name,P2.para2_name'    

 PRINT @CCMD  
 PRINT @CCMD2
 PRINT @CCMD1
 SET @CCMD= @CCMD + @CCMD2 +@CCMD1
 EXEC SP_EXECUTESQL @CCMD  
 GOTO LAST     
...
LBL21:  
 ...
 -- LEFT OUTER JOIN POD01106 BO (NOLOCK) ON D.ROW_ID = BO.WOD_ROW_ID 
  ...
  (
   ...
   LEFT OUTER JOIN 
  (
	   SELECT PRODUCT_CODE FROM  ORD_PLAN_BARCODE_DET BOC (NOLOCK)   
	   JOIN ORD_PLAN_DET JCD (NOLOCK) ON JCD.ROW_ID=BOC.REFROW_ID
	   LEFT OUTER JOIN ORD_PLAN_MST POM (NOLOCK) ON JCD.MEMO_ID = POM.MEMO_ID 
	   WHERE POM.CANCELLED= 0
  )JC  ON ISNULL(D.PRODUCT_CODE,'') = ISNULL(JC.PRODUCT_CODE,'') 
    
  WHERE   M.CANCELLED = 0 AND ISNULL(D.CANCELLED,0)=0 AND  M.APPROVED=1 AND D.ORDER_TYPE <> 1 AND BO.PRODUCT_CODE IS NULL AND JC.PRODUCT_CODE IS NULL
 ...
 --  LEFT OUTER JOIN POM01106 POM (NOLOCK) ON BO.PO_ID = POM.PO_ID AND POM.CANCELLED=0
  ...
  WHERE  M.CANCELLED = 0 AND M.APPROVED=1 AND ISNULL(D.CANCELLED,0)=0 AND ISNULL(D.PRODUCT_CODE,'')<>'' AND D.ORDER_TYPE <> 1 AND BO.PRODUCT_CODE IS NULL  AND JC.PRODUCT_CODE IS NULL
  GROUP BY M.ORDER_ID,M.ORDER_NO,M.REF_NO,M.ORDER_DT  
  ,M.TOTAL_AMOUNT,B.AC_NAME,CUST.CUSTOMER_FNAME,CUST.CUSTOMER_LNAME  
  ,M.DISCOUNT_PERCENTAGE,M.DISCOUNT_AMOUNT   ,ISNULL(M.URGENT,0) 
 END  
...
LBL27:
	  SELECT  B.BUY_PLAN_NAME,C.BUDGET_PLAN_NAME,SEASON_NAME,EXPIRY_DT,B.MEMO_NO,B.MEMO_DT, 
		  sum(A.QUANTITY) as QUANTITY,
          sum(A.QUANTITY*A.PURCHASE_PRICE) AS AMOUNT
         FROM BUY_PLAN_DET A(NOLOCK)
         JOIN BUY_PLAN_MST B (NOLOCK) ON A.MEMO_NO=B.MEMO_NO
         JOIN BUDGET_PLAN_MST C(NOLOCK) ON C.MEMO_NO=B.BUDGET_PLAN_MEMO_NO
         JOIN SEASON_MST D(NOLOCK) ON D.SEASON_ID=C.SEASON_ID
         JOIN 
		 (
			 SELECT BUY_PLAN_ROW_ID,SUM(QUANTITY) AS PO_QTY
			 FROM POD01106 E(NOLOCK)
			 WHERE PO_ID=@CWHERE
			 GROUP BY BUY_PLAN_ROW_ID
		  ) E  ON E.BUY_PLAN_ROW_ID=A.ROW_ID
     
		 group by  B.BUY_PLAN_NAME,C.BUDGET_PLAN_NAME,SEASON_NAME,EXPIRY_DT,B.MEMO_NO,B.MEMO_DT
		 
		 GOTO LAST		 
LBL30:
		SELECT CONVERT(BIT,0) AS CHK,A.PQ_ID AS ORDER_ID,C.AC_NAME AS CUSTOMER_NAME,1 AS TOTAL_QTY,0 AS TOTAL_AMOUNT,
		A.pq_dt AS ORDER_DT,A.pq_no  AS  ORDER_NO,'DIRECT'  AS MEMO_TYPE,REF_NO ,A.REMARKS
		FROM QUOTATION_MST A(NOLOCK) 
		--JOIN QUOTATION_DET B (NOLOCK) ON A.PQ_ID = B.PQ_ID
		JOIN LMV01106 C(NOLOCK) ON A.AC_CODE = C.AC_CODE
		WHERE A.CANCELLED = 0	AND ISNULL(A.Short_close,0)=0
		AND A.DELIVERY_DT>=@CWHERECLAUSE
		AND C.AC_CODE=@CWHERE
		ORDER BY ORDER_NO,ORDER_DT  
	GOTO LAST
...
LBL31:
		 ...
		 P5.PARA5_NAME,
		 P6.PARA6_NAME,
		 1 AS QUANTITY,0 AS DISCOUNT_AMOUNT ,0 AS ITEM_DISCOUNT_AMOUNT,A.PQ_ID AS ORDER_ID,A.ROW_ID,C.REMARKS,A.RATE_INR,A.RATE_FC,
		 A.TARGET_PRICE_FC,A.TARGET_PRICE_INR
		 ,(CASE WHEN ISNULL(A.alt_uom_conversion_factor,0)=0 THEN AT.alt_uom_conversion_factor ELSE A.alt_uom_conversion_factor END) AS alt_uom_conversion_factor
		 ,(CASE WHEN ISNULL(A.alt_uom_conversion_factor,0)=0 THEN AT.alt_uom_conversion_factor ELSE A.alt_uom_conversion_factor END) AS alternate_uom_quantity,
		 AT.alternate_uom_applicable,AT.alternate_uom_code,AT.conversion_factor_mode,B2.UOM_NAME AS alternate_uom_name
		 ,AT.HSN_CODE,AT.coding_scheme,cast(AT.para1_set as bit) as para1_set,cast(AT.para2_set as bit) as para2_set
		 FROM QUOTATION_DET A(NOLOCK)
		 JOIN QUOTATION_MST C(NOLOCK) ON A.pq_id = C.pq_id
		 JOIN ARTICLE AT(NOLOCK) ON AT.ARTICLE_CODE = A.ARTICLE_CODE
		 JOIN PARA1 P1(NOLOCK) ON P1.PARA1_CODE = A.PARA1_CODE
		 JOIN PARA2 P2(NOLOCK) ON P2.PARA2_CODE = A.PARA2_CODE
		 JOIN PARA3 P3(NOLOCK) ON P3.PARA3_CODE = AT.PARA3_CODE
		 ...
		 WHERE C.CANCELLED = 0 	AND ISNULL(C.Short_close,0)=0 AND C.DELIVERY_DT>=@CWHERECLAUSE
		 AND C.AC_CODE=@CWHERE
		 ORDER BY ORDER_ID
GOTO LAST
