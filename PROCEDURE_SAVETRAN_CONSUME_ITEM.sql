CREATE PROCEDURE SAVETRAN_CONSUME_ITEM
(
...
BEGIN
	...
	BEGIN TRY
		...
		SET @CTEMPDETAILTABLENAME	= 'TEMP_CON_CONSUMPTION_DET_'+LTRIM(RTRIM(STR(@NSPID)))					
		...
		BEGIN
			...
			SET @CCMD = N'UPDATE CON_CONSUMPTION_MST SET CANCELLED = 1,LAST_UPDATE=GETDATE() ' + 
		...
		SET @CMASTERTABLENAME	= 'CON_CONSUMPTION_MST'
		SET @CDETAILTABLENAME	= 'CON_CONSUMPTION_DET'

		SET @CTEMPMASTERTABLENAME	= 'TEMP_CON_CONSUMPTION_MST_'+LTRIM(RTRIM(STR(@NSPID)))
		...
		SET @CCMD = 'SELECT @CLOCID = location_code, @CKEYFIELDVAL1 = MEMO_ID FROM '+@CTEMPMASTERTABLE
	
		EXEC SP_EXECUTESQL @CCMD, N'@CLOCID VARCHAR(4) OUTPUT, @CKEYFIELDVAL1 VARCHAR(50) OUTPUT', 
								  @CLOCID OUTPUT, @CKEYFIELDVAL1 OUTPUT
		IF (@CLOCID IS NULL )
		BEGIN
		...
		BEGIN	
           ...
           PRINT 'GENERATING NEW KEY... START'			
			IF LTRIM(RTRIM(ISNULL(@CKEYFIELDVAL1,'LATER'))) = 'LATER'
			BEGIN
