create PROCEDURE SP3S_PENDINGORDER_FOR_JOBCARD
(
	@CWHERE NVARCHAR(MAX),
	@cLocID varchar(5)=''
)
...
BEGIN
	...
	BEGIN TRY
		...
		SET @CCMD=N'
		SELECT A.ORDER_ID,LMV01106.AC_NAME AS CUSTOMER_NAME,BUYER_ORDER_MST.ORDER_DT,BUYER_ORDER_MST.ORDER_NO,
		...
		IF (0= @@ROWCOUNT)
		BEGIN
			SELECT CONVERT(BIT,0) AS BCHK,ORDER_ID,CUSTOMER_NAME,CONVERT(VARCHAR(20),ORDER_DT,105) AS ORDER_DT,ORDER_NO,ROW_ID,ARTICLE_NO,ARTICLE_CODE,PARA1_CODE,PARA1_NAME,EMP_NAME,username FROM #PENDINGBUYERORDER
			RETURN
		END
		...
		SET @CCMD=N'
		SELECT ORDER_ID,CUSTOMER_NAME,ORDER_DT,ORDER_NO,ARTICLE_NO,ARTICLE_CODE,PARA1_CODE,PARA1_NAME,EMP_NAME,username,'+@CSIZE+
		...
		@CSIZE_SUM+','+@CSIZE_SUM_TOTAL +' AS TOTAL,CONVERT(INT,0) AS BEDIT,''ORD QTY'' AS QTY_MSG,CUSTOMER_NAME AS CUSTOMER_NAME_DISP, EMP_NAME,username
		FROM ##PENDINGORDER_PIVOT
		GROUP BY ORDER_ID,CUSTOMER_NAME,ORDER_DT,ORDER_NO,ARTICLE_NO,ARTICLE_CODE,PARA1_CODE,PARA1_NAME, EMP_NAME,username
		UNION
		...
		@CSIZE_SUM+','+@CSIZE_SUM_TOTAL +' AS TOTAL,CONVERT(INT,1) AS BEDIT,''JOB QTY'' AS QTY_MSG,CUSTOMER_NAME AS CUSTOMER_NAME_DISP,EMP_NAME,username
		FROM ##PENDINGORDER_PIVOT
		GROUP BY ORDER_ID,CUSTOMER_NAME,ARTICLE_CODE,PARA1_CODE, EMP_NAME,username
		UNION
		...
		@CSIZE_ZERO+',CAST(0 AS NUMERIC(14,2)) AS TOTAL,CONVERT(INT,-1) AS BEDIT,''TOTAL JOB QTY'' AS QTY_MSG,''ZZZZZZZZZZZZ'' AS CUSTOMER_NAME_DISP, EMP_NAME,username
		FROM ##PENDINGORDER_PIVOT
		GROUP BY ORDER_ID,CUSTOMER_NAME,ARTICLE_CODE,PARA1_CODE, EMP_NAME,username
