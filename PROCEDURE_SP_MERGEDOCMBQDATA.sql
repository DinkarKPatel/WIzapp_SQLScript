CREATE PROCEDURE SP_MERGEDOCMBQDATA
	@CMEMOID VARCHAR(40)
--	WITH ENCRYPTION
AS
BEGIN
	BEGIN TRY	
		...
		DECLARE @CCURDEPTID CHAR(2),@CHODEPTID CHAR(2),@CCMD NVARCHAR(MAX),
				...
				@BLOOP BIT,@CMEMONOPREFIX VARCHAR(10),@NUPDATEVALUE INT,@CMRRID VARCHAR(40),@CCMDOUTPUT VARCHAR(MAX),
				@CERRORMSG VARCHAR(MAX),@BENFORCEAUTOSERIES BIT,@CTARGETLOCID CHAR(2),
		...
		SET @CMASTERKEYFIELD='DEPT_ID'
		
		SET @NSTEP=17
		
		SET @CCMD=N'IF OBJECT_ID(''TMP_DOCMBQ_MBQ_DET_'+LTRIM(RTRIM(@CMEMOID))+''',''U'') IS NOT NULL  
		...
		BEGIN  
		   ...
		   SET @CERRORMSG='NO MBQ  DATA FOUND TO BE MERGED'  
		   GOTO LBLLAST  
		END  
...
LBLDELETE:		
		...
		SET @NSTEP=110
		SET @CCMD=N'DELETE A FROM MBQ_DET A JOIN '+@CTEMPMASTERTABLENAME+' B ON B.DEPT_ID=A.DEPT_ID'
		PRINT @CCMD
...
LBLMERGE:
		...
		BEGIN
			...
			SET @CSOURCETEMPTABLE='TMP_DOCMBQ_'+@CTABLENAME+'_'+LTRIM(RTRIM(@CMEMOID))
...
LBLLAST:
	...
	IF @@TRANCOUNT>0
	BEGIN
		IF ISNULL(@CERRORMSG,'')='' OR @BGENSKUREQUESTFROMHO=1  
			COMMIT TRANSACTION
		ELSE
			ROLLBACK 	
	END	
	...
	PRINT 'TRAPPED ERROR:'+ISNULL(@CERRORMSG,'')
       
    SELECT * FROM @TRETMSG    
	
	EXEC SP_DROPTEMPTABLES_XNS 'DOCMBQ',0,@CMEMOID,'TMP_DOCMBQ'		
END
--- END OF CREATING PROCEDURE SP_MERGEDOCMBQDATA
