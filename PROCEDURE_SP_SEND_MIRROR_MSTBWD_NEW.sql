CREATE PROCEDURE DBO.SP_SEND_MIRROR_MSTBWD_NEW--(LocId 3 digit change by Sanjay:06-11-2024)
(
...
AS
 BEGIN
   DECLARE @DTSQL NVARCHAR(MAX),@CSTEP VARCHAR(10),@CFILTERCONDITION NVARCHAR(MAX)
	...
	SET @CTABLENAME = 'BWD_MST'
	SELECT DISTINCT  'MSTBWD_BWD_MST_MIRROR' AS TARGET_TABLENAME,BWD_MST.*,1 AS PICK_FROM_CURRENT_DB FROM BWD_MST (NOLOCK)

		
	--INSERT DATA FROM MAIN TABLE TO TEMP TABLE
    SET @CSTEP=40
    ---SELECT DATA FROM LOC_BILLING_RULES TABLE
	SET @CTABLENAME = 'BWD_DET'
	SELECT DISTINCT  'MSTBWD_BWD_DET_MIRROR' AS TARGET_TABLENAME,BWD_DET.*,1 AS PICK_FROM_CURRENT_DB FROM BWD_DET (NOLOCK)
	
	END TRY
	
	BEGIN CATCH
	    SET @CERRORMSG='P: SP_SEND_MIRROR_MSTBWD_NEW, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()+ 'ERROR QUERY || '+@DTSQL
	    GOTO END_PROC
    END CATCH 
