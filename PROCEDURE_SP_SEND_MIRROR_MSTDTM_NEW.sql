CREATE PROCEDURE DBO.SP_SEND_MIRROR_MSTDTM_NEW
(
...
AS
 BEGIN
   DECLARE @DTSQL NVARCHAR(MAX),@CSTEP VARCHAR(10),@CFILTERCONDITION NVARCHAR(MAX)
	...
	SET @CTABLENAME = 'DTM'
	SELECT DISTINCT  'MSTDTM_DTM_MIRROR' AS TARGET_TABLENAME,DTM.*,1 AS PICK_FROM_CURRENT_DB FROM DTM (NOLOCK)
    
	END TRY
	
	BEGIN CATCH
	    SET @CERRORMSG='P: SP_SEND_MIRROR_MSTDTM_NEW, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()+ 'ERROR QUERY || '+@DTSQL
	    GOTO END_PROC
    END CATCH 
