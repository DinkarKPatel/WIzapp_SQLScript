CREATE PROCEDURE SAVETRAN_PRD_READYRECEIVE    
(  
...
BEGIN  
 ...
 BEGIN TRY  
  ...
  SET @CTEMPDETAILTABLENAME = 'TEMP_PRD_READY_RECEIVE_DET_'+LTRIM(RTRIM(STR(@NSPID)))       
  ...
  BEGIN  
   ...
   SET @CCMD = N'UPDATE PRD_READY_RECEIVE_MST SET CANCELLED = 1,LAST_UPDATE=GETDATE() ' +   
  ...
  SET @CMASTERTABLENAME = 'PRD_READY_RECEIVE_MST'  
  SET @CDETAILTABLENAME = 'PRD_READY_RECEIVE_DET'  
  
  SET @CTEMPMASTERTABLENAME = 'TEMP_PRD_READY_RECEIVE_MST_'+LTRIM(RTRIM(STR(@NSPID)))  
  ...
  EXEC SAVETRAN_UPDSKU_PRD 'RMR',@CKEYFIELDVAL1,@CERRORMSG OUTPUT   
  IF ISNULL(@CERRORMSG,'')<>''  
    GOTO END_PROC  
