CREATE PROCEDURE SP_RPT_APPROVAL_REPORT_CAL
(
  @DFM_DT DATETIME,
  @DTO_DT DATETIME,
...
BEGIN
	...
	FROM 
	(
	 SELECT A.MEMO_DT AS ISSUE_DT,A.AC_CODE,A.CUSTOMER_CODE, A.DEPT_ID, 'APP' AS XN_TYPE,A.MEMO_ID AS CM_ID ,B.PRODUCT_CODE,B.EMP_CODE ,B.EMP_CODE1,B.EMP_CODE2,
	 ...
	 UNION ALL
	 SELECT D.MEMO_DT AS ISSUE_DT,D.AC_CODE,D.CUSTOMER_CODE, A.DEPT_ID,'APR' AS XN_TYPE,A.MEMO_ID AS CM_ID  ,C.PRODUCT_CODE,B.EMP_CODE,C.EMP_CODE1,C.EMP_CODE2,
	 ...
	 JOIN APD01106 C (NOLOCK) ON B.APD_ROW_ID = C.ROW_ID 
	 JOIN APM01106 D ON C.MEMO_ID = D.MEMO_ID 
	 WHERE A.CANCELLED = 0  AND A.MEMO_DT BETWEEN @DFM_DT AND @DTO_DT  
	 AND (@CAC_CODE='' OR (D.AC_CODE=@CAC_CODE OR D.CUSTOMER_CODE=@CAC_CODE))
	) A

	JOIN SKU SKU (NOLOCK) ON A.PRODUCT_CODE = SKU.PRODUCT_CODE 
