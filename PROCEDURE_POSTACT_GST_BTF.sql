CREATE PROCEDURE POSTACT_GST_BTF 
(
	@DTTO DATETIME,
	@CDEPTID CHAR(4)='',
	@NLOOP NUMERIC(1,0)
...
BEGIN
    DECLARE @CSTEP VARCHAR(10),@BPOSTDISCOUNTSEPARATELY BIT,@CDISCACCODE VARCHAR(10),@BPOSTFREIGHTSEPARATELY	BIT
			...
			@BBLANKACFOUND BIT,@CERRORMSG VARCHAR(MAX)			
	SET @CSTEP=10
	...
	SELECT TOP 1 @CCUTOFFDATE=CUTOFFDATE FROM GST_ACCOUNTS_CONFIG_MST where xn_type='BTF'
    ...
    BEGIN TRY
			...
			, A.MEMO_ID
			, A.MEMO_DT
			,(CASE WHEN A.TRANSFERMODE=1 OR tm.TILL_ID IS NULL THEN LCK.AC_CODE ELSE  ISNULL(TM.AC_CODE,'0000000000') END) AS SOURCEACCODE
			, C.AC_CODE
			...
			LEFT OUTER JOIN TILL_MST TM ON TM.TILL_ID=TSM.TILL_ID
			LEFT OUTER JOIN TILL_MST LCK ON LCK.TILL_ID='0000000000'
			JOIN LM01106 C ON A.AC_CODE = C.AC_CODE
		
		
		CREATE INDEX IX_V_PROCESS_RM_ID ON #V_PROCESS(MEMO_ID)

		SELECT @CCASHACCODE=VALUE  FROM GST_ACCOUNTS_CONFIG_DET_OTHERS WHERE XN_TYPE ='SLS' AND COLUMNNAME ='CASH_AC_CODE'
		...
		BEGIN
			...
			IF ISNULL(@CVMID,'') = ''
			BEGIN
			    SET @CVMID = 'LATERBTF-'+RTRIM(LTRIM(CONVERT(VARCHAR,@NCTR)))
				SET @NCTR = @NCTR+1
			END
			...
			SELECT @CVMID
				,'LATERBTF-'+CONVERT(VARCHAR,ROW_NUMBER() OVER(ORDER BY A.AC_CODE)) AS VD_ID
				, A.AC_CODE
			...
			SELECT TOP 1 @CVMID,MEMO_DT,@CVOUCHERCODE,dept_id
...
ENDPROC:
   ...
   set @nSpId=ltrim(rtrim(str(@@spid)))  
	...
	EXEC SP_CHKXNSAVELOG 'BTFPOST',@cStep,1,@nSpId,'',1	 	
	...
	SET @CSTEP = 574
	EXEC SP_CHKXNSAVELOG 'BTFPOST',@cStep,1,@nSpId,'',1	 		
	PRINT 'START POSTING BTF:'+@CSTEP+' '+CONVERT(VARCHAR,GETDATE(),113)
...
BEGIN CATCH    
   SET @CERRORMSG=' ERROR IN POSTACT_GST_BTF  AT STEP#'+@CSTEP+' '+ERROR_MESSAGE()
   GOTO END_PROC
END CATCH    
