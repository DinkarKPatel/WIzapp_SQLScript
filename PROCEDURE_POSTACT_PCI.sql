...
BEGIN
    ...
    BEGIN TRY
	        ...
	        (
				...
				WHERE C.CANCELLED=0 AND B.XN_TYPE = 'PCI' 
			...
			BEGIN
				INSERT @ERRORS ( XN_TYPE, ERR_DESC )
						VALUES ( 'PCI', 'PLEASE CONFIGURE PETTY CASH A/C.' )
				GOTO ENDPROC			
			END
		
			SET @CSTEP = 330
			CREATE INDEX IX_TEMP_PCITABLE_RM_ID ON #POSTS(MEMO_ID)
			...
			SELECT DISTINCT PM.MEMO_ID,PM.MEMO_NO,'PCI' AS XN_TYPE
				  ,(CASE 
						 WHEN ISNULL(LCT.DEPT_AC_CODE,'0000000000') IN ('','0000000000') 
								THEN 'SOURCE LOCATION A/C NOT SPECIFIED.'
						 WHEN PM.AMOUNT=0
								THEN 'ZERO VALUE PETTY CASH IN.'
						 ELSE 'UNIDENTIFIED ERROR.' END) AS ERR_DESC		
			FROM PCI_MST PM
			JOIN #POSTS TPT ON PM.MEMO_ID=TPT.MEMO_ID AND TPT.MODE='PCI'
			JOIN LOCATION LCT ON LCT.DEPT_ID=pm.location_Code 
			WHERE 
			...
			FROM PCI_MST PM 
			JOIN LOCATION LCT ON LCT.DEPT_ID=pm.location_Code 
			JOIN #POSTS PT ON PT.MEMO_ID = PM.MEMO_ID AND PT.MODE='PCI'
		...
		BEGIN
			SET @CSTEP = 390
			SELECT TOP 1 @CMEMOID=MEMO_ID,@CXNTYPE=XN_TYPE,@CAC_CODE=AC_CODE FROM #V_PROCESS
			
			SET @CSTEP = 400
			--SORUCE LOCATION ACCOUNT SHOULD BE CREDITED WITH AMOUNT 
			INSERT @VCHC ( AC_CODE, AMOUNT,NARRATION,XN_NO,XN_ID )
			SELECT AC_CODE,-XN_AMOUNT,NARRATION,MEMO_NO,MEMO_ID
			FROM #V_PROCESS
			WHERE MEMO_ID=@CMEMOID
			
			--PETTY CASH A/C SHOULD BE DEBITED WITH THE AMOUNT
			INSERT @VCHC ( AC_CODE, AMOUNT,NARRATION,XN_NO,XN_ID )
			SELECT @CPETTYCASHACCODE,XN_AMOUNT
			...
			SELECT @CVMID,@CMEMOID,@CXNTYPE,LAST_UPDATE FROM PCI_MST 
			WHERE MEMO_ID = @CMEMOID
