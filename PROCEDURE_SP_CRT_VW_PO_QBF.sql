CREATE PROCEDURE SP_CRT_VW_PO_QBF
AS
BEGIN

IF OBJECT_ID('VW_PO_QBF','V') IS NOT NULL
	   DROP VIEW VW_PO_QBF
	
	DECLARE @VW_DEF VARCHAR(MAX),@ATTR_VIEW_COL VARCHAR(MAX)=''
		  
	--SELECT @ATTR_VIEW_COL=COALESCE(@ATTR_VIEW_COL,'')+'AV.['+COLUMN_NAME+'],' 
	--FROM INFORMATION_SCHEMA.COLUMNS 
	
	--SET @ATTR_VIEW_COL=ISNULL(@ATTR_VIEW_COL,'')
	  
	--IF LEN(@ATTR_VIEW_COL)>1
	--   BEGIN
	--  	 SET @ATTR_VIEW_COL=LTRIM(RTRIM(@ATTR_VIEW_COL))
	--     SET @ATTR_VIEW_COL=','+LEFT(@ATTR_VIEW_COL,LEN(@ATTR_VIEW_COL)-1)
	--   END
	   
	SET @VW_DEF='CREATE VIEW VW_PO_QBF AS        
		...
		LCT.DEPT_ID AS LOCATION_DEPT_ID,LCT.DEPT_NAME AS LOCATION_DEPT_NAME,
		...
		AND MBQ.DEPT_ID = SUBSTRING(A.PO_ID,0,LEN(A.LOCATION_CODE)+1)
  ...
  SET NOCOUNT OFF
END		
--************************ END OF PROCEDURE SP_CRT_VW_PO_QBF
