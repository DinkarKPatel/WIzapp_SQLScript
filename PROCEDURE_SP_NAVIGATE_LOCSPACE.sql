CREATE PROCEDURE SP_NAVIGATE_LOCSPACE 
...
BEGIN
	...
	SET @CCMD=N'SELECT @DMEMODTOUT='+@CMEMODTCOL+' FROM '+@CTABLENAME+' WHERE   '+@CMEMONOCOL+'='''+@CMEMONO+''''
				 PRINT @CCMD
	EXEC SP_EXECUTESQL @CCMD,N'@DMEMODTOUT DATETIME OUTPUT',@DMEMODTOUT=@DMEMODT OUTPUT
	
	SET @CCMD = N'SELECT TOP 1 '+@CMEMONOCOL+','+@CMEMOIDCOL+' FROM '+@CTABLENAME+' WHERE 1=1 '+
	...
	IF NOT EXISTS (SELECT TOP 1 MEMO_NO FROM @TNAVIGATE)
	BEGIN
		SET @CCMD=N'SELECT '+@CMEMONOCOL+','+@CMEMOIDCOL+' FROM '+@CTABLENAME+' WHERE  
					 '+@CMEMONOCOL+'='''+@CMEMONO+''''
		PRINT @CCMD	
