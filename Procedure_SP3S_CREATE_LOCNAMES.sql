CREATE PROCEDURE SP3S_CREATE_LOCNAMES--(LocId 3 digit change only increased the parameter width by Sanjay:04-11-2024)
(
 @CLOCid VARCHAR(4)=''
)
AS
BEGIN
	 ...
	 IF @CLOCid=''
	   TRUNCATE TABLE LOC_NAMES
	 ELSE
	   DELETE  A FROM LOC_NAMES A (NOLOCK) WHERE A.Dept_ID =@CLOCid
	 ...
	 INSERT LOC_NAMES	(Dept_ID,AREA_NAME,PINCODE,CITY,STATE,OCTROI_PERCENTAGE,REGION_NAME,dept_name,major_dept_id,ro_dept_id,group_id,pc_ho_loc_id,loc_type,loc_discount_percentage,
						...
						Locattr20_key_name,Locattr21_key_name,Locattr22_key_name,Locattr23_key_name,Locattr24_key_name,Locattr25_key_name,loc_area_code,EINVOICE_CUTOFF_DATE,LEAD_DAYS_FOR_ALTERATION,
                        ENABLE_EINVOICE_IN_RETAIL_SALE,DO_NOT_ENFORCE_LAST_RUN_DT,ENFORCE_YEAR_CODE_RETAIL_SALE 
                        ) 
	  	   ...
	  	   SELECT       a.Dept_ID,B.AREA_NAME,B.PINCODE,C.CITY,D.STATE,OCTROI_PERCENTAGE,E.REGION_NAME,A.dept_name,A.major_dept_id,A.ro_dept_id,A.group_id,A.pc_ho_loc_id,A.loc_type,A.loc_discount_percentage,
						...
						isnull(attr11_key_name,''),isnull(attr12_key_name,''),isnull(attr13_key_name,''),isnull(attr14_key_name,''),isnull(attr15_key_name,''),
						isnull(attr16_key_name,''),isnull(attr17_key_name,''),isnull(attr18_key_name,''),isnull(attr19_key_name,''),isnull(attr20_key_name,''),
						isnull(attr21_key_name,''),isnull(attr22_key_name,''),isnull(attr23_key_name,''),isnull(attr24_key_name,''),isnull(attr25_key_name,''),
						b.area_code ,a.EINVOICE_CUTOFF_DATE,a.LEAD_DAYS_FOR_ALTERATION,
                        a.ENABLE_EINVOICE_IN_RETAIL_SALE,a.DO_NOT_ENFORCE_LAST_RUN_DT,a.ENFORCE_YEAR_CODE_RETAIL_SALE 
		...
		WHERE (A.dept_id =@CLOCid OR @CLOCid='')
END