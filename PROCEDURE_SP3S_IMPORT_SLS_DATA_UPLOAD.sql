CREATE PROCEDURE SP3S_IMPORT_SLS_DATA_UPLOAD--_SLS_0000000
(    
 @iDateMode		INT,
 @CLOCID VARCHAR(10),          
 @NMODE INT=0      ,    
...
,cgst_amount NUMERIC(14,2)    
 ...
 ,EMAIL   VARCHAR(MAX)     
	...
	UPDATE A SET A.errormsg=ISNULL(A.errormsg,'')+CHAR(13)+ 'Quanity should not be zero.'    
	FROM SLS_IMPORT_DATA A(nolock)     
	WHERE A.SP_ID=@CSPID AND ISNULL(A.quantity,0)=0    
    
    
	UPDATE A SET A.errormsg=ISNULL(A.errormsg,'')+CHAR(13)+(CASE WHEN B.PRODUCt_CODE IS NULL  THEN 'Product Code not found.' ELSE '' END)    
	FROM SLS_IMPORT_DATA A(nolock)     
	LEFT JOIN SKU  B(nolock) On A.PRODUCT_CODE=B.PRODUCT_CODE    
	WHERE A.SP_ID=@CSPID AND B.PRODUCt_CODE IS NULL    
    
	UPDATE A SET A.errormsg=ISNULL(A.errormsg,'')+CHAR(13)+(CASE WHEN B.HSN_CODE IS NULL  THEN 'HSN Code not found.' ELSE '' END)    
	FROM SLS_IMPORT_DATA A(nolock)     
 ...
 END    
  ...
  --UPDATE SLS_IMPORT_DATA SET MEMO_DT=CONVERT(VARCHAR(20),GETDATE(),111) WHERE ISNULL(MEMO_DT,'')=''         
	...
	*/
	...
	sisloc_eoss_discount_percentage,sisloc_eoss_discount_amount,sisloc_mrp )            
    EXEC SP_EXECUTESQL @CCMD          
...
--SELECT * FROM CMM01106(NOLOCK) WHERE CM_ID IN (SELECT CM_ID FROM #NEWBILLS)           
  ...
  */          
  INSERT SLS_CMD01106_UPLOAD (SP_ID, PRODUCT_CODE, QUANTITY, MRP, NET, BASIC_DISCOUNT_PERCENTAGE, DISCOUNT_PERCENTAGE,BASIC_DISCOUNT_AMOUNT, DISCOUNT_AMOUNT, ROW_ID, LAST_UPDATE          
   ...
   , NRM_ID ,HSN_CODE   ,gst_percentage ,igst_amount ,cgst_amount,sgst_amount ,NET_SALE,pack_slip_row_id,SIS_NET,sisloc_eoss_discount_percentage,sisloc_eoss_discount_amount,sisloc_mrp)            
 ...
 PRINT 'PROCESSING STEP #'+@CSTEP+'#'+CONVERT(VARCHAR,GETDATE(),113)          
  ...
  EXEC SP_EXECUTESQL @CCMD          
    ...
    ,LEFT(A.CM_ID,2)+CONVERT(VARCHAR(40),NEWID()) AS ROW_ID,NET_AMOUNT AS AMOUNT,GETDATE() AS LAST_UPDATE,          
  ...
  BEGIN      
   ...
   ROW_ID,QUANTITY,LEFT(B.CM_ID,2) AS PARTY_DEPT_ID,LEFT(B.CM_ID,2) AS  DEPT_ID,A.MRP ,      
   B.CM_DT ,      
   SKU.HSN_CODE       
   FROM CMD01106  A (NOLOCK)      
   JOIN CMM01106  B (NOLOCK) ON A.CM_ID =B.CM_ID       
   ...
   JOIN LOCATION   B ON LEFT(A.CM_ID,2) =B.DEPT_ID        
   WHERE CM_ID =@CCMID       
   AND  A.PARTY_STATE_CODE IN('','00')      
   ...
   JOIN LOCATION L ON L.DEPT_ID =LEFT(A.CM_ID,2)      
   WHERE CM_ID =@CCMID       
   SET @CSTEP=690      
   IF ISNULL(@NOC,0)<>0      
 ...
 @cLocationId=@CLOCID ,    
  ...
  EXEC  UPDATERFNET 'SLS',@CCMID      
   ...
   SELECT @CERRMSG='Mismatch Between Bill Amount('+CONVERT(VARCHAR(20),NET_AMOUNT) +') and  Calculated Bill Amount ('+CONVERT(VARCHAR(20),((SUBTOTAL+subtotal_r+round_off+atd_charges)-DISCOUNT_AMOUNT))+')'    
   FROM CMM01106     
   WHERE cm_id=@CCMID    
   AND     
   (ABS((SUBTOTAL+subtotal_r+round_off+atd_charges)-DISCOUNT_AMOUNT)-ABS(NET_AMOUNT)) >1    
  ...
  GROUP BY B.PRODUCT_CODE,A.DEPT_ID ,B.BIN_ID     
   ...
   JOIN    
   (    
      SELECT A.PRODUCT_CODE,A.DEPT_ID ,A.BIN_ID,SUM(STOCK_QTY) AS STOCK_QTY    
   FROM #TMPSTOCK A    
...
BEGIN CATCH         
  ...
  SET @CERRMSG='SP3S_SYNCH_MBO_SLS_DATA_UPLOAD  : AT STEP - '+@CSTEP+', MESSAGE - '+ERROR_MESSAGE()    
  PRINT       @CERRMSG    
  GOTO END_PROC           
END CATCH          
...
END_PROC:          
 ...
 ELSE          
    SELECT PRODUCT_CODE, MEMO_NO AS REF_NO,@CLOCID AS  DEPT_ID, ISNULL(@CERRMSG,'') ERRMSG FROM SLS_IMPORT_DATA  WHERE SP_ID=@CSPID        
END



/*
CREATE PROCEDURE SP3S_IMPORT_SLS_DATA_UPLOAD    
(    
 @iDateMode		INT,
 @CLOCID VARCHAR(10),          
 @NMODE INT=0      ,    
...
BEGIN          
  ...
  @CAPPLYSALESSETUP VARCHAR(2),@CBINSALES VARCHAR(10),@CGENVENDOREANCODES VARCHAR(2),@NIMPORTMODE INT,          
  @BSTOCKNOTFOUND BIT,@BLOOP BIT,@CEANNO VARCHAR(50),@CROWID VARCHAR(40),@NPENDINGQTY NUMERIC(10,2),@CBILLNO VARCHAR(50),          
  @CPRODUCTCODE VARCHAR(50),@CFINYEAR VARCHAR(10),@NQTY NUMERIC(10,2),@CAPPLYMINDISCOUNT VARCHAR(2)    ,@CheckEffectiveSaleSetup BIT       
...
FROM #SLSIMPORT_CUSTDYM WHERE ISNULL(USER_CUSTOMER_CODE,'')<>''    
 ...
 END    
  ...
  --UPDATE SLS_IMPORT_DATA SET MEMO_DT=CONVERT(VARCHAR(20),GETDATE(),111) WHERE ISNULL(MEMO_DT,'')=''         
    ...
    SET @CCMD=N'SELECT 0 AS SR_NO,MEMO_NO AS BILL_NO,MEMO_DT AS CM_DT,DEPT_ID,A.PRODUCT_CODE,QUANTITY,A.NET AS AMOUNT,          
    CAST(ISNULL(CANCELLED,0) AS BIT) AS CANCELLED,'''' AS ERROR_MESSAGE,ISNULL(A.MRP,B.MRP) AS MRP,
	ISNULL(A.CMD_DISCOUNT_AMOUNT,0) AS DISCOUNT_AMT,A.NET CASH,0 CC_AMOUNT,'''' CC_NAME ,A.CMM_DISCOUNT_AMOUNT,A.CMM_OTHER_CHARGES,
	ISNULL(A.HSN_CODE  ,B.HSN_CODE) AS HSN_CODE ,A.gst_percentage ,A.igst_amount ,A.cgst_amount,A.sgst_amount     
	,A.NET,ISNULL(A.NET_AMOUNT,A.NET) AS NET_AMOUNT,A.NET AS NET_SALE,ISNULL(A.REMARKS,'''') AS REMARKS,ISNULL(cus_gst_state,'''') AS party_state_code
    FROM SLS_IMPORT_DATA A  (NOLOCK)         
    ...
    SET @CSTEP=25          
    INSERT #SLSIMPORT (SR_NO,BILL_NO,CM_DT,DEPT_ID,PRODUCT_CODE,QUANTITY,AMOUNT,CANCELLED,ERROR_MESSAGE          
    ,MRP,DISCOUNT_AMT,CASH,CC_AMOUNT,CC_NAME,CMM_DISCOUNT_AMOUNT,CMM_OTHER_CHARGES,    
 HSN_CODE   ,gst_percentage ,igst_amount ,cgst_amount,sgst_amount,NET,NET_AMOUNT,NET_SALE,REMARKS,party_state_code )            
    EXEC SP_EXECUTESQL @CCMD          
            --SELECT * FROM #SLSIMPORT
    SET @CSTEP=30    
          UPDATE #SLSIMPORT SET CM_DT=CONVERT(VARCHAR(20),GETDATE(),111) WHERE ISNULL(CM_DT,'')=''         
...
--return     
 ...
 -- SELECT * FROM #SLSIMPORT           
  ...
  INTO #EXISTINGBILLS          
  FROM #SLSIMPORT A          
  JOIN CMM01106 B(NOLOCK) ON A.DEPT_ID+'-'+ISNULL(A.BILL_NO,'')=B.REF_NO AND A.FIN_YEAR=B.FIN_YEAR          
  LEFT OUTER JOIN gst_state_mst GST (NOLOCK) ON GST.gst_state_name=A.party_state_code
  WHERE B.CANCELLED=0
 ...
 UPDATE A SET PARTY_STATE_CODE =CASE WHEN ISNULL(B.GST_STATE_CODE   ,'') ='' THEN '00' ELSE B.GST_STATE_CODE END       
  ...
  INTO #NEWBILLS          
  FROM #SLSIMPORT A          
  LEFT JOIN CMM01106 B(NOLOCK) ON B.CANCELLED=0 AND A.DEPT_ID+'-'+ISNULL(A.BILL_NO,'')=B.REF_NO AND A.FIN_YEAR=B.FIN_YEAR 
  LEFT OUTER JOIN gst_state_mst GST (NOLOCK) ON GST.gst_state_name=A.party_state_code
  WHERE B.CM_ID IS NULL         
...
--SELECT * FROM CMM01106(NOLOCK) WHERE CM_ID IN (SELECT CM_ID FROM #NEWBILLS)           
  ...
  */          
  INSERT SLS_CMD01106_UPLOAD (SP_ID, PRODUCT_CODE, QUANTITY, MRP, NET, BASIC_DISCOUNT_PERCENTAGE, DISCOUNT_PERCENTAGE,BASIC_DISCOUNT_AMOUNT, DISCOUNT_AMOUNT, ROW_ID, LAST_UPDATE          
   ...
   , WEIGHTED_AVG_DISC_PCT, WEIGHTED_AVG_DISC_AMT, MANUAL_DISCOUNT, FIX_MRP, SR_NO, HOLD_FOR_ALTER          
   , PACK_SLIP_ID, XN_TYPE, REPEAT_PUR_ORDER, DEPT_ID, REF_ORDER_ID, FOC_QUANTITY, CMM_DISCOUNT_AMOUNT          
   , NRM_ID ,HSN_CODE   ,gst_percentage ,igst_amount ,cgst_amount,sgst_amount ,NET_SALE,pack_slip_row_id,SIS_NET)            
  SELECT  @CSPID,  A.PRODUCT_CODE, A.QUANTITY, A.MRP,A.NET AS NET,    
  --(CASE WHEN ABS(A.MRP)<ABS(A.QUANTITY*B.MRP) THEN           
  --  ((ABS(A.QUANTITY*B.MRP)-ABS(A.MRP))/ABS(A.QUANTITY*A.MRP))*100           
  --  ELSE 0 END) AS DISCOUNT_PERCENTAGE          
  ...
  ,(CASE WHEN ISNULL(DISCOUNT_AMT,0)= 0 THEN ((A.MRP*A.QUANTITY)-A.NET) ELSE ISNULL(DISCOUNT_AMT,0) END) AS DISCOUNT_AMOUNT    
    ...
    ,0 AS REPEAT_PUR_ORDER,A.DEPT_ID,'' AS REF_ORDER_ID          
    ,0 AS FOC_QUANTITY,0 AS CMM_DISCOUNT_AMOUNT,'' AS NRM_ID,A.HSN_CODE   ,A.gst_percentage ,A.igst_amount ,A.cgst_amount,A.sgst_amount       
	,A.NET AS NET_SALE,'',A.NET AS SIS_NET
  FROM #EXISTINGBILLS A          
  JOIN SKU B ON A.PRODUCT_CODE=B.PRODUCT_CODE          
  UNION           
  SELECT  @CSPID,  A.PRODUCT_CODE, A.QUANTITY, A.MRP,A.NET AS NET          
