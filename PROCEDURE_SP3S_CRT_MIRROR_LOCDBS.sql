CREATE PROCEDURE SP3S_CRT_MIRROR_LOCDBS
(
	@CDEPT_ID VARCHAR(3)='' 
)
--WITH ENCRYPTION
...
BEGIN
	...
	*/
DECLARE @CCUR_DEPT_ID VARCHAR(3),@CHO_DEPT_ID VARCHAR(3),@CMAJOR_DEPT_ID VARCHAR(3),@CERRMSG VARCHAR(1000)
...
BEGIN
	SET @CERRMSG='P:SP3S_CRT_MIRROR_LOCDBS,MESSAGE: THIS PROCEDURE CAN ONLY BE EXECUTED AT HEAD OFFICE....'
	GOTO EXIT_PROC
END 
...
CREATE TABLE #CRT_DBS(DBNAME VARCHAR(200))
	...
	BEGIN
		...
		BEGIN
			...
			SELECT '''+@CTARGETDBNAME+''' AS DBNAME
		END'
		PRINT @DTSQL
...
BEGIN
	...
	SET @DTSQL=N'INSERT '+@CCRT_DB+'.DBO.EXE_TIME (LAST_UPDATE)
				 SELECT LAST_UPDATE FROM DBO.EXE_TIME'	
	PRINT @DTSQL
	EXEC SP_EXECUTESQL @DTSQL	
...
END TRY
BEGIN CATCH
	SET @CERRMSG='P:SP3S_CRT_MIRROR_LOCDBS, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
END CATCH
...
END	
---END OF PROCEDURE - SP3S_CRT_MIRROR_LOCDBS
