create PROCEDURE SPSIS_SEND_MIRROR_SLS_DATA
(
...
LBLTABLEINFO:
	...
	SET @CSTEP=60
	SELECT @cCols=COALESCE(@cCols+',','')+colname 	FROM ebo_xnssending_cols WHERE tablename='CMM01106' 

	SET @cCmdCols=N'
	   SELECT ''SLS_CMM01106_upload'' AS TARGET_TABLENAME,'+@cCols+' FROM CMM01106  (NOLOCK)   WHERE CM_ID='''+@CMEMOID+''''
	PRINT @cCmdCols
	...
	SET @CSTEP=65	
	SET @cCols=NULL
	SELECT @cCols=COALESCE(@cCols+',','')+'CMD01106.'+colname FROM ebo_xnssending_cols	WHERE tablename='CMD01106'
	
	SET @cCOls=REPLACE(@cCOls,'CMD01106.PRODUCT_CODE','CASE WHEN ISNULL(C.VENDOR_EAN_NO,'''')<>'''' THEN C.VENDOR_EAN_NO ELSE CMD01106.PRODUCT_CODE END AS PRODUCT_CODE')


	PRINT @cCOls
	SET @cCmdCols=N'
		SELECT ''SLS_CMD01106_upload'' AS TARGET_TABLENAME,'+@cCols+' 
		FROM CMD01106  (NOLOCK)   
		JOIN SKU C (NOLOCK) ON C.PRODUCT_CODE=CMD01106.PRODUCT_CODE
	...
	EXEC SPSIS_GET_CURSOR_SKUNAMES @cXNTYPE='SLS',@cTABLENAME='CMD01106',@cKEYNAME='CM_ID',@cKEYVALUE=@CMEMOID,@CERRMSG=@CERRMSG OUTPUT 
	IF ISNULL(@CERRMSG,'')<>'' 
		GOTO END_PROC
...
BEGIN CATCH
	SET @CERRMSG='P: SPSIS_SEND_MIRROR_SLS_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
