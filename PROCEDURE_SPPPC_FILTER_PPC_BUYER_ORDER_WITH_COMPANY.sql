CREATE PROCEDURE [DBO].[SPPPC_FILTER_PPC_BUYER_ORDER_WITH_COMPANY]        
(          
...
BEGIN          
 ...
 BEGIN TRY          
  ...
  IF (@NMODE=1)          
  BEGIN          
   SET @CCMD=N' SELECT ORDER_ID, AC_CODE, AC_NAME, REF_NO, BILL_NO, ORDER_NO, ORDER_DT, TOTAL_QTY, TOTAL_AMOUNT, CANCELLED 
           FROM (        
           ...
           JOIN LMV01106 LMV ON BOM.AC_CODE =LMV.AC_CODE ) AS VBOM        
   ...
   IF @ORDER_DTYN = 'Y'          
   BEGIN          
    SET @CCMD+=N' AND ORDER_DT BETWEEN ''' + @ORDER_DTFROM + ''' AND ''' + @ORDER_DTTO + ''' '        
   END          
   SET @CCMD+=N' ORDER BY  ORDER_ID DESC,AC_NAME, ORDER_DT DESC '          
  ...
  BEGIN          
   SET @CCMD=N' SELECT BOM.MEMO_TYPE, BOM.AC_CODE, LMV.AC_NAME AS CUSTOMER_NAME,         
             ...
             ,C.COMPANY_NAME,COMPANY_ADDRESS=C.ADDRESS1+'' ''+C.ADDRESS2, C.CITY AS COMPANY_CITY,C.[STATE] COMPANY_STATE,C.PIN AS COMPANY_PIN,A.AREA_NAME AS COMPANY_AREA ,
             C.LOGO_PATH,
          ...
          LEFT JOIN COMPANY C ON C.COMPANY_CODE=''01''
		  LEFT JOIN AREA A ON A.AREA_CODE=C.AREA_CODE 
		  LEFT OUTER JOIN LMP01106 LMP ON LMP.AC_CODE=BOM.AC_CODE       
