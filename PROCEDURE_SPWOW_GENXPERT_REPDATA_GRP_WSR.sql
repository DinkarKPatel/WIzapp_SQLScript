CREATE PROCEDURE SPWOW_GENXPERT_REPDATA_GRP_WSR
...
BEGIN
	...
	JOIN location TargetLocation on TargetLocation.dept_id= cnm01106.party_dept_id/*LEFT(CNM01106.cn_id,2)*//*Rohit 05-11-2024*/
	...
	LEFT JOIN lm01106 party_lm01106 on party_lm01106.ac_code=cnm01106.ac_code
	...
	JOIN bin TargetBin on TargetBin.bin_id=cnm01106.bin_id
	JOIN bin SourceBin on SourceBin.bin_id=cnm01106.bin_id
	...
	WHERE cnm01106.receipt_dt BETWEEN [DFROMDT] AND [DTODT]  AND  cnm01106.cancelled=0 
	AND (cnm01106.mode=2 OR (ISNULL(party_lmp01106.ac_gst_no,'''')=ISNULL(SourceLocation.loc_gst_no,'''') AND ISNULL(party_lmp01106.ac_gst_no,'''')<>'''')  
	OR (ISNULL(party_lmp01106.pan_no,'''')=ISNULL(SourceLocation.pan_no,'''') AND ISNULL(party_lmp01106.pan_no,'''')<>'''')) 
	AND [WHERE]       
	group by [GROUPBY]'			

	SET @cXnType=(CASE WHEN @bCalledfromStkAnalysis=1 THEN 'STOCK' ELSE 'GRP_WSR' END)
	...
	JOIN city  SourceLocation_city on SourceLocation_city.city_code=SourceLocation_area.city_code
	JOIN state  SourceLocation_state on SourceLocation_state.state_code=SourceLocation_city.state_code
	JOIN location TargetLocation on TargetLocation.dept_id=cnm01106.location_code/*LEFT(CNM01106.cn_id,2)*//*Rohit 05-11-2024*/
	JOIN area  TargetLocation_area on TargetLocation.area_code=TargetLocation_area.area_code
	JOIN city  TargetLocation_city on TargetLocation_city.city_code=TargetLocation_area.city_code
	JOIN state  TargetLocation_state on TargetLocation_state.state_code=TargetLocation_city.state_code
	...
	SELECT TOP 1 @cXntypeSearch=xn_type FROM wow_xpert_xntypes_alias WHERE xn_type_alias='grp_wsr'
	SET @nOhCount=1
	WHILE @nOhCount<=2
	BEGIN
		...
		SET @cOhCaption=(CASE WHEN @nOhCount=1 THEN 'Freight' ELSE 'insurance' END)
