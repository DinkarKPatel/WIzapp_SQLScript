CREATE PROCEDURE SPWOW_GENXPERT_REPDATA_DCI ----(LocId 3 digit change by Sanjay:25-11-2024 left by concerned developer)
...
BEGIN
	...
	JOIN bin SourceBin on SourceBin.bin_id=FLOOR_ST_DET.ITEM_TARGET_BIN_ID
	JOIN bin targetBin on targetBin.bin_id=FLOOR_ST_DET.SOURCE_BIN_ID
   ...
   LEFT JOIN lm01106 supplier_lm01106 on supplier_lm01106.ac_code=SKU_NAMES.ac_code
	...
	LEFT JOIN XN_ITEM_TYPE_DESC_mst (NOLOCK) on XN_ITEM_TYPE_DESC_mst.XN_ITEM_TYPE=sku_names.sku_item_type
  	WHERE FLOOR_ST_MST.RECEIPT_DT BETWEEN [DFROMDT] AND [DTODT]  AND FLOOR_ST_MST.cancelled=0 
	 AND [WHERE]       
	group by [GROUPBY]'
