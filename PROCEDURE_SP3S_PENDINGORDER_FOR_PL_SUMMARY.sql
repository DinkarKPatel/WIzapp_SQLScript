create PROCEDURE SP3S_PENDINGORDER_FOR_PL_SUMMARY
(
	@CWHERE NVARCHAR(MAX)
)
AS
BEGIN
	...
	BEGIN TRY
		SET @CCMD=N'
		SELECT DISTINCT ARTICLE_NO,A.ARTICLE_CODE,A.PARA1_CODE,A.PARA2_CODE,PARA1.PARA1_NAME,PARA2.PARA2_NAME,
		(CASE WHEN ISNULL(STK.QUANTITY_IN_STOCK,0)-ISNULL(B.QUANTITY,0)<=0 THEN NULL ELSE ISNULL(STK.QUANTITY_IN_STOCK,0)-ISNULL(B.QUANTITY,0) END) AS QUANTITY,
		CAST(1 AS BIT) AS IS_STOCK
		...
		)STK ON STK.ARTICLE_CODE=ARTICLE.ARTICLE_CODE AND STK.PARA1_CODE=PARA1.PARA1_CODE
		AND STK.PARA2_CODE=PARA2.PARA2_CODE 
		--AND STK.PARA3_CODE=PARA3.PARA3_CODE AND STK.PARA4_CODE=PARA4.PARA4_CODE 
		--AND STK.PARA5_CODE=PARA5.PARA5_CODE AND STK.PARA6_CODE=PARA6.PARA6_CODE
		...
		@CSIZE_SUM=ISNULL(@CSIZE_SUM,'')+(CASE WHEN ISNULL(@CSIZE_SUM,'')='' THEN '' ELSE ',' END ) +' SUM(['+A.PARA2_NAME+']) AS ['+A.PARA2_NAME+']',
		...
		SET @CCMD=N'
		SELECT ARTICLE_NO,ARTICLE_CODE,PARA1_CODE,PARA1_NAME,IS_STOCK, '+@CSIZE+
		...
		SET @CCMD=N'
		SELECT IS_STOCK,CONVERT(BIT,0) AS BCHK,ARTICLE_NO,ARTICLE_CODE,PARA1_CODE,PARA1_NAME,'+
		@CSIZE_SUM+','+@CSIZE_SUM_TOTAL +' AS TOTAL,CONVERT(NUMERIC(14,2),0) AS ORD_QTY
		FROM ##PENDINGORDER_PIVOT
		GROUP BY ARTICLE_NO,ARTICLE_CODE,PARA1_CODE,PARA1_NAME,IS_STOCK'
