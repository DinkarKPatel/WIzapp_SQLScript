create PROCEDURE  SP_POSTSALES_SLS_DELIVERYPRINT  
(
 @cmemo_id varchar(50)=''
)
...
--(dinkar) Replace  left(memoid,2) to Location_code 
   ...
   IF OBJECT_ID ('TEMPDB..#TMPDELIVERYPRINT','U') IS NOT NULL
      DROP TABLE #TMPDELIVERYPRINT
	...
	SELECT C.PRODUCT_CODE AS SLS_PRODUCT_CODE,  
		 ...
		 ART.STOCK_NA,M.REMARKS AS HBD_REMARKS  
		 ,ART.DT_CREATED ,H.CUSTOMER_FNAME  AS CUSTOMER_NAME ,H.ADDRESS9  AS CUSTOMER_ADDRESS 
		 ,EMP.EMP_NAME,EMP1.EMP_NAME AS [EMP_NAME2],EMP2.EMP_NAME AS [EMP_NAME3] ,H.CUST_BAL ,H.MOBILE ,M.ROW_ID AS HBD_ROW_ID,M.JOB_CODE ,
		 SL.DEPT_ID AS SOURCE_LOCATION_ID ,
		 sl.dept_name as SOURCE_LOCATION_NAME ,
		 sl.address1 SOURCE_LOCATION_ADDRESS1,sl.address2 SOURCE_LOCATION_ADDRESS2,SL.REGISTERED_ADD SOURCE_LOCATION_REGISTERED_ADDRESS ,
		 ...
		 sla.pincode SOURCE_LOCATION_PIN ,m.JOB_RATE,EMP3.EMP_NAME AS HBD_EMP_NAME,
		 N.MEMO_NO HOLDBACK_NO,N.MEMO_DT AS HOLDBACK_DT
	 INTO #TMPDELIVERYPRINT
	 ...
	 JOIN SKU (NOLOCK) ON SKU.PRODUCT_CODE=isnull(C.PRODUCT_CODE  ,b.PRODUCT_CODE)
	 ...
	 LEFT  JOIN EMPLOYEE EMP2(NOLOCK) ON EMP2.EMP_CODE=C.EMP_CODE2
	  LEFT JOIN EMPLOYEE EMP3(NOLOCK) ON EMP3.EMP_CODE=n.hbd_emp_code
	 
	 where a.memo_id=@cmemo_id


	 DECLARE @CJOB_NAME VARCHAR(1000)
	SELECT @CJOB_NAME =  COALESCE(@CJOB_NAME +  ',', '' ) +rtrim( ltrim( JOB_NAME  ))  
	FROM #TMPDELIVERYPRINT a
	JOIN POST_SALES_JOBWORK_ISSUE_DET P (NOLOCK) ON P.REF_HBD_ROW_ID = A.HBD_ROW_ID 
	JOIN POST_SALES_JOBWORK_ISSUE_MST ISSMST (NOLOCK) ON ISSMST.ISSUE_ID=P.ISSUE_ID  AND ISSMST.CANCELLED =0
	JOIN JOBS F (NOLOCK) ON F.JOB_CODE = ISNULL(P.JOB_CODE ,A.JOB_CODE)


	
		

	SELECT *,@CJOB_NAME AS JOB_NAME   FROM #TMPDELIVERYPRINT A
