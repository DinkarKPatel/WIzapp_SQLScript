create PROCEDURE SPSIS_SEND_MIRROR_APP_DATA
(
...
LBLTABLEINFO:
	...
	SET @CSTEP=60
	SELECT @cCols=COALESCE(@cCols+',','')+colname 	FROM ebo_xnssending_cols WHERE tablename='APM01106' 

	SET @cCmdCols=N'
	   SELECT ''APP_APM01106_UPLOAD'' AS TARGET_TABLENAME,'+@cCols+' FROM APM01106  (NOLOCK)   WHERE MEMO_ID='''+@CMEMOID+''''
	PRINT @cCmdCols
	...
	SET @CSTEP=65	
	SET @cCols=NULL
	SELECT @cCols=COALESCE(@cCols+',','')+'APD01106.'+colname FROM ebo_xnssending_cols	WHERE tablename='APD01106' 

	SET @cCOls=REPLACE(@cCOls,'APD01106.PRODUCT_CODE','CASE WHEN ISNULL(C.VENDOR_EAN_NO,'''')<>'''' THEN C.VENDOR_EAN_NO ELSE APD01106.PRODUCT_CODE END AS PRODUCT_CODE')

	PRINT @cCOls
	SET @cCmdCols=N'
		SELECT ''APP_APD01106_UPLOAD'' AS TARGET_TABLENAME,'+@cCols+' 
		FROM APD01106  (NOLOCK)   
		JOIN SKU C (NOLOCK) ON C.PRODUCT_CODE=APD01106.PRODUCT_CODE
		JOIN LMP01106 LMP (NOLOCK) ON LMP.AC_CODE =C.AC_CODE
		WHERE SUBSTRING(LMP.AC_GST_NO,3,10)='''+@CPANNO+'''
		AND APD01106.MEMO_ID='''+@CMEMOID+''''
	...
	EXEC SPSIS_GET_CURSOR_SKUNAMES @cXNTYPE='APP',@cTABLENAME='APD01106',@cKEYNAME='MEMO_ID',@cKEYVALUE=@CMEMOID,@CERRMSG=@CERRMSG OUTPUT 
	IF ISNULL(@CERRMSG,'')<>'' 
		GOTO END_PROC
...
BEGIN CATCH
	SET @CERRMSG='P: SPSIS_SEND_MIRROR_APP_DATA, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
