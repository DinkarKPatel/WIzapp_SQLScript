...
create PROC SP_FRANCHISEE_COMM(
@FROM_DATE DATETIME,
@TO_DATE DATETIME,
@LOC VARCHAR(10)='',
@MODE INT=1,
@SUB_SEC VARCHAR(1000)='',
@ndisplaymode int=3
)
AS 
BEGIN
SET NOCOUNT ON
--1. SUMMARY   - REMOVE CM NO AND CM DT 
--2. BILL DATE WISE   - REMOVE CM NO  
--3. BILL DAY WISE  - current
--(dinkar) Replace  left(memoid,2) to Location_code 
DECLARE @SQL VARCHAR(MAX),@CLOC VARCHAR(5),@CHO VARCHAR(5)

IF OBJECT_ID ('SP_FRANCHISEE_COMM_CUSTOM','P') IS NOT NULL
 BEGIN
	-- SET @LOC='40'
	 EXEC SP_FRANCHISEE_COMM_CUSTOM @FROM_DATE,@TO_DATE,@LOC,@MODE,@SUB_SEC
	 RETURN
...
,CAST('' AS varchar(100))  AS SCHEMENAME
INTO #TBL
FROM LOCATION A 
...
,SUM(C.QUANTITY) AS QUANTITY
,SUM(C.NET) AS TOTAL_SALE
,CASE WHEN (RIGHT(C.scheme_name,6) =''AGEING'' or RIGHT(scheme_name,7)=''Agening'') THEN ''AGEING'' ELSE '''' END 
...
,ISNULL(C.DISCOUNT_PERCENTAGE,0),CASE WHEN (RIGHT(C.scheme_name,6) =''AGEING'' or RIGHT(scheme_name,7)=''Agening'') THEN ''AGEING'' ELSE '''' END  '
PRINT @SQL
EXEC(@SQL)
...
,SALE_VALUE_AT_MRP, TAX_AMOUNT,DISC_PER,DIC_AMOUNT , QUANTITY, REALIZED_SALE_VALUE,
CASE WHEN ISNULL(SCHEMENAME,'')<>'' AND COMM_PCT>10 THEN 10 ELSE COMM_PCT END COMM_PCT,
CASE WHEN ISNULL(SCHEMENAME,'')<>'' AND COMM_PCT>10 THEN 'Ageing' ELSE '' END Ageing,
B.TOTAL_SALE
INTO #GENREPO
...
begin
	...
	,G.CM_NO +case when ageing<>'' then   +'('+ageing+')' else '' end  AS CM_NO,CM_DT
	,QUANTITY
	,DISC_PER DISCPER
	...
	,ROUND(((REALIZED_SALE_VALUE)*COMM_PCT/100),2) [COMM]
	,COMM_PCT [COMM_PER]
	FROM #GENREPO G 
...
begin
	...
	,DISC_PER  DISCPER
	,sum(SALE_VALUE_AT_MRP) SALE
	,sum(DIC_AMOUNT) [DISCN]  
	,sum((SALE_VALUE_AT_MRP -REALIZED_SALE_VALUE))[DISC]
	,sum(REALIZED_SALE_VALUE) as  REALIZED_SALE_VALUE
	,sum(G.CNT) as CNT
	,sum(ATD_CHARGES) [RO_ATD_CHARGES]
	--,CASE WHEN FLOOR_DISC_PCT=0 THEN @NTOTATDAMT ELSE 0 END [RO_CHARGES]
	,sum(ROFF) as ROFF--[RO_CHARGES]
	--,TOTAL_SALE [NET_SLS_AMT]
	,sum(REALIZED_SALE_VALUE+ATD_CHARGES+ROFF) [NET_SLS_AMT]
	,ROUND(sum(((REALIZED_SALE_VALUE)*COMM_PCT/100)),2) [COMM]
	,COMM_PCT [COMM_PER]
	FROM #GENREPO G 
	WHERE G.QUANTITY IS NOT NULL
	group by DEPT_ID+'-'+DEPT_NAME,DISC_PER,COMM_PCT
...
begin
	...
	FROM #GENREPO G 
	WHERE G.QUANTITY IS NOT NULL
	group by DEPT_ID+'-'+DEPT_NAME,G.CM_DT,DISC_PER ,COMM_PCT
	 

end
 --UPDATE #GENREPO SET COMM_PCT=100 WHERE Ageing ='AGEING'

IF OBJECT_ID('TEMPDB..#GENC') IS NOT NULL DROP TABLE #GENC
SELECT  DEPT_ID+'-'+DEPT_NAME LOC_NAME,FLOOR_DISC_PCT, CEIL_DISC_PCT, COMM_PCT, 
SUM(QUANTITY) AS QTY
...
INTO #GENC
FROM #GENREPO 
GROUP BY AGEING,DEPT_ID+'-'+DEPT_NAME,FLOOR_DISC_PCT, CEIL_DISC_PCT, COMM_PCT,Ageing
...
,@COMM_AMT FLOAT ,@DISC_AMT FLOAT ,@ATD_CHARGES FLOAT,@ROFF_CHARGES FLOAT,@CAGEING VARCHAR(10)
...
,CTEXT9 DECIMAL(15,2),CTEXT10 DECIMAL(15,2),CTEXT11 DECIMAL(15,2),CTEXT12 DECIMAL(15,2),CTEXT13 VARCHAR(10))
...
,ROUND(((REALIZED_SALE_VALUE)*COMM_PCT/100),2) AS COMM_AMT
,(SALE_VALUE_AT_MRP -(REALIZED_SALE_VALUE)) AS DISC_AMT
,ATD_CHARGES--=CASE WHEN FLOOR_DISC_PCT=0 THEN @NTOTATDAMT ELSE 0 END 
,ISNULL(ROFF,0)ROFF,Ageing
FROM #GENC 
--WHERE DIC_AMOUNT>0

OPEN GENC
FETCH NEXT FROM GENC INTO @FLOOR_DISC_PCT,@CEIL_DISC_PCT,@COMM_PCT,@QTY,@SALE_VALUE_AT_MRP,@DIC_AMOUNT,@REALIZED_SALE_VALUE,@COMM_AMT,@DISC_AMT,@ATD_CHARGES,@ROFF_CHARGES,@CAGEING
WHILE @@FETCH_STATUS=0
  BEGIN
...
SELECT CTEXT1=CAST(CTEXT1 AS varchar(100))+CASE WHEN CTEXT13<>'' THEN  ' (A)' ELSE '' END, * FROM #CTEXT WHERE CTEXT1 IS NOT NULL   ORDER BY CTEXT13, 1
...
--SELECT * FROM #CTEXT WHERE CTEXT1 IS NOT NULL AND  ISNULL(CTEXT13,'')<>''  ORDER BY 1
SET NOCOUNT OFF
END
