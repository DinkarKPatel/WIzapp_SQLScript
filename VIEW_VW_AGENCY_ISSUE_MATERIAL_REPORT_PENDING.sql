CREATE  VIEW VW_AGENCY_ISSUE_MATERIAL_REPORT_PENDING       
AS  
SELECT D2.ARTICLE_NO AS ARTICLE_SET,T3.ITEM_REMARKS AS ITEM_REMARKS,P1.PARA1_NAME COM_COLOR,P2.PARA2_NAME AS COM_SIZE, WO.MEMO_NO AS [WORK_ORDER_NO],WO.REF_NO ,D.ARTICLE_NAME AS [COMPONENT NAME] ,D.ARTICLE_DESC AS [COMPONENT DESC] 
,D.ARTICLE_NO AS [COMPONENT NO],       
...
,EMP.EMP_FNAME+' '+EMP.EMP_LNAME AS PC_NAME,T4.ISSUE_QTY AS ORD_QUANTITY ,T3.ROW_ID   
FROM PRD_AGENCY_ISSUE_MATERIAL_MST_PENDING T1       
JOIN USERS T2 ON T1.USER_CODE = T2.USER_CODE              
JOIN PRD_AGENCY_ISSUE_MATERIAL_DET_PENDING T3 ON T1.MEMO_ID=T3.MEMO_ID       
...
LEFT OUTER JOIN 
(SELECT B.REF_PRD_WORKORDER_MEMOID,A.MEMO_ID,PARA1_CODE,PARA2_CODE,A.ISSUE_QTY 
FROM PRD_AGENCY_ISSUE_ROW_MATERIAL_DET A JOIN PRD_AGENCY_ISSUE_MATERIAL_DET B
ON A.MEMO_ID =B.MEMO_ID AND A.ROW_ID =B.REF_MATERIAL_ROW_ID GROUP BY B.REF_PRD_WORKORDER_MEMOID,A.MEMO_ID,
PARA1_CODE,PARA2_CODE,A.ISSUE_QTY 
) T4 ON T4.MEMO_ID=T3.REF_ISSUE_ID 
AND P1.PARA1_CODE=T4.PARA1_CODE AND P2.PARA2_CODE =T4.PARA2_CODE
