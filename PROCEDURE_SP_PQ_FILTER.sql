CREATE PROC SP_PQ_FILTER    
(    
 --@NMODE_TYPE NUMERIC(1), /*@NMODE_TYPE:0 FOR ALL,1 FOR HO,2 FOR STORES AND 3 FOR AGAINST BUYINGORDER */    
 --@NBILL_LEVEL_TAX_METHOD NUMERIC(1), /*@NBILL_LEVEL_TAX_METHOD:0 FOR ALL , 1 FOR EXCLUSIVE BILL LEVEL TAX METHOD  AND 2 FOR INCLUSIVE */    
 ...
 --@CDEPT_ID VARCHAR(2), /*@CDEPT_ID:'' THEN NO FILTER ELSE FILTER LOCATION*/    
...
BEGIN    
SET NOCOUNT ON    
 SELECT CONVERT(VARCHAR,A.PQ_DT,106) AS MEMO_DT,A.PQ_ID AS MEMO_ID,A.PQ_NO AS MEMO_NO,    
B.AC_NAME AS SUPPLIER_NAME,    
 ...
 A.REF_NO,    
 (CASE WHEN (A.APPROVED = 2 or A.ApprovedLevelNo =99) THEN 'YES' ELSE 'NO' END) AS APPROVED
 ...
 --) J ON J.PO_ID = A.PO_ID      
   ...
   --  AND (@NTAXFORM_STORAGE_MODE=0 OR (A.TAXFORM_STORAGE_MODE= @NTAXFORM_STORAGE_MODE ))              
     AND ((@NDT_FLAG = 0 AND A.DELIVERY_DT BETWEEN  @DFROMDATE AND @DTODATE)     
        OR (@NDT_FLAG = 1 AND A.PQ_DT BETWEEN  @DFROMDATE AND @DTODATE))    
     ...
     --AND (@POTYPE='' OR A.PQ_TYPE_ID =@POTYPE)     
     AND (@LOC='' OR LEFT(A.PQ_ID,2)=@LOC OR A.DEPT_ID = @LOC)     
  AND ( ISNULL(@XN_ITEM_TYPE ,0)=0 OR A.xn_item_type= @XN_ITEM_TYPE)  
--AND (ISNULL(@PO_RECEIVING_MODE   ,0)=0 OR A.PQ_RECEIVING_MODE= @PO_RECEIVING_MODE)  
