CREATE PROCEDURE DBO.SP_MERGE_MIRROR_MSTGVSCH
  (@NMODE INT
  ,@XN_TYPE VARCHAR(50) ='MSTGVSCH'
  )
AS
 BEGIN
...
BEGIN TRY
	  ...
	  BEGIN
			SET @CERRORMSG='P:SP_MERGE_MIRROR_MSTGVSCH, CANNOT CALL MERGING PROC AT HEAD OFFICE'
			GOTO END_PROC
  	  END					
	  ...
	  DELETE FROM GV_SCHEME_LOCS	
	  DELETE FROM GV_SCHEME_1
	  DELETE FROM GV_SCHEME_MST
	 ...
	 --INSERT DATA INTO LOCATION TABLE
	   ...
	   SET @CSTEP = 20
	   SET @CTABLENAME = 'GV_SCHEME_MST'
	   SET @CSOURCETABLE ='MSTGVSCH_GV_SCHEME_MST_MIRROR'
	   SET @CKEYFIELD1 ='SCHEME_CODE'
	   EXEC UPDATEMASTERXN
	   ...
	   SET @CSTEP = 30
	   SET @CTABLENAME = 'GV_SCHEME_1'
	   SET @CSOURCETABLE ='MSTGVSCH_GV_SCHEME_1_MIRROR'
	   SET @CKEYFIELD1 ='SCHEME_CODE'
	   EXEC UPDATEMASTERXN
	   @CSOURCEDB=@CSOURCEDB, @CSOURCETABLE=@CSOURCETABLE,
	   @CDESTDB='',@CDESTTABLE=@CTABLENAME,@CKEYFIELD1=@CKEYFIELD1,
	   ...
	   @CFILTERCONDITION='',@LUPDATEONLY=0,@BALWAYSUPDATE=1
	    ...
	    --INSERT AND UPDATE DATA INTO LOC_STOCK_LEVEL_DET TABLE
	   SET @CSTEP = 40
	   SET @CTABLENAME = 'GV_SCHEME_LOCS'
	   SET @CSOURCETABLE ='MSTGVSCH_GV_SCHEME_LOCS_MIRROR'
	   SET @CKEYFIELD1 ='SCHEME_CODE'
	   EXEC UPDATEMASTERXN
	   @CSOURCEDB=@CSOURCEDB, @CSOURCETABLE=@CSOURCETABLE,
	   @CDESTDB='',@CDESTTABLE=@CTABLENAME,@CKEYFIELD1=@CKEYFIELD1,
...
BEGIN CATCH
	SET @CERRORMSG='P:SP_MERGE_MIRROR_MSTGVSCH, STEP:'+@CSTEP+', MESSAGE:'+ERROR_MESSAGE()
	GOTO END_PROC
END CATCH
...
END_PROC:
	...
	BEGIN   
	    ...
	    DELETE FROM MSTGVSCH_GV_SCHEME_MST_MIRROR
	    DELETE FROM MSTGVSCH_GV_SCHEME_1_MIRROR
		DELETE FROM MSTGVSCH_GV_SCHEME_LOCS_MIRROR
	END
		
	SELECT @XN_TYPE AS MEMO_ID,@CERRORMSG AS ERRMSG   	  
END

--END PROCEDURE SP_MERGE_MIRROR_MSTGVSCH
