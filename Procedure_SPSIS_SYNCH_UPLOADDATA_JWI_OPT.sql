CREATE PROCEDURE SPSIS_SYNCH_UPLOADDATA_JWI_OPT
(
    @CSPID VARCHAR(50),
...
LBLSTART:
    ...
    SELECT @CMEMOID=''
		...
		UPDATE A SET 
				 ...
				 USER_CODE ='0000000',
				 BIN_ID='000',
				 issue_id=@CDEPTID+SUBSTRING(issue_id,LEN(@CDEPTID)+1,LEN(issue_id))/*Rohit 01-11-2024*/ ,
				 ISSUE_NO=@CDEPTID+SUBSTRING(ISSUE_NO,LEN(@CDEPTID)+1,LEN(ISSUE_NO))/*Rohit 01-11-2024*/
				
	FROM JWI_JOBWORK_ISSUE_MST_UPLOAD A (NOLOCK)
	WHERE SP_ID =@CSPID

	UPDATE A SET BIN_ID ='000',
	          issue_id=@CDEPTID+SUBSTRING(issue_id,LEN(@CDEPTID)+1,LEN(issue_id))/*Rohit 01-11-2024*/ ,
			  DEPT_ID=@CDEPTID
	FROM JWI_JOBWORK_ISSUE_DET_UPLOAD A (NOLOCK)
	WHERE SP_ID =@CSPID

	
	 UPDATE A SET PRODUCT_CODE=LEFT(A.PRODUCT_CODE, ISNULL(NULLIF(CHARINDEX ('@',A.PRODUCT_CODE)-1,-1),LEN(A.PRODUCT_CODE )))  
	 FROM JWI_JOBWORK_ISSUE_DET_UPLOAD A (NOLOCK)	WHERE SP_ID=@CSPID
	...
	IF @BADDMODE=1 AND NOT EXISTS (SELECT TOP 1 'U' FROM JWI_JOBWORK_ISSUE_DET_UPLOAD WHERE SP_ID =@CSPID)
	    GOTO EXIT_PROC
    IF @BADDMODE=0 AND NOT EXISTS (SELECT TOP 1 'U' FROM JWI_JOBWORK_ISSUE_DET_UPLOAD WHERE SP_ID =@CSPID)
		 SET @BCANCELLED=1
LBLMERGE:
	...
	BEGIN	
		 ...
		 SELECT 	'000'  BIN_ID,/*LEFT(A.issue_id,2)*//*Rohit 01-11-2024*/@CDEPTID DEPT_ID, PRODUCT_CODE,@CSPID SP_ID,A.quantity AS XN_QTY 
		...
		IF @bCancelled=1 
		BEGIN
			UPDATE JOBWORK_ISSUE_MST WITH (ROWLOCK) SET cancelled=1 WHERE issue_id=@cMemoid
			GOTO lblupdatepmt
		END
	...
	EXEC UPDATEMASTERXN_MIRROR @CSOURCEDB=@CSOURCEDB,@CSOURCETABLE=@CTMP_TABLENAME,@CDESTDB=@CMERGEDB
	  ...
	  SELECT a.product_code as new_product_code into #tmpbarcode FROM JWI_JOBWORK_ISSUE_DET_UPLOAD A (NOLOCK)
		...
		BEGIN
			SET @CSTEP=84.2
			EXEC SP_CHKXNSAVELOG 'JWI',@CSTEP,0,@CSPID,'',1

			 EXEC SPSIS_INSERTSKUBARCODE @CSPID, 'JWI',@CERRMSG OUTPUT
	...
	EXEC UPDATEMASTERXN_MIRROR @CSOURCEDB=@CSOURCEDB,@CSOURCETABLE=@CTMP_TABLENAME,@CDESTDB=@CMERGEDB
	    ...
	    INSERT SAVETRAN_BARCODE_NETQTY	( BIN_ID, DEPT_ID, PRODUCT_CODE, sp_id, XN_QTY ) 
		 SELECT 	'000'  BIN_ID,/*LEFT(A.ISSUE_ID,2)*//*Rohit 01-11-2024*/@CDEPTID DEPT_ID, PRODUCT_CODE,@CSPID SP_ID,-1*A.quantity AS XN_QTY 
		 FROM JOBWORK_ISSUE_DET A (NOLOCK)
		 WHERE A.ISSUE_ID =@CMEMOID
