CREATE PROCEDURE SP3S_PROCESS_FILTERWISE_TARGET_ACHIEVED
--WITH ENCRYPTION 
AS 
BEGIN
/*
	THIS PROCEDURE WILL CALCULATE SALE TARGET ACHIEVED METRICS BASED ON FILTER PARAMETERS DEFINED FOR CURRENT AND 
	LAST FINANCIAL YEARS 
*/
--EXEC SP3S_PROCESS_FILTERWISE_TARGET_ACHIEVED
...
INTO #FILTER_CURSOR
FROM SALEPERSON_TARGET_MST MST
JOIN SALEPERSON_TARGET_DET DET ON MST.MEMO_ID=DET.MEMO_ID
JOIN #FINYEARS FIN ON DET.FIN_YEAR=FIN.FIN_YEAR

SET @CSTEP=70
IF OBJECT_ID('TEMPDB..#TARGET_SET','U') IS NOT NULL
	DROP TABLE #TARGET_SET
...
BEGIN
	...
	INSERT #TARGET_SET(MEMO_ID,DEPT_ID,MONTH_NAME,MONTH_NO,FIN_YEAR,VALUE_TYPE,TARGET_VALUE)
	SELECT MEMO_ID,DEPT_ID
			  ...
			  ,FIN_YEAR,VALUE_TYPE,TARGET_VALUE
		FROM 
		(
			SELECT A.MEMO_ID,A.DEPT_ID,A.FIN_YEAR,A.VALUE_TYPE,A.APRIL,A.MAY,A.JUNE,A.JULY,A.AUGUST,A.SEPTEMBER,A.OCTOBER,A.NOVEMBER
		...
		UNPIVOT
		(
			TARGET_VALUE FOR MONTH_NAME IN 
			(
				  APRIL
				 ,MAY
				 ,JUNE
	...
	SET @CCMD=N'SELECT Cmm.location_code AS DEPT_ID,DATENAME(MONTH,CMM.CM_DT) AS MONTH_NAME
					  ,CMM.FIN_YEAR	
					  ,SUM(CMD.QUANTITY) AS SOLD_QUANTITY,SUM(CMD.RFNET) AS SOLD_VALUE
	...
	SET @CSTEP=160
	INSERT #ACTUAL_SALE(DEPT_ID,MONTH_NAME,FIN_YEAR,SALE_QUANTITY,SALE_VALUE)
	EXEC SP_EXECUTESQL @CCMD
	
	SET @CSTEP=170
	--UPDATING THE SALE_VALUE AGAINST THE TARGET SET FOR EACH LOCATION 
	UPDATE A SET SALE_VALUE=(CASE WHEN A.VALUE_TYPE=1 THEN B.SALE_QUANTITY ELSE B.SALE_VALUE END)
	FROM #TARGET_SET A
	JOIN #ACTUAL_SALE B ON A.DEPT_ID=B.DEPT_ID AND A.MONTH_NAME=B.MONTH_NAME AND A.FIN_YEAR=B.FIN_YEAR 
	
	SET @CSTEP=180
	DELETE FILTER_TARGET_ACHIEVED_DET WHERE MEMO_ID=@CMEMO_ID
	DELETE FILTER_TARGET_ACHIEVED_MST WHERE MEMO_ID=@CMEMO_ID
	
	SET @CSTEP=190
	INSERT FILTER_TARGET_ACHIEVED_MST	( TITLE_NAME, MEMO_ID, FILTER, PROCESS_DT )  
	SELECT @CTITLE_NAME AS TITLE_NAME,@CMEMO_ID AS MEMO_ID,@CFILTER AS FILTER,GETDATE() AS PROCESS_DT 
	
	SET @CSTEP=200
	INSERT FILTER_TARGET_ACHIEVED_DET	( MEMO_ID, DEPT_ID, MONTH_NAME, MONTH_NO, FIN_YEAR, VALUE_TYPE
										, TARGET_VALUE, SALE_VALUE, ACHIEVED_PER, SHORTFALL )  
	SELECT 	  MEMO_ID, DEPT_ID, MONTH_NAME, MONTH_NO, FIN_YEAR
			 ,(CASE WHEN VALUE_TYPE=1 THEN 'QUANTITY BASED' ELSE 'VALUE BASED' END) AS VALUE_TYPE
			 , TARGET_VALUE, ISNULL(SALE_VALUE,0) 
			 ,(CASE WHEN ISNULL(TARGET_VALUE,0)<>0 THEN (ISNULL(SALE_VALUE,0)/TARGET_VALUE)*100 ELSE 0 END)
			 ,(CASE WHEN ISNULL(TARGET_VALUE,0)>ISNULL(SALE_VALUE,0) THEN TARGET_VALUE-ISNULL(SALE_VALUE,0) ELSE 0 END)
	FROM #TARGET_SET
...
END	
--END OF PROCEDURE - SP3S_PROCESS_FILTERWISE_TARGET_ACHIEVED
