...
create Procedure SP3S_CALCULATE_FCNET_WSLORD
(
 @CORDERID VARCHAR(50),
...
BEGIN
		...
		BEGIN
		 ...
		 END
	       ...
	       INSERT INTO #TMPFCRATE(ROW_ID,FC_NET,FC_GST_PERCENTAGE,FC_TAX_METHOD,IS_IGST)
				SELECT A.ROW_ID ,
					   FC_NET = ROUND(((A.WS_PRICE *A.QUANTITY)),2)/B.fc_rate,
					   A.GST_PERCENTAGE,B.GST_CAL_METHOD ,
		...
		BEGIN
		 ...
		 DECLARE @NFCOTH NUMERIC(10,2)

		   SELECT @NFCOTH=ROUND(other_charges+CASE WHEN GST_CAL_METHOD =1 THEN  ISNULL(OTHER_CHARGES_IGST_AMOUNT,0)+ISNULL(other_charges_CGST_AMOUNT,0)+ISNULL(other_charges_SGST_AMOUNT,0)
		        ELSE 0 END /ISNULL(FC_RATE,0),2)
		   FROM BUYER_ORDER_MST WITH (NOLOCK) WHERE order_id = @CORDERID
		   AND other_charges <>0 AND ISNULL(FC_RATE,0)<>0

		   
		   SELECT SUM(FC_XN_VALUE_WITH_GST)+isnull(@NFCOTH,0) AS FC_TOTAL_AMOUNT
		   from #TMPFCRATE
