CREATE PROCEDURE SP3S_PREPARE_ARS_ID_UPLOAD_DETAIL_OLD(@cPlan_Name VARCHAR(100),@DT VARCHAR(10)='',@LOC VARCHAR(MAX))      
AS      
BEGIN      
SET NOCOUNT ON      
--DECLARE @cPlan_Name VARCHAR(100)='TEST',@DT VARCHAR(10)='',@LOC VARCHAR(MAX)='IN (''15'',''16'',''18'')'
...
SELECT TOP 1 @ARS_ID=ARS_ID,@SEASON_PERIOD_EXISTS=CASE WHEN YEAR(ISNULL(FROM_DT,''))=1900 THEN 0 ELSE 1 END 
FROM ARS_MST (NOLOCK) 
...
JOIN (SELECT A.DEPT_ID,ARTICLE.ARTICLE_CODE,SKU.PARA1_CODE,SKU.PARA2_CODE,SKU.PARA3_CODE,SKU.PARA4_CODE,SKU.PARA5_CODE,SKU.PARA6_CODE
		...
		FROM '+@cRfDbName+@RF_TABLE+' A (NOLOCK)
		...
		WHERE A.XN_DT BETWEEN  REPLACE(CONVERT(VARCHAR,DATEADD(DD,-'+CAST(@CAL_DAYS AS VARCHAR)+',GETDATE()),102),''.'',''-'') 
		AND REPLACE(CONVERT(VARCHAR,GETDATE(),102),''.'',''-'')
		GROUP BY A.DEPT_ID,ARTICLE.ARTICLE_CODE,SKU.PARA1_CODE,SKU.PARA2_CODE,SKU.PARA3_CODE,SKU.PARA4_CODE,SKU.PARA5_CODE,SKU.PARA6_CODE
		,SECTIONM.SECTION_CODE,SECTIOND.SUB_SECTION_CODE,AF.ATTR1_KEY_CODE,AF.ATTR2_KEY_CODE,AF.ATTR3_KEY_CODE,AF.ATTR4_KEY_CODE,AF.ATTR5_KEY_CODE
		,AF.ATTR6_KEY_CODE,AF.ATTR7_KEY_CODE,AF.ATTR8_KEY_CODE,AF.ATTR9_KEY_CODE,AF.ATTR10_KEY_CODE,AF.ATTR11_KEY_CODE,AF.ATTR12_KEY_CODE,AF.ATTR13_KEY_CODE
		,AF.ATTR14_KEY_CODE,AF.ATTR15_KEY_CODE,AF.ATTR16_KEY_CODE,AF.ATTR17_KEY_CODE,AF.ATTR18_KEY_CODE,AF.ATTR19_KEY_CODE,AF.ATTR20_KEY_CODE
		,AF.ATTR21_KEY_CODE,AF.ATTR22_KEY_CODE,AF.ATTR23_KEY_CODE,AF.ATTR24_KEY_CODE,AF.ATTR25_KEY_CODE
...
IF EXISTS(SELECT TOP 1 * FROM ARTICLE (NOLOCK) WHERE PARA1_SET=1 OR PARA2_SET=1) and 1=2
   SET @PARA_SET=1  
...
--select 161,* from ##tplan
UPDATE ##tPlan SET DYS=DATEDIFF(DD,Season_From,Season_Till)+1,WeekF1=DATEADD(DD,-6,Season_Till),WeekT1=Season_Till
...
IF @AUTO_CAL=1
   BEGIN
      ...
      UPDATE ##tPlan SET Season_From=DATEADD(DD,-DYS,Season_Till)
   END
/*
IF (SELECT DATEDIFF(DD,SEASON_FROM,SEASON_TILL)+1 FROM ##TPLAN)>21
   UPDATE ##TPLAN SET WEEKF1=SEASON_FROM,WEEKT1=DATEADD(DD,6,SEASON_FROM)
   ,WEEKF2=DATEADD(DD,7,SEASON_FROM),WEEKT2=DATEADD(DD,13,SEASON_FROM)
   ,WEEKF3=DATEADD(DD,14,SEASON_FROM),WEEKT3=DATEADD(DD,20,SEASON_FROM)
   ,WEEKF4=DATEADD(DD,21,SEASON_FROM),WEEKT4=CASE WHEN DATEADD(DD,27,SEASON_FROM)<SEASON_TILL THEN DATEADD(DD,27,SEASON_FROM) ELSE SEASON_TILL END 
ELSE IF (SELECT DATEDIFF(DD,SEASON_FROM,SEASON_TILL)+1 FROM ##TPLAN)>14
   UPDATE ##TPLAN SET WEEKF1=SEASON_FROM,WEEKT1=DATEADD(DD,6,SEASON_FROM)
   ,WEEKF2=DATEADD(DD,7,SEASON_FROM),WEEKT2=DATEADD(DD,13,SEASON_FROM)
   ,WEEKF3=DATEADD(DD,14,SEASON_FROM),WEEKT3=CASE WHEN DATEADD(DD,20,SEASON_FROM)<SEASON_TILL THEN DATEADD(DD,20,SEASON_FROM) ELSE SEASON_TILL END 
   ,WEEKF4=NULL,WEEKT4=NULL
ELSE IF (SELECT DATEDIFF(DD,SEASON_FROM,SEASON_TILL)+1 FROM ##TPLAN) BETWEEN 8 AND 14
   UPDATE ##TPLAN SET WEEKF1=SEASON_FROM,WEEKT1=DATEADD(DD,6,SEASON_FROM)
   ,WEEKF2=DATEADD(DD,7,SEASON_FROM),WEEKT2=CASE WHEN DATEADD(DD,13,SEASON_FROM)<SEASON_TILL THEN DATEADD(DD,13,SEASON_FROM) ELSE SEASON_TILL END 
   ,WEEKF3=NULL,WEEKT3=NULL,WEEKF4=NULL,WEEKT4=NULL
ELSE
   UPDATE ##TPLAN SET WEEKF1=SEASON_FROM,WEEKT1=SEASON_TILL
   ,WEEKF2=NULL,WEEKT2=NULL,WEEKF3=NULL,WEEKT3=NULL,WEEKF4=NULL,WEEKT4=NULL
select 191,* from ##tplan
*/
--14MAR19
--IF EXISTS (SELECT TOP 1 * FROM ##tPlan WHERE SEASON_FROM>CONVERT(DATE,GETDATE()))
--   UPDATE ##tPlan SET Season_From=DATEADD(YY,-1,Season_From),Season_Till=DATEADD(YY,-1,Season_Till)
--14MAR19
...
FROM      
(SELECT JN= CASE when STK_LEVEL_PARA_NAME like 'PARA%' and (SUBSTRING(stk_level_para_name,5,1) not in ('1','2') OR @PARA_SET=0) then CHAR(13)+'JOIN '+REPLACE(stk_level_para_name,'_NAME','')+' P'+SUBSTRING(stk_level_para_name,5,1)+' (NOLOCK) ON P'+SUBSTRING(stk_level_para_name,5,1)+'.PARA'+SUBSTRING(stk_level_para_name,5,1)+'_CODE=SKU.PARA'+SUBSTRING(stk_level_para_name,5,1)+'_CODE'  
...
IF OBJECT_ID('tempdb..##REPORT_PLAN') IS NOT NULL DROP TABLE ##REPORT_PLAN      
...
,SUM(CASE WHEN RF.DEPT_ID=AD.DEPT_ID THEN ISNULL(RF.GIT_QTY,0) ELSE 0 END)AS Indent_Stock'
+CHAR(13)+REPLACE(@SALE,'Qty_Sold','')+'/CASE T.DYS WHEN 0 THEN 1 ELSE T.DYS END AQSPD'      
+CHAR(13)+'INTO ##REPORT'      
+CHAR(13)+'FROM '+@cRfDbName+@RF_TABLE+' RF (NOLOCK) ' 
+@JOIN     
...
EXEC(@SQL)      
/*
IF EXISTS(SELECT * FROM ##tPlan WHERE CONVERT(DATE,GETDATE()) BETWEEN SEASON_FROM AND SEASON_TILL)
   BEGIN
	SET @SQL='UPDATE RPT SET Qty_Sold=ISNULL(Qty_Sold,0)+ISNULL(S.Sale,0)
    FROM (SELECT '+CASE WHEN ISNULL(@SALESEL,'')!='' THEN @SALESEL ELSE '' END+'SUM(A.QUANTITY) Sale'
    +CHAR(13)+'FROM CMD01106 A WITH (NOLOCK)      
	JOIN CMM01106 B WITH (NOLOCK) ON A.CM_ID = B.CM_ID      
    JOIN LOCATION C WITH (NOLOCK) ON C.DEPT_ID=LEFT(A.CM_ID,2) 
    JOIN #LOC L ON L.LOC=C.DEPT_ID'
    +CHAR(13)+REPLACE(REPLACE(REPLACE(@JOIN,'JOIN #LOC ON #LOC.LOC=RF.DEPT_ID',''),'ON SKU.PRODUCT_CODE=A.PRODUCT_CODE',''),'ON SKU.PRODUCT_CODE=RF.PRODUCT_CODE','ON SKU.PRODUCT_CODE=A.PRODUCT_CODE')
    +CHAR(13)+'WHERE B.CM_DT=CONVERT(DATE,GETDATE())'
    +CASE WHEN ISNULL(@SALESEL,'')!='' THEN CHAR(13)+'GROUP BY '+LEFT(@SALESEL,LEN(@SALESEL)-1) ELSE '' END
    +')S
    JOIN ##REPORT RPT'
    +CHAR(13)+'ON '+CASE WHEN ISNULL(@SALEJN,'')='' THEN '1=1' ELSE CASE ISNULL(@SALEJN,'') WHEN '' THEN '' ELSE LEFT(RTRIM(@SALEJN),LEN(RTRIM(@SALEJN))-3)END END
    SET @SQL=REPLACE(REPLACE(@SQL,'SECTIONM.','SM.'),'SECTIOND.','SD.')
    --PRINT '307 '+@SQL
    EXEC(@SQL)
   END

IF EXISTS(SELECT * FROM ##tPlan WHERE CONVERT(DATE,GETDATE()) BETWEEN WEEKF1 AND WEEKT1)
   BEGIN
	SET @SQL='UPDATE RPT SET Week1_Sale=ISNULL(Week1_Sale,0)+ISNULL(S.Sale,0)
    FROM (SELECT '+CASE WHEN ISNULL(@SALESEL,'')!='' THEN @SALESEL ELSE '' END+'SUM(A.QUANTITY) Sale'
    +CHAR(13)+'FROM CMD01106 A WITH (NOLOCK)      
	JOIN CMM01106 B WITH (NOLOCK) ON A.CM_ID = B.CM_ID      
    JOIN LOCATION C WITH (NOLOCK) ON C.DEPT_ID=LEFT(A.CM_ID,2) 
...
IF EXISTS(SELECT * FROM ##tPlan WHERE CONVERT(DATE,GETDATE()) BETWEEN WEEKF2 AND WEEKT2)
   BEGIN
	SET @SQL='UPDATE RPT SET Week2_Sale=ISNULL(Week2_Sale,0)+ISNULL(S.Sale,0)
    FROM (SELECT '+CASE WHEN ISNULL(@SALESEL,'')!='' THEN @SALESEL ELSE '' END+'SUM(A.QUANTITY) Sale'
    +CHAR(13)+'FROM CMD01106 A WITH (NOLOCK)      
	JOIN CMM01106 B WITH (NOLOCK) ON A.CM_ID = B.CM_ID      
    JOIN LOCATION C WITH (NOLOCK) ON C.DEPT_ID=LEFT(A.CM_ID,2) 
...
IF EXISTS(SELECT * FROM ##tPlan WHERE CONVERT(DATE,GETDATE()) BETWEEN WEEKF3 AND WEEKT3)
   BEGIN
	SET @SQL='UPDATE RPT SET Week3_Sale=ISNULL(Week3_Sale,0)+ISNULL(S.Sale,0)
    FROM (SELECT '+CASE WHEN ISNULL(@SALESEL,'')!='' THEN @SALESEL ELSE '' END+'SUM(A.QUANTITY) Sale'
    +CHAR(13)+'FROM CMD01106 A WITH (NOLOCK)      
	JOIN CMM01106 B WITH (NOLOCK) ON A.CM_ID = B.CM_ID      
    JOIN LOCATION C WITH (NOLOCK) ON C.DEPT_ID=LEFT(A.CM_ID,2) 
...
IF EXISTS(SELECT * FROM ##tPlan WHERE CONVERT(DATE,GETDATE()) BETWEEN WEEKF4 AND WEEKT4)
   BEGIN
	SET @SQL='UPDATE RPT SET Week4_Sale=ISNULL(Week4_Sale,0)+ISNULL(S.Sale,0)
    FROM (SELECT '+CASE WHEN ISNULL(@SALESEL,'')!='' THEN @SALESEL ELSE '' END+'SUM(A.QUANTITY) Sale'
    +CHAR(13)+'FROM CMD01106 A WITH (NOLOCK)      
	JOIN CMM01106 B WITH (NOLOCK) ON A.CM_ID = B.CM_ID      
    JOIN LOCATION C WITH (NOLOCK) ON C.DEPT_ID=LEFT(A.CM_ID,2) 
   ...
   END
*/   
UPDATE ##REPORT SET AQSPD=ISNULL(Qty_Sold,0)/(SELECT TOP 1 CASE T.DYS WHEN 0 THEN 1 ELSE T.DYS END FROM ##tPlan T)

--
SET @SQL=''
SELECT @SQL=COALESCE(@SQL,'')+'RF.'+COLUMN_NAME+'=WH.'+COLUMN_NAME+' AND ' 
FROM TEMPDB.INFORMATION_SCHEMA.COLUMNS 
WHERE TABLE_NAME LIKE '##REPORT' AND (COLUMN_NAME='ARTICLE_CODE' OR COLUMN_NAME LIKE 'PARA%CODE')
...
FROM 
(SELECT '+CASE WHEN ISNULL(@SALESEL,'')='' THEN '' ELSE @SALESEL+'' END
...
+CHAR(13)+'WHERE SKU.ER_FLAG IN (''0'',''1'') AND loc_view.REPORT_BLOCKED=0 AND SECTIONM.ITEM_TYPE=1 AND loc_view.INACTIVE=0 '
+CHAR(13)+CASE WHEN ISNULL(@SALESEL,'')='' THEN '' ELSE CHAR(13)+'GROUP BY '+LEFT(@SALESEL,LEN(@SALESEL)-1)+'' END
+CHAR(13)+')S
JOIN ##REPORT RPT
...
SELECT @cPlan_Name PLANNAME,*      
,Stock CBS      
,FLOOR(CONVERT(NUMERIC(18,2),ROUND(Stock/CASE AQSPD WHEN 0 THEN 1 ELSE AQSPD END,2))) as Days_of_Stock      
      
,SOS_Variance=CASE WHEN Stock<Minimum_Order THEN ABS(Minimum_Order-Stock) ELSE 0 END      
,ReOrder_Variance=CASE WHEN Stock BETWEEN Minimum_Order AND Re_Order THEN ABS(Re_Order-Stock) ELSE 0 END      
,MaxLevel_Variance=CASE WHEN Stock>Maximum_Order THEN ABS(Maximum_Order-Stock) ELSE 0 END      
      
,ReOrder_Qty=
			CASE WHEN Stock<Re_Order THEN Re_Order-Stock-Indent_Stock
...
LEFT JOIN (SELECT * FROM TEMPDB.INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='##REPORT_PLAN')T ON U.COLUMN_NAME=T.COLUMN_NAME      
)T      
...
SET @SQL='SELECT '''+@cPlan_Name+''' PlanName,'''+@ARS_ID+'''ARS_ID,DEPT_ID,'+@SELECT+CHAR(13)
	   +' FROM ##REPORT_PLAN R' 
